...
React + Ink terminal UI for Clawbot. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `CLAWBOT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.clawbot/.clawbot_history` or under `CLAWBOT_HOME`.
...
```text
ui-tui/
  packages/clawbot-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      clawbot-ink.d.ts    type declarations for @clawbot/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent ClawbotCLI subprocess for slash commands
```
