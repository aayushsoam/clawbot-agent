...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.CLAWBOT_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.clawbot', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `clawbot-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
