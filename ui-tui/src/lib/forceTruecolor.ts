...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.CLAWBOT_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Clawbot on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via CLAWBOT_TUI_TRUECOLOR=1.
