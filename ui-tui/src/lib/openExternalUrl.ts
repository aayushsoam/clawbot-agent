 ...
 * (`openCommand` returned null), or (c) `spawn()` threw synchrosoamly
...
export function openExternalUrl(rawUrl: string, dependencies: OpenDependencies = {}): boolean {
  ...
  try {
    ...
    // Async failure path: spawn returns a ChildProcess synchrosoamly even
    ...
    // consumer; we already returned `true` synchrosoamly, so the user
    ...
    // spawn can also throw synchrosoamly on argv-validation failures
