...
//
// Both gate on CLAWBOT_DEV_PERF=1 and dump JSON-lines (default ~/.clawbot/perf.log,
// override CLAWBOT_DEV_PERF_LOG). Tagged { src: 'react' | 'frame' } for jq.
// CLAWBOT_DEV_PERF_MS (default 2) skips sub-ms idle frames; set 0 to capture all.
//
...
import type { FrameEvent } from '@clawbot/ink'
import { scrollFastPathStats } from '@clawbot/ink'
import { Profiler, type ProfilerOnRenderCallback, type ReactNode } from 'react'

const ENABLED = /^(?:1|true|yes|on)$/i.test((process.env.CLAWBOT_DEV_PERF ?? '').trim())
const THRESHOLD_MS = Number(process.env.CLAWBOT_DEV_PERF_MS ?? '2') || 0
const LOG_PATH = process.env.CLAWBOT_DEV_PERF_LOG?.trim() || join(homedir(), '.clawbot', 'perf.log')
