import type { RunExternalProcess } from '@clawbot/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`clawbot ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching clawbot: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`clawbot ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
