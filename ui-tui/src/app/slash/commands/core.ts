import { forceRedraw } from '@clawbot/ink'
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys(
            'clipboard copy failed — try CLAWBOT_TUI_FORCE_OSC52=1 to force the escape sequence; CLAWBOT_TUI_DEBUG_CLIPBOARD=1 for details'
          )
        }
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `You #${i + 1}` : `Clawbot #${i + 1}`
