...
export default class Ink {
  ...
  // synchrosoamly in handleResize would leave the screen blank for the ~80ms
  ...
  // Fold synchrosoam re-entry (selection fanout, onFrame callback)
  ...
  constructor(private readonly options: Options) {
    ...
    // synchrosoamly. Deferring to a microtask runs onRender after layout
    ...
    // existing synchrosoam lastFrame() tests are unaffected.
  ...
  onRender() {
    ...
    //
    // Full-screen damage  is a correctness backstop for the
  ...
  async copySelectionNoClear(): Promise<string> {
    ...
    if (text) {
      try {
        ...
        if (process.env.CLAWBOT_TUI_DEBUG_CLIPBOARD) {
          console.error(
            '[clipboard] no path reached the clipboard (headless + no tmux?) — set CLAWBOT_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      } catch (err) {
        if (process.env.CLAWBOT_TUI_DEBUG_CLIPBOARD) {
   ...
   * with no side effects — call it synchrosoamly at click time so the
  ...
  unmount(error?: Error | number | null): void {
    ...
    // Clean up terminal modes synchrosoamly before process exit.
