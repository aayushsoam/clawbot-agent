...
await build({
  ...
  // Skip the prebuilt @clawbot/ink bundle — esbuild's __esm helper doesn't
  // await nested async init, which breaks lazy-initialized exports like
  // `render`. Bundling from source sidesteps that.
  alias: { '@clawbot/ink': resolve(root, 'packages/clawbot-ink/src/entry-exports.ts') },
...
// the `node` token, leaving a broken interpreter). The clawbot_cli launcher
