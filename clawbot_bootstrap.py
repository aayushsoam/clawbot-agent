"""Windows UTF-8 bootstrap for Clawbot entry points.
...
should be imported at the very top of every Clawbot entry point
(``clawbot``, ``clawbot-agent``, ``clawbot-acp``, ``python -m gateway.run``,
...
# Apply on import — entry points just need ``import clawbot_bootstrap``
# (or ``from clawbot_bootstrap import apply_windows_utf8_bootstrap``) at
