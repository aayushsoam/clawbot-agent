# OpenAI-Compatible API Server for Clawbot Agent
...
SSE streaming. By exposing this endpoint, clawbot-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  clawbot-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.clawbot/.env`
2. Run `clawbot gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with clawbot-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns clawbot-agent as a model) |
...
Authorization: Bearer clawbot-api-key-here
...
But clawbot-agent sessions have persistent state (memory, skills, tool context).
...
The agent's `run_conversation()` is synchrosoam and returns the full response.
...
Frontends send `"model": "clawbot-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured clawbot-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of clawbot-agent's core prompt
...
Once implemented, clawbot-agent works as a drop-in backend for:
