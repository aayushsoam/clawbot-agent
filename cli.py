#!/usr/bin/env python3
"""
Clawbot Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Clawbot Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills clawbot-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: clawbot_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See clawbot_bootstrap.py for full rationale.
try:
    import clawbot_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when clawbot_bootstrap isn't registered in the venv
    # yet — happens during partial ``clawbot update`` where git-reset landed
...
os.environ["CLAWBOT_QUIET"] = "1"  # Our own modules
...
try:
    from clawbot_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from clawbot_cli.banner import _format_context_length, format_banner_version_label
...
from clawbot_cli.browser_connect import (
...
_clawbot_home = get_clawbot_home()
_project_env = Path(__file__).parent / '.env'
load_clawbot_dotenv(clawbot_home=_clawbot_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.clawbot/.
...
def _parse_reasoning_config(effort: str) -> dict | None:
    ...
    from clawbot_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.clawbot/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If CLAWBOT_IGNORE_USER_CONFIG=1 is set (via ``clawbot chat --ignore-user-config``),
    the user config at ``~/.clawbot/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({CLAWBOT_HOME}/config.yaml)
    user_config_path = _clawbot_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("CLAWBOT_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Clawbot, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Clawbot - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and ClawbotCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `clawbot model`).
    ...
    from clawbot_cli.config import _expand_env_vars
    defaults = _expand_env_vars(defaults)

    # Apply terminal config to environment variables (so terminal_tool picks them up)
    terminal_config = defaults.get("terminal", {})
    
    # Normalize config key: the new config system (clawbot_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && clawbot` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _CLAWBOT_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_CLAWBOT_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["CLAWBOT_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.clawbot/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from clawbot_logging import setup_logging
...
try:
    from clawbot_cli.config import print_config_warnings
...
try:
    from clawbot_cli.skin_engine import init_skin_from_config
...
from clawbot_cli.banner import build_welcome_banner
from clawbot_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from clawbot_cli.callbacks import prompt_for_secret
...
def _run_cleanup():
    ...
    try:
        from clawbot_cli.plugins import invoke_hook as _invoke_hook
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run clawbot -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"clawbot-{short_id}"
    branch_name = f"clawbot/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`clawbot_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from clawbot_cli.config import load_config as _load_full_config
        from clawbot_constants import get_clawbot_home as _get_clawbot_home
        _clawbot_home_maint = _get_clawbot_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_clawbot_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`clawbot_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``clawbot/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("clawbot-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``clawbot/clawbot-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``clawbot -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("clawbot/clawbot-") or b.startswith("pr-"))
    ]
...
# - red: #FF6600 (headers, highlights)
# - Amber: #FF8C00 (secondary highlights)
# - Bronze: #FF4500 (tertiary elements)
# - Light: #FFE4C4 (text)
# - Dim: #CC5500 (muted text)

# ANSI building blocks for conversation display
_ACCENT_ANSI_DEFAULT = "\033[1;38;2;255;215;0m"  # True-color #FF6600 bold — fallback
_BOLD = "\033[1m"
_RST = "\033[0m"
...
# to remap "near-white" skin colors (e.g. #FFE4C4 banner_text, #CC5500
# banner_dim) to darker equivalents that are readable on a light
# Terminal.app / iTerm2 background.
#
# Detection priority:
#   1. CLAWBOT_LIGHT / CLAWBOT_TUI_LIGHT env (true/false) — explicit override
#   2. CLAWBOT_TUI_THEME=light|dark — explicit theme
#   3. CLAWBOT_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Clawbot assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("CLAWBOT_LIGHT", "CLAWBOT_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("CLAWBOT_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("CLAWBOT_TUI_BACKGROUND") or ""
...
_LIGHT_MODE_REMAP: dict[str, str] = {
    ...
    "#FFE4C4": "#1A1A1A",   # bisque -> near-black
    "#FF6600": "#9A6B00",   # red -> dark goldenrod (readable on cream)
    "#FF8C00": "#8A5A00",   # dark orange -> dark amber
    "#CC5500": "#5C4500",   # burnt orange -> deeper brown (more contrast)
    "#F1E6CF": "#1A1A1A",   # cream -> near-black
    "#c9d1d9": "#24292F",   # github-light fg
    "#EAF7FF": "#0F1B26",   # ice
    "#F5F5F5": "#1A1A1A",
    "#FFF0D4": "#1A1A1A",
    "#FF4500": "#8A4F1A",   # bronze -> darker bronze
    "#FFEFB5": "#3A2A00",
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from clawbot_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_clawbot_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._clawbot_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __init__(self, skin_key: str, fallback_hex: str = "#FF6600", *, bold: bool = False):
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from clawbot_cli.skin_engine import get_active_skin
...
_ACCENT = _SkinAwareAnsi("response_border", "#FF6600", bold=True)
...
# Terminal.app modes.  Hardcoded skin colors like #CC5500
...
def _accent_hex() -> str:
    ...
    try:
        from clawbot_cli.skin_engine import get_active_skin
        return get_active_skin().get_color("ui_accent", "#FF8C00")
    except Exception:
        return "#FF8C00"
...
def _replay_output_history() -> None:
    ...
    try:
        ...
        if rendered_lines:
            ...
            # per-line prompt_toolkit print forces one synchrosoam terminal I/O
...
from clawbot_constants import is_termux as _is_termux_environment
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``ClawbotCLI._busy_command()``.
        """
        yield self

# ASCII Art - CLAWBOT-AGENT logo (full width, single line - requires ~95 char terminal)
CLAWBOT_AGENT_LOGO = """[bold #FF6600] ██████╗██╗      █████╗ ██╗    ██╗██████╗  ██████╗ ████████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FF6600]██╔════╝██║     ██╔══██╗██║    ██║██╔══██╗██╔═══██╗╚══██╔══╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FF8C00]██║     ██║     ███████║██║ █╗ ██║██████╔╝██║   ██║   ██║        ███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FF8C00]██║     ██║     ██╔══██║██║███╗██║██╔══██╗██║   ██║   ██║        ██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#FF4500]╚██████╗███████╗██║  ██║╚███╔███╔╝██████╔╝╚██████╔╝   ██║        ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
def _build_compact_banner() -> str:
    ...
    border_color = _skin.get_color("banner_border", "#FF6600") if _skin else "#FF6600"
    title_color = _skin.get_color("banner_title", "#FF6600") if _skin else "#FF6600"
    dim_color = _skin.get_color("banner_dim", "#CC5500") if _skin else "#CC5500"

    if skin_name == "default":
        line1 = "⚡SOAM CLAWBOT - AI Agent Framework"
        tiny_line = "⚡SOAM CLAWBOT"
    else:
        agent_name = _skin.get_branding("agent_name", "Clawbot Agent") if _skin else "Clawbot Agent"
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Aayush soam[/]\n"
...
def _looks_like_slash_command(text: str) -> bool:
    ...
    File paths like ``/Users/aayushsoam/file.md:45-46 can you fix this?``
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from clawbot_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.clawbot/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.clawbot/config.yaml on first use)
...
class ClawbotCLI:
    """
    Interactive CLI for the Clawbot Agent.
    ...
    def __init__(
        ...
        """
        Initialize the Clawbot CLI.

        Args:
            model: Model to use (default: from env or claude-sonnet)
            toolsets: List of toolsets to enable (default: all)
            provider: Inference provider ("auto", "openrouter", "soam", "openai-codex", "zai", "kimi-coding", "minimax", "minimax-cn")
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from clawbot_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("CLAWBOT_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("CLAWBOT_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("CLAWBOT_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `clawbot chat --ignore-rules` in clawbot_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("CLAWBOT_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("CLAWBOT_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # it's shared across all Clawbot processes for this CLAWBOT_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.clawbot/checkpoints/.  Opt-in via
        ...
        self._history_file = _clawbot_home / ".clawbot_history"
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from clawbot_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            yolo_active = bool(os.getenv("CLAWBOT_YOLO_MODE"))
            if width < 52:
            ...
            return f"⚡{self.model if getattr(self, 'model', None) else 'Clawbot'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from clawbot_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from clawbot_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from clawbot_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from clawbot_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "⚡Clawbot")
                _text_hex = _skin.get_color("banner_text", "#FFE4C4")
            except Exception:
                label = "⚡Clawbot"
                _text_hex = "#FFE4C4"
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - ClawbotCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        """
        from clawbot_cli.runtime_provider import (
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: clawbot setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: clawbot setup")
            return False
        ...
        # `clawbot chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `clawbot auth add openai-codex`
        # without `clawbot model`), fall back to the provider's first catalog
        # model so the API call doesn't fail with "model must be non-empty".
        if not self.model and resolved_provider:
            try:
                from clawbot_cli.models import get_default_model_for_provider
    ...
    def _resolve_turn_agent_config(self, user_message: str) -> dict:
        ...
        """
        from clawbot_cli.models import resolve_fast_mode_overrides
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (clawbot sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``clawbot doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``clawbot doctor`` so the banner stays
        small.
        """
        try:
            from clawbot_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   Clawbot needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
        ...
        # Warn if the configured model is a Soam Clawbot LLM (not agentic)
        from clawbot_cli.model_switch import is_soam_clawbot_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_soam_clawbot_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Aayush soam Clawbot 3 & 4 models are NOT agentic and are not "
                "designed for use with Clawbot Agent.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        try:
            from clawbot_cli.skin_engine import get_active_skin
            _skin = get_active_skin()
            _history_text_c = _skin.get_color("banner_text", "#FFE4C4")
            _session_label_c = _skin.get_color("session_label", "#FF8C00")
            _session_border_c = _skin.get_color("session_border", "#8B8682")
            _assistant_label_c = _skin.get_color("ui_ok", "#8FBC8F")
        except Exception:
            _history_text_c = "#FFE4C4"
            _session_label_c = "#FF8C00"
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Clawbot: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Clawbot: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.clawbot/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from clawbot_cli.clipboard import save_clipboard_image

        img_dir = get_clawbot_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: clawbot --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Clawbot config/state.
        ...
        """
        from clawbot_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_clawbot_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from clawbot_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run clawbot chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'clawbot setup' to configure[/]")
        except Exception:
    ...
    def _show_status(self):
        ...
        try:
            ...
            separator_color = skin.get_color("banner_dim", "#CC5500")
            accent_color = skin.get_color("ui_accent", "#FF8C00")
            label_color = skin.get_color("ui_label", "#FF8C00")
        except Exception:
            separator_color, accent_color, label_color = "#CC5500", "#FF8C00", "#FF8C00"
        toolsets_info = ""
    ...
    def _show_session_status(self):
        ...
        # 2.1.114's /recap.
        try:
            from clawbot_cli.session_recap import build_recap
            recap = build_recap(
    ...
    def _fast_command_available(self) -> bool:
        try:
            from clawbot_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from clawbot_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from clawbot_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Clawbot!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in clawbot_cli/colors.py
        ...
        from clawbot_cli.tools_config import _get_platform_tools
    ...
    def _handle_profile_command(self):
        ...
        from clawbot_constants import display_clawbot_home
        from clawbot_cli.profiles import get_active_profile_name

        display = display_clawbot_home()
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from clawbot_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("CLAWBOT_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        """
        from clawbot_state import format_session_db_unavailable
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `clawbot gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `clawbot sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from clawbot_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `clawbot sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.clawbot/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``clawbot --resume <id>``
        ...
        saved_dir = get_clawbot_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"clawbot_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: clawbot --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from clawbot_cli.curses_ui import curses_single_select
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # Copilot, and Soam-enforced caps win over the raw models.dev entry
        # (e.g. gpt-5.5 is 1.05M on openai but 272K on Codex OAuth).
        mi = result.model_info
        try:
            from clawbot_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `clawbot model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from clawbot_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from clawbot_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from clawbot_cli.model_switch import switch_model, parse_model_flags
        from clawbot_cli.providers import get_label
        ...
        # clobber disk config.
        from clawbot_cli.inventory import build_models_payload, load_picker_context
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from clawbot_cli.commands import resolve_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to clawbot_cli.curator so the CLI and the `clawbot curator`
        ...
        try:
            from clawbot_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from clawbot_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to clawbot_cli.skills_hub."""
        from clawbot_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_clawbot_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_clawbot_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in clawbot_cli/commands.py instead of touching every dispatch site.
        from clawbot_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from clawbot_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from clawbot_cli.skin_engine import get_active_skin
                        _tip_color = get_active_skin().get_color("banner_dim", "#CC5500")
                    except Exception:
                        _tip_color = "#CC5500"
            ...
            from clawbot_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from clawbot_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_clawbot_home()}/plugins/ to get started.")
                else:
            ...
            # Check for plugin-registered slash commands
                ...
                from clawbot_cli.plugins import (
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from clawbot_cli.commands import COMMANDS
            ...
            try:
                ...
                if response:
                    try:
                        ...
                        _skin = get_active_skin()
                        label = _skin.get_branding("response_label", "⚡Clawbot")
                        _resp_color = _maybe_remap_for_light_mode(_skin.get_color("response_border", "#FF4500"))
                        _resp_text = _maybe_remap_for_light_mode(_skin.get_color("banner_text", "#FFE4C4"))
                    except Exception:
                        label = "⚡Clawbot"
                        _resp_color = "#FF4500"
                        _resp_text = "#FFE4C4"
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from clawbot_cli.goals import GoalManager
            from clawbot_cli.config import load_config
        except Exception as exc:
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Clawbot keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from clawbot_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_clawbot_home()}/skins/\n")
            return
    ...
    def _handle_footer_command(self, cmd_original: str) -> None:
        ...
        """
        from clawbot_cli.config import load_config
        from clawbot_cli.colors import Colors as _Colors
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("CLAWBOT_YOLO_MODE"))
        if current:
            os.environ.pop("CLAWBOT_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Clawbot is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Clawbot is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Clawbot is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from clawbot_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from clawbot_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by clawbot_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from clawbot_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.clawbot/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_clawbot_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "clawbot_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "clawbot_voice",
        ...
        try:
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _clawbot_home / "interrupt_debug.log"
            ...
            if response and not response_previewed:
                ...
                try:
                    ...
                    label = "⚡Clawbot"
                    _resp_color = _maybe_remap_for_light_mode("#FF4500")
                    _resp_text = _maybe_remap_for_light_mode("#FFE4C4")
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  clawbot --resume {self.session_id}")
            if session_title:
                print(f"  clawbot -c \"{session_title}\"")
            print()
            ...
            try:
                from clawbot_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from clawbot_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from clawbot_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Clawbot Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFE4C4")
        except Exception:
            _welcome_text = "Welcome to Clawbot Agent! Type your message or /help for commands."
            _welcome_color = "#FFE4C4"
        ...
        try:
            _redact_raw = os.getenv("CLAWBOT_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(CLAWBOT_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Clawbot migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                try:
                    _resid_color = _welcome_skin.get_color("banner_dim", "#CC5500")
                except Exception:
                    _resid_color = "#CC5500"
                self._console_print(f"[{_resid_color}]{openclaw_residue_hint_cli()}[/]")
                try:
                    from clawbot_cli.config import get_config_path as _get_cfg_path_resid
        ...
        try:
            ...
            _tip = get_random_tip()
            try:
                _tip_color = _welcome_skin.get_color("banner_dim", "#CC5500")
            except Exception:
                _tip_color = "#CC5500"
            self._console_print(f"[dim {_tip_color}]✦ Tip: {_tip}[/]")
        except Exception:
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_clawbot_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Clawbot binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from clawbot_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Clawbot Agent")
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if line_count >= 5 and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _clawbot_home / "pastes"
        ...
        # Dynamic prompt: shows Clawbot symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Clawbot needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Clawbot needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Clawbot needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = ClawbotCLI._compute_model_picker_viewport(
        ...
        self._tui_style_base = {
            ...
            # color schemes.  (Hardcoding a near-white #FFE4C4 made
            ...
            'status-bar-warn': 'bg:#1a1a2e #FF6600 bold',
            ...
            'completion-menu': 'bg:#1a1a2e #FFE4C4',
            'completion-menu.completion': 'bg:#1a1a2e #FFE4C4',
            'completion-menu.completion.current': 'bg:#333355 #FF6600',
            'completion-menu.meta.completion': 'bg:#1a1a2e #888888',
            'completion-menu.meta.completion.current': 'bg:#333355 #FF8C00',
            # Clarify question panel
            'clarify-border': '#FF4500',
            'clarify-title': '#FF6600 bold',
            'clarify-question': '#FFE4C4 bold',
            'clarify-choice': '#AAAAAA',
            'clarify-selected': '#FF6600 bold',
            'clarify-active-other': '#FF6600 italic',
            'clarify-countdown': '#FF4500',
            # Sudo password panel
            ...
            'approval-border': '#FF4500',
            'approval-title': '#FF8C00 bold',
            'approval-desc': '#FFE4C4 bold',
            'approval-cmd': '#AAAAAA italic',
            'approval-choice': '#AAAAAA',
            'approval-selected': '#FF6600 bold',
            # Voice mode
            'voice-prompt': '#87CEEB',
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_clawbot_osd_patched", False):
                ...
                _pt_renderer._clawbot_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``CLAWBOT_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("CLAWBOT_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the clawbot process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: clawbot setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                if getattr(self, '_delete_session_on_exit', False):
                    try:
                        from clawbot_constants import get_clawbot_home as _ghh
...
def main(
    ...
    """
    Clawbot Agent CLI - Interactive AI Assistant
    ...
    os.environ["CLAWBOT_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Clawbot Gateway (messaging platforms)...")
    ...
    # Default to clawbot-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside ClawbotCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  CLAWBOT_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "Clawbot" box, no tool-gen
            ...
            # Single-query mode (`clawbot chat -q "…"`): skip the welcome
