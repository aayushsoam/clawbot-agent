...
_CLAWBOT_CORE_TOOLS = [
    ...
    # spawned as a kanban worker (CLAWBOT_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `clawbot tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``clawbot tools`` → Video Generation."
        ),
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (CLAWBOT_KANBAN_TASK env "
    ...
    # ==========================================================================
    # Full Clawbot toolsets (CLI + messaging platforms)
    #
    ...
    "clawbot-cli": {
        ...
        "tools": _CLAWBOT_CORE_TOOLS,
        "includes": []
    },

    "clawbot-cron": {
        # Mirrors clawbot-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `clawbot tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as clawbot-cli; gated by `clawbot tools`",
        "tools": _CLAWBOT_CORE_TOOLS,
        "includes": []
    },
    ...
    "clawbot-email": {
        "description": "Email bot toolset - interact with Clawbot via email (IMAP/SMTP)",
        "tools": _CLAWBOT_CORE_TOOLS,
        "includes": []
    },
    ...
    "clawbot-sms": {
        "description": "SMS bot toolset - interact with Clawbot via SMS (Twilio)",
        "tools": _CLAWBOT_CORE_TOOLS,
        "includes": []
    },
    ...
    "clawbot-gateway": {
        ...
        "tools": [],
        "includes": ["clawbot-telegram", "clawbot-discord", "clawbot-whatsapp", "clawbot-slack", "clawbot-signal", "clawbot-bluebubbles", "clawbot-homeassistant", "clawbot-email", "clawbot-sms", "clawbot-mattermost", "clawbot-matrix", "clawbot-dingtalk", "clawbot-feishu", "clawbot-wecom", "clawbot-wecom-callback", "clawbot-weixin", "clawbot-qqbot", "clawbot-webhook", "clawbot-yuanbao"]
    }
}
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (clawbot-<name>).
        # Gives them _CLAWBOT_CORE_TOOLS plus any tools the plugin registered
        # into a toolset matching the platform name.
        if name.startswith("clawbot-"):
            platform_name = name[len("clawbot-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_CLAWBOT_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
