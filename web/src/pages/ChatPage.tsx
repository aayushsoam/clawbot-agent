/**
 * ChatPage — embeds `clawbot --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (clawbot_cli/web_server.py)                          .
 *          ▼                                                              .
...
import { Button } from "@aayushsoam/ui/ui/components/button";
import { Typography } from "@/components/NouiTypography";
import { CLAWBOT_BASE_PATH } from "@/lib/api";
...
function buildWsUrl(
  ...
  return `${proto}//${window.location.host}${CLAWBOT_BASE_PATH}/api/pty?${qs.toString()}`;
}
...
// Colors for the terminal body.  Matches the dashboard's black canvas
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    typeof window !== "undefined" && !window.__CLAWBOT_SESSION_TOKEN__
      ? "Session token unavailable. Open this page through `clawbot dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Clawbot TUI has enabled xterm mouse-events
      ...
      // let the inner Clawbot TUI own transcript history/scroll behavior.
    ...
    // Clawbot TUI own transcript scrolling.
    //
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[clawbot-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Clawbot. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2 normal-case">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="clawbot-chat-xterm-host min-h-0 min-w-0 flex-1"
          />
...
declare global {
  interface Window {
    __CLAWBOT_SESSION_TOKEN__?: string;
  }
}
