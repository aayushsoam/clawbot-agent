...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      clawbot-agent = self'.packages.default;
      clawbotVenv = clawbot-agent.clawbotVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "clawbot-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${clawbotVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "clawbot-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "clawbot-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${clawbot-agent}/bin/clawbot || (echo "FAIL: clawbot binary missing"; exit 1)
          test -x ${clawbot-agent}/bin/clawbot-agent || (echo "FAIL: clawbot-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${clawbot-agent}/bin/clawbot version 2>&1 | grep -qi "clawbot" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "clawbot-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in clawbot clawbot-agent clawbot-acp; do
            test -x ${clawbot-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "clawbot-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking clawbot --help ==="
          ${clawbot-agent}/bin/clawbot --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${clawbot-agent}/bin/clawbot --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "clawbot-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${clawbot-agent}/share/clawbot-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${clawbot-agent}/share/clawbot-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "CLAWBOT_BUNDLED_SKILLS" ${clawbot-agent}/bin/clawbot || \
            (echo "FAIL: CLAWBOT_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: CLAWBOT_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "clawbot-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${clawbot-agent}/share/clawbot-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${clawbot-agent}/share/clawbot-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "CLAWBOT_BUNDLED_PLUGINS" ${clawbot-agent}/bin/clawbot || \
            (echo "FAIL: CLAWBOT_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: CLAWBOT_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "clawbot-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${clawbot-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${clawbot-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "CLAWBOT_TUI_DIR" ${clawbot-agent}/bin/clawbot || \
            (echo "FAIL: CLAWBOT_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: CLAWBOT_TUI_DIR set in wrapper"
        ...
        # Verify CLAWBOT_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        clawbot-node = pkgs.runCommand "clawbot-node-version" { } ''
          set -e
          echo "=== Checking CLAWBOT_NODE in wrapper ==="
          grep -q "CLAWBOT_NODE" ${clawbot-agent}/bin/clawbot || \
            (echo "FAIL: CLAWBOT_NODE not set in wrapper"; exit 1)
          echo "PASS: CLAWBOT_NODE present in wrapper"

          CLAWBOT_NODE=$(sed -n "s/^export CLAWBOT_NODE='\(.*\)'/\1/p" ${clawbot-agent}/bin/clawbot)
          test -x "$CLAWBOT_NODE" || (echo "FAIL: CLAWBOT_NODE=$CLAWBOT_NODE not executable"; exit 1)
          echo "PASS: CLAWBOT_NODE executable at $CLAWBOT_NODE"

          NODE_MAJOR=$("$CLAWBOT_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All CLAWBOT_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify CLAWBOT_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "clawbot-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(CLAWBOT_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking CLAWBOT_MANAGED guards ==="
          check_blocked "config set" ${clawbot-agent}/bin/clawbot config set model foo
          check_blocked "config edit" ${clawbot-agent}/bin/clawbot config edit
        ...
        extra-python-packages = let
          ...
          clawbotWithExtra = clawbot-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "clawbot-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${clawbotWithExtra}/bin/clawbot || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${clawbotWithExtra}/bin/clawbot || \
          ...
          if grep -q "PYTHONPATH" ${clawbot-agent}/bin/clawbot; then
        ...
        extra-dependency-groups = let
          clawbotWithGroups = clawbot-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "clawbot-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${clawbotWithGroups}"
          echo "venv: ${clawbotWithGroups.clawbotVenv}"
        ...
        in pkgs.runCommand "clawbot-config-roundtrip" {
          ...
          merge_and_load() {
            local clawbot_home="$1"
            export CLAWBOT_HOME="$clawbot_home"
            ${configMergeScript} ${nixSettings} "$clawbot_home/config.yaml"
            ${clawbotVenv}/bin/python3 -c '
import json, sys
