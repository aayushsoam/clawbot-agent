# nix/nixosModules.nix — NixOS module for clawbot-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: clawbot runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $CLAWBOT_HOME/.env
# and read by clawbot at startup — no container recreation needed for env changes.
#
# Tool resolution: the clawbot wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.clawbot-agent;
    effectivePackage =
    ...
    clawbot-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-clawbot-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "clawbot-config-attrs";
      description = "Clawbot YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "clawbot-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "clawbot-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'CLAWBOT_DOC_EOF'\n${value}\nCLAWBOT_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "clawbot-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/clawbot";
    ...
    # clawbot user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "clawbot-container-entrypoint" ''
      set -eu

      CLAWBOT_UID="''${CLAWBOT_UID:?CLAWBOT_UID must be set}"
      CLAWBOT_GID="''${CLAWBOT_GID:?CLAWBOT_GID must be set}"

      # ── Group: ensure a group with GID=$CLAWBOT_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$CLAWBOT_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="clawbot"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$CLAWBOT_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$CLAWBOT_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$CLAWBOT_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$CLAWBOT_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="clawbot"
        TARGET_HOME="/home/clawbot"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$CLAWBOT_UID" -g "$CLAWBOT_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$CLAWBOT_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$CLAWBOT_UID:$CLAWBOT_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure CLAWBOT_HOME is owned by the target user.
      ...
      if [ -n "''${CLAWBOT_HOME:-}" ] && [ -d "$CLAWBOT_HOME" ]; then
        find "$CLAWBOT_HOME" \! -user "$CLAWBOT_UID" -exec chown "$CLAWBOT_UID:$CLAWBOT_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/clawbot-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/clawbot-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/clawbot ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/clawbot
        chmod 0440 /etc/sudoers.d/clawbot
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$CLAWBOT_UID" --regid="$CLAWBOT_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $CLAWBOT_HOME/.env.
    ...
    # Default: /var/lib/clawbot/workspace → /data/workspace.
  ...
  in {
    options.services.clawbot-agent = with lib; {
      enable = mkEnableOption "Clawbot Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = clawbot-agent;
        description = "The clawbot-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/clawbot";
        description = "State directory. Contains .clawbot/ subdir (CLAWBOT_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Clawbot config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $CLAWBOT_HOME/.env at activation time.
          Clawbot reads this file on every startup via load_clawbot_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $CLAWBOT_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $CLAWBOT_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `clawbot gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the clawbot user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the clawbot plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Clawbot discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          clawbot_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as clawbot (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in clawbot-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the clawbot CLI to environment.systemPackages and export
          CLAWBOT_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.clawbot symlink to the service
            stateDir. These users are automatically added to the clawbot group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.clawbot-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the clawbot CLI to system PATH and export CLAWBOT_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.clawbot/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.CLAWBOT_HOME = "${cfg.stateDir}/.clawbot";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.clawbot-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the clawbot user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.clawbot-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed clawbot
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure clawbot is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.clawbot        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawbot/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawbot/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawbot/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawbot/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.clawbot/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."clawbot-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.clawbot
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.clawbot ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.clawbot ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.clawbot -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.clawbot/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.clawbot/$_subdir"
            chmod 2770 "${cfg.stateDir}/.clawbot/$_subdir"
            find "${cfg.stateDir}/.clawbot/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.clawbot/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.clawbot/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawbot/config.yaml
            chmod 0640 ${cfg.stateDir}/.clawbot/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.clawbot/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawbot/.managed
          chmod 0644 ${cfg.stateDir}/.clawbot/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.clawbot/.container-mode <<'CLAWBOT_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    clawbot_bin=${containerDataDir}/current-package/bin/clawbot
    CLAWBOT_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawbot/.container-mode
            chmod 0644 ${cfg.stateDir}/.clawbot/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.clawbot/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.clawbot";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.clawbot" ]; then
                  rm -f "${symlinkPath}"
                  echo "clawbot-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.clawbot -> stateDir/.clawbot for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.clawbot";
                target = "${cfg.stateDir}/.clawbot";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "clawbot-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.clawbot/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.clawbot/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.clawbot/auth.json
              fi
            ''}
          ''}
          ...
          # Clawbot reads $CLAWBOT_HOME/.env at startup via load_clawbot_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.clawbot/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'CLAWBOT_NIX_ENV_EOF'
    ${envFileContent}
    CLAWBOT_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.clawbot/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.clawbot/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.clawbot/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.clawbot-agent = {
          description = "Clawbot Agent Gateway";
          ...
          environment = {
            HOME = cfg.stateDir;
            CLAWBOT_HOME = "${cfg.stateDir}/.clawbot";
            CLAWBOT_MANAGED = "true";
          ...
          serviceConfig = {
            ...
            # $CLAWBOT_HOME/.env by the activation script. load_clawbot_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/clawbot"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the clawbot group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.clawbot-agent = {
          description = "Clawbot Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              CLAWBOT_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              CLAWBOT_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env CLAWBOT_UID="$CLAWBOT_UID" \
                --env CLAWBOT_GID="$CLAWBOT_GID" \
                --env CLAWBOT_HOME=${containerDataDir}/.clawbot \
                --env CLAWBOT_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/clawbot gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
