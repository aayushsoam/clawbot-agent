...
CLAWBOT_HOME_DIR="${CLAWBOT_HOME:-$HOME/.clawbot}"
CLAWBOT_CFG="${CLAWBOT_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Clawbot config has twozero_td MCP entry ──
if [[ ! -f "$CLAWBOT_CFG" ]]; then
    echo -e " ${FAIL} Clawbot config not found at ${CLAWBOT_CFG}"
    manual_steps+=("Create ${CLAWBOT_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$CLAWBOT_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Clawbot config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Clawbot config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$CLAWBOT_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${CLAWBOT_CFG} manually"); }
    manual_steps+=("Restart Clawbot session to pick up config change")
fi
