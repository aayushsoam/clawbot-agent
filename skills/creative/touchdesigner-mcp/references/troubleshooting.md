...
## 5. Clawbot Configuration

### Config location

`$CLAWBOT_HOME/config.yaml` (defaults to `~/.clawbot/config.yaml` when `CLAWBOT_HOME` is unset)
...
Restart the Clawbot session for changes to take effect. The MCP connection is
