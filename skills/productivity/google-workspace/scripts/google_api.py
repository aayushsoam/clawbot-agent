#!/usr/bin/env python3
"""Google Workspace API CLI for Clawbot Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Clawbot-facing JSON contract and falls back to the Python client
...
from _clawbot_home import get_clawbot_home
...
def _gws_binary() -> str | None:
    override = os.getenv("CLAWBOT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Clawbot Agent")
