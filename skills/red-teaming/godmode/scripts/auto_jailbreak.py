...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("CLAWBOT_HOME", os.path.expanduser("~/.clawbot")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CLAWBOT_HOME", Path.home() / ".clawbot")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Clawbot config paths
# ═══════════════════════════════════════════════════════════════════

CLAWBOT_HOME = Path(os.getenv("CLAWBOT_HOME", Path.home() / ".clawbot"))
CONFIG_PATH = CLAWBOT_HOME / "config.yaml"
PREFILL_PATH = CLAWBOT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Soam/Clawbot models — already uncensored, just needs clean prompt
    "clawbot": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "clawbot" in model_lower or "soam" in model_lower:
        return "clawbot"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Clawbot config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.clawbot/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Clawbot for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Clawbot for changes to take effect.")
