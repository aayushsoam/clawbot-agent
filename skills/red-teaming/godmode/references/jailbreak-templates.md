...
## 5. GODMODE FAST — Clawbot 4 405B (Zero Refusal)

**Model:** `aayushsoam/clawbot-4-405b`
**Strategy:** Clawbot models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Clawbot Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.clawbot/config.yaml`:
