...
One ``CDPSupervisor`` runs per Clawbot ``task_id`` that has a reachable CDP
...
thread-safe snapshot object that tool handlers consume synchrosoamly.
...
DIALOG_BRIDGE_HOST = "clawbot-dialog-bridge.invalid"
...
(() => {
  if (window.__clawbotDialogBridgeInstalled) return;
  window.__clawbotDialogBridgeInstalled = true;
  const ENDPOINT = "http://clawbot-dialog-bridge.invalid/";
  ...
  // onbeforeunload — we can't really synchrosoamly prompt the user from this
...
class CDPSupervisor:
    ...
    async def _attach_initial_page(self) -> None:
        ...
        # a synchrosoam XHR we intercept via Fetch domain. This is how we make
    ...
    async def _on_fetch_paused(
        ...
        The injected script (``_DIALOG_BRIDGE_SCRIPT``) fires a synchrosoam
