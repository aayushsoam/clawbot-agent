...
running under the dispatcher (env var ``CLAWBOT_KANBAN_TASK`` set). A
normal ``clawbot chat`` session sees **zero** kanban tools in its schema.

Why tools instead of just shelling out to ``clawbot kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``clawbot kanban complete …``
   inside the container, where ``clawbot`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.clawbot/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``clawbot kanban …``), the dashboard
(``clawbot dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``CLAWBOT_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``clawbot chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("CLAWBOT_KANBAN_TASK")
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``CLAWBOT_KANBAN_TASK`` set
    ...
    ``CLAWBOT_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set CLAWBOT_KANBAN_TASK in the env)"
        )
    try:
        kb, conn = _connect()
        try:
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # CLAWBOT_KANBAN_CLAIM_LOCK in the worker env at spawn time
            # (see _default_spawn in kanban_db.py); falling back to the
            # default _claimer_id() covers locally-driven workers that
            # never went through the dispatcher path.
            claim_lock = os.environ.get("CLAWBOT_KANBAN_CLAIM_LOCK")
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``clawbot-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("CLAWBOT_PROFILE") or "worker"
    try:
        kb, conn = _connect()
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("CLAWBOT_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=os.environ.get("CLAWBOT_PROFILE") or "worker",
            )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to CLAWBOT_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to CLAWBOT_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
