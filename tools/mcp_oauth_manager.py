...
`mcp_tool.py`, and `clawbot_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# ClawbotMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_clawbot_provider_class() -> Optional[type]:
    ...
    class ClawbotMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._clawbot_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`ClawbotTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, ClawbotTokenStorage)
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            but runs synchrosoamly before the first request instead of
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._clawbot_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import ClawbotTokenStorage
                        if isinstance(storage, ClawbotTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._clawbot_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import ClawbotTokenStorage
            if not isinstance(storage, ClawbotTokenStorage):
                return
    ...
    return ClawbotMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_CLAWBOT_PROVIDER_CLS: Optional[type] = _make_clawbot_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`ClawbotMCPOAuthProvider` directly using the helpers
        ...
        storage = ClawbotTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``clawbot mcp remove <name>`` and (indirectly) by
        ``clawbot mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
