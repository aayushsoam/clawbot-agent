...
# Clawbot-internal env vars that should NOT leak into terminal subprocesses.
_CLAWBOT_PROVIDER_ENV_FORCE_PREFIX = "_CLAWBOT_FORCE_"
...
_CLAWBOT_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Clawbot-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_CLAWBOT_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _CLAWBOT_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_CLAWBOT_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_CLAWBOT_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _CLAWBOT_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("CLAWBOT_GIT_BASH_PATH")
    ...
    #   PortableGit: %LOCALAPPDATA%\clawbot\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\clawbot\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _clawbot_portable_git = os.path.join(_local_appdata, "clawbot", "git") if _local_appdata else ""
    if _clawbot_portable_git:
        for candidate in (
            os.path.join(_clawbot_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_clawbot_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    raise RuntimeError(
        "Git Bash not found. Clawbot Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set CLAWBOT_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_CLAWBOT_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_CLAWBOT_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _CLAWBOT_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # Clawbot is driving via _find_bash (Git Bash), and Git Bash itself
    ...
    # npm …) into {CLAWBOT_HOME}/home/ when that directory exists.  Only the
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Clawbot trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``CLAWBOT_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under CLAWBOT_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_clawbot_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "clawbot_terminal"
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._clawbot_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_clawbot_pgid", None)
                    if pgid is None:
                        raise
