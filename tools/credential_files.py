...
def _resolve_clawbot_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.clawbot",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``CLAWBOT_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside CLAWBOT_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    clawbot_home = _resolve_clawbot_home()

    # Reject absolute paths — they bypass the CLAWBOT_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to CLAWBOT_HOME)",
            relative_path,
        )
        return False

    host_path = clawbot_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape CLAWBOT_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, clawbot_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.clawbot/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="clawbot-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching clawbot_constants.get_clawbot_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_clawbot_dir()`` for backward compatibility with old directory layouts.
    """
    from clawbot_constants import get_clawbot_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_clawbot_dir(new_subpath, old_name)
        if host_dir.is_dir():
