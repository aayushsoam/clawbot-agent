...
Backend is selected during ``clawbot tools`` setup (web.backend in config.yaml).
When available, Clawbot can route Firecrawl calls through a Soam-hosted tool-gateway
for Soam Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract, crawl; direct or derived firecrawl-gateway.<domain> for Soam Subscribers)
...
from tools.managed_tool_gateway import (  # noqa: F401 — backward-compat names for tests
    ...
    read_soam_access_token as _read_soam_access_token,
    resolve_managed_tool_gateway,
)
from tools.tool_backend_helpers import managed_soam_tools_enabled, prefers_gateway  # noqa: F401
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.clawbot/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``clawbot tools``).
    ...
    # tool gateway is configured for Soam subscribers.
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_soam_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchrosoam HTTP
    refresh against the Soam portal on every CLI startup (invoked at
...
def _is_soam_auxiliary_client(client: Any) -> bool:
    """Return True when the resolved auxiliary backend is Soam Portal."""
    from urllib.parse import urlparse

    base_url = str(getattr(client, "base_url", "") or "")
    host = (urlparse(base_url).hostname or "").lower()
    return host == "aayushsoam.com" or host.endswith(".aayushsoam.com")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_soam_auxiliary_client(client):
        from agent.auxiliary_client import get_auxiliary_extra_body
        from agent.portal_tags import soam_portal_tags
        extra_body = get_auxiliary_extra_body() or {"tags": soam_portal_tags()}
        ...
        if provider is None:
            response_data = {
                ...
                "error": (
                    ...
                    "Run `clawbot tools` to set one up."
                ),
            }
        else:
            logger.info(
...
if __name__ == "__main__":
    ...
    soam_available = check_auxiliary_model()
    ...
    if not soam_available:
        print("❌ No auxiliary model available for LLM content processing")
        print("Set OPENROUTER_API_KEY, configure Soam Portal, or set OPENAI_BASE_URL + OPENAI_API_KEY")
    ...
    print("")
    print("  # Search (synchrosoam)")
    print("  results = web_search_tool('Python tutorials')")
    print("")
    print("  # Extract and crawl (asynchrosoam)")
