...
This module provides the glue:
    - ``ClawbotTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Clawbot Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses CLAWBOT_HOME so each profile gets its own OAuth tokens.
    Layout: ``CLAWBOT_HOME/mcp-tokens/``
    """
    try:
        from clawbot_constants import get_clawbot_home
        base = Path(get_clawbot_home())
    except ImportError:
        base = Path(os.environ.get("CLAWBOT_HOME", str(Path.home() / ".clawbot")))
...
# ---------------------------------------------------------------------------
# ClawbotTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class ClawbotTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        CLAWBOT_HOME/mcp-tokens/<server_name>.json         -- tokens
        CLAWBOT_HOME/mcp-tokens/<server_name>.client.json   -- client info
        CLAWBOT_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Clawbot records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Clawbot.</p></body></html>"
            ) if code else (
...
async def _redirect_handler(authorization_url: str) -> None:
    ...
    if _oauth_port and (os.getenv("SSH_CLIENT") or os.getenv("SSH_TTY")):
        print(
            ...
            f"\n"
            f"  Then open the URL above. See: https://clawbot-agent.aayushsoam.com/docs/guides/oauth-over-ssh\n",
            file=sys.stderr,
        )
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Clawbot Agent")
    scope = cfg.get("scope")
