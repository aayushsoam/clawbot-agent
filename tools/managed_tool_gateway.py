"""Generic managed-tool gateway helpers for Soam-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "aayushsoam.com"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_SOAM_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
def auth_json_path():
    """Return the Clawbot auth store path, respecting CLAWBOT_HOME overrides."""
    return get_clawbot_home() / "auth.json"


def _read_soam_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        soam_provider = providers.get("soam", {})
        if isinstance(soam_provider, dict):
            return soam_provider
    except Exception:
        pass
    return None
...
def read_soam_access_token() -> Optional[str]:
    """Read a Soam Subscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    soam_provider = _read_soam_provider_state() or {}
    access_token = soam_provider.get("access_token")
    ...
    if cached_token and not _access_token_is_expiring(
        soam_provider.get("expires_at"),
        _SOAM_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from clawbot_cli.auth import resolve_soam_access_token

        refreshed_token = resolve_soam_access_token(
            refresh_skew_seconds=_SOAM_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Soam access token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_soam_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    soam_user_token = resolved_token_reader()
    if not gateway_origin or not soam_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        soam_user_token=soam_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and Soam access token are available."""
