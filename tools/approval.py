...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("CLAWBOT_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("CLAWBOT_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("CLAWBOT_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set CLAWBOT_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind CLAWBOT_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds CLAWBOT_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("CLAWBOT_CRON_SESSION"):
        return False
    if env_var_enabled("CLAWBOT_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $CLAWBOT_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_CLAWBOT_ENV_PATH = (
    r'(?:~\/\.clawbot/|'
    r'(?:\$home|\$\{home\})/\.clawbot/|'
    r'(?:\$clawbot_home|\$\{clawbot_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bclawbot\s+gateway\s+(stop|restart)\b', "stop/restart clawbot gateway (kills running agents)"),
    (r'\bclawbot\s+update\b', "clawbot update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart clawbot-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart clawbot-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(clawbot|gateway|cli\.py)\b', "kill clawbot/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill clawbot` but not
    # `kill -9 $(pgrep -f clawbot)` because the substitution is opaque
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchrosoam input() flow)
# =========================================================================
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("CLAWBOT_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = env_var_enabled("CLAWBOT_INTERACTIVE")
    ...
    if is_gateway or env_var_enabled("CLAWBOT_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("CLAWBOT_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = env_var_enabled("CLAWBOT_INTERACTIVE")
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("CLAWBOT_EXEC_ASK")
    ...
    # responds with /approve or /deny, mirroring the CLI's synchrosoam
