...
supports multiple backends — **Browser Use** (cloud, default for Soam
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    clawbot_node_bin = str(clawbot_home / "node" / "bin")
    clawbot_node_root = str(clawbot_home / "node")
    clawbot_nm_bin = str(clawbot_home / "node_modules" / ".bin")
    return [clawbot_node_bin, clawbot_node_root, clawbot_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
# plugins drop in under ``~/.clawbot/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed Soam gateway or
    ...
    browser plugins (``~/.clawbot/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed Soam gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Clawbot silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            #   in the Clawbot CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-clawbot_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed clawbot processes — this way every clean clawbot exit sweeps
    ...
    # Sweep orphans from other crashed clawbot processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live clawbot processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current clawbot PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live clawbot process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning clawbot process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced clawbot PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent clawbot instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-clawbot_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live clawbot process.
            continue
...
BROWSER_TOOL_SCHEMAS = [
    {
        ...
        "description": "Navigate to a URL in the browser. Initializes the session and loads the page. Must be called before other browser tools. IMPORTANT: Clawbot Desktop has an integrated browser engine called 'Clawbot Browser' (which renders inside the chat window split pane) and an external browser called 'Local Browser' (which spawns in a separate window). When you receive a browser automation or web task that requires browser, you MUST explicitly ask the user for permission first, presenting these two choices: 'Clawbot Browser' or 'Local Browser'. Once the user responds and selects/approves one, proceed to call browser_navigate. For simple information retrieval, prefer web_search or web_extract (faster, cheaper). For plain-text endpoints — URLs ending in .md, .txt, .json, .yaml, .yml, .csv, .xml, raw.githubusercontent.com, or any documented API endpoint — prefer curl via the terminal tool or web_extract; the browser stack is overkill and much slower for these. Use browser tools when you need to interact with a page (click, fill forms, dynamic content). Returns a compact page snapshot with interactive elements and ref IDs — no need to call browser_snapshot separately after navigating.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Clawbot-managed
    ...
    # Build an extended search PATH including Clawbot-managed Node, macOS
    ...
    try:
        ...
        if ensure_dependency("browser"):
            ...
            if not recheck:
                clawbot_nm = str(get_clawbot_home() / "node_modules" / ".bin")
                recheck = shutil.which("agent-browser", path=clawbot_nm)
            if not recheck:
                clawbot_node_bin = str(get_clawbot_home() / "node" / "bin")
                recheck = shutil.which("agent-browser", path=clawbot_node_bin)
            if not recheck:
                clawbot_node_root = str(get_clawbot_home() / "node")
                recheck = shutil.which("agent-browser", path=clawbot_node_root)
            if recheck:
...
def _run_browser_command(
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this clawbot PID as the session owner (cross-process safe
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = clawbot_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    from clawbot_constants import get_clawbot_dir
    screenshots_dir = get_clawbot_dir("cache/screenshots", "browser_screenshots")
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/aayushsoam/clawbot-agent:latest")
                else:
