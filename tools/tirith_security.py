...
it is automatically downloaded from GitHub releases to $CLAWBOT_HOME/bin/tirith.
...
def _get_clawbot_home() -> str:
    """Return the Clawbot home directory, respecting CLAWBOT_HOME env var."""
    return str(get_clawbot_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_clawbot_home(), ".tirith-install-failed")
...
def _clawbot_bin_dir() -> str:
    """Return $CLAWBOT_HOME/bin, creating it if needed."""
    d = os.path.join(_get_clawbot_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CLAWBOT_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_clawbot_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CLAWBOT_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CLAWBOT_HOME/bin/tirith
    ...
    clawbot_bin = os.path.join(_clawbot_bin_dir(), "tirith")
    if os.path.isfile(clawbot_bin) and os.access(clawbot_bin, os.X_OK):
        _resolved_path = clawbot_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return clawbot_bin
...
def ensure_installed(*, log_failures: bool = True):
    ...
    Quick PATH/local checks are synchrosoam; network download runs in a
    ...
    # Explicit path: synchrosoam check only, no download
    if explicit:
        if os.path.isfile(expanded) and os.access(expanded, os.X_OK):
            _resolved_path = expanded
            return expanded
