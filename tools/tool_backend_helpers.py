...
def managed_soam_tools_enabled() -> bool:
    """Return True when the user has an active paid Soam subscription.

    The Tool Gateway is available to any Soam subscriber who is NOT on
    ...
    try:
        ...
        from clawbot_cli.models import check_soam_free_tier

        if check_soam_free_tier():
...
def resolve_modal_backend_state(
    ...
    *,
    ...
    managed_mode_blocked = (
        requested_mode == "managed" and not managed_soam_tools_enabled()
    )

    if normalized_mode == "managed":
        selected_backend = "managed" if managed_soam_tools_enabled() and managed_ready else None
    elif normalized_mode == "direct":
        selected_backend = "direct" if has_direct else None
    else:
        selected_backend = "managed" if managed_soam_tools_enabled() and managed_ready else "direct" if has_direct else None
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.clawbot/.env`` (via
    ``clawbot_cli.config.get_env_value`` when available) so tool-side
