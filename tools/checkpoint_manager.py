...
-----------------------------------------------------------------------------
    ...
    ~/.clawbot/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/clawbot/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_clawbot_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/clawbot"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Clawbot convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("CLAWBOT_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(
    ...
    The shared store is internal Clawbot infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``clawbot checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `clawbot checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "clawbot@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Clawbot Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``CLAWBOT_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at CLAWBOT_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "CLAWBOT_WORKDIR").write_text(
    ...
    * its ``last_touch`` is older than ``retention_days`` days.
    ...
    for child in base.iterdir():
        ...
        if delete_orphans:
            ...
            wd_marker = child / "CLAWBOT_WORKDIR"
            if wd_marker.exists():
                try:
...
# ---------------------------------------------------------------------------
# Public helpers for `clawbot checkpoints` CLI
# ---------------------------------------------------------------------------
