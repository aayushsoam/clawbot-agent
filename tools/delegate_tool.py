...
# Must match clawbot_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
# (clawbot-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("clawbot-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``clawbot-cli`` (which bundles
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on Soam Portal).
    """
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. clawbot-cli) so that individual
    ...
    # Extract parent's API key so subagents inherit auth (e.g. Soam Portal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.clawbot/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, soam, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'clawbot auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (clawbot_cli/config.py load_config()) so that
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "notify_on_complete=True) instead. delegate_task runs SYNCHROSOAMLY "
        ...
        "properties": {
            ...
            "acp_command": {
                ...
                "description": (
                    ...
                    "Leave empty to use the parent's default transport (Clawbot subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
