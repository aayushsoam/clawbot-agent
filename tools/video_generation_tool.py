...
provider** — turn on a backend by enabling a plugin (``clawbot plugins
enable video_gen/<name>``) and selecting it in ``clawbot tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/clawbot-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `clawbot tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `clawbot tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `clawbot tools` → "
...
# `clawbot tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`clawbot tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `clawbot tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}
