"""disk_cleanup — ephemeral file cleanup for Clawbot Agent.

Library module wrapping the deterministic cleanup rules maintained by @aayushsoam. The plugin ``__init__.py`` wires these
...
Rules:
  ...
  - empty dirs    → always delete (under CLAWBOT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly CLAWBOT_HOME and /tmp/clawbot-*
Never touches: ~/.clawbot/logs/ or any system directory.
"""
...
try:
    ...
    def get_clawbot_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("CLAWBOT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".clawbot").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$CLAWBOT_HOME/logs/``."""
    return get_clawbot_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$CLAWBOT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under CLAWBOT_HOME or ``/tmp/clawbot-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    clawbot_home = get_clawbot_home()
    try:
        path.resolve().relative_to(clawbot_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/clawbot-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("clawbot-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside CLAWBOT_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under CLAWBOT_HOME (but leave CLAWBOT_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "clawbot-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(clawbot_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == clawbot_home:
                continue
            try:
                rel_parts = dirpath.relative_to(clawbot_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    clawbot_home = get_clawbot_home()
    try:
        rel = path.resolve().relative_to(clawbot_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "clawbot-agent",
        }:
            return None
        ...
        # Path isn't under CLAWBOT_HOME (e.g. /tmp/clawbot-*) — fall through.
        pass
