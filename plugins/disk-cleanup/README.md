...
Auto-tracks and cleans up ephemeral files created during Clawbot Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$CLAWBOT_HOME` and `/tmp/clawbot-*`.

Maintained by [@aayushsoam](https://github.com/aayushsoam) as a
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `CLAWBOT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `CLAWBOT_HOME` or `/tmp/clawbot-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$CLAWBOT_HOME/disk-cleanup/` is itself excluded
- `$CLAWBOT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
