...
``BROWSER_USE_API_KEY`` for self-billed users, or the managed Soam tool
gateway (which Clawbot uses to bill Browser Use sessions to a Soam
...
Auth env vars (one of)::
    ...
    # OR a managed Soam gateway entry (configured via 'clawbot setup')
"""
...
# Idempotency tracking for managed-mode session creation. The managed Soam
...
class BrowserUseBrowserProvider(BrowserProvider):
    ...
    to the managed Soam tool gateway when ``tool_gateway.browser`` config
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Soam gateway)
    # ------------------------------------------------------------------

    def _get_config_or_none(self) -> Optional[Dict[str, Any]]:
        # Import here to avoid a hard dependency at module-import time —
        # managed_tool_gateway pulls in the Soam auth stack which can be
        ...
        # managed Soam gateway via ``tool_gateway.browser: gateway``.
        ...
        return {
            "api_key": managed.soam_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when Clawbot only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"clawbot_{task_id}_{uuid.uuid4().hex[:8]}"
        external_call_id = (
