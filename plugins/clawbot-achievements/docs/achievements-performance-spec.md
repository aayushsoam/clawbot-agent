# Clawbot Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: clawbot-achievements plugin
...
### `GET /api/plugins/clawbot-achievements/achievements`
...
### `POST /api/plugins/clawbot-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/clawbot-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/clawbot-achievements/overview`
...
- Persist plugin data under:
  - `~/.clawbot/plugins/clawbot-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.clawbot/plugins/clawbot-achievements/scan_checkpoint.json`
...
- `~/.clawbot/plugins/clawbot-achievements/`
