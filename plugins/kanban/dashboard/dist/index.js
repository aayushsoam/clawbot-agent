/**
 * Clawbot Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.clawbot/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__CLAWBOT_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__CLAWBOT_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "clawbot-kanban-dot-triage",
    todo: "clawbot-kanban-dot-todo",
    ready: "clawbot-kanban-dot-ready",
    running: "clawbot-kanban-dot-running",
    blocked: "clawbot-kanban-dot-blocked",
    done: "clawbot-kanban-dot-done",
    archived: "clawbot-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-clawbot-task";
  ...
  const DOCS_URL = "https://clawbot-agent.aayushsoam.com/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://clawbot-agent.aayushsoam.com/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "clawbot.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="clawbot-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "clawbot-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "clawbot-kanban-md",
  ...
  // standard `drop` event and our `clawbot-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("clawbot-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (col !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("clawbot-kanban-column--drop");
          if (col) col.classList.add("clawbot-kanban-column--drop");
          lastTarget = col;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("clawbot-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          lastTarget.dispatchEvent(new CustomEvent("clawbot-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        const token = window.__CLAWBOT_SESSION_TOKEN__ || "";
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "clawbot-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "clawbot-kanban-attention",
        "clawbot-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "clawbot-kanban-attention-bar" },
        h("span", { className: "clawbot-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "clawbot-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "clawbot-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "clawbot-kanban-attention-row",
                  "clawbot-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "clawbot-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "clawbot-kanban-attention-row-id" }, task.id),
                h("span", { className: "clawbot-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "clawbot-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "clawbot-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "clawbot-kanban-diag-action-btn",
      action.suggested ? "clawbot-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " clawbot-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".clawbot-kanban-drawer-comment-row input, .clawbot-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "clawbot-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("clawbot-kanban-diag", sevClass) },
      h("div", { className: "clawbot-kanban-diag-header" },
        h("span", { className: "clawbot-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "clawbot-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "clawbot-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "clawbot-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "clawbot-kanban-diag-data-row" },
                  h("span", { className: "clawbot-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "clawbot-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "clawbot-kanban-diag-data-row" },
                h("span", { className: "clawbot-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "clawbot-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "clawbot-kanban-diag-reassign-row" },
            h("span", { className: "clawbot-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "clawbot-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "clawbot-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "clawbot-kanban-diag-msg",
              msg.ok ? "clawbot-kanban-diag-msg--ok" : "clawbot-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "clawbot-kanban-section" },
      h("div", { className: "clawbot-kanban-section-head-row" },
        h("span", { className: "clawbot-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "clawbot-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "clawbot-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "clawbot-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "clawbot-kanban-docs-link",
      title: "Open Clawbot Kanban docs in a new tab",
      "aria-label": "Clawbot Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `clawbot kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "clawbot-kanban-msg-ok" : "clawbot-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "clawbot-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "clawbot-kanban-boardswitcher" },
      h("div", { className: "clawbot-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "clawbot-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "clawbot-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "clawbot-kanban-dialog-title" },
        ...
        h("div", { className: "clawbot-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Clawbot profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "clawbot-kanban-bulk" },
      h("span", { className: "clawbot-kanban-bulk-count" },
      ...
      h("div", { className: "clawbot-kanban-bulk-priority",
      ...
      h("div", { className: "clawbot-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Clawbot profile. Pick a profile (or unassign) and click Apply." },
        h(Select, {
          value: assignee,
      ...
      h("label", { className: "clawbot-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h("input", {
          type: "checkbox",
  ...
  function BoardColumns(props) {
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".clawbot-kanban-card");
      if (!card) return;
    ...
    return h("div", { className: "clawbot-kanban-columns", onDragStart: handleDragStart, onDragEnd: handleDragEnd },
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("clawbot-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("clawbot-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "clawbot-kanban-column",
        dragOver ? "clawbot-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "clawbot-kanban-column-header",
                 title: colHelp || "" },
        h("input", {
          type: "checkbox",
          className: "clawbot-kanban-col-check",
        ...
        h("span", { className: cn("clawbot-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "clawbot-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "clawbot-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "clawbot-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "clawbot-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "clawbot-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "clawbot-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "clawbot-kanban-lane" },
                  h("div", { className: "clawbot-kanban-lane-head" },
                    h("span", { className: "clawbot-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "clawbot-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "clawbot-kanban-card--stale-red";
    if (age >= tier.amber) return "clawbot-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".clawbot-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "clawbot-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "clawbot-kanban-card",
        props.selected ? "clawbot-kanban-card--selected" : "",
        props.failed ? "clawbot-kanban-card--failed" : "",
        props.draggingSource ? "clawbot-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "clawbot-kanban-card-content" },
          h("div", { className: "clawbot-kanban-card-row" },
            h("label", {
              className: "clawbot-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h("input", {
                type: "checkbox",
                className: "clawbot-kanban-card-check",
            ...
            h("span", { className: "clawbot-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or clawbot kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "clawbot-kanban-warning-badge",
                    "clawbot-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "clawbot-kanban-needs-assignee",
          ...
          h("div", { className: "clawbot-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "clawbot-kanban-card-row clawbot-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "clawbot-kanban-assignee",
                            title: `Assigned to Clawbot profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "clawbot-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "clawbot-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "clawbot-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "clawbot-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Clawbot profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Clawbot profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "clawbot-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "clawbot-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "clawbot-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "clawbot-kanban-drawer-close",
        ...
        data ? h("div", { className: "clawbot-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "clawbot-kanban-drawer-body" },
      h("div", { className: "clawbot-kanban-drawer-title" },
        h("span", { className: cn("clawbot-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "clawbot-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "clawbot-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "clawbot-kanban-section" },
        h("div", { className: "clawbot-kanban-section-head" }, tx(i18n, "result", "Result")),
        h(MarkdownBlock, { source: t.result, enabled: props.renderMarkdown }),
      ) : null,
      h("div", { className: "clawbot-kanban-section" },
        h("div", { className: "clawbot-kanban-section-head" },
          `${tx(i18n, "comments", "Comments")} (${comments.length})`),
        comments.length === 0
          ? h("div", { className: "text-xs text-muted-foreground" },
              tx(i18n, "noComments", "— no comments —"))
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "clawbot-kanban-comment" },
                h("div", { className: "clawbot-kanban-comment-head" },
                  h("span", { className: "clawbot-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "clawbot-kanban-comment-ago" },
      ...
      h("div", { className: "clawbot-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "clawbot-kanban-event",
              isDiag ? "clawbot-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "clawbot-kanban-event-header" },
                  h("span", { className: "clawbot-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "clawbot-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "clawbot-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "clawbot-kanban-event-header-plain" },
                  h("span", { className: "clawbot-kanban-event-kind" }, e.kind),
                  h("span", { className: "clawbot-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "clawbot-kanban-event-phantom-row" },
                  h("span", { className: "clawbot-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "clawbot-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "clawbot-kanban-event-payload" },
    ...
    return h("div", { className: "clawbot-kanban-section" },
      h("div", { className: "clawbot-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "clawbot-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `clawbot-kanban-run--${r.outcome || r.status || "ended"}`
          : "clawbot-kanban-run--active";
        return h("div", { key: r.id, className: cn("clawbot-kanban-run", outcomeClass) },
          h("div", { className: "clawbot-kanban-run-head" },
            h("span", { className: "clawbot-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "clawbot-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "clawbot-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "clawbot-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "clawbot-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "clawbot-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "clawbot-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "clawbot-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "clawbot-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "clawbot-kanban-pre clawbot-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "clawbot-kanban-meta-row" },
      h("span", { className: "clawbot-kanban-meta-label" }, props.label),
      h("span", { className: "clawbot-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "clawbot-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clawbot-kanban-meta-row" },
        h("span", { className: "clawbot-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "clawbot-kanban-meta-value clawbot-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "clawbot-kanban-meta-row" },
        h("span", { className: "clawbot-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "clawbot-kanban-meta-value clawbot-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "clawbot-kanban-section" },
      h("div", { className: "clawbot-kanban-section-head-row" },
        h("span", { className: "clawbot-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "clawbot-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "clawbot-kanban-section" },
      h("div", { className: "clawbot-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "clawbot-kanban-deps-row" },
        h("span", { className: "clawbot-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "clawbot-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "clawbot-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "clawbot-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "clawbot-kanban-dep-chip-x",
      ...
      h("div", { className: "clawbot-kanban-deps-row" },
        h("span", { className: "clawbot-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "clawbot-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "clawbot-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "clawbot-kanban-msg-ok"
          : "clawbot-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "clawbot-kanban-section" },
      ...
      h("div", { className: "clawbot-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "clawbot-kanban-home-sub clawbot-kanban-home-sub--on"
              : "clawbot-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__CLAWBOT_PLUGINS__ && typeof window.__CLAWBOT_PLUGINS__.register === "function") {
    window.__CLAWBOT_PLUGINS__.register("kanban", KanbanPage);
  }
})();
