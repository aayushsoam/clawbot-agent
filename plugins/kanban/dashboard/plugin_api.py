...
``clawbot_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
own pages inject it via ``window.__CLAWBOT_SESSION_TOKEN__`` so logged-in
...
the in-browser PTY bridge in ``clawbot_cli/web_server.py``.

This means ``clawbot dashboard --host 0.0.0.0`` is safe to run on a LAN:
...
from clawbot_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from clawbot_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``clawbot_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from clawbot_cli.kanban_diagnostics import SEVERITY_ORDER
...
def get_board(
    ...
    through to the active board (``CLAWBOT_KANBAN_BOARD`` env → on-disk
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from clawbot_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``clawbot kanban
...
# spawn failures, stuck-blocked). See clawbot_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def list_diagnostics(
    ...
    AND for ``clawbot kanban diagnostics`` which shells to this
...
def reclaim_task_endpoint(
    ...
    ``clawbot kanban reclaim <task_id> --reason ...``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
def specify_task_endpoint(
    ...
    it to ``todo``. Maps 1:1 to ``clawbot kanban specify <task_id>``.
    ...
    try:
        os.environ["CLAWBOT_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        # Import lazily so a missing auxiliary client at import time
        # doesn't break plugin load.
        from clawbot_cli import kanban_specify  # noqa: WPS433 (intentional)
...
def reassign_task_endpoint(
    ...
    Maps 1:1 to ``clawbot kanban reassign <task_id> <profile> [--reclaim]``.
    """
    board = _resolve_board(board)
    conn = _conn(board=board)
    try:
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.clawbot/config.yaml.
...
def get_assignees(board: Optional[str] = Query(None)):
    ...
    Returns the union of ``~/.clawbot/profiles/*`` on disk and every
...
def update_profile_description(profile_name: str, payload: DescribeBody):
    ...
    try:
        ...
        if canon == "default":
            from clawbot_constants import get_clawbot_home  # type: ignore
            from pathlib import Path as _Path
            profile_dir = _Path(get_clawbot_home())
        else:
...
def auto_describe_profile(profile_name: str, payload: DescribeAutoBody):
    ...
    Maps 1:1 to ``clawbot profile describe <name> --auto``. Non-OK
    ...
    """
    try:
        from clawbot_cli import profile_describer  # noqa: WPS433 (intentional)
...
def decompose_task_endpoint(
    ...
    1:1 to ``clawbot kanban decompose <task_id>``.
    ...
    try:
        os.environ["CLAWBOT_KANBAN_BOARD"] = board or kanban_db.DEFAULT_BOARD
        from clawbot_cli import kanban_decompose  # noqa: WPS433 (intentional)
...
def set_orchestration_settings(payload: OrchestrationSettingsBody):
    """Update the kanban orchestration knobs in ~/.clawbot/config.yaml.
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in clawbot_cli/web_server.py.
