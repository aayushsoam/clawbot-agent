...
``_check_spotify_available()`` — when the user has not run ``clawbot auth
spotify``, the tools remain registered (so they appear in ``clawbot
...
The Spotify auth flow (``clawbot auth spotify``), CLI plumbing, and docs
