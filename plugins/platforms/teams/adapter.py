"""
Microsoft Teams platform adapter for Clawbot Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Clawbot"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        clawbot_action = data.get("clawbot_action", "")
        session_key = data.get("session_key", "")

        if not clawbot_action or not session_key:
        ...
        choice = choice_map.get(clawbot_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
    ...
    async def send_exec_approval(
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="clawbot_approve",
                    data={**btn_data_base, "clawbot_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="clawbot_approve",
                    data={**btn_data_base, "clawbot_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="clawbot_approve",
                    data={**btn_data_base, "clawbot_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="clawbot_approve",
                    data={**btn_data_base, "clawbot_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Clawbot\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.clawbot/.env")
