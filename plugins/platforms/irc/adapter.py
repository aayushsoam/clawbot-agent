"""
IRC Platform Adapter for Clawbot Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Clawbot agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "clawbot-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Clawbot Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Clawbot Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: clawbot_, clawbot_1, clawbot_2...
...
def check_requirements() -> bool:
    ...
    # clawbot setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `clawbot gateway setup` flow for the IRC platform.

    Lazy-imports ``clawbot_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Clawbot to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #clawbot — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.clawbot/.env")
    print_info("Restart the gateway for changes to take effect: clawbot gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "clawbot-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "clawbot-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Clawbot Agent (cron)")
