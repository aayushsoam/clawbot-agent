"""
LINE Messaging API platform adapter for Clawbot Agent.
...
to Clawbot Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``CLAWBOT_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            clawbot_home = Path.home().joinpath(".clawbot").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``clawbot status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``clawbot status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clawbot setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.clawbot/.env`` via ``clawbot_cli.config``.
    """
    print()
    ...
    try:
        from clawbot_cli.config import get_env_var, set_env_var
    except ImportError:
        print("clawbot_cli.config not available; set LINE_* vars manually in ~/.clawbot/.env")
        return
