...
googleapiclient is synchrosoam. This keeps the event loop responsive.
...
# ``model_tools`` import time, so ``clawbot status``, ``clawbot chat``, etc.
...
# ``clawbot_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Clawbot is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a clawbot MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${CLAWBOT_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _clawbot_home = _get_clawbot_home()
        except (ModuleNotFoundError, ImportError):
            _clawbot_home = _Path.home() / ".clawbot"
        self._thread_count_store = _ThreadCountStore(
            _clawbot_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Clawbot is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("CLAWBOT_HOME", str(_Path.home() / ".clawbot"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${CLAWBOT_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a clawbot MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Clawbot is thinking…' marker message.
        ...
        "Clawbot is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Clawbot is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Clawbot is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Clawbot is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Clawbot is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``clawbot setup``.

    The setup wizard at ``clawbot_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.clawbot/.env`` so the next
    gateway restart picks them up.
    """
    from clawbot_cli.cli_output import (
        print_info,
        print_success,
        print_warning,
        prompt,
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. clawbot-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.clawbot/.env")
    print_info("Restart the gateway: clawbot gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``clawbot cron`` running as a
    separate process from ``clawbot gateway``).  Without this hook,
    ...
    # Bound the synchrosoam urllib3-backed token refresh so a hung Google
...
def register(ctx) -> None:
    """Plugin entry point — called by the Clawbot plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'clawbot-agent[google_chat]'",
        ...
        # the "Clawbot is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Clawbot is thinking…' marker message "
