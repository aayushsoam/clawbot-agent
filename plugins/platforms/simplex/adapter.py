"""SimpleX Chat platform adapter (Clawbot plugin).
...
This adapter ships as a Clawbot platform plugin under
``plugins/platforms/simplex/``. The Clawbot plugin loader scans the
...
discoverable and `clawbot setup` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Clawbot — no
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        payload = {
            "corrId": f"clawbot-snd-{int(time.time() * 1000)}",
            "cmd": cmd_str,
        }
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``clawbot setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / home channel.
    Writes to ``~/.clawbot/.env`` via ``clawbot_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("clawbot_cli.config not available; set SIMPLEX_* vars manually in ~/.clawbot/.env")
        return
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        # env-only setups show up in `clawbot gateway status` without
