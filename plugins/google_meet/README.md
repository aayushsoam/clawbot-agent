...
Let the clawbot agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where clawbot runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `clawbot meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `clawbot meet` CLI |
...
| `cli.py` | `clawbot meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$CLAWBOT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `clawbot meet node {run,list,approve,remove,status,ping}` |
...
clawbot meet install                                      # pip + Chromium
clawbot meet setup                                        # preflight
clawbot meet auth                                         # optional
clawbot meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
clawbot meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.clawbot/.env
clawbot meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
clawbot meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
clawbot meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.clawbot/.env
clawbot meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, clawbot will **not** switch your system audio input automatically — the
...
clawbot plugins enable google_meet
clawbot meet node run --display-name my-mac --host 0.0.0.0 --port 18789
