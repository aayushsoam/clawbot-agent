"""`clawbot meet node ...` subcommand tree.

Wired into the existing ``clawbot meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``clawbot meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="clawbot-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``clawbot meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             clawbot meet node approve <name> ws://<host>:{args.port} {token}")
        try:
