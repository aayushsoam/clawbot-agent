...
session-scoped state file under ``$CLAWBOT_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $CLAWBOT_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_clawbot_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this clawbot install, leave it first —
    ...
    env = os.environ.copy()
    env["CLAWBOT_MEET_URL"] = url
    env["CLAWBOT_MEET_OUT_DIR"] = str(out)
    env["CLAWBOT_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["CLAWBOT_MEET_HEADED"] = "1"
    if auth_state:
        env["CLAWBOT_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["CLAWBOT_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if CLAWBOT_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["CLAWBOT_MEET_MODE"] = mode
    if realtime_model:
        env["CLAWBOT_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["CLAWBOT_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["CLAWBOT_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["CLAWBOT_MEET_REALTIME_KEY"] = realtime_api_key
