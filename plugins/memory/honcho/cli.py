...
Handles: clawbot honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "clawbot" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Clawbot profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'clawbot honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'clawbot honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `clawbot update` -- no output, no exceptions.
    """
    try:
        from clawbot_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Clawbot profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CLAWBOT_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_clawbot_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Clawbot persistent cross-session memory.")
    ...
    clawbot_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'clawbot honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = clawbot_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        clawbot_host["peerName"] = new_peer

    current_ai = clawbot_host.get("aiPeer") or cfg.get("aiPeer", "clawbot")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        clawbot_host["aiPeer"] = new_ai

    current_workspace = clawbot_host.get("workspace") or cfg.get("workspace", "clawbot")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        clawbot_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = clawbot_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in {"unified", "directional"}:
        clawbot_host["observationMode"] = new_obs
    else:
        clawbot_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(clawbot_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        clawbot_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        clawbot_host["writeFrequency"] = new_wf if new_wf in {"async", "turn", "session"} else "async"

    # --- 6. Recall mode ---
    _raw_recall = clawbot_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in {"hybrid", "context", "tools"}:
        clawbot_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = clawbot_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in {"none", "uncapped", "no limit"}:
        clawbot_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                clawbot_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(clawbot_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            clawbot_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        clawbot_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        clawbot_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in {"minimal", "low", "medium", "high", "max"}:
        clawbot_host["dialecticReasoningLevel"] = new_reasoning
    else:
        clawbot_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = clawbot_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in {"per-session", "per-repo", "per-directory", "global"}:
        clawbot_host["sessionStrategy"] = new_strat

    clawbot_host["enabled"] = True
    clawbot_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        clawbot_config = load_config()
        clawbot_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(clawbot_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: clawbot config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    clawbot honcho status     -- show full config")
    print("    clawbot honcho mode       -- change recall/observation mode")
    print("    clawbot honcho tokens     -- tune context and dialectic budgets")
    print("    clawbot honcho peer       -- update peer names")
    print("    clawbot honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Clawbot profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: clawbot honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'clawbot honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: clawbot honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        clawbot = hosts.get(_host_key(), {})
        user = clawbot.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = clawbot.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = clawbot.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = clawbot.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Clawbot' identity in Honcho. Seed with 'clawbot honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: clawbot honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: clawbot honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = clawbot.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = clawbot.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = clawbot.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Clawbot asks Honcho's AI peer a question")
        ...
        print("\n  Set with: clawbot honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'clawbot honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'clawbot honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    clawbot honcho identity --show        — show both peer representations")
        print("    clawbot honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Clawbot + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Clawbot\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Clawbot persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  clawbot honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'clawbot honcho setup' now?", default="y")
        if answer.lower() in {"y", "yes"}:
            ...
            print()
            print("  Run 'clawbot honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  clawbot honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'clawbot'")
        print("  with Honcho configured and no prior session history.")
        print("  (Clawbot calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    clawbot honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'clawbot honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Clawbot, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'clawbot honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    clawbot honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  clawbot honcho identity <path/to/SOUL.md>")
    ...
    print("    Clawbot:   cloud-backed Honcho peers. Files can stay on disk as source")
    print("              of truth; Honcho holds the live representation.")
    print()
    print("  Context injection")
    print("    OpenClaw: file excerpts injected synchrosoamly before each LLM call.")
    print("    Clawbot:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Clawbot:   Honcho observes every message and updates representations")
    ...
    print("    Clawbot:   per-session by default — each run gets its own session")
    print("              Map a custom name:  clawbot honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. clawbot honcho setup              — configure API key (required)")
        print("  2. clawbot honcho migrate            — re-run this walkthrough")
    else:
        print("  1. clawbot honcho status             — verify Honcho connection")
        print("  2. clawbot                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. clawbot honcho identity --show    — verify AI peer representation")
        print("  4. clawbot honcho tokens             — tune context and dialectic budgets")
        print("  5. clawbot honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'clawbot memory setup'...\n")
        from clawbot_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``clawbot honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``clawbot honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to clawbot memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Clawbot Honcho",
    )
