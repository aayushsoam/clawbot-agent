...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/clawbot>
...
```bash
clawbot honcho setup    # full interactive wizard (cloud or local)
clawbot memory setup    # generic picker, also works
```

Or manually:
```bash
clawbot config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.clawbot/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$CLAWBOT_HOME/honcho.json` | Profile-local (isolated Clawbot instances) |
| 2 | `~/.clawbot/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Clawbot profile: `clawbot` (default) or `clawbot.<profile>`.
...
| 4 | `per-session` strategy | Clawbot session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`clawbot-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`clawbot`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-clawbot-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening clawbot in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Clawbot session ID (timestamp + hex). Every `clawbot` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Clawbot profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`clawbot`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Clawbot profile: `clawbot` (default) or `clawbot.<profile>` (e.g. `clawbot -p coder` → host key `clawbot.coder`).
...
| `CLAWBOT_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `clawbot honcho setup` | Full interactive setup wizard |
| `clawbot honcho status` | Show resolved config for active profile |
| `clawbot honcho enable` / `disable` | Toggle Honcho for active profile |
| `clawbot honcho mode <mode>` | Change recall or observation mode |
| `clawbot honcho peer --user <name>` | Update user peer name |
| `clawbot honcho peer --ai <name>` | Update AI peer name |
| `clawbot honcho tokens --context <N>` | Set context token budget |
| `clawbot honcho tokens --dialectic <N>` | Set dialectic max chars |
| `clawbot honcho map <name>` | Map current directory to a session name |
| `clawbot honcho sync` | Create host blocks for all Clawbot profiles |
