...
Config file: `$CLAWBOT_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `clawbot` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `clawbot-{identity}` → `clawbot-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per Clawbot profile:

```json
{
  "container_tag": "clawbot-{identity}"
}
```

For a profile named `coder`, this resolves to `clawbot-coder`. The default profile resolves to `clawbot-default`. Without `{identity}`, all profiles share the same container.
