...
_DEFAULT_CONTAINER_TAG = "clawbot"
...
def _load_supermemory_config(clawbot_home: str) -> dict:
    config = _default_config()
    config_path = Path(clawbot_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, clawbot_home: str) -> None:
    config_path = Path(clawbot_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `clawbot memory setup`.
        # All other options are documented for $CLAWBOT_HOME/supermemory.json
    ...
    def save_config(self, values, clawbot_home):
        ...
        _save_supermemory_config(sanitized, clawbot_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from clawbot_constants import get_clawbot_home
        self._clawbot_home = kwargs.get("clawbot_home") or str(get_clawbot_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._clawbot_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "clawbot", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "clawbot_memory", "target": target, "type": "explicit_memory"},
