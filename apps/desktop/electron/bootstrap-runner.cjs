 ...
 *
 * Drives apps/desktop's first-launch install of Clawbot Agent by spawning
...
function resolveLocalInstallScript(sourceRepoRoot) {
  ...
  const override = readableFile(process.env.CLAWBOT_DESKTOP_INSTALL_SCRIPT)
  if (override) return override

  const candidates = []
  if (sourceRepoRoot) candidates.push(path.join(sourceRepoRoot, 'scripts', scriptName))
  if (process.resourcesPath) candidates.push(path.join(process.resourcesPath, 'scripts', scriptName))
  candidates.push(path.resolve(__dirname, '..', '..', '..', 'scripts', scriptName))

  for (const candidate of candidates) {
    const resolved = readableFile(candidate)
    if (resolved) return resolved
  }
  return null
}

function bootstrapCacheDir(clawbotHome) {
  return path.join(clawbotHome, 'bootstrap-cache')
}

function cachedScriptPath(clawbotHome, commit) {
  return path.join(bootstrapCacheDir(clawbotHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const scriptName = installScriptName()
  const url = `https://raw.githubusercontent.com/ClawbotAgent/clawbot-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, clawbotHome, emit }) {
  ...
  const cached = cachedScriptPath(clawbotHome, installStamp.commit)
  try {
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, clawbotHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, {
      ...
      env: {
        ...process.env,
        // Pass CLAWBOT_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        CLAWBOT_HOME: clawbotHome || process.env.CLAWBOT_HOME || ''
      }
    })
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, clawbotHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, clawbotHome }) {
  const args = ['--dir', activeRoot, '--clawbot-home', clawbotHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, clawbotHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, clawbotHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, clawbotHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, clawbotHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(clawbotHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, clawbotHome, emit })
