...
const USER_DATA_OVERRIDE = process.env.CLAWBOT_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.CLAWBOT_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with CLAWBOT_DESKTOP_DISABLE_GPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[clawbot] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[clawbot] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[clawbot] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// CLAWBOT_HOME — the user-facing root for everything Clawbot-related. Mirrors
// scripts/install.ps1's $ClawbotHome and scripts/install.sh's $CLAWBOT_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\clawbot (matches install.ps1)
//   macOS / Linux: ~/.clawbot (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.clawbot directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\clawbot yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// CLAWBOT_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// CLAWBOT_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.clawbot / %LOCALAPPDATA%\clawbot.
function resolveClawbotHome() {
  if (process.env.CLAWBOT_HOME) return path.resolve(process.env.CLAWBOT_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'clawbot-home')
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'clawbot')
    const legacy = path.join(app.getPath('home'), '.clawbot')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.clawbot setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.clawbot')
}

const CLAWBOT_HOME = resolveClawbotHome()
// ACTIVE_CLAWBOT_ROOT — the canonical mutable Clawbot install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_CLAWBOT_ROOT = path.join(CLAWBOT_HOME, 'clawbot-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_CLAWBOT_ROOT, 'venv')
...
// boots, going straight to `resolveClawbotBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_CLAWBOT_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_CLAWBOT_ROOT, '.clawbot-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Clawbot profile the desktop launches its
// local backend as. When set, startClawbot() passes `clawbot --profile <name>
// dashboard …`, which deterministically pins CLAWBOT_HOME (see
// _apply_profile_override in clawbot_cli/main.py) and bypasses the sticky
// ~/.clawbot/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors clawbot_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// clawbotDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under CLAWBOT_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by clawbot_logging.setup_logging — one log
...
app.setAboutPanelOptions({
  ...
  copyright: 'Copyright © 2026 Clawbot Agent'
})
...
const MEDIA_PROTOCOL = 'clawbot-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(clawbot-media://…)`.
...
let mainWindow = null
let clawbotProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by clawbotProcess +
// connectionPromise + startClawbot(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.CLAWBOT_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.CLAWBOT_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startClawbot() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Clawbot backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (!text) return
  const lines = text.split(/\r?\n/).map(line => `[clawbot] ${line}`)
  clawbotLog.push(...lines)
  if (clawbotLog.length > 300) {
    clawbotLog.splice(0, clawbotLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-clawbot-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('clawbot:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the clawbot:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clawbot:bootstrap:event', ev)
}
...
function isClawbotSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'clawbot_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.CLAWBOT_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Clawbot deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Clawbot' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\clawbot\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clawbot', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'clawbot', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\clawbot\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'clawbot', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'clawbot', 'git', 'bin', 'git.exe'))
  }
...
function recentClawbotLog() {
  return clawbotLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's clawbot root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_CLAWBOT_ROOT.
// CLAWBOT_DESKTOP_CLAWBOT_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.CLAWBOT_DESKTOP_CLAWBOT_ROOT && path.resolve(process.env.CLAWBOT_DESKTOP_CLAWBOT_ROOT),
    !IS_PACKAGED && isClawbotSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isClawbotSourceRoot(ACTIVE_CLAWBOT_ROOT) ? ACTIVE_CLAWBOT_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_CLAWBOT_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('clawbot:updates:progress', merged)
  }
}
...
// CLAWBOT_HOME/clawbot-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_clawbot_home). That binary owns
// ALL repo mutation — running `clawbot update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'clawbot-setup.exe' : 'clawbot-setup'
  const candidate = path.join(CLAWBOT_HOME, name)
...
// Path to the venv shim whose lock decides whether `clawbot update` can write
// fresh entry points. On Windows this is the file the running backend
// `clawbot.exe` holds open; on POSIX it's never mandatory-locked.
function venvClawbotShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'clawbot.exe')
    : path.join(updateRoot, 'venv', 'bin', 'clawbot')
}
...
// only signals the direct child, so on Windows a backend `clawbot.exe` that
// spawned its own grandchildren (a `clawbot` REPL, a pty terminal session, the
...
// `clawbotProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `clawbot.exe`, the quarantine
...
async function releaseBackendLockForUpdate(updateRoot) {
  ...
  const pids = []
  if (clawbotProcess && Number.isInteger(clawbotProcess.pid)) pids.push(clawbotProcess.pid)
  ...
  if (clawbotProcess && !clawbotProcess.killed) {
    try {
  ...
  const shim = venvClawbotShimPath(updateRoot)
...
// `clawbot update`). Instead we spawn the staged Clawbot-Setup binary with
// --update and quit, so it can run `clawbot update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri clawbot-setup. Unlike Windows
      ...
      // whole update itself: `clawbot update` (backend) + `clawbot desktop
    ...
    if (!updater) {
      ...
      // `clawbot desktop`, never the Tauri installer that self-copies
      // clawbot-setup.exe into CLAWBOT_HOME). They DO have a working `clawbot`
      ...
      // checkout they're on — bare `clawbot update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'clawbot update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `clawbot update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `clawbot update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, clawbotRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Clawbot updater…', percent: 100 })
    ...
    // clawbot.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `clawbot update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: CLAWBOT_HOME,
      env: {
        ...process.env,
        CLAWBOT_HOME,
        PATH: [path.join(CLAWBOT_HOME, 'node', 'bin'), venvBin, process.env.PATH].filter(Boolean).join(path.delimiter)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
// Resolve the clawbot CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `clawbot` on PATH.
function resolveClawbotCliBinary(updateRoot) {
  const venvClawbot = path.join(updateRoot, 'venv', 'bin', 'clawbot')
  if (fileExists(venvClawbot)) return venvClawbot
  return findOnPath('clawbot') || null
}
...
// macOS/Linux in-app update: backend (`clawbot update`) + OS-aware GUI rebuild
// (`clawbot desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const clawbot = resolveClawbotCliBinary(updateRoot)
  if (!clawbot) {
    emitUpdateProgress({ stage: 'manual', message: 'clawbot update', percent: null })
    return { ok: true, manual: true, command: 'clawbot update', clawbotRoot: updateRoot }
  }

  // Put the Clawbot-managed Node and the venv on PATH so `clawbot desktop`'s
  // npm build can find them on a machine with no system Node.
  const extraPath = [path.join(CLAWBOT_HOME, 'node', 'bin'), path.join(updateRoot, 'venv', 'bin')]
    .filter(Boolean)
    .join(path.delimiter)
  const env = {
    CLAWBOT_HOME,
  ...
  // `clawbot update` reaps stale `clawbot dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads CLAWBOT_DESKTOP_CHILD_PID and excludes
  ...
  if (clawbotProcess && Number.isInteger(clawbotProcess.pid)) {
    desktopChildPids.push(clawbotProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.CLAWBOT_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Clawbot (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(clawbot, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'clawbot update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'clawbot update failed' }
  }

  emitUpdateProgress({ stage: 'rebuild', message: 'Rebuilding the desktop app…', percent: 60 })
  const rebuilt = await runStreamedUpdate(clawbot, ['desktop', '--build-only'], {
    cwd: updateRoot,
    env,
    stage: 'rebuild'
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Clawbot to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Clawbot.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Clawbot.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Clawbot to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.clawbot-update-new"; then
    rm -rf "$DST.clawbot-update-old" 2>/dev/null || true
    mv "$DST" "$DST.clawbot-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.clawbot-update-new" "$DST"
    rm -rf "$DST.clawbot-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `clawbot-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Clawbot to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `clawbot
  ...
  return isClawbotSourceRoot(ACTIVE_CLAWBOT_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.CLAWBOT_DESKTOP_WEB_DIST
  const candidates = [
    override ? path.resolve(override) : null,
    path.join(ACTIVE_CLAWBOT_ROOT, 'clawbot_cli', 'web_dist'),
    path.join(SOURCE_REPO_ROOT, 'clawbot_cli', 'web_dist'),
    path.join(unpackedPathFor(APP_ROOT), 'dist'),
    path.join(APP_ROOT, 'dist')
  ].filter(Boolean)

  for (const candidate of candidates) {
    if (fileExists(path.join(candidate, 'index.html'))) return candidate
  }
...
function resolveClawbotCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Clawbot.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.CLAWBOT_DESKTOP_CWD,
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'clawbot_cli.main', ...dashboardArgs],
    env: {
...
// createActiveBackend — build a backend pointing at ACTIVE_CLAWBOT_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Clawbot at ${ACTIVE_CLAWBOT_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'clawbot_cli.main', ...dashboardArgs],
    env: {
      PYTHONPATH: [ACTIVE_CLAWBOT_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
    },
    root: ACTIVE_CLAWBOT_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveClawbotBackend(dashboardArgs) {
  // 1. Explicit override -- CLAWBOT_DESKTOP_CLAWBOT_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.CLAWBOT_DESKTOP_CLAWBOT_ROOT && path.resolve(process.env.CLAWBOT_DESKTOP_CLAWBOT_ROOT)
  if (overrideRoot && isClawbotSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Clawbot source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `clawbot` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isClawbotSourceRoot.)
  if (!IS_PACKAGED && isClawbotSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Clawbot source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_CLAWBOT_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\clawbot\clawbot-agent (Windows) or ~/.clawbot/clawbot-agent.
  ...
  //    to spawning clawbot. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `clawbot update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `clawbot` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    CLAWBOT_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.CLAWBOT_DESKTOP_IGNORE_EXISTING !== '1') {
    let clawbotCommand = null
    const clawbotOverride = process.env.CLAWBOT_DESKTOP_CLAWBOT

    if (clawbotOverride) {
      const resolvedOverride = findOnPath(clawbotOverride)
      if (resolvedOverride) {
        clawbotCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(clawbotOverride, { isWsl: IS_WSL })) {
        clawbotCommand = clawbotOverride
      } else {
        rememberLog(`Ignoring Windows Clawbot override under WSL: ${clawbotOverride}`)
      }
    } else {
      clawbotCommand = findOnPath('clawbot')
    }

    if (clawbotCommand) {
      if (looksLikeDesktopAppBinary(clawbotCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Clawbot CLI: ${clawbotCommand}`)
        clawbotCommand = null
      }
    }

    if (clawbotCommand) {
      // Smoke-test the candidate before trusting it. A `clawbot` shim
      ...
      const shellForProbe = isCommandScript(clawbotCommand)
      if (verifyClawbotCli(clawbotCommand, { shell: shellForProbe })) {
        return {
          label: `existing Clawbot CLI at ${clawbotCommand}`,
      ...
      rememberLog(
        `Ignoring existing Clawbot CLI at ${clawbotCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed clawbot_cli module via system Python.
  ...
  if (python) {
    ...
    // having clawbot_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\clawbot\clawbot-agent\venv.
    if (canImportClawbotCli(python)) {
      return {
        kind: 'python',
        label: `installed clawbot_cli module via ${python}`,
        command: python,
        args: ['-m', 'clawbot_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: clawbot_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveClawbotBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Clawbot Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
    ...
    activeRoot: ACTIVE_CLAWBOT_ROOT,
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveClawbotBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Clawbot install found; starting first-launch bootstrap')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      clawbotHome: CLAWBOT_HOME,
      logRoot: path.join(CLAWBOT_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Clawbot install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Clawbot bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(CLAWBOT_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startClawbot() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // clawbot:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveClawbotBackend(backend.args))
  }
  ...
  if (!isClawbotSourceRoot(ACTIVE_CLAWBOT_ROOT)) {
    throw new Error(
      `Clawbot install at ${ACTIVE_CLAWBOT_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Clawbot' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\clawbot\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `clawbot` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Clawbot on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch Clawbot.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // isClawbotSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Clawbot venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Clawbot at ${ACTIVE_CLAWBOT_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Clawbot runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Clawbot backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Clawbot backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Clawbot backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Clawbot-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('clawbot:link-titles', { cache: false })
...
function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveClawbotCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clawbot:preview-file-changed', payload)
}
...
async function waitForClawbot(baseUrl, token) {
  ...
  throw new Error(`Clawbot backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clawbot:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clawbot:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Clawbot...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clawbot:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('clawbot:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('clawbot:window-state-changed', state)
}
...
//
// Hosted Clawbot gateways gate the dashboard behind an OAuth provider (e.g.
// Clawbot Agent) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``clawbot_session_at``),
...
//   * Clawbot Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / clawbot #37247).
//     Both are set as HttpOnly cookies (``clawbot_session_at`` ~15 min,
//     ``clawbot_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:clawbot-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to Clawbot gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Clawbot backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root CLAWBOT_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  return {
    ...
    // scope is never overridden by CLAWBOT_DESKTOP_REMOTE_URL.
    envOverride: key ? false : Boolean(process.env.CLAWBOT_DESKTOP_REMOTE_URL)
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (clawbot #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Clawbot gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Clawbot gateway is selected, but no session token is saved. ' +
...
//   2. env override (CLAWBOT_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.CLAWBOT_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.CLAWBOT_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'CLAWBOT_DESKTOP_REMOTE_URL is set but CLAWBOT_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Clawbot backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.CLAWBOT_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Clawbot
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with Clawbot Agent" instead of a generic
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startClawbot())
  ...
  // connect to Clawbot gateway". Mirror the renderer's connect here so the test
...
function resetClawbotConnection() {
...
// startClawbot() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetClawbotConnection() nulls clawbotProcess.
  const dying = clawbotProcess && !clawbotProcess.killed ? clawbotProcess : null
  resetClawbotConnection()
...
// profile to startClawbot() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startClawbot() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForClawbot(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: clawbotLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  // --profile wins over the inherited CLAWBOT_HOME env (see _apply_profile_override
  // step 3 in clawbot_cli/main.py), so the child re-homes to this profile.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', String(port)]
  const backend = await ensureRuntime(resolveClawbotBackend(dashboardArgs))
  const clawbotCwd = resolveClawbotCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Clawbot backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(backend.command, backend.args, {
    ...
    env: {
      ...
      CLAWBOT_HOME,
      ...backend.env,
      CLAWBOT_DASHBOARD_TUI: '1',
      CLAWBOT_DASHBOARD_SESSION_TOKEN: token,
      CLAWBOT_WEB_DIST: webDist
    },
    shell: backend.shell,
    stdio: ['ignore', 'pipe', 'pipe']
  })
  ...
  child.once('error', error => {
    rememberLog(`Clawbot backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Clawbot backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`Clawbot backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })

  const baseUrl = `http://127.0.0.1:${port}`
  await Promise.race([waitForClawbot(baseUrl, token), startFailed])
  ready = true
...
async function startClawbot() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startClawbot() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Clawbot backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Clawbot backend at ${remote.baseUrl}`, 24)
      await waitForClawbot(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Clawbot backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.clawbot/active_profile file) and
    // resolves CLAWBOT_HOME the same way `clawbot -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Clawbot runtime', 28)
    const backend = await ensureRuntime(resolveClawbotBackend(dashboardArgs))
    const clawbotCwd = resolveClawbotCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Clawbot backend via ${backend.label}`, 84)
    rememberLog(`Starting Clawbot backend via ${backend.label}`)

    clawbotProcess = spawn(backend.command, backend.args, {
      cwd: clawbotCwd,
      env: {
        ...process.env,
        // Explicitly pin CLAWBOT_HOME for the child so Python's get_clawbot_home()
        // resolves to the SAME location our resolveClawbotHome() picked. Without
        // this pin, Python falls back to ~/.clawbot on every platform — fine on
        // mac/linux (where our default matches), but on Windows our default is
        // %LOCALAPPDATA%\clawbot, which differs from C:\Users\<u>\.clawbot.
        // Mismatch would split config / sessions / .env / logs across two
        // directories. install.ps1 sets CLAWBOT_HOME via setx; the desktop
    ...
    clawbotProcess.stdout.on('data', rememberLog)
    clawbotProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    clawbotProcess.once('error', error => {
      rememberLog(`Clawbot backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Clawbot backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    clawbotProcess.once('exit', (code, signal) => {
      rememberLog(`Clawbot backend exited (${signal || code})`)
      clawbotProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Clawbot backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Clawbot backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentClawbotLog()}`
          )
        )
      }
    })
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Clawbot backend to become ready', 90)
    await Promise.race([waitForClawbot(baseUrl, token), backendStartFailed])
    backendReady = true
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Clawbot backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startClawbot().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('clawbot:connection', async (_event, profile) => ensureBackend(profile))
ipcMain.handle('clawbot:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('clawbot:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('clawbot:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startClawbot() call restarts the
...
ipcMain.handle('clawbot:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startClawbot() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('clawbot:bootstrap:cancel', async () => {
...
ipcMain.handle('clawbot:boot-progress:get', async () => bootProgressState)
ipcMain.handle('clawbot:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('clawbot:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('clawbot:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('clawbot:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('clawbot:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('clawbot:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('clawbot:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('clawbot:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('clawbot:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('clawbot:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new CLAWBOT_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('clawbot:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('clawbot:requestMicrophoneAccess', async () => {
...
ipcMain.handle('clawbot:api', async (_event, request) => {
...
ipcMain.handle('clawbot:notify', (_event, payload) => {
  if (!Notification.isSupported()) return false
  new Notification({
    title: payload?.title || 'Clawbot',
    body: payload?.body || '',
...
ipcMain.handle('clawbot:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('clawbot:readFileText', async (_event, filePath) => {
...
ipcMain.handle('clawbot:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('clawbot:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('clawbot:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('clawbot:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('clawbot:saveClipboardImage', async () => {
...
ipcMain.handle('clawbot:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('clawbot:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('clawbot:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('clawbot:titlebar-theme', (_event, payload) => {
...
ipcMain.handle('clawbot:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveClawbotCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('clawbot:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: path.join(app.getPath('home'), 'clawbot-projects')
}))

ipcMain.handle('clawbot:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('clawbot:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('clawbot:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('clawbot:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Clawbot' TUI into
...
function terminalChannel(id, suffix) {
  return `clawbot:terminal:${id}:${suffix}`
}
...
ipcMain.handle('clawbot:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Clawbot.')
  }
...
ipcMain.handle('clawbot:terminal:write', (_event, id, data) => {
...
ipcMain.handle('clawbot:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('clawbot:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('clawbot:updates:check', async () =>
...
ipcMain.handle('clawbot:updates:apply', async (_event, payload) =>
...
ipcMain.handle('clawbot:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('clawbot:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Clawbot version (the one `release.py` bumps in
// clawbot_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Clawbot version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveClawbotVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'clawbot_cli', '__init__.py')
...
ipcMain.handle('clawbot:version', async () => ({
  appVersion: resolveClawbotVersion(),
