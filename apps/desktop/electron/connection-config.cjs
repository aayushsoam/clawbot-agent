 ...
 *     `X-Clawbot-Session-Token` header; WS uses `?token=`.
...
// clawbot_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - clawbot_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - clawbot_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / clawbot #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-clawbot_session_at', '__Secure-clawbot_session_at', 'clawbot_session_at']
const RT_COOKIE_VARIANTS = ['__Host-clawbot_session_rt', '__Secure-clawbot_session_rt', 'clawbot_session_rt']
...
/**
 * True if any cookie in `cookies` is a clawbot session ACCESS-token cookie
