...
// bug that drives the REAL useGatewayBoot hook + REAL ClawbotGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshClawbotConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { clawbotDesktop?: unknown }).clawbotDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { clawbotDesktop?: unknown }).clawbotDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForClawbot) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startClawbot()'s remote branch awaits waitForClawbot() for 45s before it
    ...
    ;(window as { clawbotDesktop?: unknown }).clawbotDesktop = desktop
    ...
    // After ~45s waitForClawbot gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Clawbot backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
