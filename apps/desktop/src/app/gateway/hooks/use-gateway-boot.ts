...
import type { ClawbotConnection } from '@/global'
import { ClawbotGateway } from '@/clawbot'
...
import type { RpcEvent } from '@/types/clawbot'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.clawbotDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: ClawbotGateway | null) => void
  refreshClawbotConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: ClawbotConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Clawbot...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // as an opaque "Could not connect to Clawbot gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshClawbotConfig().catch(() => undefined)
    ...
    const gateway = new ClawbotGateway()
    ...
    const offExit = desktop.onBackendExit(() => {
      if ($desktopBoot.get().running || $desktopBoot.get().visible) {
        failDesktopBoot('Clawbot background process exited during startup.')
      }

      notify({
        kind: 'error',
        title: 'Backend stopped',
        message: 'Clawbot background process exited.',
        durationMs: 0
      })
    })
    ...
    async function boot() {
      try {
        ...
        setDesktopBootStep({
          ...
          message: 'Loading Clawbot settings',
          progress: 97
        })
        await callbacksRef.current.refreshClawbotConfig()
