...
import { getSessionMessages, listAllProfileSessions, type SessionInfo } from '../clawbot'
...
import { useClawbotConfig } from './session/hooks/use-clawbot-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.clawbotDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.clawbotDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.clawbotDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshClawbotConfig, sttEnabled, voiceMaxRecordingSeconds } = useClawbotConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      window.dispatchEvent(new CustomEvent('clawbot:new-session-shortcut'))
    }
