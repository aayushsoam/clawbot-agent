...
import { getClawbotConfig, getClawbotConfigDefaults } from '@/clawbot'
...
interface ClawbotConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useClawbotConfig({ activeSessionIdRef, refreshProjectBranch }: ClawbotConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshClawbotConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getClawbotConfig(), getClawbotConfigDefaults().catch(() => ({}))])
  ...
  return { refreshClawbotConfig, sttEnabled, voiceMaxRecordingSeconds }
}
