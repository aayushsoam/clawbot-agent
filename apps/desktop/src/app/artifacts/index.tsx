...
import { getSessionMessages, listSessions } from '@/clawbot'
...
import type { SessionInfo, SessionMessage } from '@/types/clawbot'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.clawbotDesktop?.openExternal) {
        await window.clawbotDesktop.openExternal(href)
      } else {
