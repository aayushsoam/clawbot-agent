...
import { CLAWBOT_PATHS_MIME } from '@/app/chat/hooks/use-composer-actions'
...
async function readTextPreview(filePath: string) {
  if (window.clawbotDesktop.readFileText) {
    try {
      return await window.clawbotDesktop.readFileText(filePath)
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error)

      if (!message.includes("No handler registered for 'clawbot:readFileText'")) {
        throw error
      }
    }
  }
  ...
  const dataUrl = await window.clawbotDesktop.readFileDataUrl(filePath)
...
function startLineDrag(event: ReactDragEvent<HTMLElement>, filePath: string, { end, start }: LineSelection) {
  ...
  event.dataTransfer.setData(CLAWBOT_PATHS_MIME, JSON.stringify([{ line: start, lineEnd, path: filePath }]))
      ...
      <PreviewEmptyState
        body={
          binary
            ...
            : `${target.label} is ${formatBytes(size)}. Clawbot will only show the first 512 KB.`
        }
