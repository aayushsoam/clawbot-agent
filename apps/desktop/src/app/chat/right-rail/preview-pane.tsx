    ...
    <PreviewEmptyState
        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.clawbotDesktop?.openExternal(error.url)
            }}
          >
      ...
      secondaryAction={
        onRestartServer
          ? {
              disabled: restarting,
              label: restarting ? 'Clawbot is restarting...' : 'Ask Clawbot to restart the server',
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Clawbot to restart the server" looked like it did nothing —
    ...
    try {
      ...
      appendConsoleEntry({
        level: 1,
        message: `Clawbot is looking for a preview server to restart (${taskId})`
      })

      notify({
        kind: 'info',
        title: 'Restarting preview server',
        message: 'Clawbot is working in the background. Watch the preview console for progress.',
        durationMs: 4000
      })
    } catch (error) {
      appendConsoleEntry({
  ...
  useEffect(() => {
      ...
      message:
        previewServerRestart.status === 'running'
          ...
          : previewServerRestart.status === 'complete'
            ? `Clawbot finished restarting the preview server${
    ...
    if (previewServerRestart.status === 'complete') {
      ...
      notify({
        ...
        message: previewServerRestart.message?.slice(0, 200) || 'Clawbot could not restart the server.',
        durationMs: 6000
      })
    }
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      failPreviewServerRestart(
        taskId,
        'Clawbot is still working, but no restart result has arrived yet. The server command may be running in the foreground.'
      )
  ...
  useEffect(() => {
    if (
      target.kind !== 'file' ||
      !window.clawbotDesktop?.watchPreviewFile ||
      !window.clawbotDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.clawbotDesktop.onPreviewFileChanged(payload => {
    ...
    void window.clawbotDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.clawbotDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.clawbotDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:clawbot-preview')
