...
import { clawbotDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, CLAWBOT_PATHS_MIME } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // cold start ("Starting Clawbot...") from a dropped connection we're trying to
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = clawbotDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, CLAWBOT_PATHS_MIME)) {
      return
    }
  ...
  const handleDrop = (event: ReactDragEvent<HTMLFormElement>) => {
    ...
    if (Array.from(event.dataTransfer.types || []).includes(CLAWBOT_PATHS_MIME)) {
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, CLAWBOT_PATHS_MIME)) {
      return
    }
    ...
    <>
      <ComposerPrimitive.Unstable_TriggerPopoverRoot>
        <ComposerPrimitive.Root
          className="group/composer absolute bottom-0 left-1/2 z-30 w-[min(var(--composer-width),calc(100%-2rem))] max-w-full -translate-x-1/2 rounded-[1.55rem] pt-2 pb-[var(--composer-shell-pad-block-end)]"
          ...
          <div className="relative w-full rounded-[inherit]">
            ...
            >
              ...
              >
                ...
                <div className="flex min-h-[6rem] w-full flex-col justify-between gap-2">
...
export function ChatBarFallback() {
  return (
    <div
      className={cn(
        'group/composer absolute bottom-0 left-1/2 z-30 w-[min(var(--composer-width),calc(100%-2rem))] max-w-full -translate-x-1/2 rounded-[1.55rem] pt-2 pb-[var(--composer-shell-pad-block-end)]',
