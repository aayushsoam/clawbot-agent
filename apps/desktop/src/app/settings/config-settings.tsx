...
import {
  getElevenLabsVoices,
  getClawbotConfigDefaults,
  getClawbotConfigRecord,
  getClawbotConfigSchema,
  saveClawbotConfig
} from '@/clawbot'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, ClawbotConfigRecord } from '@/types/clawbot'
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<ClawbotConfigRecord | null>(null)
  const [_defaults, setDefaults] = useState<ClawbotConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getClawbotConfigRecord(), getClawbotConfigDefaults(), getClawbotConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await saveClawbotConfig(config)
  ...
  const updateConfig = (next: ClawbotConfigRecord) => {
  ...
  if (!config || !schema) {
    return <LoadingState label="Loading Clawbot configuration..." />
  }
