...
export const EMPTY_SELECT_VALUE = '__clawbot_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Clawbot Portal',
    description: 'Hosted Clawbot & Clawbot-compatible models',
    docsUrl: 'https://portal.clawbot.com',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'CLAWBOT_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'CLAWBOT_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  timezone: 'Used when Clawbot needs local time context. Blank uses the system timezone.',
  ...
  file_read_max_chars: 'Maximum characters Clawbot can read from one file request.',
  'approvals.mode': 'How Clawbot handles commands that need explicit approval.',
  ...
  'agent.max_turns': 'Upper bound for tool-calling turns before Clawbot stops a run.',
  'updates.non_interactive_local_changes':
    'When Clawbot updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
}
