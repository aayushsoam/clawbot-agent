...
import type { ClawbotConfigRecord } from '@/types/clawbot'
...
describe('settings helpers', () => {
  it('reads and writes nested config paths', () => {
    const config: ClawbotConfigRecord = { display: { theme: 'mono' } }
  ...
  it('rejects prototype-polluting config paths', () => {
    const config: ClawbotConfigRecord = {}
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('NOUS_API_KEY')).toBe('Clawbot Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // CLAWBOT_QWEN_ and CLAWBOT_GEMINI_ both share the CLAWBOT_ stem.
      expect(providerGroup('CLAWBOT_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('CLAWBOT_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
