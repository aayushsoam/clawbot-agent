...
const CLAWBOT_PATHS_MIME = 'application/x-clawbot-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(CLAWBOT_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(CLAWBOT_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.clawbotDesktop?.terminal
    ...
    try {
      ...
      console.warn('[clawbot-terminal] WebGL unavailable; falling back to DOM', err)
    }
