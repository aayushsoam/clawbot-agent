...
import type { ClawbotReadDirResult } from '@/global'

import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<ClawbotReadDirResult>>()

beforeEach(() => {
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { clawbotDesktop: { readDir: typeof readDir } }).clawbotDesktop = { readDir }
})

afterEach(() => {
  resetProjectTreeState()
  delete (window as unknown as { clawbotDesktop?: unknown }).clawbotDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): ClawbotReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: ClawbotReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<ClawbotReadDirResult>(resolve => {
  ...
  it('returns no-bridge gracefully when window.clawbotDesktop is missing', async () => {
    delete (window as unknown as { clawbotDesktop?: unknown }).clawbotDesktop
