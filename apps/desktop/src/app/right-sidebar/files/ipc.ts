...
import type { ClawbotReadDirEntry, ClawbotReadDirResult } from '@/global'

export type ProjectTreeEntry = ClawbotReadDirEntry
...
async function gitRootFor(start: string) {
  if (!window.clawbotDesktop?.gitRoot) {
    return null
  }
  ...
  if (!cached) {
    cached = window.clawbotDesktop.gitRoot(key)
...
async function readGitignore(dir: string): Promise<GitignoreRule | null> {
  if (!window.clawbotDesktop?.readDir || !window.clawbotDesktop.readFileDataUrl) {
    return null
  }

  try {
    const listing = await window.clawbotDesktop.readDir(dir)

    if (!listing.entries.some(e => e.name === '.gitignore' && !e.isDirectory)) {
      return null
    }

    const text = decodeDataUrl(await window.clawbotDesktop.readFileDataUrl(`${dir}/.gitignore`))
...
function ignoredBy(rules: GitignoreRule[], entry: ClawbotReadDirEntry) {
...
async function filterIgnored(entries: ClawbotReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<ClawbotReadDirResult> {
  if (!window.clawbotDesktop) {
    return { entries: [], error: 'no-bridge' }
  }

  const result = await window.clawbotDesktop.readDir(dirPath)
