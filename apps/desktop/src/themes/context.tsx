...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.clawbotTheme = skinName
  root.dataset.clawbotMode = rendered
  ...
  window.clawbotDesktop?.setTitleBarTheme?.({
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.clawbotThemeFont = 'true'
...
/** Sync the desktop skin with the active Clawbot backend theme on connect. */
