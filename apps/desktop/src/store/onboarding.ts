...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/clawbot'
...
const CONFIGURED_CACHE_KEY = 'clawbot-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'clawbot-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Clawbot is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the Clawbot Tool Gateway,
// mirroring clawbot_cli/nous_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to Clawbot Portal auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your Clawbot subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `clawbot model` does (for Clawbot Portal it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Clawbot still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Clawbot still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.clawbotDesktop?.openExternal) {
    try {
      await window.clawbotDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Clawbot still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Clawbot still cannot reach ${url}.` }
    }
