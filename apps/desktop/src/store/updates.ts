...
const UPDATE_TOAST_SNOOZE_KEY = 'clawbot:update-toast-snooze-until'
...
// value means GUI<->backend skew. Some resume paths and older persisted
// session events do not carry the field, so absence alone is not actionable.
...
export function reportBackendContract(contract: number | undefined): void {
  if (contract == null || contract >= REQUIRED_BACKEND_CONTRACT) {
  ...
  notify({
    action: { label: translateNow('notifications.updateClawbot'), onClick: () => void applyUpdates() },
    durationMs: 0,
    id: SKEW_TOAST_ID,
    kind: 'warning',
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.clawbotDesktop?.getVersion?.()
...
export async function checkUpdates(): Promise<DesktopUpdateStatus | null> {
  const bridge = window.clawbotDesktop?.updates
  ...
  try {
    ...
    // The update check pulls the latest clawbot_cli + bundled package metadata
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `clawbot update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'clawbot update',
        command: result.command ?? 'clawbot update'
      })
    }
...
function onFocus() {
  ...
  // updated Clawbot from another window/CLI between focuses, and About should
