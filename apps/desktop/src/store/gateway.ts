import type { ConnectionState, GatewayEvent } from '@clawbot/shared'
...
const isOpen = (gateway: ClawbotGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<ClawbotGateway | null>(null)
...
let primaryGateway: ClawbotGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: ClawbotGateway | null, profile = 'default'): void {
...
export function activeGateway(): ClawbotGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<ClawbotGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
