...
import { getProfiles, setApiRequestProfile } from '@/clawbot'
...
import type { ProfileInfo } from '@/types/clawbot'
...
// /api/profiles/active `current`). "default" is the root ~/.clawbot. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'clawbot.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'clawbot.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.clawbotDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new CLAWBOT_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.clawbotDesktop.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'clawbot.desktop.showAllProfiles'
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.clawbotDesktop?.touchBackend?.(target).catch(() => undefined)
}
