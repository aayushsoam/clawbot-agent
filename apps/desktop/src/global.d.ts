...
declare global {
  interface Window {
    clawbotDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<ClawbotConnection>
      ...
      profile: {
        ...
        // backend under the new CLAWBOT_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: ClawbotApiRequest) => Promise<T>
      notify: (payload: ClawbotNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<ClawbotReadFileTextResult>
      selectPaths: (options?: ClawbotSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<ClawbotPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<ClawbotPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: ClawbotTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<ClawbotReadDirResult>
      gitRoot?: (path: string) => Promise<string | null>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: ClawbotTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<ClawbotTerminalSession>
        write: (id: string, data: string) => Promise<boolean>
      }
      onClosePreviewRequested?: (callback: () => void) => () => void
      onOpenUpdatesRequested?: (callback: () => void) => () => void
      onWindowStateChanged?: (callback: (payload: ClawbotWindowState) => void) => () => void
      onPreviewFileChanged: (callback: (payload: ClawbotPreviewFileChanged) => void) => () => void
...
export interface ClawbotTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface ClawbotTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `clawbot update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  clawbotRoot?: string
}
...
export interface ClawbotTitleBarTheme {
  background: string
  foreground: string
}

export interface ClawbotWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface ClawbotApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface ClawbotNotification {
  title?: string
  body?: string
  silent?: boolean
}

export interface ClawbotPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface ClawbotReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface ClawbotPreviewWatch {
  id: string
  path: string
}

export interface ClawbotReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface ClawbotReadDirResult {
  entries: ClawbotReadDirEntry[]
  error?: string
}

export interface ClawbotPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface ClawbotSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
