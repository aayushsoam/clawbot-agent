...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'clawbot:saveImageFromUrl'")
}
...
export function ZoomableImage({ className, containerClassName, src, alt, slot, ...props }: ZoomableImageProps) {
  ...
  async function handleDownload() {
    ...
    try {
      if (window.clawbotDesktop?.saveImageFromUrl) {
        const saved = await window.clawbotDesktop.saveImageFromUrl(src)
      ...
      if (isMissingIpcHandler(error)) {
        try {
          ...
          notify({
            ...
            message: 'Restart Clawbot Desktop to use Save Image.'
          })
        } catch (fallbackError) {
          notifyError(fallbackError, 'Restart Clawbot Desktop to save images')
        }
