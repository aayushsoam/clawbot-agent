...
const CLAWBOT_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type ClawbotRefType = (typeof CLAWBOT_REF_TYPES)[number]
...
const ICON_PATHS: Record<ClawbotRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as ClawbotRefType] ?? ICON_FALLBACK
...
const CLAWBOT_DIRECTIVE_RE = new RegExp(
...
export const clawbotDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(CLAWBOT_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as ClawbotRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: ClawbotRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing Clawbot directives (`@file:...`, `@image:...`) as
...
export function DirectiveContent({ text }: { text: string }) {
  ...
  const segments = useMemo(() => clawbotDirectiveFormatter.parse(cleanedText), [cleanedText])
