...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.clawbotDesktop && ['audio', 'video'].includes(mediaKind(path))) {
    return mediaStreamUrl(path)
  }

  if (!window.clawbotDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.clawbotDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.clawbotDesktop?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
