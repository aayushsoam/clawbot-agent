...
import { formatRefValue, clawbotDirectiveFormatter } from './directive-text'
...
describe('clawbotDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = clawbotDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = clawbotDirectiveFormatter.parse('@file:src/main.tsx the entry point')
