...
import { getGlobalModelOptions } from '@/clawbot'
import {
  Check,
  ChevronDown,
  ChevronLeft,
...
import type { OAuthProvider } from '@/types/clawbot'
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    description: 'Point Clawbot at a local or self-hosted OpenAI-compatible endpoint (vLLM, llama.cpp, Ollama, etc).',
    docsUrl: 'https://github.com/ClawbotAgent/clawbot-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

const PROVIDER_DISPLAY: Record<string, { order: number; title: string }> = {
  nous: { order: 0, title: 'Clawbot Portal' },
...
const FLOW_SUBTITLES: Record<OAuthProvider['flow'], string> = {
  ...
  device_code: 'Opens a verification page in your browser — Clawbot connects automatically',
  loopback: 'Opens your browser to sign in — Clawbot connects automatically',
  ...
  return (
    <div className="grid gap-3" role="status">
      <p className="text-sm text-muted-foreground">
        {installing
          ? 'Clawbot is finishing install. This usually takes under a minute on first run.'
          : 'Starting Clawbot…'}
      </p>
  ...
  return (
    <div className="border-b border-(--ui-stroke-tertiary) bg-(--ui-chat-bubble-background) px-5 py-4">
      <div className="flex items-start gap-3">
        ...
        <div>
          <h2 className="text-[0.9375rem] font-semibold tracking-tight">Let's get you setup with Clawbot Agent</h2>
...
const FEATURED_PITCH = 'One subscription, 300+ frontier models — the recommended way to run Clawbot'
const SHOW_ALL_KEY = 'clawbot-onboarding-show-all-v1'
...
function FlowPanel({ ctx, flow }: { ctx: OnboardingContext; flow: OnboardingFlow }) {
  ...
  if (flow.status === 'awaiting_browser') {
    return (
      <Step title={`Sign in with ${title}`}>
        <p className="text-sm text-muted-foreground">
          We opened {title} in your browser. Authorize Clawbot there and you'll be connected automatically — nothing to
          copy or paste.
        </p>
