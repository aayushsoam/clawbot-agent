...
const DESKTOP_COMMAND_META = [
  ...
  ['/profile', 'Switch the active Clawbot profile'],
  ...
  ['/version', 'Show Clawbot Agent version'],
...
export function isDesktopSlashCommand(command: string): boolean {
  ...
  return DESKTOP_COMMANDS.has(canonical) || !isKnownClawbotSlashCommand(normalized)
}

/**
 * An "extension" command is anything the backend surfaces that is NOT one of
 * Clawbot' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownClawbotSlashCommand(normalized)
}
...
function isKnownClawbotSlashCommand(command: string): boolean {
