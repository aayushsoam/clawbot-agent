...
export const zh: Translations = {
  ...
  boot: {
    ready: 'Clawbot Desktop 已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败:${message}`,
    steps: {
      connectingGateway: '正在连接实时桌面网关',
      loadingSettings: '正在加载 Clawbot 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingClawbotDesktop: '正在启动 Clawbot Desktop…'
    },
    errors: {
      backgroundExited: 'Clawbot 后台进程已退出。',
      backgroundExitedDuringStartup: 'Clawbot 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Clawbot 后端早于当前桌面构建,可能无法正常工作。请更新以保持一致。',
    updateClawbot: '更新 Clawbot',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求(405 Method Not Allowed)。请尝试重启 Clawbot Desktop。',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Clawbot 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      gateway: '网关',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Clawbot 跟随系统设置。',
    ...
    fieldDescriptions: {
      ...
      timezone: '当 Clawbot 需要本地时间上下文时使用。留空则使用系统时区。',
      ...
      file_read_max_chars: 'Clawbot 单次文件读取可读取的最大字符数。',
      'approvals.mode': 'Clawbot 如何处理需要显式审批的命令。',
      ...
      'agent.max_turns': 'Clawbot 停止一次运行前工具调用轮次的上限。'
    },
    about: {
      ...
      automaticUpdatesDesc: 'Clawbot 会在后台自动检查更新,并在有可用更新时通知你。',
  ...
  plugins: {
    ...
    providersHint: '选择由哪个插件为新会话提供记忆与上下文。',
    memoryProviderLabel: '记忆提供方',
    contextEngineLabel: '上下文引擎',
    providerDefault: '（默认）',
    saveProviders: '保存提供方',
    savedProviders: '提供方已保存',
    saveProvidersFailed: '保存提供方失败',
    installHeading: '安装插件',
    installHint: '输入 GitHub owner/repo 或完整 URL 以安装代理插件。',
    identifierLabel: '插件标识',
    forceReinstall: '强制重装',
    enableAfterInstall: '安装后启用',
    installBtn: '安装',
    installed: name => `${name} 已安装`,
    installFailed: '安装失败',
    missingEnv: vars => `缺少环境变量：${vars}`,
    rescanHint: '手动添加了文件？重新扫描以发现新的仪表盘插件。',
    rescanBtn: '重新扫描',
    rescanned: count => `已重新扫描（${count}）`,
    rescanFailed: '重新扫描失败',
    pluginListHeading: '已安装插件',
    noPlugins: '尚未安装任何插件。',
    orphanHeading: '孤立的仪表盘插件',
    actionFailed: '操作失败',
    status: { enabled: '已启用', disabled: '已禁用', inactive: '未激活' },
    authRequired: '需要授权',
    enable: '启用',
    disable: '禁用',
    update: '更新',
    ...
    shownToast: '已在侧栏显示',
    hiddenToast: '已从侧栏隐藏',
    removedToast: name => `${name} 已移除`,
    showInSidebar: '在侧栏显示',
    hideFromSidebar: '从侧栏隐藏',
    removeConfirmTitle: '移除插件？',
    removeConfirmDesc: name => `这将从你的代理中移除 "${name}" 插件。`
  },
  ...
  commandCenter: {
    ...
    nav: {
      ...
      settings: { title: '设置', detail: '配置 Clawbot 桌面端' },
    ...
    clawbotActiveSessions: (version, count) => `Clawbot ${version} · 活跃会话 ${count}`,
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接,然后把 Clawbot 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Clawbot 自带的 WhatsApp 桥接,首次运行时扫描二维码,然后启用该平台。',
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server,暴露其 API,然后用服务器密码把 Clawbot 指向该 URL。',
      ...
      weixin: '登录微信公众平台,复制 AppID 和 Token,并把消息回调 URL 指向 Clawbot。',
      qqbot: '在 QQ 开放平台(q.qq.com)注册一个应用,复制 App ID 和 Client Secret。',
      api_server: '把 Clawbot 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥,然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 Clawbot 环境:各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
  ...
  cron: {
    ...
    emptyDescNew: '按 cron 表达式排程一个提示词。Clawbot 会运行它,并把结果发送到你选择的目的地。',
  ...
  composer: {
    message: '消息',
    placeholderStarting: '正在启动 Clawbot…',
    placeholderReconnecting: '正在重新连接 Clawbot…',
    ...
    attachUrlDesc: 'Clawbot 将抓取该页面并作为本回合的上下文。',
