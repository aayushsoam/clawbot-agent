...
import { getClawbotConfigRecord, type ClawbotConfigRecord, saveClawbotConfig } from '@/clawbot'
...
export interface I18nConfigClient {
  getConfig: () => Promise<ClawbotConfigRecord>
  saveConfig: (config: ClawbotConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.clawbotDesktop?.api) {
      return Promise.resolve({})
    }

    return getClawbotConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.clawbotDesktop?.api) {
    ...
    return saveClawbotConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: ClawbotConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: ClawbotConfigRecord, locale: Locale): ClawbotConfigRecord {
