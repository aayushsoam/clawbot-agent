...
export type ClawbotConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
  ...
  /** Clawbot portal only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Clawbot portal only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
   ...
   *  CLI `clawbot tools` active-provider detection). */
  is_active: boolean
}
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the Clawbot Tool Gateway as a result of
   *  switching the main provider to Clawbot Portal. Empty unless provider === 'nous'
