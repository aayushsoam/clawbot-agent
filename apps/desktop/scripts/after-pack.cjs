 ...
 *
 * Stamps the Clawbot icon + identity onto the packed Windows Clawbot.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `clawbot desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Clawbot')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Clawbot'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Clawbot.exe keeps the stock Electron icon`)
  }
}
