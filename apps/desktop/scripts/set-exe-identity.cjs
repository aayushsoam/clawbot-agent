#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Clawbot icon + version metadata onto the
// built Clawbot.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Clawbot.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `clawbot desktop`, the installer's
...
// path (which rebuilds via `clawbot desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Clawbot.exe>
//
...
// Stamp the Clawbot icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Clawbot',
      FileDescription: 'Clawbot',
      CompanyName: 'Clawbot Agent',
      LegalCopyright: 'Copyright (c) 2026 Clawbot Agent'
    }
  })

  console.log('[set-exe-identity] done — Clawbot icon + identity stamped')
}
