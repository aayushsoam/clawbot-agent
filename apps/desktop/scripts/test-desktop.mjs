...
// no longer bundles the Clawbot Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Clawbot.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Clawbot'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Clawbot.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'clawbot'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default CLAWBOT_HOME for non-sandboxed runs -- matches main.cjs's
// resolveClawbotHome(). On Windows it's %LOCALAPPDATA%\clawbot; elsewhere
// it's ~/.clawbot. The fresh-install sandbox launchFresh() sets its own
// CLAWBOT_HOME and never touches this.
const DEFAULT_CLAWBOT_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'clawbot')
  }
  return path.join(os.homedir(), '.clawbot')
})()
const VENV_ROOT = path.join(DEFAULT_CLAWBOT_HOME, 'clawbot-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'clawbot-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.CLAWBOT_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Clawbot-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Clawbot-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Clawbot-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Clawbot-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.CLAWBOT_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.CLAWBOT_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const clawbotHome = path.join(sandbox, 'clawbot-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(clawbotHome, { recursive: true })
  ...
  env.CLAWBOT_DESKTOP_CWD = cwd
  env.CLAWBOT_DESKTOP_IGNORE_EXISTING = '1'
  env.CLAWBOT_DESKTOP_TEST_MODE = 'fresh-install'
  env.CLAWBOT_DESKTOP_USER_DATA_DIR = userDataDir
  env.CLAWBOT_HOME = clawbotHome
  delete env.CLAWBOT_DESKTOP_CLAWBOT
  delete env.CLAWBOT_DESKTOP_CLAWBOT_ROOT
  ...
  console.log(`  CLAWBOT_HOME: ${clawbotHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(clawbotHome, 'clawbot-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Clawbot Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of clawbot_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'clawbot-agent', 'clawbot_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Clawbot
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + CLAWBOT_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  CLAWBOT_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
