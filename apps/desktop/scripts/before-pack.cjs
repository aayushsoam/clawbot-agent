 ...
 * product name (`Clawbot`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Clawbot'
 *
 * This is a hard failure with no obvious cause for the user — `clawbot desktop`
 ...
 * (the mac-unpacked Clawbot.app bundle) and Windows (win-unpacked), so we
