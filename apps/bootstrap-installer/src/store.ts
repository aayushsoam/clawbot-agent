...
export const $clawbotHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, clawbotHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_clawbot_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $clawbotHome.set(clawbotHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Clawbot" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Clawbot-Setup.exe --update);
...
export async function launchClawbotDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_clawbot_desktop', { installRoot })
}
