...
//!
//! Mirrors `clawbot_constants.get_clawbot_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\clawbot
//!   macOS:   ~/.clawbot
//!   Linux:   ~/.clawbot  (override via $CLAWBOT_HOME)
//!
//! NOTE (macOS): Python's get_clawbot_home(), scripts/install.sh, and the
//! Electron desktop's resolveClawbotHome() ALL use ~/.clawbot on macOS — there
...
/// Returns the canonical Clawbot home directory, respecting $CLAWBOT_HOME if set.
pub fn clawbot_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("CLAWBOT_HOME") {
    ...
    {
        // %LOCALAPPDATA%\clawbot — matches scripts/install.ps1's $ClawbotHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("clawbot");
        }
    }

    // macOS + Linux + fallback: ~/.clawbot (matches Python get_clawbot_home(),
    // install.sh, and the Electron desktop's resolveClawbotHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".clawbot");
    }

    // Last resort — current dir, almost certainly wrong but at least
    // doesn't panic.
    PathBuf::from(".clawbot")
}

pub fn log_dir() -> PathBuf {
    clawbot_home().join("logs")
}
...
pub fn bootstrap_cache_dir() -> PathBuf {
    clawbot_home().join("bootstrap-cache")
}
...
/// CLAWBOT_HOME so it survives repo checkout deletion (unlike anything under
/// clawbot-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\clawbot\clawbot-setup.exe`; on other
...
pub fn copy_self_to_clawbot_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to CLAWBOT_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_CLAWBOT_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".clawbot-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under CLAWBOT_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[clawbot-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("CLAWBOT_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_clawbot_home() -> String {
    clawbot_home().to_string_lossy().into_owned()
}
