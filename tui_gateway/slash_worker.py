"""Persistent slash-command worker — one ClawbotCLI per TUI session.
...
def _run(cli: ClawbotCLI, command: str) -> str:
...
def main():
    ...
    os.environ["CLAWBOT_SESSION_KEY"] = args.session_key
    os.environ["CLAWBOT_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = ClawbotCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
