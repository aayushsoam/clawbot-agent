...
# 5. Symlinks the 'clawbot' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚡Clawbot Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/clawbot-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink clawbot into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up clawbot command..."

CLAWBOT_BIN="$SCRIPT_DIR/venv/bin/clawbot"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$CLAWBOT_BIN" "$COMMAND_LINK_DIR/clawbot"
echo -e "${GREEN}✓${NC} Symlinked clawbot → $COMMAND_LINK_DISPLAY_DIR/clawbot"
...
# ============================================================================
# Seed bundled skills into ~/.clawbot/skills/
# ============================================================================

CLAWBOT_SKILLS_DIR="${CLAWBOT_HOME:-$HOME/.clawbot}/skills"
mkdir -p "$CLAWBOT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.clawbot/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CLAWBOT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  clawbot status        # Check configuration"
if is_termux; then
    echo "  clawbot gateway       # Run gateway in foreground"
else
    echo "  clawbot gateway install # Install gateway service (messaging + cron)"
fi
echo "  clawbot cron list     # View scheduled jobs"
echo "  clawbot doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m clawbot_cli.main setup
fi
