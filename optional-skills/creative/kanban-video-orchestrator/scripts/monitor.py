#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `clawbot kanban list` and
...
def kanban_list(tenant: str) -> list[dict]:
    ...
    output isn't supported by the installed clawbot CLI."""
    try:
        out = subprocess.run(
            ["clawbot", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `clawbot kanban list`
    out = subprocess.run(
        ["clawbot", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["clawbot", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not clawbot_available():
        print("ERROR: 'clawbot' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
