...
Maps each role archetype to the Clawbot skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public clawbot-agent
...
## Clawbot skills relevant to video production

### Visual / rendering skills (`clawbot-agent/skills/creative/`)
...
### Design / pre-production skills (`clawbot-agent/skills/creative/`)
...
### Audio / media skills (`clawbot-agent/skills/creative/` + `skills/media/`)
...
### Kanban infrastructure (`clawbot-agent/skills/devops/`)
...
These are **not** Clawbot skills but external CLIs / APIs that profiles invoke.
...
## Built-in Clawbot tools for media review

These are native Clawbot tools — not invoked via terminal but through their own
...
key is present in `~/.clawbot/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Clawbot profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `~/.clawbot/.env` → environment variable.
...
When a new Clawbot-public video skill ships:
