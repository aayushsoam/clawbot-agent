...
clawbot kanban watch --tenant <project-slug>

# Snapshot of the board
clawbot kanban list --tenant <project-slug>
clawbot kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
clawbot kanban stats --tenant <project-slug>

# Visual dashboard (browser)
clawbot dashboard

# Inspect a specific task (includes comments + events)
clawbot kanban show <task-id>

# Follow a single task's event stream
clawbot kanban tail <task-id>
```

Verify available subcommands with `clawbot kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `clawbot kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `clawbot kanban block <id>` then `clawbot kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `clawbot kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `clawbot kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `clawbot profile list` (profile exists?), `clawbot status` (gateway/dispatcher up?), and verify tenant. |
...
clawbot kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
clawbot kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(clawbot kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
clawbot kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
clawbot kanban block <task-id>
clawbot kanban archive <task-id>

# Diagnose what happened
clawbot kanban show <task-id>      # task body, comments, recent events
clawbot kanban tail <task-id>      # follow the live event stream
clawbot kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `clawbot kanban create` for the director
...
while true; do
    clear
    clawbot kanban list --tenant <slug>
    echo "---"
    clawbot kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `clawbot kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `clawbot profile list` and start
