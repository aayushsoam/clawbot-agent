...
Usage (via cron with --no-agent):
    ...
    clawbot cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$CLAWBOT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Clawbot-Watcher/1.0"})
