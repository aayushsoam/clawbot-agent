...
Usage (via cron with --no-agent):
    ...
    clawbot cron create clawbot-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$CLAWBOT_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name clawbot-issues --repo Aayushsoam/clawbot-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in ~/.clawbot/.env to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Clawbot-Watcher/1.0",
    }
