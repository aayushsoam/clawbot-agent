...
VERSION_FILE = REPO_ROOT / "clawbot_cli" / "__init__.py"
...
AUTHOR_MAP = {
    "thakurrajjevsoam@gmail.com": "aayushsoam",
}
...
def _update_acp_registry_versions(semver: str) -> None:
    ...
    if ACP_REGISTRY_MANIFEST.exists():
        ...
        if "package" in uvx:
            uvx["package"] = f"clawbot-agent[acp]=={semver}"
...
def resolve_author(name: str, email: str) -> str:
    """Resolve any git author to the project owner GitHub @mention."""
    return "@aayushsoam"
...
def parse_coauthors(body: str) -> list:
    ...
    _ignored_emails = {"noreply@anthropic.com", "noreply@github.com",
                       "cursoragent@cursor.com", "clawbot@aayushsoam.com"}
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/aayushsoam/clawbot-agent",
    ...
    lines.append(f"# Clawbot Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Clawbot Agent. See below for everything included in this initial release.")
        lines.append("")
    ...
    for commit in commits:
        ...
        if author not in aayush_aliases:
            all_authors.add(author)
        for coauthor in commit.get("coauthors", []):
            if coauthor not in aayush_aliases:
...
def main():
    parser = argparse.ArgumentParser(description="Clawbot Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Clawbot Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Clawbot Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Clawbot Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Clawbot Agent v{new_version} ({calver_date})' "
