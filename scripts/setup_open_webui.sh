...
# Bootstrap Open WebUI against Clawbot Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.clawbot/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-clawbot.sh
...
#   OPEN_WEBUI_NAME='Johnny Clawbot'
...
#   CLAWBOT_API_PORT=8642
#   CLAWBOT_API_HOST=127.0.0.1
#   CLAWBOT_API_MODEL_NAME='Clawbot Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Clawbot Agent WebUI}"
...
CLAWBOT_ENV_FILE="${CLAWBOT_ENV_FILE:-$HOME/.clawbot/.env}"
CLAWBOT_API_PORT="${CLAWBOT_API_PORT:-8642}"
CLAWBOT_API_HOST="${CLAWBOT_API_HOST:-127.0.0.1}"
CLAWBOT_API_CONNECT_HOST="${CLAWBOT_API_CONNECT_HOST:-127.0.0.1}"
CLAWBOT_API_MODEL_NAME="${CLAWBOT_API_MODEL_NAME:-Clawbot Agent}"
CLAWBOT_API_BASE_URL="http://${CLAWBOT_API_CONNECT_HOST}:${CLAWBOT_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-clawbot.sh"
LOG_DIR="$HOME/.clawbot/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$CLAWBOT_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$CLAWBOT_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.clawbot.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.clawbot</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.clawbot"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.clawbot"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-clawbot.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Clawbot Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-clawbot.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.clawbot/logs/openwebui.log
StandardError=append:%h/.clawbot/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-clawbot.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$CLAWBOT_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring Clawbot API server is configured...'
  upsert_env API_SERVER_ENABLED true "$CLAWBOT_ENV_FILE"
  upsert_env API_SERVER_HOST "$CLAWBOT_API_HOST" "$CLAWBOT_ENV_FILE"
  upsert_env API_SERVER_PORT "$CLAWBOT_API_PORT" "$CLAWBOT_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$CLAWBOT_API_MODEL_NAME" "$CLAWBOT_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$CLAWBOT_ENV_FILE"
  ensure_env_permissions

  log 'Restarting Clawbot gateway so API server settings take effect...'
  clawbot gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${CLAWBOT_API_CONNECT_HOST}:${CLAWBOT_API_PORT}/health" >/dev/null; then
    log 'Clawbot API server did not answer on the first check. Trying to start gateway in the background...'
    nohup clawbot gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${CLAWBOT_API_CONNECT_HOST}:${CLAWBOT_API_PORT}/health" >/dev/null
  ...
  log "Clawbot API endpoint: ${CLAWBOT_API_BASE_URL}"
