...
param(
    ...
    [switch]$SkipSetup,
    [string]$Branch = "main",
    [string]$ClawbotHome = "$env:LOCALAPPDATA\clawbot",
    [string]$InstallDir = "$env:LOCALAPPDATA\clawbot\clawbot-agent",
...
# 5.1's progress UI repaints synchrosoamly on every received byte, which
...
$RepoUrlSsh = "git@github.com:Aayushsoam/clawbot-agent.git"
$RepoUrlHttps = "https://github.com/aayushsoam/clawbot-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $ClawbotHome)) {
        New-Item -ItemType Directory -Force -Path $ClawbotHome | Out-Null
    }
    $envFile = Join-Path $ClawbotHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $ClawbotHome "node"
...
function Install-Git {
    ...
    which Clawbot uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\clawbot\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Clawbot needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Clawbot copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\clawbot\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``CLAWBOT_GIT_BASH_PATH`` (User scope) so Clawbot can find it in a fresh
    ...
    # Download PortableGit into $ClawbotHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $ClawbotHome\git\ ..."
    ...
    try {
        ...
        $release = Invoke-RestMethod -Uri $releaseApi -UseBasicParsing -Headers @{ "User-Agent" = "clawbot-installer" }

        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Clawbot features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Clawbot needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``CLAWBOT_GIT_BASH_PATH`` (User env scope) so Clawbot can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $ClawbotHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $ClawbotHome\git\usr\bin\bash.exe
    $candidates += "$ClawbotHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$ClawbotHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("CLAWBOT_GIT_BASH_PATH", $candidate, "User")
            $env:CLAWBOT_GIT_BASH_PATH = $candidate
            Write-Info "Set CLAWBOT_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Clawbot may not find Git Bash."
    Write-Info "If needed, set CLAWBOT_GIT_BASH_PATH manually to your bash.exe path."
}

function Test-NodeVersionForDashboard {
    param([string]$VersionText)
    if (-not $VersionText) { return $false }
    $trimmed = $VersionText.Trim()
    if ($trimmed -notmatch '^v?(\d+)\.(\d+)') { return $false }
    $major = [int]$Matches[1]
    $minor = [int]$Matches[2]
...
function Test-Node {
    Write-Info "Checking Node.js (for browser tools and dashboard)..."

    if (Get-Command node -ErrorAction SilentlyContinue) {
        $version = node --version
        if (Test-NodeVersionForDashboard $version) {
            if (Test-NpmUsable) {
                Write-Success "Node.js $version found"
                $script:HasNode = $true
                return $true
            }
            Write-Warn "Node.js $version found, but npm is not runnable"
        } else {
            Write-Warn "Node.js $version found, but dashboard needs Node.js 20.19+ or 22.12+"
        }
    }

    # Check our own managed install from a previous run
    $managedNode = "$ClawbotHome\node\node.exe"
    if (Test-Path $managedNode) {
        $version = & $managedNode --version
        if (Test-NodeVersionForDashboard $version) {
            $env:Path = "$ClawbotHome\node;$env:Path"
            if (Test-NpmUsable) {
                Write-Success "Node.js $version found (Clawbot-managed)"
                $script:HasNode = $true
                return $true
            }
            Write-Warn "Clawbot-managed Node.js $version found, but npm is not runnable; reinstalling Node.js $NodeVersion"
        } else {
            Write-Warn "Clawbot-managed Node.js $version is too old; reinstalling Node.js $NodeVersion"
        }
    }

    Write-Info "Installing portable Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $ClawbotHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $ClawbotHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\clawbot-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$ClawbotHome\node") { Remove-Item -Recurse -Force "$ClawbotHome\node" }
                Move-Item $extractedDir.FullName "$ClawbotHome\node"
                ...
                $version = & "$ClawbotHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $ClawbotHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        try {
            ...
            if (Get-Command node -ErrorAction SilentlyContinue) {
                ...
                Write-Warn "Node.js $version installed via winget, but dashboard needs Node.js 20.19+ or 22.12+"
            }
        } catch {
            if ($prevEAP) { $ErrorActionPreference = $prevEAP }
        }
...
function Install-Repository {
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running clawbot processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                $zipUrl = "https://github.com/aayushsoam/clawbot-agent/archive/refs/heads/$Branch.zip"
                $zipPath = "$env:TEMP\clawbot-agent-$Branch.zip"
                $extractPath = "$env:TEMP\clawbot-agent-extract"
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `clawbot.exe` exists
...
try:
    ...
    out = []
    for s in specs:
        m = re.search(r'clawbot-agent\[([\w-]+)\]', s)
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install clawbot-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and clawbot.exe broken with
    ...
    # here, not 30 seconds later when the user runs `clawbot`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `clawbot dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `clawbot dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `clawbot dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up clawbot command..."
    
    if ($NoVenv) {
        $clawbotBin = "$InstallDir"
    } else {
        $clawbotBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so clawbot is globally available
    # On Windows, the clawbot.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$clawbotBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$clawbotBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $clawbotBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set CLAWBOT_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\clawbot instead
    # of the Unix default ~/.clawbot
    $currentClawbotHome = [Environment]::GetEnvironmentVariable("CLAWBOT_HOME", "User")
    if (-not $currentClawbotHome -or $currentClawbotHome -ne $ClawbotHome) {
        [Environment]::SetEnvironmentVariable("CLAWBOT_HOME", $ClawbotHome, "User")
        Write-Success "Set CLAWBOT_HOME=$ClawbotHome"
    }
    $env:CLAWBOT_HOME = $ClawbotHome
    
    # Update current session
    $env:Path = "$clawbotBin;$env:Path"
    
    Write-Success "clawbot command ready"
}

function Copy-ConfigTemplates {
    Write-Info "Setting up configuration files..."
    
    # Create ~/.clawbot directory structure
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$ClawbotHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$ClawbotHome\.env"
    if (-not (Test-Path $envPath)) {
        $examplePath = "$InstallDir\.env.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $envPath
            Write-Success "Created ~/.clawbot/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.clawbot/.env"
        }
    } else {
        Write-Info "~/.clawbot/.env already exists, keeping it"
    }
    
    # Create config.yaml
    $configPath = "$ClawbotHome\config.yaml"
    if (-not (Test-Path $configPath)) {
        $examplePath = "$InstallDir\cli-config.yaml.example"
        if (Test-Path $examplePath) {
            Copy-Item $examplePath $configPath
            Write-Success "Created ~/.clawbot/config.yaml from template"
        }
    } else {
        Write-Info "~/.clawbot/config.yaml already exists, keeping it"
    }
    ...
    # (the default PS5 behaviour), and Clawbot's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$ClawbotHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        $soulContent = @"
# Clawbot Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Clawbot communicates with you.
...
"@
        ...
        Write-Success "Created ~/.clawbot/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.clawbot/"
    
    # Seed bundled skills into ~/.clawbot/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.clawbot/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.clawbot/skills/"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$ClawbotHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to ~/.clawbot/skills/"
            }
        }
    }
}
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'clawbot setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\clawbot-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\clawbot-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\clawbot-npm-tui-$(Get-Random).log"
...
function Install-PlatformSdks {
    ...
    # ~/.clawbot/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'clawbot setup'."
        return
    }
    ...
    # Run clawbot setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m clawbot_cli.main setup
    } else {
        python -m clawbot_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $clawbotCmd = "$InstallDir\venv\Scripts\clawbot.exe"
    if (-not (Test-Path $clawbotCmd)) {
        $clawbotCmd = "clawbot"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$ClawbotHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'clawbot whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: clawbot gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$ClawbotHome\logs\gateway.log"
            Start-Process -FilePath $clawbotCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$ClawbotHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: clawbot gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: clawbot gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$ClawbotHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClawbotHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClawbotHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$ClawbotHome\clawbot-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   clawbot              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   clawbot setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   clawbot config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   clawbot config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   clawbot gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   clawbot update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Clawbot repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Clawbot to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Aayushsoam/clawbot-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
