#!/usr/bin/env bash
# Canonical test runner for clawbot-agent. Run this instead of calling
...
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.clawbot/clawbot-agent/venv"; do
...
# Unset CLAWBOT_* behavioral vars too.
unset CLAWBOT_YOLO_MODE CLAWBOT_INTERACTIVE CLAWBOT_QUIET CLAWBOT_TOOL_PROGRESS \
      CLAWBOT_TOOL_PROGRESS_MODE CLAWBOT_MAX_ITERATIONS CLAWBOT_SESSION_PLATFORM \
      CLAWBOT_SESSION_CHAT_ID CLAWBOT_SESSION_CHAT_NAME CLAWBOT_SESSION_THREAD_ID \
      CLAWBOT_SESSION_SOURCE CLAWBOT_SESSION_KEY CLAWBOT_GATEWAY_SESSION \
      CLAWBOT_CRON_SESSION \
      CLAWBOT_PLATFORM CLAWBOT_INFERENCE_PROVIDER CLAWBOT_MANAGED CLAWBOT_DEV \
      CLAWBOT_CONTAINER CLAWBOT_EPHEMERAL_SYSTEM_PROMPT CLAWBOT_TIMEZONE \
      CLAWBOT_REDACT_SECRETS CLAWBOT_BACKGROUND_NOTIFICATIONS CLAWBOT_EXEC_ASK \
      CLAWBOT_HOME_MODE 2>/dev/null || true
...
# If a system-wide clawbot pytest_live_guard plugin is installed at
# $HOME/.clawbot/pytest_live_guard.py, force-load it here so every test run
...
if [ -f "$HOME/.clawbot/pytest_live_guard.py" ]; then
  case ":${PYTHONPATH:-}:" in
    *":$HOME/.clawbot:"*) ;;
    *) export PYTHONPATH="${PYTHONPATH:+$PYTHONPATH:}$HOME/.clawbot" ;;
  esac
...
WORKERS="${CLAWBOT_TEST_WORKERS:-4}"
...
echo "▶ running pytest with $WORKERS workers, clawbotic env, in $REPO_ROOT"
