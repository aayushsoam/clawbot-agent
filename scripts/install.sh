...
# Python-driven tool session (e.g. Clawbot terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:Aayushsoam/clawbot-agent.git"
REPO_URL_HTTPS="https://github.com/aayushsoam/clawbot-agent.git"
CLAWBOT_HOME="${CLAWBOT_HOME:-$HOME/.clawbot}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${CLAWBOT_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$CLAWBOT_INSTALL_DIR"
...
#   data still at /root/.clawbot (CLAWBOT_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   default (root, Linux): /usr/local/lib/clawbot-agent"
            echo "  --clawbot-home PATH  Data directory (default: ~/.clawbot, or \$CLAWBOT_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Clawbot installs the code under"
            echo "  /usr/local/lib/clawbot-agent and links the command into"
            echo "  /usr/local/bin/clawbot (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$CLAWBOT_HOME"
            echo "  (default /root/.clawbot).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$CLAWBOT_HOME/clawbot-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs clawbot setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $CLAWBOT_HOME/clawbot-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $CLAWBOT_HOME/clawbot-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/clawbot-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $CLAWBOT_HOME/clawbot-agent — then preserve it)
#
# Always no-op when the user set --dir or $CLAWBOT_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in CLAWBOT_HOME.
    if is_termux; then
        INSTALL_DIR="$CLAWBOT_HOME/clawbot-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$CLAWBOT_HOME/clawbot-agent/.git" ]; then
            INSTALL_DIR="$CLAWBOT_HOME/clawbot-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/clawbot-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/clawbot-agent"
        ROOT_FHS_LAYOUT=true
        log_info "Root install on Linux — using FHS layout"
        log_info "  Code:    $INSTALL_DIR"
        log_info "  Command: /usr/local/bin/clawbot"
        log_info "  Data:    $CLAWBOT_HOME (unchanged)"
        return 0
    fi
...
get_clawbot_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/clawbot" ]; then
        echo "$link_dir/clawbot"
    else
        echo "clawbot"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/Aayushsoam/clawbot-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/clawbot-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/clawbot-uv-installer.$$.sh")"
...
node_version_supports_dashboard() {
    local ver="${1#v}"
    local major="${ver%%.*}"
    local rest="${ver#*.}"
    local minor="${rest%%.*}"
...
check_node() {
    ...
    if command -v node &> /dev/null; then
        ...
        found_ver=$("$node_path" --version 2>/dev/null || true)
        if node_version_supports_dashboard "$found_ver"; then
            if npm_is_usable; then
                log_success "Node.js $found_ver found"
                HAS_NODE=true
                return 0
            fi
            log_warn "Node.js $found_ver found, but npm is not runnable"
        else
            log_warn "Node.js ${found_ver:-unknown} found, but dashboard needs Node.js 20.19+ or 22.12+"
        fi
    fi

    # Check our own managed install from a previous run
    if [ -x "$CLAWBOT_HOME/node/bin/node" ]; then
        local found_ver
        found_ver=$("$CLAWBOT_HOME/node/bin/node" --version 2>/dev/null || true)
        if node_version_supports_dashboard "$found_ver"; then
            export PATH="$CLAWBOT_HOME/node/bin:$PATH"
            if npm_is_usable; then
                log_success "Node.js $found_ver found (Clawbot-managed)"
                HAS_NODE=true
                return 0
            fi
            log_warn "Clawbot-managed Node.js $found_ver found, but npm is not runnable; reinstalling Node.js $NODE_VERSION"
        else
            log_warn "Clawbot-managed Node.js ${found_ver:-unknown} is too old; reinstalling Node.js $NODE_VERSION"
        fi
    fi
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_info "Installing Clawbot-managed Node.js $NODE_VERSION LTS..."
    fi
    install_node
}
...
install_node() {
    ...
    log_info "Extracting to ~/.clawbot/node/..."
    ...
    # Place into ~/.clawbot/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$CLAWBOT_HOME/node"
    mkdir -p "$CLAWBOT_HOME"
    mv "$extracted_dir" "$CLAWBOT_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$CLAWBOT_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$CLAWBOT_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$CLAWBOT_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$CLAWBOT_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CLAWBOT_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.clawbot/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Clawbot install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Clawbot Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="clawbot-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Clawbot behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up clawbot command..."

    if [ "$USE_VENV" = true ]; then
        CLAWBOT_BIN="$INSTALL_DIR/venv/bin/clawbot"
    else
        CLAWBOT_BIN="$(which clawbot 2>/dev/null || echo "")"
        if [ -z "$CLAWBOT_BIN" ]; then
            log_warn "clawbot not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$CLAWBOT_BIN" ]; then
        log_warn "clawbot entry point not found at $CLAWBOT_BIN"
    ...
    # Create a user-facing shim for the clawbot command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $CLAWBOT_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$CLAWBOT_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/clawbot"
    cat > "$command_link_dir/clawbot" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$CLAWBOT_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/clawbot"
    log_success "Installed clawbot launcher → $command_link_display_dir/clawbot"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "clawbot command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v clawbot' \
                >/dev/null 2>&1; then
        ...
        log_info "clawbot not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Clawbot Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Clawbot Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Clawbot Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so clawbot works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.clawbot directory structure (config at top level, code in subdir)
    mkdir -p "$CLAWBOT_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.clawbot/.env (top level, easy to find)
    if [ ! -f "$CLAWBOT_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CLAWBOT_HOME/.env"
            log_success "Created ~/.clawbot/.env from template"
        else
            touch "$CLAWBOT_HOME/.env"
            log_success "Created ~/.clawbot/.env"
        fi
    else
        log_info "~/.clawbot/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$CLAWBOT_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.clawbot/config.yaml (top level, easy to find)
    if [ ! -f "$CLAWBOT_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CLAWBOT_HOME/config.yaml"
            log_success "Created ~/.clawbot/config.yaml from template"
        fi
    else
        log_info "~/.clawbot/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$CLAWBOT_HOME/SOUL.md" ]; then
        cat > "$CLAWBOT_HOME/SOUL.md" << 'SOUL_EOF'
...
-->
SOUL_EOF
        log_success "Created ~/.clawbot/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.clawbot/"

    # Seed bundled skills into ~/.clawbot/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.clawbot/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.clawbot/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CLAWBOT_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$CLAWBOT_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.clawbot/skills/"
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    local env_file="$CLAWBOT_HOME/.env"
    ...
    {
        echo ""
        echo "# Clawbot Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; Clawbot will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (clawbot --tui may not work)"
        }
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'clawbot setup' after install."
        return 0
    fi
    ...
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m clawbot_cli.main setup < /dev/tty
    else
        python -m clawbot_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CLAWBOT_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Clawbot to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CLAWBOT_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'clawbot whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                CLAWBOT_CMD="$(get_clawbot_command_path)"
                $CLAWBOT_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'clawbot whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'clawbot gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $CLAWBOT_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CLAWBOT_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: clawbot gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: clawbot gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $CLAWBOT_CMD gateway > "$CLAWBOT_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.clawbot/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: clawbot gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: clawbot gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $CLAWBOT_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $CLAWBOT_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $CLAWBOT_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}clawbot${NC}              Start chatting"
    echo -e "   ${GREEN}clawbot setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}clawbot config${NC}       View/edit configuration"
    echo -e "   ${GREEN}clawbot config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}clawbot gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}clawbot update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'clawbot' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'clawbot' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'clawbot' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$CLAWBOT_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$CLAWBOT_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$CLAWBOT_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$CLAWBOT_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$CLAWBOT_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    CLAWBOT_CMD="$(command -v clawbot 2>/dev/null || echo "")"
    if [ -n "$CLAWBOT_CMD" ]; then
        log_info "Running clawbot setup..."
        "$CLAWBOT_CMD" setup
    else
        log_warn "clawbot command not found on PATH"
        log_info "Try: python -m clawbot_cli.main setup"
    fi
}
