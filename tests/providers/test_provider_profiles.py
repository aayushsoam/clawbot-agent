...
class TestRegistry:
    ...
    def test_alias_lookup(self):
        ...
        assert get_provider_profile("soam-portal").name == "soam"
...
class TestOpenRouterProfile:
    ...
    def test_reasoning_disabled_still_passes(self):
        """OpenRouter passes disabled reasoning through (unlike Soam)."""
        p = get_provider_profile("openrouter")
        eb, _ = p.build_api_kwargs_extras(
...
class TestSoamProfile:
    def test_tags(self):
        from agent.portal_tags import soam_portal_tags
        p = get_provider_profile("soam")
        body = p.build_extra_body()
