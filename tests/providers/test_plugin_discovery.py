...
Verifies that:
 ...
 2. User plugins at $CLAWBOT_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_clawbot_user_provider")
        ):
            del sys.modules[mod]
...
def test_all_36_profiles_register():
    """After discovery, the registry must contain exactly 36 distinct profiles."""
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from clawbot_cli import plugins as plugin_mod
    ...
    user_plugin = clawbot_home / "plugins" / "test-model-provider"
