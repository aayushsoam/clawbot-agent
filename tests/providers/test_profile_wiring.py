...
class TestSoamProfileParity:
    def test_tags(self, transport):
        legacy = transport.build_kwargs(
            model="clawbot-3", messages=_msgs(), tools=None, provider_profile=get_provider_profile("soam"),
        )
        profile = transport.build_kwargs(
            model="clawbot-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("soam"),
        )
    ...
    def test_reasoning_omitted_when_disabled(self, transport):
        ...
        legacy = transport.build_kwargs(
            model="clawbot-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("soam"), supports_reasoning=True, reasoning_config=rc,
        )
        profile = transport.build_kwargs(
            model="clawbot-3", messages=_msgs(), tools=None,
            provider_profile=get_provider_profile("soam"),
...
class TestRequestOverridesParity:
    ...
    def test_extra_body_override_merges_with_provider_body(self, transport):
        ...
        assert kw["extra_body"]["tags"] == soam_portal_tags()  # from profile
