"""
Tests for mcp_serve — Clawbot MCP server.
...
def _isolate_clawbot_home(tmp_path, monkeypatch):
    """Redirect CLAWBOT_HOME to a temp directory."""
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    try:
        import clawbot_constants
        monkeypatch.setattr(clawbot_constants, "get_clawbot_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking clawbot_state schema."""
