...
- Path resolution (absolute, relative to CLAWBOT_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp CLAWBOT_HOME."""
    clawbot_home = tmp_path / ".clawbot"
    clawbot_home.mkdir()
    (clawbot_home / "cron").mkdir()
    (clawbot_home / "cron" / "output").mkdir()
    (clawbot_home / "scripts").mkdir()
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "CLAWBOT_DIR", clawbot_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", clawbot_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", clawbot_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", clawbot_home / "cron" / "output")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.clawbot/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("CLAWBOT_SESSION_PLATFORM") is None
        assert os.environ.get("CLAWBOT_SESSION_CHAT_ID") is None
        assert os.environ.get("CLAWBOT_SESSION_CHAT_NAME") is None
