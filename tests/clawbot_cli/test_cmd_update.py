...
from clawbot_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_refreshes_repo_and_tui_node_dependencies(
        ...
        from clawbot_cli import main as hm
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "clawbot_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "clawbot_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("clawbot_cli.config.check_config_version", return_value=(1, 2)), patch(
            "clawbot_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("clawbot_cli.main.sys") as mock_sys:
            ...
            from clawbot_cli.config import migrate_config
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        default_p = SimpleNamespace(name="default", path=Path("/fake/.clawbot"))
        active_p = SimpleNamespace(name="bit", path=Path("/fake/.clawbot/profiles/bit"))
        other_p = SimpleNamespace(name="work", path=Path("/fake/.clawbot/profiles/work"))
        ...
        with (
            patch("clawbot_cli.profiles.list_profiles", return_value=all_profiles),
            patch("clawbot_cli.profiles.seed_profile_skills", side_effect=fake_seed),
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("clawbot_cli.profiles.list_profiles", return_value=[default_p]),
            patch("clawbot_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
