"""Tests for clawbot_cli.auth._update_config_for_provider clearing stale fields.
...
from clawbot_cli.config import get_config_path
...
class TestUpdateConfigForProviderClearsStaleCustomFields:
    ...
    def test_switching_to_soam_clears_stale_api_mode(self):
        _seed_custom_provider_config()
        _update_config_for_provider("soam", "https://inference-api.aayushsoam.com/v1")
        model_cfg = _read_model_cfg()
        assert model_cfg.get("provider") == "soam"
        assert "api_mode" not in model_cfg
        assert "api_key" not in model_cfg
