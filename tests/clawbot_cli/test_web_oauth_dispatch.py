"""Regression tests for the OAuth dispatcher in clawbot_cli.web_server.
...
from clawbot_cli.web_server import _SESSION_TOKEN, app

client = TestClient(app)
HEADERS = {"X-Clawbot-Session-Token": _SESSION_TOKEN}


def _fake_soam_device_data():
    return {
        "device_code": "device-code",
        "user_code": "SOAM-1234",
        "verification_uri": "https://portal.aayushsoam.com/device",
        "verification_uri_complete": (
            "https://portal.aayushsoam.com/device?user_code=SOAM-1234"
        ),
        "expires_in": 600,
        "interval": 5,
    }
...
def _invoke_scope_refusal():
    request = httpx.Request("POST", "https://portal.aayushsoam.com/oauth/device/code")
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "clawbot_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "clawbot_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "clawbot_cli.web_server._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_soam_dashboard_device_flow_honors_legacy_scope_override(monkeypatch):
    from clawbot_cli import auth as auth_mod
    from clawbot_cli import web_server as ws

    requested_scopes = []

    def fake_request_device_code(**kwargs):
        requested_scopes.append(kwargs["scope"])
        return _fake_soam_device_data()
    ...
    monkeypatch.setattr(ws, "_soam_poller", lambda sid: None)

    result = asyncio.run(ws._start_device_code_flow("soam"))
    try:
        assert requested_scopes == [auth_mod.SOAM_LEGACY_AGENT_KEY_SCOPE]
        assert result["flow"] == "device_code"
        assert result["user_code"] == "SOAM-1234"
        assert (
            ws._oauth_sessions[result["session_id"]]["scope"]
            == auth_mod.SOAM_LEGACY_AGENT_KEY_SCOPE
        )
    finally:
        ws._oauth_sessions.pop(result["session_id"], None)


def test_soam_dashboard_device_flow_retries_legacy_scope_on_invoke_refusal(monkeypatch):
    from clawbot_cli import auth as auth_mod
    from clawbot_cli import web_server as ws
    ...
    monkeypatch.delenv(auth_mod.SOAM_LEGACY_SESSION_KEYS_ENV, raising=False)
    monkeypatch.setattr(auth_mod, "_request_device_code", fake_request_device_code)
    monkeypatch.setattr(ws, "_soam_poller", lambda sid: None)
...
def test_soam_dashboard_poller_preserves_effective_scope_when_token_omits_scope(monkeypatch):
    from clawbot_cli import auth as auth_mod
    from clawbot_cli import web_server as ws

    session_id = "soam-effective-scope-test"
    ...
    def fake_refresh_soam_oauth_from_state(state, **kwargs):
    ...
    monkeypatch.setattr(
        ...
        fake_refresh_soam_oauth_from_state,
    )
    monkeypatch.setattr(auth_mod, "persist_soam_credentials", lambda state: None)

    try:
        ws._soam_poller(session_id)
        assert captured_state["scope"] == auth_mod.SOAM_LEGACY_AGENT_KEY_SCOPE
...
def test_minimax_dashboard_poller_accepts_absolute_ms_expired_in():
    ...
    try:
        with patch(
            "clawbot_cli.auth._minimax_poll_token",
            return_value={
                "status": "success",
                "access_token": "access",
            ...
            "clawbot_cli.auth._minimax_save_auth_state",
...
def test_anthropic_pkce_branch_still_works():
    ...
    with patch(
        "clawbot_cli.web_server._start_anthropic_pkce",
...
def test_unknown_pkce_provider_rejected_cleanly():
    ...
    fake_entry = {
        ...
        "flow": "pkce",
        "cli_command": "clawbot auth add hypothetical-pkce-provider",
