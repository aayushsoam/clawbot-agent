"""Tests for clawbot_cli.status model/provider display."""
...
def _patch_common_status_deps(monkeypatch, status_mod, tmp_path, *, openai_base_url=""):
    ...
    monkeypatch.setattr(status_mod, "get_clawbot_home", lambda: tmp_path, raising=False)
    ...
    monkeypatch.setattr(auth_mod, "get_soam_auth_status", lambda: {}, raising=False)
...
def test_show_status_displays_configured_dict_model_and_provider_label(monkeypatch, capsys, tmp_path):
    from clawbot_cli import status as status_mod
...
def test_show_status_reports_managed_soam_features(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("clawbot_cli.status.managed_soam_tools_enabled", lambda: True)
    from clawbot_cli import status as status_mod
    ...
    monkeypatch.setattr(
        status_mod,
        "load_config",
        lambda: {"model": {"default": "claude-opus-4-6", "provider": "soam"}},
        raising=False,
    )
    monkeypatch.setattr(status_mod, "resolve_requested_provider", lambda requested=None: "soam", raising=False)
    monkeypatch.setattr(status_mod, "resolve_provider", lambda requested=None, **kwargs: "soam", raising=False)
    monkeypatch.setattr(status_mod, "provider_label", lambda provider: "Soam Portal", raising=False)
    monkeypatch.setattr(
        status_mod,
    ...
    out = capsys.readouterr().out
    assert "Soam Tool Gateway" in out
    assert "Browser automation" in out
    assert "active via Soam subscription" in out


def test_show_status_hides_soam_subscription_section_when_feature_flag_is_off(monkeypatch, capsys, tmp_path):
    monkeypatch.setattr("clawbot_cli.status.managed_soam_tools_enabled", lambda: False)
    from clawbot_cli import status as status_mod
    ...
    out = capsys.readouterr().out
    assert "Soam Tool Gateway" not in out
...
def test_show_status_reports_empty_lmstudio_listing_as_reachable(monkeypatch, capsys, tmp_path):
    ...
    monkeypatch.setattr(
        "clawbot_cli.models.probe_lmstudio_models",
