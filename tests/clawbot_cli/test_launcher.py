"""Tests for the top-level `./clawbot` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./clawbot` should use `clawbot_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "clawbot"
    called = []

    fake_main_module = types.ModuleType("clawbot_cli.main")

    def fake_main():
        called.append("clawbot_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "clawbot_cli.main", fake_main_module)
    ...
    assert called == ["clawbot_cli.main"]
