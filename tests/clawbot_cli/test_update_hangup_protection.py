"""Tests for SIGHUP protection and stdout mirroring in ``clawbot update``.

Covers ``_UpdateOutputStream``, ``_install_hangup_protection``, and
``_finalize_update_output`` in ``clawbot_cli/main.py``.  These exist so
that ``clawbot update`` survives a terminal disconnect mid-install
...
class TestInstallHangupProtection:
    ...
    def test_installs_sighup_ignore(self, tmp_path, monkeypatch):
        ...
        # Clear cached get_clawbot_home if present
        import clawbot_cli.config as _cfg
        if hasattr(_cfg, "_CLAWBOT_HOME_CACHE"):
            _cfg._CLAWBOT_HOME_CACHE = None  # type: ignore[attr-defined]
    ...
    def test_wraps_stdout_and_stderr_with_mirror(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            assert "clawbot update started" in contents
        finally:
            _finalize_update_output(state)
    ...
    def test_non_fatal_if_log_setup_fails(self, monkeypatch):
        """If get_clawbot_home() raises, stdio must be left untouched but SIGHUP still handled."""
        prev_out, prev_err = sys.stdout, sys.stderr
        ...
        monkeypatch.setattr(
            "clawbot_cli.config.get_clawbot_home", _boom, raising=True
        )
