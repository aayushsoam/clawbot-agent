"""Regression tests for TOCTOU-safe credential file writers in ``clawbot_cli.auth``.
...
# ---------------------------------------------------------------------------
# _save_auth_store  (~/.clawbot/auth.json — every native OAuth provider)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Soam shared-credential store write (inside _write_shared_soam_state)
# ---------------------------------------------------------------------------


def test_shared_soam_store_writes_0o600_with_0o700_parent(tmp_path, monkeypatch):
    """The Soam shared-credential store must land at 0o600 / parent 0o700."""
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    # _soam_shared_store_path() refuses to touch the real shared store during
    ...
    # home" reference — which currently tracks CLAWBOT_HOME via
    # get_default_clawbot_root() — can't collide with our override and
    # falsely claim we're writing to the real user's shared store.
    monkeypatch.setenv("CLAWBOT_SHARED_AUTH_DIR", str(tmp_path / "shared_override"))
    old_umask = os.umask(0o022)
    try:
        from clawbot_cli import auth as auth_mod

        state = {
            "access_token": "soam-access-xxx",
            "refresh_token": "soam-refresh-xxx",
            "token_type": "Bearer",
        ...
        auth_mod._write_shared_soam_state(state)
        path = auth_mod._soam_shared_store_path()
    finally:
        os.umask(old_umask)

    assert path.exists(), "shared Soam store was not written"
    mode = stat.S_IMODE(path.stat().st_mode)
    parent_mode = stat.S_IMODE(path.parent.stat().st_mode)

    assert mode == 0o600, (
        f"Soam shared store mode 0o{mode:o} != 0o600 — TOCTOU race regressed"
    )
    assert parent_mode == 0o700, (
        f"Soam shared store parent dir mode 0o{parent_mode:o} != 0o700"
    )

    data = json.loads(path.read_text())
    assert data["refresh_token"] == "soam-refresh-xxx"
