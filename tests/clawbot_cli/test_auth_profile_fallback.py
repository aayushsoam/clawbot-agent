...
When ``CLAWBOT_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.clawbot/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.clawbot            (has its own auth.json fixture)
    * Profile     -> tmp_path/.clawbot/profiles/coder   (active, CLAWBOT_HOME points here)
...
def test_provider_auth_state_falls_back_to_global_when_profile_has_none(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "soam": {"access_token": "soam-global", "refresh_token": "rt-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={}))

    state = get_provider_auth_state("soam")
    assert state is not None
    assert state["access_token"] == "soam-global"
...
def test_provider_auth_state_profile_wins_when_present(profile_env):
    ...
    _write(profile_env["global"] / "auth.json", _make_auth_store(providers={
        "soam": {"access_token": "soam-global"},
    }))
    _write(profile_env["profile"] / "auth.json", _make_auth_store(providers={
        "soam": {"access_token": "soam-profile"},
    }))

    state = get_provider_auth_state("soam")
    assert state is not None
    assert state["access_token"] == "soam-profile"
...
def test_provider_auth_state_returns_none_when_neither_has_it(profile_env):
    ...
    assert get_provider_auth_state("soam") is None
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (CLAWBOT_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.clawbot as the "real home" — which is NOT equal
    # to the CLAWBOT_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    clawbot_home = tmp_path / "classic"
    clawbot_home.mkdir()
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

    _write(clawbot_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from clawbot_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: CLAWBOT_HOME is set to a custom path that is NOT under
    # ~/.clawbot/profiles/ — get_default_clawbot_root() returns CLAWBOT_HOME
...
def test_write_credential_pool_targets_profile_not_global(profile_env):
    from clawbot_cli.auth import read_credential_pool, write_credential_pool
