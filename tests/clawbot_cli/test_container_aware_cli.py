...
def container_env(tmp_path, monkeypatch):
    """Set up a fake CLAWBOT_HOME with .container-mode file."""
    clawbot_home = tmp_path / ".clawbot"
    clawbot_home.mkdir()
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    monkeypatch.delenv("CLAWBOT_DEV", raising=False)

    container_mode = clawbot_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=clawbot-agent\n"
        "exec_user=clawbot\n"
        "clawbot_bin=/data/current-package/bin/clawbot\n"
    )
    return clawbot_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("clawbot_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "clawbot-agent"
    assert info["exec_user"] == "clawbot"
    assert info["clawbot_bin"] == "/data/current-package/bin/clawbot"


def test_get_container_exec_info_none_inside_container(container_env):
    """Returns None when we're already inside a container."""
    with patch("clawbot_constants.is_container", return_value=True):
        info = get_container_exec_info()
...
def test_get_container_exec_info_skipped_when_clawbot_dev(container_env, monkeypatch):
    """Returns None when CLAWBOT_DEV=1 is set (dev mode bypass)."""
    monkeypatch.setenv("CLAWBOT_DEV", "1")
...
def test_get_container_exec_info_not_skipped_when_clawbot_dev_zero(container_env, monkeypatch):
    """CLAWBOT_DEV=0 does NOT trigger bypass — only '1' does."""
    monkeypatch.setenv("CLAWBOT_DEV", "0")
...
def test_get_container_exec_info_defaults():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        clawbot_home = Path(tmpdir) / ".clawbot"
        clawbot_home.mkdir()
        (clawbot_home / ".container-mode").write_text(
            "# minimal file with no keys\n"
        )

        with patch("clawbot_constants.is_container", return_value=False), \
             patch.dict(get_container_exec_info.__globals__, {"get_clawbot_home": lambda: clawbot_home}), \
             patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CLAWBOT_DEV", None)
            info = get_container_exec_info()
...
def test_get_container_exec_info_docker_backend(container_env):
    ...
    (container_env / ".container-mode").write_text(
        "backend=docker\n"
        "container_name=clawbot-custom\n"
        "exec_user=myuser\n"
        "clawbot_bin=/opt/clawbot/bin/clawbot\n"
    )
    ...
    assert info["backend"] == "docker"
    assert info["container_name"] == "clawbot-custom"
    assert info["exec_user"] == "myuser"
    assert info["clawbot_bin"] == "/opt/clawbot/bin/clawbot"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from clawbot_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "clawbot"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "clawbot-agent" in cmd
    assert "/data/current-package/bin/clawbot" in cmd
    assert "chat" in cmd
