"""Unit tests for clawbot_cli.pty_bridge — PTY spawning + byte forwarding.
...
class TestPtyBridgeIO:
    def test_reads_child_stdout(self):
        bridge = PtyBridge.spawn(["/bin/sh", "-c", "printf clawbot-ok"])
        try:
            output = _read_until(bridge, b"clawbot-ok")
            assert b"clawbot-ok" in output
        finally:
            bridge.close()
...
class TestPtyBridgeEnv:
    ...
    def test_env_is_forwarded(self):
        bridge = PtyBridge.spawn(
            ["/bin/sh", "-c", "printf %s \"$CLAWBOT_PTY_TEST\""],
            env={**os.environ, "CLAWBOT_PTY_TEST": "pty-env-works"},
        )
        try:
