"""Tests for `clawbot curator run` CLI behavior."""
...
def test_run_defaults_to_synchrosoam(monkeypatch, capsys):
    import agent.curator as curator_state
    import clawbot_cli.curator as curator_cli
    ...
    assert calls[0]["synchrosoam"] is True
...
def test_run_background_opts_into_async(monkeypatch, capsys):
    ...
    assert calls[0]["synchrosoam"] is False
...
def test_run_sync_wins_over_background(monkeypatch):
    ...
    assert curator_cli._cmd_run(_args(synchrosoam=True, background=True)) == 0

    assert calls[0]["synchrosoam"] is True


def test_dry_run_default_reports_synchrosoam_wording(monkeypatch, capsys):
    import agent.curator as curator_state
    import clawbot_cli.curator as curator_cli
    ...
    assert "Read the report with `clawbot curator status`" in out
