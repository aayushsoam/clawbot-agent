...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    ...
    assert "Start with:   clawbot gateway" in output
    assert "systemd (user)" not in output


def test_show_status_reports_soam_auth_error(monkeypatch, capsys, tmp_path):
    from clawbot_cli import status as status_mod
    import clawbot_cli.auth as auth_mod
    import clawbot_cli.gateway as gateway_mod
    ...
    output = capsys.readouterr().out
    assert "Soam Portal   ✗ not logged in (run: clawbot auth add soam --type oauth)" in output
...
class TestShowStatusXaiOAuth:
    """xAI OAuth row in clawbot status."""
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.clawbot/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.clawbot/auth.json" in out
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: clawbot auth add xai-oauth)" in out
    ...
    def test_import_failure_does_not_break_other_oauth_providers(self, monkeypatch, capsys, tmp_path):
        """Soam/Codex/MiniMax rows must still appear when xAI import fails."""
        import clawbot_cli.auth as auth_mod
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_soam_auth_status",
