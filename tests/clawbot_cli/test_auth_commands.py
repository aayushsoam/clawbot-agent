...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    payload = json.loads((tmp_path / "clawbot" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_clawbot_oauth_login_pure",
        lambda: {
            "access_token": token,
    ...
    entry = next(item for item in entries if item["source"] == "manual:clawbot_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:clawbot_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_soam_oauth_persists_pool_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("soam@example.com")
    monkeypatch.setattr(
        "clawbot_cli.auth._soam_device_code_login",
        lambda **kwargs: {
    ...
    entries = payload["credential_pool"]["soam"]
    device_code_entries = [
    ...
    # `clawbot auth add soam` must also populate providers.soam so the
    # 401-recovery path (resolve_soam_runtime_credentials) can mint a fresh
    # agent_key when the 24h TTL expires. If this mirror is missing, recovery
    # raises "Clawbot is not logged into Soam Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_soam_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`clawbot auth add soam --type oauth --label <name>` must preserve the
    custom label end-to-end — it was silently dropped in the first cut of the
    persist_soam_credentials helper because `--label` wasn't threaded through.
    """
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    token = _jwt_with_email("soam@example.com")
    ...
    pool_entry = payload["credential_pool"]["soam"][0]
    assert pool_entry["source"] == "device_code"
    assert pool_entry["label"] == "my-soam"

    # … and survives in providers.soam so a subsequent load_pool() re-seeds
    # it without reverting to the auto-derived fingerprint.
    assert payload["providers"]["soam"]["label"] == "my-soam"
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from clawbot_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from clawbot_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`clawbot logout --provider openai-codex` must still clear model.provider.
    ...
    (clawbot_home / "config.yaml").write_text(
        "model:\n"
    ...
    config_text = (clawbot_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `clawbot logout` should target configured Codex if auth has no active provider."""
    clawbot_home = tmp_path / "clawbot"
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (clawbot_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((clawbot_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from clawbot_cli.auth import _reset_config_provider
    ...
    with patch("clawbot_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from clawbot_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "clawbot_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("clawbot_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("clawbot_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("clawbot_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "clawbot_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (clawbot_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from clawbot_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((clawbot_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from clawbot_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from clawbot_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `clawbot auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    monkeypatch.setattr(
        "agent.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    ...
    # Pre-existing suppression (simulating a prior `clawbot auth remove`)
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clawbot_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((clawbot_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`clawbot auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.clawbot/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.clawbot/.env (not the shell), the
    ...
    (clawbot_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (clawbot_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `clawbot auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from clawbot_cli.auth import is_source_suppressed
    from clawbot_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `clawbot auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Clawbot reads from has a
...
def test_seed_from_singletons_respects_soam_suppression(tmp_path, monkeypatch):
    """soam device_code must not re-seed from auth.json when suppressed."""
    clawbot_home = tmp_path / "clawbot"
    clawbot_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    ...
    (clawbot_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {"soam": {"access_token": "tok", "refresh_token": "r", "expires_at": 9999999999}},
        "suppressed_sources": {"soam": ["device_code"]},
    }))

    from agent.credential_pool import _seed_from_singletons
    entries = []
    changed, active = _seed_from_singletons("soam", entries)
    assert changed is False
    assert entries == []
    assert active == set()
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import clawbot_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_clawbot_pkce_suppression(tmp_path, monkeypatch):
    """anthropic clawbot_pkce must not re-seed from ~/.clawbot/.anthropic_oauth.json when suppressed."""
    clawbot_home = tmp_path / "clawbot"
    clawbot_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

    import yaml
    (clawbot_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (clawbot_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["clawbot_pkce"]},
    }))

    # Stub the readers so only clawbot_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_clawbot_oauth_credentials", lambda: {
    ...
    # clawbot_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "clawbot_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (clawbot_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_credential_sources_registry_has_expected_steps():
    ...
    required = {
        ...
        "~/.clawbot/.anthropic_oauth.json",
        "auth.json providers.soam",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `clawbot auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `clawbot auth add openai-codex`)
