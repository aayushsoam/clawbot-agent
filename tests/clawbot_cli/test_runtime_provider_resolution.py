...
from clawbot_cli import runtime_provider as rp
...
def test_named_custom_provider_does_not_shadow_builtin_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: {
            ...
            "api_key": "soam-runtime-key",
            "source": "portal",
            "expires_at": None,
        },
    )

    resolved = rp.resolve_runtime_provider(requested="soam")

    assert resolved["provider"] == "soam"
    assert resolved["base_url"] == "https://inference-api.aayushsoam.com/v1"
    assert resolved["api_key"] == "soam-runtime-key"
    assert resolved["requested_provider"] == "soam"
...
def test_named_custom_provider_skipped_for_canonical_built_in(monkeypatch):
    """Companion to the test above: ``soam`` is a canonical provider name
    (``resolve_provider('soam') == 'soam'``), so a custom entry with that name
    ...
    entry = rp._get_named_custom_provider("soam")
...
def test_resolve_requested_provider_precedence(monkeypatch):
    ...
    monkeypatch.setattr(rp, "_get_model_config", lambda: {})
    assert rp.resolve_requested_provider() == "soam"
...
def test_auto_detected_soam_auth_failure_falls_through_to_openrouter(monkeypatch):
    """When auto-detect picks Soam but credentials are revoked, fall through to OpenRouter."""
    from clawbot_cli.auth import AuthError
    ...
    # resolve_provider returns "soam" (stale active_provider in auth.json)
    monkeypatch.setattr(rp, "resolve_provider", lambda *a, **k: "soam")
    ...
    # Soam credential resolution fails with revoked token
    monkeypatch.setattr(
        rp, "resolve_soam_runtime_credentials",
        lambda **kw: (_ for _ in ()).throw(
            AuthError("Refresh session has been revoked",
                      provider="soam", code="invalid_grant", relogin_required=True)
        ),
    )
...
def test_explicit_soam_auth_failure_still_raises(monkeypatch):
    """When user explicitly requests Soam and auth fails, the error should propagate."""
    from clawbot_cli.auth import AuthError
    import pytest
    ...
    # With explicit "soam", should raise — don't silently switch providers
    with pytest.raises(AuthError, match="Refresh session has been revoked"):
        rp.resolve_runtime_provider(requested="soam")
...
class TestAzureFoundryResolution:
    ...
    def test_azure_foundry_missing_api_key_raises(self, monkeypatch):
        ...
        # `get_env_value` reads from ~/.clawbot/.env — mock it to return None
        # so the resolver can't find a key there either.
        import clawbot_cli.config as cfg_mod
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from clawbot_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from clawbot_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from clawbot_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
def test_minimax_oauth_runtime_uses_inference_base_url(monkeypatch):
    ...
    from clawbot_cli.auth import MINIMAX_OAUTH_CN_INFERENCE
