"""Tests for IRC gateway configuration via `clawbot setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Clawbot install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Clawbot install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#clawbot")
            monkeypatch.setenv("IRC_NICKNAME", "clawbot-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `clawbot setup gateway` experience with IRC present."""
