"""Tests for the Command Installation check in clawbot doctor."""
...
import clawbot_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal CLAWBOT_HOME + PROJECT_ROOT for doctor tests."""
    ...
    clawbot_bin = venv_bin_dir / "clawbot"
    clawbot_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    clawbot_bin.chmod(0o755)
    ...
    return home, project, clawbot_bin
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, clawbot_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "clawbot"
        cmd_link.symlink_to(clawbot_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "clawbot doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "clawbot"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == clawbot_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "clawbot"
        wrong_target = tmp_path / "wrong_clawbot"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        clawbot_bin = project / ".venv" / "bin" / "clawbot"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "clawbot"
        cmd_link.symlink_to(clawbot_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/clawbot" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/clawbot is a regular file (not symlink), accept it."""
        home, project, clawbot_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "clawbot"
        cmd_link.write_text("#!/bin/sh\nexec python -m clawbot_cli.main \"$@\"\n")
