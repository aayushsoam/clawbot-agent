"""Tests for clawbot_cli.tools_config platform tool persistence."""
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `clawbot tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "clawbot_cli.tools_config._xai_credentials_present", lambda: True
    )
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "clawbot_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `clawbot tools`,
    ...
    # User explicitly opted into spotify but not x_search via `clawbot tools`.
    config = {"platform_toolsets": {"cli": ["clawbot-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[clawbot-cli, spotify]`` (composite + configurable) must keep the full
    ``clawbot-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``clawbot-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when clawbot-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["clawbot-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``clawbot-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["clawbot-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _CLAWBOT_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `clawbot tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("clawbot_cli.tools_config.save_config"):
...
def test_toolset_has_keys_for_vision_accepts_codex_auth(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/aayushsoam/clawbot-agent/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (clawbot-cli, clawbot-telegram, etc.) must NOT
    ...
    ``clawbot tools``, but clawbot-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "clawbot-cli",  # <-- the culprit
    ...
    # clawbot-cli must NOT survive — it's a platform default, not an MCP server
    assert "clawbot-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_clawbot_telegram():
    """Same bug for Telegram — clawbot-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "clawbot-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "clawbot-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "clawbot-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_soam_subscription_when_logged_in(monkeypatch):
    monkeypatch.setattr("clawbot_cli.tools_config.managed_soam_tools_enabled", lambda: True)
    config = {"model": {"provider": "soam"}}

    monkeypatch.setattr(
        "clawbot_cli.soam_subscription.get_soam_auth_status",
        lambda: {"logged_in": True},
    )

    providers = _visible_providers(TOOL_CATEGORIES["browser"], config)

    assert providers[0]["name"].startswith("Soam Subscription")


def test_visible_providers_hide_soam_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.setattr("clawbot_cli.tools_config.managed_soam_tools_enabled", lambda: False)
    config = {"model": {"provider": "soam"}}
    ...
    assert all(not provider["name"].startswith("Soam Subscription") for provider in providers)
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("clawbot_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.tools_config._toolset_has_keys",
    ...
    monkeypatch.setattr("clawbot_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "clawbot_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config: configured.append(ts_key),
    )
    monkeypatch.setattr("clawbot_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_soam_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("clawbot_cli.tools_config.managed_soam_tools_enabled", lambda: True)
    monkeypatch.setattr("clawbot_cli.soam_subscription.managed_soam_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "soam"},
    ...
    monkeypatch.setattr(
        "clawbot_cli.tools_config._prompt_toolset_checklist",
    ...
    # apply_soam_managed_defaults per platform; a second iteration sees values
    # set by the first as "explicit" and skips them.
    monkeypatch.setattr(
        "clawbot_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "clawbot_cli.soam_subscription.get_soam_auth_status",
    ...
    configured = []
    monkeypatch.setattr(
        "clawbot_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from clawbot_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """clawbot-gateway includes list should cover all messaging platforms."""
        from clawbot_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["clawbot-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from clawbot_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from clawbot_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/aayushsoam/clawbot-agent/issues/6901
    """
    config = {
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from clawbot_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        """Both Soam Subscription and FAL.ai providers must carry the
        imagegen_backend tag so _configure_provider fires the picker."""
        from clawbot_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from clawbot_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("clawbot_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("clawbot_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("clawbot_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`clawbot tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["clawbot-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "clawbot-_test_platform"},
    }

    with mock_patch("clawbot_cli.tools_config.PLATFORMS", {**PLATFORMS, **test_platforms}):
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'clawbot-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `clawbot tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from clawbot_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `clawbot tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from clawbot_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `clawbot tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from clawbot_cli.tools_config import _get_effective_configurable_toolsets
...
@pytest.mark.parametrize("provider,config_key,expected", [
    ...
    ({"name": "T", "tts_provider": "elevenlabs", "managed_soam_feature": "tts", "env_vars": []}, "tts", True),
    ({"name": "B", "browser_provider": "browserbase", "managed_soam_feature": "browser", "env_vars": []}, "browser", True),
    ({"name": "W", "web_backend": "tavily", "managed_soam_feature": "web", "env_vars": []}, "web", True),
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("clawbot_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("clawbot_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("clawbot_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("clawbot_cli.tools_config.save_env_value", lambda k, v: None)
