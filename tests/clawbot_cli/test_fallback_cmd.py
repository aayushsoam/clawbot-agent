"""Tests for `clawbot fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate CLAWBOT_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".clawbot" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from clawbot_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "soam", "model": "Clawbot-4-Llama-3.1-405B"},
            ]
        }
    ...
    def test_skips_incomplete_entries(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "soam", "model": "foo"},  # valid
                "not-a-dict",                          # noise
            ]
        }
        assert _read_chain(cfg) == [{"provider": "soam", "model": "foo"}]

    def test_returns_copies_not_aliases(self):
        from clawbot_cli.fallback_cmd import _read_chain
        cfg = {"fallback_providers": [{"provider": "soam", "model": "foo"}]}
        result = _read_chain(cfg)
        result[0]["provider"] = "mutated"
        assert cfg["fallback_providers"][0]["provider"] == "soam"
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from clawbot_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("clawbot_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("clawbot_cli.main._require_tty"):
            from clawbot_cli.fallback_cmd import cmd_fallback_add
...
class TestRemoveCommand:
    def test_remove_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from clawbot_cli.fallback_cmd import cmd_fallback_remove
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "soam / Clawbot-4")
        with patch("clawbot_cli.setup._curses_prompt_choice", return_value=1):
            from clawbot_cli.fallback_cmd import cmd_fallback_remove
            cmd_fallback_remove(types.SimpleNamespace())
...
class TestClearCommand:
    def test_clear_empty_chain(self, isolated_home, capsys):
        _write_config(isolated_home, {})
        from clawbot_cli.fallback_cmd import cmd_fallback_clear
...
class TestArgparseWiring:
    """Verify `clawbot fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "clawbot_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
