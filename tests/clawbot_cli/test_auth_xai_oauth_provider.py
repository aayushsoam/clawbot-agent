"""Tests for xAI Grok OAuth — tokens stored in Clawbot auth store (~/.clawbot/auth.json)."""
...
def _setup_clawbot_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Clawbot auth store at the given root."""
    ...
    auth_file = clawbot_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("clawbot_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_authorize_url_includes_referrer_clawbot_agent():
    """Attribution: xAI's OAuth server can identify Clawbot-originated logins
    ...
    params = _parse_authorize_url(url)
    assert params["referrer"] == "clawbot-agent"
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    clawbot_home = tmp_path / "clawbot"
    _setup_clawbot_auth(clawbot_home, access_token="")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    clawbot_home = tmp_path / "clawbot"
    _setup_clawbot_auth(clawbot_home, refresh_token="")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    clawbot_home = tmp_path / "clawbot"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_clawbot_auth(clawbot_home, access_token=fresh)
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    ...
    assert creds["source"] == "clawbot-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clawbot_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_clawbot_auth(clawbot_home, access_token=fresh)
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    monkeypatch.setenv("CLAWBOT_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from clawbot_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Clawbot that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("clawbot_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `clawbot model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `clawbot auth list` reflects truth and
    refreshes route through the pool consistently with codex."""
    from agent.credential_pool import load_pool

    clawbot_home = tmp_path / "clawbot"
    fresh = _jwt_with_exp(int(time.time()) + 3600)
    _setup_clawbot_auth(clawbot_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`clawbot auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `clawbot auth remove`.
    from clawbot_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``clawbot auth remove xai-oauth 1`` for a
    ...
    pool = load_pool("xai-oauth")
    assert pool.has_credentials()
    raw = json.loads((clawbot_home / "auth.json").read_text())
    assert "xai-oauth" in raw.get("providers", {})

    # Act: the user runs `clawbot auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((clawbot_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Clawbot invocation."
    )
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_clawbot_auth(clawbot_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    ...
    monkeypatch.setattr("clawbot_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from clawbot_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clawbot_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    clawbot_home = tmp_path / "clawbot"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_clawbot_auth(clawbot_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Clawbot process refreshed the singleton
    ...
    Mirrors the proactive sync codex/soam already perform for the same
    reason, and is what makes the pool actually safe to share across
    profiles + Clawbot processes."""
    from agent.credential_pool import load_pool

    clawbot_home = tmp_path / "clawbot"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_clawbot_auth(clawbot_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    ...
    (clawbot_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_clawbot_auth(clawbot_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``clawbot model`` -> xAI Grok OAuth (or another process
    ...
    cooldown elapses.  Mirrors the codex/soam self-heal pattern."""
    from agent.credential_pool import load_pool, STATUS_EXHAUSTED
    from dataclasses import replace

    clawbot_home = tmp_path / "clawbot"
    stale_at = _jwt_with_exp(int(time.time()) + 3600)
    _setup_clawbot_auth(clawbot_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    ...
    # Simulate the user re-running `clawbot model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``clawbot auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_clawbot_auth(clawbot_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`clawbot auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_auxiliary_client_routes_xai_oauth_through_responses_api(tmp_path, monkeypatch):
    ...
    Soam, etc.).  Users on xAI Grok OAuth would then see surprise charges
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    using it on the sync-back path means every xAI/Codex/Soam refresh in a
    multi-provider setup silently overrides the user's chosen active
    provider (visible to ``clawbot auth status``, ``clawbot setup``, and the
    ``clawbot`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    clawbot_home = tmp_path / "clawbot"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_clawbot_auth(clawbot_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
    ...
    assert raw_after["active_provider"] == "openrouter", (
        ...
        "Soam token rotations silently take over multi-provider users' "
