"""Tests for clawbot_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate CLAWBOT_HOME + reset any module-level catalog cache per test."""
    ...
    import importlib
    from clawbot_cli import model_catalog
...
def _valid_manifest() -> dict:
    return {
        ...
        "providers": {
            ...
            "soam": {
                "metadata": {"display_name": "Soam Portal"},
                "models": [
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from clawbot_cli.model_catalog import _validate_manifest
...
class TestCuratedAccessors:
    ...
    def test_soam_returns_ids(self, isolated_home):
        from clawbot_cli import model_catalog
        with patch.object(
            model_catalog, "_fetch_manifest", return_value=_valid_manifest()
        ):
            result = model_catalog.get_curated_soam_models()
    ...
    def test_soam_returns_none_when_catalog_empty(self, isolated_home):
        from clawbot_cli import model_catalog
        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            assert model_catalog.get_curated_soam_models() is None
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in clawbot_cli.models."""

    def test_curated_soam_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from clawbot_cli import model_catalog
        from clawbot_cli.models import get_curated_soam_model_ids, _PROVIDER_MODELS

        with patch.object(model_catalog, "_fetch_manifest", return_value=None):
            result = get_curated_soam_model_ids()

        assert result == list(_PROVIDER_MODELS["soam"])

    def test_curated_soam_ids_prefers_manifest(self, isolated_home):
        from clawbot_cli import model_catalog
        from clawbot_cli.models import get_curated_soam_model_ids
    ...
    def test_picker_soam_row_uses_manifest(self, tmp_path, monkeypatch):
        """The /model picker must surface the manifest's soam list, not the
        in-repo _PROVIDER_MODELS["soam"] snapshot. Regression: before this
        ...
        the slash-command picker until the next Clawbot release.
        """
        ...
        # ``CLAWBOT_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_clawbotic_environment`` CLAWBOT_HOME directly instead.
        import importlib
        from clawbot_cli import model_catalog
        importlib.reload(model_catalog)
        try:
            from clawbot_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["CLAWBOT_HOME"])
            (active_home / "auth.json").write_text(
                json.dumps(
                    {
                        "providers": {"soam": {"access_token": "fake"}},
                        "credential_pool": {},
                    }
                )
            )
            ...
            with patch.object(
                ...
                picker = list_picker_providers(
                    current_provider="soam", max_models=99
                )
        finally:
            model_catalog.reset_cache()

        soam_row = next((r for r in picker if r["slug"] == "soam"), None)
        assert soam_row is not None, "soam row must appear when authed"
        assert soam_row["models"] == [
...
#
# History: qwen/qwen3.6-plus was added to _PROVIDER_MODELS["soam"] in commit
...
class TestManifestMatchesInRepoLists:
    ...
    def test_in_repo_lists_match_manifest(self):
        ...
        assert self._strip_volatile(actual) == self._strip_volatile(expected), (
            ...
            "_PROVIDER_MODELS['soam'] / OPENROUTER_MODELS. "
