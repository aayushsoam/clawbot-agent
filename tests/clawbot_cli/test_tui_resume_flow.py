...
def main_mod(monkeypatch):
    import clawbot_cli.main as mod
...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["clawbot", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "clawbot_cli.plugins",
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["clawbot", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "clawbot_cli.plugins",
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from clawbot_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "clawbot -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """clawbot -z bypasses ClawbotCLI, but recall still needs SessionDB."""
    from clawbot_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "clawbot_state", mod("clawbot_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "clawbot_cli.config",
        mod("clawbot_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "clawbot_cli.models",
        mod("clawbot_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
        "clawbot_cli.runtime_provider",
        mod(
            "clawbot_cli.runtime_provider",
    ...
    monkeypatch.setitem(
        sys.modules,
        "clawbot_cli.tools_config",
        mod("clawbot_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["CLAWBOT_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="soam/clawbot-test", provider="soam", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["CLAWBOT_MODEL"] == "soam/clawbot-test"
    assert env["CLAWBOT_INFERENCE_MODEL"] == "soam/clawbot-test"
    assert env["CLAWBOT_TUI_PROVIDER"] == "soam"
    assert env["CLAWBOT_INFERENCE_PROVIDER"] == "soam"
    assert env["CLAWBOT_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["CLAWBOT_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("clawbot-tui-active-session-")
...
def test_make_tui_argv_dev_prebuilds_clawbot_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("CLAWBOT_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    import clawbot_cli.main as main_mod
    ...
    monkeypatch.setitem(
        sys.modules, "clawbot_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )
    ...
    assert "clawbot --tui --resume 20260409_000001_abc123" in out
    assert 'clawbot --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "clawbot --tui --resume actual_session" in out
