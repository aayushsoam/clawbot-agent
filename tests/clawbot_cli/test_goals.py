"""Tests for clawbot_cli/goals.py — persistent cross-turn goals."""
...
def clawbot_home(tmp_path, monkeypatch):
    """Isolated CLAWBOT_HOME so SessionDB.state_meta writes don't clobber the real one."""
    ...
    # Bust the goal-module's DB cache for each test so it re-resolves CLAWBOT_HOME.
    from clawbot_cli import goals
...
class TestParseJudgeResponse:
    def test_clean_json_done(self):
        from clawbot_cli.goals import _parse_judge_response
...
class TestJudgeGoal:
    def test_empty_goal_skipped(self):
        from clawbot_cli.goals import judge_goal
...
class TestGoalManager:
    def test_no_goal_initial(self, clawbot_home):
    ...
    def test_set_then_status(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_set_rejects_empty(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_pause_and_resume(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_clear(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_persistence_across_managers(self, clawbot_home):
    ...
    def test_evaluate_after_turn_done(self, clawbot_home):
    ...
    def test_evaluate_after_turn_continue_under_budget(self, clawbot_home):
        from clawbot_cli import goals
        from clawbot_cli.goals import GoalManager
    ...
    def test_evaluate_after_turn_budget_exhausted(self, clawbot_home):
    ...
    def test_evaluate_after_turn_inactive(self, clawbot_home):
    ...
    def test_continuation_prompt_shape(self, clawbot_home):
        ...
        mgr.set("port goal command to clawbot")
        prompt = mgr.next_continuation_prompt()
        assert prompt is not None
        assert "port goal command to clawbot" in prompt
...
def test_goal_command_dispatches_in_cli_registry_helpers():
    ...
    from clawbot_cli.commands import COMMANDS, COMMANDS_BY_CATEGORY
...
class TestJudgeParseFailureAutoPause:
    ...
    def test_auto_pause_after_three_consecutive_parse_failures(self, clawbot_home):
        """N=3 consecutive parse failures → auto-pause with config pointer."""
        from clawbot_cli import goals
        from clawbot_cli.goals import GoalManager, DEFAULT_MAX_CONSECUTIVE_PARSE_FAILURES
    ...
    def test_parse_failure_counter_resets_on_good_reply(self, clawbot_home):
    ...
    def test_parse_failure_counter_not_incremented_by_api_errors(self, clawbot_home):
    ...
    def test_consecutive_parse_failures_persists_across_goalmanager_reloads(
        ...
        from clawbot_cli import goals
        from clawbot_cli.goals import GoalManager, load_goal
...
class TestGoalStateSubgoalsBackcompat:
    def test_old_state_meta_row_loads_without_subgoals(self):
        ...
        import json
        from clawbot_cli.goals import GoalState
...
class TestGoalManagerSubgoals:
    def test_add_subgoal(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_add_subgoal_requires_active_goal(self, clawbot_home):
        import pytest
        from clawbot_cli.goals import GoalManager
    ...
    def test_add_empty_subgoal_rejected(self, clawbot_home):
        import pytest
        from clawbot_cli.goals import GoalManager
    ...
    def test_remove_subgoal(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_remove_subgoal_out_of_range(self, clawbot_home):
        import pytest
        from clawbot_cli.goals import GoalManager
    ...
    def test_clear_subgoals(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_subgoals_persist_across_reloads(self, clawbot_home):
...
class TestContinuationPromptWithSubgoals:
    def test_empty_subgoals_uses_original_template(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_with_subgoals_includes_them(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
...
class TestJudgeGoalWithSubgoals:
    def test_judge_uses_subgoals_template_when_provided(self, clawbot_home):
    ...
    def test_judge_uses_original_template_when_no_subgoals(self, clawbot_home):
...
class TestStatusLineSubgoalCount:
    def test_status_line_no_subgoals(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
    ...
    def test_status_line_with_subgoals(self, clawbot_home):
        from clawbot_cli.goals import GoalManager
