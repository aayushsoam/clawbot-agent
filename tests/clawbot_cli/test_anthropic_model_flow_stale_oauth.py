"""Tests for Bug #12905 fix — stale OAuth token detection in clawbot model flow.

Bug 3: `clawbot model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.clawbot/.env but no valid
...
from clawbot_cli.config import load_env, save_env_value
...
class TestStaleOAuthTokenDetection:
    ...
    def test_stale_oauth_token_triggers_reauth(self, tmp_path, monkeypatch, capsys):
        ...
        from clawbot_cli.main import _model_flow_anthropic
        cfg = {}
