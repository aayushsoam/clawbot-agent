...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (CLAWBOT_OVERLAYS) — bedrock
...
class TestProviderModelIdsBedrock:
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        from clawbot_cli.models import _PROVIDER_MODELS, provider_model_ids
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via CLAWBOT_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in CLAWBOT_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from clawbot_cli.providers import CLAWBOT_OVERLAYS
        assert "bedrock" in CLAWBOT_OVERLAYS

    def test_bedrock_overlay_transport(self):
        from clawbot_cli.providers import CLAWBOT_OVERLAYS
        assert CLAWBOT_OVERLAYS["bedrock"].transport == "bedrock_converse"

    def test_bedrock_overlay_auth_type(self):
        from clawbot_cli.providers import CLAWBOT_OVERLAYS
        assert CLAWBOT_OVERLAYS["bedrock"].auth_type == "aws_sdk"
