...
from clawbot_cli import setup as setup_mod
from clawbot_cli.setup import setup_model_provider
...
def test_setup_syncs_soam_from_disk(tmp_path, monkeypatch):
    """Soam OAuth writes config to disk; wizard config dict must pick it up."""
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    _clear_provider_env(monkeypatch)
    _stub_tts(monkeypatch)
    ...
    def fake_select():
        _write_model_config(tmp_path, "soam", "https://inference.example.com/v1", "gemini-3-flash")
    ...
    reloaded = load_config()
    assert isinstance(reloaded["model"], dict)
    assert reloaded["model"]["provider"] == "soam"
...
def test_setup_gateway_skips_service_install_when_systemctl_missing(monkeypatch, capsys):
    ...
    import clawbot_cli.gateway as gateway_mod
...
def test_setup_gateway_in_container_shows_docker_guidance(monkeypatch, capsys):
    ...
    monkeypatch.setattr(clawbot_constants, "is_container", lambda: True)
...
def test_select_provider_and_model_warns_if_named_custom_provider_disappears(
    ...
    monkeypatch.setattr("clawbot_cli.auth.resolve_provider", lambda provider: None)
    monkeypatch.setattr("clawbot_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
    monkeypatch.setattr(
        "clawbot_cli.main._model_flow_named_custom",
...
def test_select_provider_and_model_accepts_named_provider_from_providers_section(
    ...
    monkeypatch.setattr(
        "clawbot_cli.main._prompt_provider_choice",
...
def test_modal_setup_can_use_soam_subscription_without_modal_creds(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr("clawbot_cli.setup.managed_soam_tools_enabled", lambda: True)
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    config = load_config()
    ...
    monkeypatch.setattr("clawbot_cli.setup.prompt", fake_prompt)
    monkeypatch.setattr("clawbot_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "clawbot_cli.setup.get_soam_subscription_features",
        lambda config: type("Features", (), {"soam_auth_present": True})(),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    from clawbot_cli.setup import setup_terminal_backend
...
def test_modal_setup_persists_direct_mode_when_user_chooses_their_own_account(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("clawbot_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("clawbot_cli.setup.prompt", lambda *args, **kwargs: next(prompt_values))
    monkeypatch.setattr("clawbot_cli.setup._prompt_container_resources", lambda config: None)
    monkeypatch.setattr(
        "clawbot_cli.setup.get_soam_subscription_features",
        lambda config: type("Features", (), {"soam_auth_present": True})(),
