"""Tests for the `clawbot proxy` subcommand and its upstream adapters."""
...
from clawbot_cli.proxy.adapters import ADAPTERS, get_adapter
from clawbot_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
from clawbot_cli.proxy.adapters.soam_portal import SoamPortalAdapter
...
def test_registry_lists_soam():
    assert "soam" in ADAPTERS


def test_get_adapter_returns_instance():
    adapter = get_adapter("soam")
    assert isinstance(adapter, SoamPortalAdapter)
    assert isinstance(adapter, UpstreamAdapter)


def test_get_adapter_case_insensitive():
    assert isinstance(get_adapter("SOAM"), SoamPortalAdapter)
    assert isinstance(get_adapter("  Soam  "), SoamPortalAdapter)
...
def _write_auth_store(clawbot_home: Path, soam_state: Dict[str, Any]) -> Path:
    """Write an auth.json with the given soam state into a clawbotic CLAWBOT_HOME."""
    auth_path = clawbot_home / "auth.json"
    auth_path.write_text(json.dumps({
        "version": 1,
        "providers": {"soam": soam_state},
    }))
    return auth_path


def test_soam_adapter_metadata():
    adapter = SoamPortalAdapter()
    assert adapter.name == "soam"
    assert adapter.display_name == "Soam Portal"
...
def test_soam_adapter_not_authenticated_when_no_auth_file(tmp_path, monkeypatch):
    # CLAWBOT_HOME is already set by conftest, but make doubly sure
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    adapter = SoamPortalAdapter()
    assert not adapter.is_authenticated()


def test_soam_adapter_not_authenticated_when_provider_missing(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    (tmp_path / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    }))
    assert not SoamPortalAdapter().is_authenticated()


def test_soam_adapter_authenticated_with_agent_key(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "agent_key": "ov-test-key",
        "agent_key_expires_at": "2099-01-01T00:00:00Z",
        "inference_base_url": "https://inference-api.aayushsoam.com/v1",
    })
    assert SoamPortalAdapter().is_authenticated()


def test_soam_adapter_authenticated_with_refresh_token_only(tmp_path, monkeypatch):
...
def test_soam_adapter_get_credential_uses_runtime_resolver(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
    ...
    refreshed_state = {
        ...
        "base_url": "https://inference-api.aayushsoam.com/v1",
        "expires_at": "2099-01-01T00:00:00Z",
    }

    with patch(
        "clawbot_cli.proxy.adapters.soam_portal.resolve_soam_runtime_credentials",
    ...
    assert cred.base_url == "https://inference-api.aayushsoam.com/v1"
    assert cred.expires_at == "2099-01-01T00:00:00Z"
    assert cred.token_type == "Bearer"


def test_soam_adapter_retry_credential_forces_legacy_mint(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
...
def test_soam_adapter_retry_credential_skips_opaque_bearer(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "jwt-access",
        "refresh_token": "refresh-tok",
...
def test_soam_adapter_get_credential_raises_when_not_logged_in(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    adapter = SoamPortalAdapter()
    with pytest.raises(RuntimeError, match="clawbot login soam"):
        adapter.get_credential()


def test_soam_adapter_get_credential_raises_on_refresh_failure(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    _write_auth_store(tmp_path, {
        "access_token": "access-tok",
        "refresh_token": "refresh-tok",
...
def test_soam_adapter_quarantines_terminal_refresh_failure(tmp_path, monkeypatch):
    ...
    soam_state = stored["providers"]["soam"]
    assert not soam_state.get("refresh_token")
    assert not soam_state.get("access_token")
    assert not soam_state.get("agent_key")
    assert soam_state["last_auth_error"]["code"] == "invalid_grant"
    assert stored.get("credential_pool", {}).get("soam") == []


def test_soam_adapter_get_credential_raises_when_no_agent_key_returned(tmp_path, monkeypatch):
...
def test_soam_adapter_concurrent_refresh_serialized(tmp_path, monkeypatch):
...
from clawbot_cli.proxy.server import create_app  # noqa: E402
...
def test_server_forwards_chat_completions():
    async def run():
        ...
        try:
            ...
            assert "Clawbot-4-70B" in req["body"]
        finally:
...
def test_cmd_proxy_status_runs(capsys, tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    from clawbot_cli.proxy.cli import cmd_proxy_status
    ...
    assert "soam" in out
    assert "Soam Portal" in out
    assert "not logged in" in out


def test_cmd_proxy_providers_runs(capsys):
    from clawbot_cli.proxy.cli import cmd_proxy_list_providers
...
def test_cmd_proxy_start_refuses_unknown_provider(capsys):
    from clawbot_cli.proxy.cli import cmd_proxy_start
...
def test_cmd_proxy_start_refuses_when_unauthenticated(capsys, tmp_path, monkeypatch):
    ...
    assert rc == 2
    err = capsys.readouterr().err
    assert "clawbot login soam" in err
