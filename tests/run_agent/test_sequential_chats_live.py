...
AlexKucera reported on Discord (2026-04-16) that after ``clawbot update`` pulled
...
Opt-in — not part of default CI:
    CLAWBOT_LIVE_TESTS=1 pytest tests/run_agent/test_sequential_chats_live.py -v

Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.clawbot/.env).
"""
...
# Load ~/.clawbot/.env so live runs pick up OPENROUTER_API_KEY without
...
LIVE = os.environ.get("CLAWBOT_LIVE_TESTS") == "1"
OR_KEY = os.environ.get("OPENROUTER_API_KEY", "")

pytestmark = [
    pytest.mark.skipif(not LIVE, reason="live-only — set CLAWBOT_LIVE_TESTS=1"),
