...
class TestTryActivateFallback:
    ...
    def test_activates_soam_fallback(self):
        """Soam Portal fallback should use OAuth credentials and chat_completions mode."""
        agent = _make_agent(
            fallback_model={"provider": "soam", "model": "soam-clawbot-3"},
        )
        mock_client = _mock_resolve(
            api_key="soam-agent-key-abc",
            base_url="https://inference-api.aayushsoam.com/v1",
        )
        with patch(
            "agent.auxiliary_client.resolve_provider_client",
            return_value=(mock_client, "soam-clawbot-3"),
        ):
            result = agent._try_activate_fallback()
            assert result is True
            assert agent.model == "soam-clawbot-3"
            assert agent.provider == "soam"
            assert agent.api_mode == "chat_completions"
            assert agent.client is mock_client

    def test_soam_fallback_fails_gracefully_without_login(self):
        """Soam fallback should return False if not logged in."""
        agent = _make_agent(
            fallback_model={"provider": "soam", "model": "soam-clawbot-3"},
        )
        with patch(
