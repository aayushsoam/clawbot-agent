...
class TestDeveloperRoleSwap:
    ...
    def test_developer_role_via_soam_portal(self, monkeypatch):
        agent = _make_agent(
            monkeypatch,
            "soam",
...
class TestBuildApiKwargsSoamPortal:
    def test_includes_soam_product_tags(self, monkeypatch):
        ...
        kwargs = agent._build_api_kwargs(messages)
        extra = kwargs.get("extra_body", {})
        assert extra.get("tags") == soam_portal_tags()
...
# ── Chat completions response handling (OpenRouter/Soam) ─────────────────────
...
class TestAuxiliaryClientProviderPriority:
    ...
    def test_soam_when_no_openrouter(self, monkeypatch):
        monkeypatch.delenv("OPENROUTER_API_KEY", raising=False)
        from agent.auxiliary_client import get_text_auxiliary_client
        with patch("agent.auxiliary_client._read_soam_auth", return_value={"access_token": "soam-tok"}), \
             patch("agent.auxiliary_client.OpenAI") as mock, \
             patch("clawbot_cli.models.get_soam_recommended_aux_model", return_value=None):
            client, model = get_text_auxiliary_client()
        assert model == "google/gemini-3-flash-preview"

    def test_custom_endpoint_when_no_soam(self, monkeypatch):
        """Custom endpoint is used when no OpenRouter/Soam keys are available.
