...
def test_aiagent_reuses_existing_errors_log_handler():
    ...
    error_log_path = (run_agent._clawbot_home / "logs" / "errors.log").resolve()
...
class TestBuildSystemPrompt:
    ...
    def test_includes_soam_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_soam_subscription_prompt", lambda tool_names: "SOAM SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "SOAM SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_soam_route(self, agent):
        agent.provider = "soam"
        agent.base_url = "https://inference-api.aayushsoam.com/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestConcurrentToolExecution:
    ...
    def test_invoke_tool_blocked_returns_error_and_skips_execution(self, agent, monkeypatch):
        ...
        monkeypatch.setattr(
            "clawbot_cli.plugins.get_pre_tool_call_block_message",
...
class TestRunConversation:
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch("clawbot_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_soam_401_refreshes_after_remint_and_retries(self, agent):
        self._setup_agent(agent)
        agent.provider = "soam"
        ...
        with (
            ...
            patch.object(agent, "_interruptible_api_call", side_effect=_fake_api_call),
            patch.object(
                agent, "_try_refresh_soam_client_credentials", side_effect=_fake_refresh
            ),
        ):
            result = agent.run_conversation("hello")
    ...
    def test_kanban_block_called_on_iteration_exhaustion(self, agent, monkeypatch):
        ...
        monkeypatch.setenv("CLAWBOT_KANBAN_TASK", "t_test_task_123")
    ...
    def test_no_kanban_block_when_not_in_kanban_mode(self, agent, monkeypatch):
        """kanban_block must NOT be called when CLAWBOT_KANBAN_TASK is unset."""
        self._setup_agent(agent)
        agent.max_iterations = 2

        monkeypatch.delenv("CLAWBOT_KANBAN_TASK", raising=False)
...
class TestSoamCredentialRefresh:
    """Verify Soam credential refresh rebuilds the runtime client."""

    def test_try_refresh_soam_client_credentials_rebuilds_client(
        ...
        monkeypatch.setattr(
            "clawbot_cli.auth.resolve_soam_runtime_credentials", _fake_resolve
        )

        agent.client = _ExistingClient()
        with patch("run_agent.OpenAI", side_effect=_fake_openai):
            ok = agent._try_refresh_soam_client_credentials(force=True)

        assert ok is True
        assert closed["value"] is True
        assert captured["inference_auth_mode"] == "legacy"
        assert rebuilt["kwargs"]["api_key"] == "new-soam-key"
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.aayushsoam.com/v1"
        )
...
class TestGpt5ApiModeRouting:
    ...
    def test_soam_gpt5_stays_on_chat_completions(self, agent):
        """Soam serves gpt-5.x on /chat/completions — must not upgrade to codex_responses."""
        agent.provider = "soam"
        agent.base_url = "https://inference-api.aayushsoam.com/v1"
        agent.api_mode = "chat_completions"
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Clawbot Agent" in agent._cached_system_prompt
...
class TestSafeWriter:
    ...
    # Honcho integration extracted to plugin.
