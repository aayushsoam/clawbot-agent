...
def test_session_search_lazily_opens_db_when_entrypoint_did_not_pass_one(monkeypatch):
    ...
    clawbot_state = ModuleType("clawbot_state")
    clawbot_state.SessionDB = FakeSessionDB
    monkeypatch.setitem(sys.modules, "clawbot_state", clawbot_state)
    ...
    result = json.loads(agent._invoke_tool("session_search", {"query": "Clawbot"}, "task-id"))

    assert result["success"] is True
    assert captured["db"] is sentinel_db
    assert captured["query"] == "Clawbot"
