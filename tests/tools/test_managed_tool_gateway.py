...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        ...
        clear=False,
    ), patch.object(managed_tool_gateway, "managed_soam_tools_enabled", return_value=True):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "soam-token",
        )

    assert result is not None
    assert result.gateway_origin == "https://firecrawl-gateway.aayushsoam.com"
    assert result.soam_user_token == "soam-token"
...
def test_resolve_managed_tool_gateway_is_inactive_without_soam_token():
    with patch.dict(
        os.environ,
        {
            "TOOL_GATEWAY_DOMAIN": "aayushsoam.com",
...
def test_resolve_managed_tool_gateway_is_disabled_without_subscription():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "aayushsoam.com"}, clear=False), \
         patch.object(managed_tool_gateway, "managed_soam_tools_enabled", return_value=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "soam-token",
        )
...
def test_read_soam_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.auth.resolve_soam_access_token",
        lambda refresh_skew_seconds=120: "fresh-token",
    )

    assert managed_tool_gateway.read_soam_access_token() == "fresh-token"
