...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        clawbot_home.mkdir()
        (clawbot_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(clawbot_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.clawbot/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        (clawbot_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        (clawbot_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        (clawbot_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        clawbot_home = tmp_path / ".clawbot"
        skills_dir = clawbot_home / "skills"
        ...
        assert mounts[0]["container_path"] == "/root/.clawbot/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        clawbot_home = tmp_path / ".clawbot"
        (clawbot_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"CLAWBOT_HOME": str(clawbot_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.clawbot")

        assert mounts[0]["container_path"] == "/home/user/.clawbot/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.clawbot/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.clawbot/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape CLAWBOT_HOME."""
        monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / ".clawbot"))
        (tmp_path / ".clawbot").mkdir()

        # Create a sensitive file one level above clawbot_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside clawbot_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside CLAWBOT_HOME must still be registered."""
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
        (clawbot_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_clawbot_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of CLAWBOT_HOME must be allowed."""
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        subdir = clawbot_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside CLAWBOT_HOME pointing outside must be rejected."""
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

        # Create a sensitive file outside clawbot_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside clawbot_home pointing outside
        symlink = clawbot_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes CLAWBOT_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, clawbot_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape CLAWBOT_HOME."""
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
        ...
        sensitive.write_text("{}")
        self._write_config(clawbot_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(clawbot_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside CLAWBOT_HOME via config must still mount."""
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

        (clawbot_home / "oauth.json").write_text("{}")
        self._write_config(clawbot_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        (clawbot_home / "cache" / "documents").mkdir(parents=True)
        (clawbot_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.clawbot/cache/documents" in paths
        assert "/root/.clawbot/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.clawbot/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        # Use legacy dir name — get_clawbot_dir prefers old if it exists
        (clawbot_home / "document_cache").mkdir()
        (clawbot_home / "image_cache").mkdir()
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(clawbot_home / "document_cache") in host_paths
        assert str(clawbot_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.clawbot/cache/documents" in container_paths
        assert "/root/.clawbot/cache/images" in container_paths
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        clawbot_home = tmp_path / ".clawbot"
        doc_dir = clawbot_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        clawbot_home = tmp_path / ".clawbot"
        ss_dir = clawbot_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.clawbot/cache/screenshots/session_abc/screen1.png"
