...
def _restore_tool_modules():
    original_clawbot_home = os.environ.get("CLAWBOT_HOME")
    original_modules = {
        name: module
        ...
        or name == "clawbot_cli"
        or name.startswith("clawbot_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_clawbot_home is None:
            os.environ.pop("CLAWBOT_HOME", None)
        else:
            os.environ["CLAWBOT_HOME"] = original_clawbot_home
        _reset_modules(("tools", "clawbot_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    clawbot_cli = types.ModuleType("clawbot_cli")
    clawbot_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["clawbot_cli"] = clawbot_cli
    clawbot_home = tmp_path / "clawbot-home"
    os.environ["CLAWBOT_HOME"] = str(clawbot_home)
    sys.modules["clawbot_cli.config"] = types.SimpleNamespace(
        get_clawbot_home=lambda: clawbot_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="clawbot-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": clawbot_home / "modal_snapshots.json",
