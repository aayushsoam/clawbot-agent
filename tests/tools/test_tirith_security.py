...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._clawbot_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_clawbot_bin(self):
        """After _INSTALL_FAILED, manual install in CLAWBOT_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        clawbot_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(clawbot_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(clawbot_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._clawbot_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == clawbot_bin
            assert _tirith_mod._resolved_path == clawbot_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# CLAWBOT_HOME isolation
# ---------------------------------------------------------------------------

class TestClawbotHomeIsolation:
    def test_clawbot_bin_dir_respects_clawbot_home(self):
        """_clawbot_bin_dir must use CLAWBOT_HOME, not hardcoded ~/.clawbot."""
        from tools.tirith_security import _clawbot_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CLAWBOT_HOME": tmpdir}):
            result = _clawbot_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_clawbot_home(self):
        """_failure_marker_path must use CLAWBOT_HOME, not hardcoded ~/.clawbot."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CLAWBOT_HOME": "/custom/clawbot"}):
            result = _failure_marker_path()
        assert result == "/custom/clawbot/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CLAWBOT_HOME; verify it's active."""
        clawbot_home = os.getenv("CLAWBOT_HOME")
        assert clawbot_home is not None, "CLAWBOT_HOME should be set by conftest"
        assert "clawbot_test" in clawbot_home, "Should point to test temp dir"

    def test_get_clawbot_home_fallback(self):
        """Without CLAWBOT_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_clawbot_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CLAWBOT_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".clawbot")
            result = _get_clawbot_home()
