...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_clawbot_cli_expands_web(self):
        """clawbot-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"clawbot-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("clawbot-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['clawbot-cli'] yields ['web']."""
        parent_toolsets = {"clawbot-cli"}
