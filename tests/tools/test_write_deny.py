...
class TestWriteDenyExactPaths:
    ...
    def test_clawbot_env(self):
        # ``.env`` under the active CLAWBOT_HOME (profile-aware, not just
        # ``~/.clawbot``) must be write-denied. The clawbotic test conftest
        # points CLAWBOT_HOME at a tempdir — resolve via get_clawbot_home()
        # to match the denylist.
        from clawbot_constants import get_clawbot_home
        path = str(get_clawbot_home() / ".env")
        assert _is_write_denied(path) is True
...
class TestWriteAllowed:
    ...
    def test_clawbot_config_not_env(self):
        path = os.path.join(str(Path.home()), ".clawbot", "config.yaml")
