...
HAS_POSIX_FIND = os.name != "nt" and shutil.which("find") is not None
HAS_GREP = os.name != "nt" and shutil.which("grep") is not None
...
class TestIgnoreFileWritten:
    ...
    def test_write_index_cache_creates_ignore_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hub_mod, "CLAWBOT_HOME", tmp_path)
