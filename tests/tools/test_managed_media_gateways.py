...
@pytest.fixture(autouse=True)
def _enable_managed_soam_tools(monkeypatch):
    """Patch the source modules so managed_soam_tools_enabled() returns True
    even after tool modules are dynamically reloaded."""
    monkeypatch.setattr("clawbot_cli.auth.get_soam_auth_status", lambda: {"logged_in": True})
    monkeypatch.setattr("clawbot_cli.models.check_soam_free_tier", lambda: False)
...
def test_managed_fal_submit_uses_gateway_origin_and_soam_token(monkeypatch):
    captured = {}
    _install_fake_tools_package()
    _install_fake_fal_client(captured)
    monkeypatch.delenv("FAL_KEY", raising=False)
    monkeypatch.setenv("FAL_QUEUE_GATEWAY_URL", "http://127.0.0.1:3009")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "soam-token")
    ...
    assert captured["client_key"] == "soam-token"
...
def test_openai_tts_uses_managed_audio_gateway_when_direct_key_absent(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TOOL_GATEWAY_DOMAIN", "aayushsoam.com")
    monkeypatch.setenv("TOOL_GATEWAY_USER_TOKEN", "soam-token")
    ...
    assert captured["api_key"] == "soam-token"
    assert captured["base_url"] == "https://openai-audio-gateway.aayushsoam.com/v1"
...
def test_transcription_uses_model_specific_response_formats(monkeypatch, tmp_path):
    ...
    assert whisper_capture["base_url"] == "https://openai-audio-gateway.aayushsoam.com/v1"
