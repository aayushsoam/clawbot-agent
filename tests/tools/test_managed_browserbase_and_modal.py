...
def _enable_managed_soam_tools(monkeypatch):
    """Ensure managed_soam_tools_enabled() returns True even after module reloads.
    ...
    clawbot_cli.auth and clawbot_cli.models — so the function body returns True.
    """
...
def test_browser_use_explicit_local_mode_stays_local_even_when_managed_gateway_is_ready(tmp_path):
    ...
    env.update({
        ...
        "TOOL_GATEWAY_USER_TOKEN": "soam-token",
...
def test_browser_use_managed_gateway_adds_idempotency_key_and_persists_external_call_id():
    ...
    assert sent_headers["X-Browser-Use-API-Key"] == "soam-token"
