...
code paths Clawbot takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``clawbot_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect CLAWBOT_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("clawbot_cli.stdio", None)
        # Fresh import now; tests import from clawbot_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import clawbot_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("clawbot_cli.stdio", None)

    def test_no_op_on_posix(self):
        from clawbot_cli import stdio
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("CLAWBOT_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("CLAWBOT_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("clawbot_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "clawbot_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """cli.py, clawbot_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["cli.py", "clawbot_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call clawbot_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """clawbot_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from clawbot_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from clawbot_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_flags_zero_on_posix(self):
        from clawbot_cli._subprocess_compat import (
...
# ---------------------------------------------------------------------------
# clawbot_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "clawbot_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the CLAWBOT_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / clawbot_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "clawbot_cli/tools_config.py",
            "clawbot_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_clawbot_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_clawbot_home" in source
        assert 'cache_dir = get_clawbot_home() / "cache" / "terminal"' in source
...
class TestGitBashPathNormalization:
    ...
    def test_posix_noop(self):
        ...
        if sys.platform != "win32":
            assert _normalize_git_bash_path("/home/aayush/foo") == "/home/aayush/foo"
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_clawbot_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "clawbot_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "clawbot_cli/gateway.py must use the platform-aware detach helper"
        )
