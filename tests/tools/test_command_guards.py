...
def _clean_state():
    ...
    saved = {}
    for k in ("CLAWBOT_INTERACTIVE", "CLAWBOT_GATEWAY_SESSION", "CLAWBOT_EXEC_ASK", "CLAWBOT_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["CLAWBOT_INTERACTIVE"] = "1"
        session_key = os.getenv("CLAWBOT_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No CLAWBOT_INTERACTIVE or CLAWBOT_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
