...
def test_clawbot_provider_subclass_exists():
    """ClawbotMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _CLAWBOT_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _CLAWBOT_PROVIDER_CLS is not None
    assert issubclass(_CLAWBOT_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_clawbot_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns ClawbotMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _CLAWBOT_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path))
    ...
    assert _CLAWBOT_PROVIDER_CLS is not None
    assert isinstance(provider, _CLAWBOT_PROVIDER_CLS)
    assert provider._clawbot_server_name == "srv"
