...
See: https://github.com/aayushsoam/clawbot-agent/issues/4426
"""
...
class TestGetSubprocessHome:
    """Unit tests for clawbot_constants.get_subprocess_home()."""

    def test_returns_none_when_clawbot_home_unset(self, monkeypatch):
    ...
    def test_returns_path_when_home_dir_exists(self, tmp_path, monkeypatch):
        clawbot_home = tmp_path / ".clawbot"
        clawbot_home.mkdir()
        profile_home = clawbot_home / "home"
        profile_home.mkdir()
        monkeypatch.setenv("CLAWBOT_HOME", str(clawbot_home))
        from clawbot_constants import get_subprocess_home
    ...
    def test_two_profiles_get_different_homes(self, tmp_path, monkeypatch):
        base = tmp_path / ".clawbot" / "profiles"
        ...
        monkeypatch.setenv("CLAWBOT_HOME", str(base / "alpha"))
        home_a = get_subprocess_home()

        monkeypatch.setenv("CLAWBOT_HOME", str(base / "beta"))
...
class TestMakeRunEnvHomeInjection:
    ...
    def test_injects_home_when_profile_home_exists(self, tmp_path, monkeypatch):
        ...
        assert result["HOME"] == str(clawbot_home / "home")
    ...
    def test_no_injection_when_clawbot_home_unset(self, monkeypatch):
        monkeypatch.delenv("CLAWBOT_HOME", raising=False)
...
class TestProfileBootstrap:
    ...
    def test_profile_dirs_includes_home(self):
        from clawbot_cli.profiles import _PROFILE_DIRS
