"""Tests for acp_adapter.server — ClawbotACPAgent ACP server."""
...
from acp_adapter.server import ClawbotACPAgent, CLAWBOT_VERSION
...
def agent(mock_manager):
    """ClawbotACPAgent backed by a mock session manager."""
    return ClawbotACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "clawbot-agent"
        assert resp.agent_info.version == CLAWBOT_VERSION
        ...
        assert payloads == [
            {
                "args": ["--setup"],
                "description": (
                    "Open Clawbot' interactive model/provider setup in a terminal. "
                    "Use this when Clawbot has not been configured on this machine yet."
                ),
                "id": TERMINAL_SETUP_AUTH_METHOD_ID,
                "name": "Configure Clawbot provider",
                "type": "terminal",
            }
        ]
...
class TestSessionOps:
    ...
    async def test_new_session_returns_model_state(self):
        ...
        acp_agent = ClawbotACPAgent(session_manager=manager)

        with patch(
            "clawbot_cli.models.curated_models_for_provider",
    ...
    async def test_load_session_replays_persisted_history_to_client(self, agent):
        ...
        state.history = [
            ...
            {"role": "assistant", "content": "ClawbotACPAgent._ADVERTISED_COMMANDS controls them."},
            {
                "role": "assistant",
                "content": "",
                "tool_calls": [
        ...
        assert replay_calls[1].kwargs["update"].content.text.startswith("ClawbotACPAgent")
    ...
    async def test_load_session_replays_history_before_returning_response(self, agent):
        ...
        counts synchrosoamly against the load response — see #12285 follow-up.
        """
        new_resp = await agent.new_session(cwd="/tmp")
        state = agent.session_manager.get_session(new_resp.session_id)
...
class TestSessionConfiguration:
    ...
    async def test_set_session_model_accepts_provider_prefixed_choice(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("clawbot_cli.config.load_config", lambda: {
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert CLAWBOT_VERSION in result
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["clawbot-acp"]
    ...
    async def test_refreshes_agent_tool_surface(self, agent, mock_manager):
        ...
        mock_defs.assert_called_once_with(
            enabled_toolsets=["clawbot-acp", "mcp-srv"],
            disabled_toolsets=None,
            quiet_mode=True,
        )
        assert state.agent.enabled_toolsets == ["clawbot-acp", "mcp-srv"]
