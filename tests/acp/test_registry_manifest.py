"""Tests for ACP Registry metadata shipped with Clawbot."""
...
def test_agent_json_matches_official_registry_required_fields():
    ...
    assert data["id"] == "clawbot-agent"
    assert re.fullmatch(r"[a-z][a-z0-9-]*", data["id"])
    assert data["name"] == "Clawbot Agent"
    assert data["description"]
    assert data["repository"] == "https://github.com/aayushsoam/clawbot-agent"
    assert data["website"].startswith("https://clawbot-agent.aayushsoam.com/")
    assert data["authors"] == ["Aayush soam"]
...
def test_agent_json_uses_uvx_distribution_without_local_command_fields():
    ...
    assert uvx["package"] == f"clawbot-agent[acp]=={data['version']}"
    assert uvx["args"] == ["clawbot-acp"]
...
def test_agent_json_pins_uvx_package_to_pyproject_version():
    ...
    assert _manifest()["distribution"]["uvx"]["package"] == (
        f"clawbot-agent[acp]=={_pyproject_version()}"
    )
