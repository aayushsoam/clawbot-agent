"""Tests for clawbot_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Clawbot entry point
(clawbot, clawbot-agent, clawbot-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Clawbot entry point imports clawbot_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported clawbot_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("clawbot_bootstrap", None)
    import clawbot_bootstrap  # noqa: WPS433
    return clawbot_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Clawbot CLI
...
class TestEntryPointsImportBootstrap:
    """Every Clawbot entry point must import clawbot_bootstrap as its
    ...
    # Entry points that invoke Clawbot as a process.  Each one must
    # import clawbot_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "clawbot_cli/main.py",   # clawbot CLI (console_script)
        "run_agent.py",          # clawbot-agent (console_script)
        "acp_adapter/entry.py",  # clawbot-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import clawbot_bootstrap' and that
        ...
        half-finished ``clawbot update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``clawbot_bootstrap`` as a top-level module) leaves clawbot
        ...
        """
        # Resolve relative to the clawbot-agent repo root.  Tests live
        # at tests/test_clawbot_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # clawbot start even when clawbot_bootstrap hasn't been
        ...
        assert first_import_name == "clawbot_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'clawbot_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import clawbot_bootstrap' line to be the first import."
        )
