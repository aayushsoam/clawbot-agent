...
def server():
    with patch.dict(
        "sys.modules",
        {
            "clawbot_constants": MagicMock(
                get_clawbot_home=MagicMock(return_value="/tmp/clawbot_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'clawbot-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'clawbot-release' patched"
    }
