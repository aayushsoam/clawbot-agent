"""Shared fixtures for the clawbot-agent test suite.

Clawbotic-test invariants enforced here (see AGENTS.md for rationale):

1. **No credential env vars.** All provider/credential-shaped env vars
   (ending in _API_KEY, _TOKEN, _SECRET, _PASSWORD, _CREDENTIALS, etc.)
   are unset before every test. Local developer keys cannot leak in.
2. **Isolated CLAWBOT_HOME.** CLAWBOT_HOME points to a per-test tempdir so
   code reading ``~/.clawbot/*`` via ``get_clawbot_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".clawbot"`` instead of the canonical
   ``get_clawbot_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No CLAWBOT_SESSION_* inheritance** — the agent's current gateway
...
# CLAWBOT_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_CLAWBOT_BEHAVIORAL_VARS = frozenset({
    "CLAWBOT_YOLO_MODE",
    "CLAWBOT_INTERACTIVE",
    "CLAWBOT_QUIET",
    ...
    "CLAWBOT_SESSION_CHAT_ID",
    "CLAWBOT_SESSION_CHAT_NAME",
    "CLAWBOT_SESSION_THREAD_ID",
    "CLAWBOT_SESSION_SOURCE",
    "CLAWBOT_SESSION_KEY",
    "CLAWBOT_GATEWAY_SESSION",
    "CLAWBOT_PLATFORM",
    "CLAWBOT_MODEL",
    "CLAWBOT_INFERENCE_MODEL",
    "CLAWBOT_INFERENCE_PROVIDER",
    "CLAWBOT_TUI_PROVIDER",
    "CLAWBOT_MANAGED",
    "CLAWBOT_DEV",
    "CLAWBOT_CONTAINER",
    "CLAWBOT_EPHEMERAL_SYSTEM_PROMPT",
    "CLAWBOT_TIMEZONE",
    "CLAWBOT_REDACT_SECRETS",
    "CLAWBOT_BACKGROUND_NOTIFICATIONS",
    "CLAWBOT_EXEC_ASK",
    "CLAWBOT_HOME_MODE",
    "CLAWBOT_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.clawbot/kanban.db instead of the per-test CLAWBOT_HOME.
    "CLAWBOT_KANBAN_DB",
    "CLAWBOT_KANBAN_BOARD",
    "CLAWBOT_KANBAN_WORKSPACES_ROOT",
    "CLAWBOT_KANBAN_LOGS_ROOT",
    "CLAWBOT_KANBAN_TASK",
    "CLAWBOT_KANBAN_WORKSPACE",
    "CLAWBOT_TENANT",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _clawbotic_environment(tmp_path, monkeypatch):
    """Blank out all credential/behavioral env vars so local and CI match.

    Also redirects HOME and CLAWBOT_HOME to per-test tempdirs so code that
    reads ``~/.clawbot/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral CLAWBOT_* vars that could change test semantics.
    for name in _CLAWBOT_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect CLAWBOT_HOME to a per-test tempdir. Code that reads
    #    ``~/.clawbot/*`` via ``get_clawbot_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.clawbot/*`` via
    #    ``Path.home() / ".clawbot"`` instead of ``get_clawbot_home()``
    #    is a bug to fix at the callsite.
    fake_clawbot_home = tmp_path / "clawbot_test"
    fake_clawbot_home.mkdir()
    (fake_clawbot_home / "sessions").mkdir()
    (fake_clawbot_home / "cron").mkdir()
    (fake_clawbot_home / "memories").mkdir()
    (fake_clawbot_home / "skills").mkdir()
    monkeypatch.setenv("CLAWBOT_HOME", str(fake_clawbot_home))
    ...
    #    ~/.clawbot/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import clawbot_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_clawbot_home(_clawbotic_environment):
...
def _reset_module_state():
    ...
    for _logger_name in ("tools", "run_agent", "trajectory_compressor", "cron", "clawbot_cli"):
...
@pytest.fixture()
def mock_config():
    """Return a minimal clawbot config dict suitable for unit tests."""
    return {
...
def _ensure_current_event_loop(request):
    ...
    Python 3.11+ no longer guarantees a current loop for plain synchrosoam tests.
...
# ``find_gateway_pids`` helper, the real call leaks out of the clawbotic
# environment and finds the developer's live ``clawbot-gateway`` process
...
# correlated with a ``tests/clawbot_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> clawbot-gateway``
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    """Register markers used by clawbotic conftest."""
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart clawbot-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting clawbot/python patterns are also blocked.
    """
    ...
    def _matches_clawbot_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _CLAWBOT_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_clawbot_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch clawbot-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m clawbot_cli.main".
                if (
                    "clawbot" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live clawbot-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting clawbot/python could hit the live gateway. "
