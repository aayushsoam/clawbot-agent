...
class TestClawbotConstantsFallback:
    """Tests for _clawbot_home.py fallback when clawbot_constants is unavailable."""

    HELPER_PATH = (
        Path(__file__).resolve().parents[2]
        / "skills/productivity/google-workspace/scripts/_clawbot_home.py"
    )

    def _load_helper(self, monkeypatch):
        """Load _clawbot_home.py with clawbot_constants blocked."""
        monkeypatch.setitem(sys.modules, "clawbot_constants", None)
        spec = importlib.util.spec_from_file_location("_clawbot_home_test", self.HELPER_PATH)
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        return module

    def test_fallback_uses_clawbot_home_env_var(self, monkeypatch, tmp_path):
        """When clawbot_constants is missing, CLAWBOT_HOME comes from env var."""
        monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "custom-clawbot"))
        module = self._load_helper(monkeypatch)
        assert module.get_clawbot_home() == tmp_path / "custom-clawbot"

    def test_fallback_defaults_to_dot_clawbot(self, monkeypatch):
        """When clawbot_constants is missing and CLAWBOT_HOME unset, default to ~/.clawbot."""
        monkeypatch.delenv("CLAWBOT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.get_clawbot_home() == Path.home() / ".clawbot"

    def test_fallback_ignores_empty_clawbot_home(self, monkeypatch):
        """Empty/whitespace CLAWBOT_HOME is treated as unset."""
        monkeypatch.setenv("CLAWBOT_HOME", "  ")
        module = self._load_helper(monkeypatch)
        assert module.get_clawbot_home() == Path.home() / ".clawbot"

    def test_fallback_display_clawbot_home_shortens_path(self, monkeypatch):
        """Fallback display_clawbot_home() uses ~/ shorthand like the real one."""
        monkeypatch.delenv("CLAWBOT_HOME", raising=False)
        module = self._load_helper(monkeypatch)
        assert module.display_clawbot_home() == "~/.clawbot"

    def test_fallback_display_clawbot_home_profile_path(self, monkeypatch):
        """Fallback display_clawbot_home() handles profile paths under ~/."""
        monkeypatch.setenv("CLAWBOT_HOME", str(Path.home() / ".clawbot/profiles/coder"))
        module = self._load_helper(monkeypatch)
        assert module.display_clawbot_home() == "~/.clawbot/profiles/coder"

    def test_fallback_display_clawbot_home_custom_path(self, monkeypatch):
        """Fallback display_clawbot_home() returns full path for non-home locations."""
        monkeypatch.setenv("CLAWBOT_HOME", "/opt/clawbot-custom")
        module = self._load_helper(monkeypatch)
        assert module.display_clawbot_home() == "/opt/clawbot-custom"

    def test_delegates_to_clawbot_constants_when_available(self):
        """When clawbot_constants IS importable, _clawbot_home delegates to it."""
        spec = importlib.util.spec_from_file_location(
            "_clawbot_home_happy", self.HELPER_PATH
        )
        module = importlib.util.module_from_spec(spec)
        assert spec.loader is not None
        spec.loader.exec_module(module)
        import clawbot_constants
        assert module.get_clawbot_home is clawbot_constants.get_clawbot_home
        assert module.display_clawbot_home is clawbot_constants.display_clawbot_home
