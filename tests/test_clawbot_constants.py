"""Tests for clawbot_constants module."""
...
from clawbot_constants import (
    ...
    get_default_clawbot_root,
    is_container,
    parse_reasoning_effort,
)


class TestGetDefaultClawbotRoot:
    """Tests for get_default_clawbot_root() — Docker/custom deployment awareness."""

    def test_no_clawbot_home_returns_native(self, tmp_path, monkeypatch):
        """When CLAWBOT_HOME is not set, returns ~/.clawbot."""
        monkeypatch.delenv("CLAWBOT_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_clawbot_root() == tmp_path / ".clawbot"

    def test_clawbot_home_is_native(self, tmp_path, monkeypatch):
        """When CLAWBOT_HOME = ~/.clawbot, returns ~/.clawbot."""
        native = tmp_path / ".clawbot"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWBOT_HOME", str(native))
        assert get_default_clawbot_root() == native

    def test_clawbot_home_is_profile(self, tmp_path, monkeypatch):
        """When CLAWBOT_HOME is a profile under ~/.clawbot, returns ~/.clawbot."""
        native = tmp_path / ".clawbot"
    ...
    def test_clawbot_home_is_docker(self, tmp_path, monkeypatch):
        """When CLAWBOT_HOME points outside ~/.clawbot (Docker), returns CLAWBOT_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWBOT_HOME", str(docker_home))
        assert get_default_clawbot_root() == docker_home

    def test_clawbot_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any CLAWBOT_HOME outside ~/.clawbot is treated as the root."""
        custom = tmp_path / "my-clawbot-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWBOT_HOME", str(custom))
        assert get_default_clawbot_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (CLAWBOT_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("CLAWBOT_HOME", str(profile))
        assert get_default_clawbot_root() == docker_root
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(clawbot_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(clawbot_constants, "_container_detected", True)
        assert is_container() is True
