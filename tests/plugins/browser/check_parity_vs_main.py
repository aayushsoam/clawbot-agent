...
Run from the PR worktree:
    ...
    cd ~/.clawbot/clawbot-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.clawbot/clawbot-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.clawbot/clawbot-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a clawbot-agent checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a clawbot-agent checkout"
)
...
# Isolated CLAWBOT_HOME for the config write.
