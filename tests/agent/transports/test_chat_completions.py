...
class TestChatCompletionsBuildKwargs:
    ...
    def test_soam_tags(self, transport):
        ...
        assert kw["extra_body"]["tags"] == soam_portal_tags()
    ...
    def test_soam_omits_disabled_reasoning(self, transport):
        from providers import get_provider_profile
        profile = get_provider_profile("soam")
        msgs = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
        ...
        # Soam rejects enabled=false; reasoning omitted entirely
    ...
    def test_gemma_does_not_receive_thinking_config(self, transport):
        ...
        # when Clawbot has reasoning enabled, the field must be omitted for
...
class TestChatCompletionsKimi:
    ...
    def test_moonshot_tool_schemas_are_sanitized_by_model_name(self, transport):
        """Aggregator routes (Soam, OpenRouter) hit Moonshot by model name, not base URL."""
        tools = [
            {
                "type": "function",
                "function": {
