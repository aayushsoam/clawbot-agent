...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Clawbot passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("CLAWBOT_HOME", "/users/alice/.clawbot/profiles/backend-worker")
        monkeypatch.setenv("CLAWBOT_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "CLAWBOT_KANBAN_DB",
            "/users/alice/.clawbot/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.clawbot/kanban/boards/smoke"]'
            in cmd
        )
