...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "clawbot" / "config.yaml"
...
def test_explicit_reset_timestamp_overrides_default_429_ttl(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.auth._import_codex_cli_tokens",
        lambda: None,
    )
    _write_auth_store(
        tmp_path,
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "clawbot" / "auth.json").read_text())
...
def test_load_pool_prefers_dotenv_over_stale_os_environ(tmp_path, monkeypatch):
    ...
    from a parent shell) must NOT shadow the fresh key in ~/.clawbot/.env when
    ...
    # User edited ~/.clawbot/.env with the fresh key
...
def test_load_pool_falls_back_to_os_environ_when_dotenv_empty(tmp_path, monkeypatch):
    """When ~/.clawbot/.env does not define OPENROUTER_API_KEY (typical Docker /
    ...
    (clawbot_home / ".env").write_text("SOME_OTHER_VAR=unrelated\n")
...
def test_load_pool_migrates_soam_provider_state(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_mirrors_soam_invoke_jwt_agent_key_runtime_api_key(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    ...
    auth_payload = json.loads((tmp_path / "clawbot" / "auth.json").read_text())
    pool_entry = auth_payload["credential_pool"]["soam"][0]
    assert pool_entry["agent_key"] == token
    assert pool_entry["agent_key_expires_at"] == expires_at


def test_soam_pool_terminal_refresh_removes_device_code_entry(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    monkeypatch.setenv("CLAWBOT_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    pool.add_entry(PooledCredential.from_dict("soam", {
        "id": "legacy-seeded",
        "source": "manual:device_code",
        "auth_type": "oauth",
    ...
    pool.add_entry(PooledCredential.from_dict("soam", {
        ...
        "source": "manual",
        "auth_type": "api_key",
        "access_token": "manual-soam-key",
    }))

    monkeypatch.setattr(auth_mod, "resolve_soam_runtime_credentials", _terminal_refresh_failure)
    ...
    auth_payload = json.loads((tmp_path / "clawbot" / "auth.json").read_text())
    soam_state = auth_payload["providers"]["soam"]
    ...
    assert [entry["id"] for entry in auth_payload["credential_pool"]["soam"]] == ["manual-key"]

    assert pool.try_refresh_current() is None
    assert refresh_calls["count"] == 1


def test_load_pool_removes_soam_device_code_when_singleton_quarantined(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    _write_auth_store(
        tmp_path,
        {
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_clawbot_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_load_pool_migrates_soam_provider_state_preserves_tls(tmp_path, monkeypatch):
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
    _write_auth_store(
        tmp_path,
        {
    ...
    auth_payload = json.loads((tmp_path / "clawbot" / "auth.json").read_text())
    assert auth_payload["credential_pool"]["soam"][0]["tls"] == {
        "insecure": True,
        "ca_bundle": "/tmp/soam-ca.pem",
    }
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("ANTHROPIC_TOKEN", raising=False)
    monkeypatch.delenv("CLAUDE_CODE_OAUTH_TOKEN", raising=False)
    monkeypatch.setattr("clawbot_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    _write_auth_store(
        tmp_path,
        {
            "version": 1,
    ...
    assert {entry.source for entry in entries} == {"manual:clawbot_pkce", "clawbot_pkce"}
...
def test_load_pool_seeds_copilot_via_gh_auth_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.copilot_auth.resolve_copilot_token",
...
def test_load_pool_seeds_qwen_oauth_via_cli_tokens(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "clawbot_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: {
            "provider": "qwen-oauth",
...
def test_soam_seed_from_singletons_preserves_obtained_at_timestamps(tmp_path, monkeypatch):
    """Regression test for #15099 secondary issue.

    When ``_seed_from_singletons`` materialises a device_code pool entry from
    the ``providers.soam`` singleton, it must carry the mint/refresh
    ...
    assert e.agent_key == "sk-soam-AAAA"
...
# ── PR #10160 salvage: Soam OAuth cross-process sync tests ─────────────────

def test_sync_soam_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    synced = pool._sync_soam_entry_from_auth_store(entry)
...
def test_sync_soam_entry_noop_when_tokens_match(tmp_path, monkeypatch):
...
def test_soam_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    """An exhausted Soam entry should recover when auth.json has newer tokens."""
    monkeypatch.setenv("CLAWBOT_HOME", str(tmp_path / "clawbot"))
...
def test_sync_codex_entry_from_auth_store_adopts_newer_tokens(tmp_path, monkeypatch):
    ...
    # Simulate `clawbot auth openai-codex` replacing the token pair on disk.
...
def test_codex_exhausted_entry_recovers_via_auth_store_sync(tmp_path, monkeypatch):
    ...
    rate-limit reset the user ran `clawbot model` to reauth, but the pool
    ...
    # Simulate `clawbot model` / `clawbot auth` refreshing the tokens.
