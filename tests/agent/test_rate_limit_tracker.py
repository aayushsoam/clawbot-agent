...
# ── Sample headers from Soam inference API ──────────────────────────────
...
class TestParseHeaders:
    def test_basic_parsing(self):
        state = parse_rate_limit_headers(SOAM_HEADERS, provider="soam")
        assert state is not None
        assert state.provider == "soam"
...
class TestAgentIntegration:
    ...
    def test_capture_rate_limits_from_headers(self):
        ...
        class MockResponse:
            headers = SOAM_HEADERS

        # Import AIAgent minimally
        from unittest.mock import MagicMock, patch

        # Test the parsing directly
        state = parse_rate_limit_headers(MockResponse.headers, provider="soam")
        assert state is not None
        assert state.requests_min.limit == 800
        assert state.tokens_hour.limit == 336000000
