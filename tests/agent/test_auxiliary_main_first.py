...
Prior to this change, aggregator users (OpenRouter / Soam Portal) had aux
...
class TestResolveAutoMainFirst:
    ...
    def test_soam_main_uses_main_model_for_aux(self, monkeypatch):
        """Soam Portal main user → aux uses their picked Soam model, not free-tier MiMo."""
        # No OPENROUTER_API_KEY → ensures if main failed we'd fall to chain
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="soam",
        ), patch(
            "agent.auxiliary_client._read_main_model",
        ...
        assert mock_resolve.call_args.args[0] == "soam"
...
class TestResolveVisionMainFirst:
    ...
    def test_soam_main_vision_uses_paid_soam_vision_backend(self):
        """Paid Soam main → aux vision uses the dedicated Soam vision backend."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="soam",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_soam_main_vision_uses_free_tier_soam_vision_backend(self):
        """Free-tier Soam main → aux vision uses MiMo omni, not the text main model."""
        with patch(
            "agent.auxiliary_client._read_main_provider", return_value="soam",
        ), patch(
            "agent.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            ...
            "clawbot_cli.copilot_auth.copilot_request_headers",
    ...
    def test_main_unavailable_vision_falls_through_to_aggregators(self):
        """Main provider fails → fall back to OpenRouter/Soam strict backends."""
        ...
        assert provider in {"openrouter", "soam"}
    ...
    def test_explicit_provider_override_still_wins(self):
        ...
        with patch(
            ...
            "agent.auxiliary_client._resolve_task_provider_model",
            return_value=("soam", None, None, None, None),  # explicit override
        ), patch(
            "agent.auxiliary_client._resolve_strict_vision_backend"
        ) as mock_strict:
            mock_strict.return_value = (MagicMock(), "soam-default-model")

            from agent.auxiliary_client import resolve_vision_provider_client

            provider, client, model = resolve_vision_provider_client()

        # Explicit "soam" override → uses strict backend, NOT main model path
        assert provider == "soam"
        mock_strict.assert_called_once_with("soam", None)
