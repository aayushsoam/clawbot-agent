...
minimal ``ClawbotCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def clawbot_home(tmp_path, monkeypatch):
    """Isolated CLAWBOT_HOME so SessionDB.state_meta writes stay clawbotic."""
    home = tmp_path / ".clawbot"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("CLAWBOT_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves CLAWBOT_HOME each test.
...
def _make_cli_with_goal(session_id: str, goal_text: str = "build a thing"):
    """Build a minimal ClawbotCLI stub with an active goal wired in."""
    from cli import ClawbotCLI
    from clawbot_cli.goals import GoalManager

    cli = ClawbotCLI.__new__(ClawbotCLI)
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, clawbot_home):
        ...
        with patch("clawbot_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, clawbot_home):
        ...
        with patch("clawbot_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, clawbot_home):
    ...
    def test_no_assistant_message_skipped(self, clawbot_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, clawbot_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, clawbot_home):
        ...
        src = inspect.getsource(ClawbotCLI.chat)
