...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("clawbot/clawbot-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned clawbot/* and pr-* branches."""

    def test_prunes_orphaned_clawbot_branch(self, git_repo):
        """clawbot/clawbot-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "clawbot/clawbot-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "clawbot/clawbot-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "clawbot/clawbot-deadbeef" in result.stdout
        ...
        assert "clawbot/clawbot-deadbeef" in orphaned
        ...
        assert "clawbot/clawbot-deadbeef" not in result.stdout
