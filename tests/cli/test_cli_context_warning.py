...
def _isolate(tmp_path, monkeypatch):
    """Isolate CLAWBOT_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal ClawbotCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = ClawbotCLI.__new__(ClawbotCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["clawbot-core"]
        obj.compact = False
