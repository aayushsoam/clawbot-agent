...
class TestToolsetConsistency:
    ...
    def test_clawbot_platforms_share_core_tools(self):
        """All clawbot-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on clawbot-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["clawbot-cli", "clawbot-telegram", "clawbot-discord", "clawbot-whatsapp", "clawbot-slack", "clawbot-signal", "clawbot-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_clawbot_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("clawbot-whatsapp")

    def test_clawbot_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("clawbot-api-server")
