# Clawbot Agent Has Had "Routines" Since March
...
Clawbot equivalent — works today:
```bash
...
Clawbot equivalent — works today:
```bash
clawbot webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Clawbot implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Clawbot Agent |
|---|---|---|
...
## Things Clawbot Does That Routines Can't
...
Clawbot has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Clawbot Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
clawbot gateway setup    # enable webhooks
clawbot webhook subscribe pr-review \
  --events "pull_request" \
...
Full automation templates gallery: [clawbot-agent.aayushsoam.com/docs/guides/automation-templates](https://clawbot-agent.aayushsoam.com/docs/guides/automation-templates)

Documentation: [clawbot-agent.aayushsoam.com](https://clawbot-agent.aayushsoam.com)

GitHub: [github.com/aayushsoam/clawbot-agent](https://github.com/aayushsoam/clawbot-agent)

---

*Clawbot Agent is built by [Aayush soam](https://github.com/aayushsoam). Open source, model-agnostic, runs on your infrastructure.*
