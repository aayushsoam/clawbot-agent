...
The gateway's ``shutdown_signal_handler`` runs synchrosoamly inside the
...
the async helper, never in the synchrosoam probe.
"""
...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        ...
        if clawbot_home_str:
            takeover_path = Path(clawbot_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(clawbot_home_str) / ".gateway-planned-stop.json"
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded clawbot-agent but never re-ran ``clawbot setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../clawbot-gateway.service"
    ...
    # that's the common case for clawbot.
    ...
    # constant in clawbot_cli/gateway.py.
    headroom = 30.0
