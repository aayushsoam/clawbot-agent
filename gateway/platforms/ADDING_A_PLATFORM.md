...
There are two ways to add a platform to the Clawbot gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.clawbot/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Clawbot code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `clawbot gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `clawbot_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Clawbot core.
...
And add it to the `clawbot-gateway` composite:

```python
"clawbot-gateway": {
    "includes": [..., "clawbot-your-platform"]
}
```
...
## 12. Status Display (`clawbot_cli/status.py`)
...
## 13. Gateway Setup Wizard (`clawbot_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ clawbot_cli/ toolsets.py \
  --include="*.py" -l | sort -u
