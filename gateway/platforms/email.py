"""
Email platform adapter for the Clawbot gateway.

Allows users to interact with Clawbot by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "clawbot-agent" "version" "{_clawbot_version}" '
            '"vendor" "Aayushsoam" '
            '"support-email" "noreply@aayushsoam.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Clawbot Agent")
        ...
        msg_id = f"<clawbot-{uuid.uuid4().hex[:12]}@{self._address.split('@')[1]}>"
