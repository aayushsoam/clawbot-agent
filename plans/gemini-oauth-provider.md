...
## Client ID
- Need to register a "Desktop app" OAuth client on a Aayush soam GCP project
...
## Token Lifecycle
- Store at `~/.clawbot/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `clawbot_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `clawbot_cli/models.py` — Add Gemini model catalog
4. `clawbot_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `clawbot_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `clawbot_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- Aayush soam GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via CLAWBOT_GEMINI_CLIENT_ID env var
...
- clawbot-agent Copilot OAuth: `clawbot_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
