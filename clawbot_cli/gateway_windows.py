...
* We write two files: a shared ``gateway.cmd`` wrapper script (cwd + env + the
  actual ``python -m clawbot_cli.main gateway run --replace`` invocation) and
...
_TASK_NAME_DEFAULT = "Clawbot_Gateway"
_TASK_DESCRIPTION = "Clawbot Agent Gateway - Messaging Platform Integration"
...
def get_task_name() -> str:
    ...
    Default profile: ``Clawbot_Gateway``
    Named profile X: ``Clawbot_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during clawbot_cli boot.
    from clawbot_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\clawbot\\gateway-service\\<task_name>.cmd``
    (or ``<CLAWBOT_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Clawbot installs stay self-contained).
    """
    _assert_windows()
    from clawbot_cli.config import get_clawbot_home

    script_dir = Path(get_clawbot_home()) / "gateway-service"
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports CLAWBOT_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``python -m clawbot_cli.main [--profile X] gateway run --replace``
    ...
    lines.append(f'set "CLAWBOT_HOME={clawbot_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "CLAWBOT_GATEWAY_DETACHED=1"')
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports clawbot_constants-based logic during startup.
    venv_dir = str(Path(python_path).resolve().parent.parent)
    lines.append(f'set "VIRTUAL_ENV={venv_dir}"')

    prog_args = [python_path, "-m", "clawbot_cli.main"]
    if profile_arg:
...
def _write_task_script() -> Path:
    ...
    clawbot_home = str(Path(get_clawbot_home()).resolve())
    profile_arg = _profile_arg(clawbot_home)

    content = _build_gateway_cmd_script(python_path, working_dir, clawbot_home, profile_arg)
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    argv = [python_exe, "-m", "clawbot_cli.main"]
    if profile_arg:
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m clawbot_cli.main gateway run --replace``
    ...
    log_dir = Path(get_clawbot_home()) / "logs"
...
def _wait_for_gateway_ready(timeout_s: float = 6.0, interval_s: float = 0.4) -> list[int]:
    ...
    """
    from clawbot_cli.gateway import find_gateway_pids
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from clawbot_cli.config import get_clawbot_home
        print(f"    type {Path(get_clawbot_home()).resolve()}\\logs\\gateway.log")
        print(f"    type {Path(get_clawbot_home()).resolve()}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from clawbot_cli.config import get_clawbot_home

    clawbot_home = Path(get_clawbot_home()).resolve()
    print()
    print("Next steps:")
    print("  clawbot gateway status                      # Check status")
    print(f"  type {clawbot_home}\\logs\\gateway.log       # View logs")
...
def stop() -> None:
    ...
    _assert_windows()
    from clawbot_cli.gateway import kill_gateway_processes
