"""clawbot hooks — inspect and manage shell-script hooks.

Usage::

    clawbot hooks list
    clawbot hooks test <event> [--for-tool X] [--payload-file F]
    clawbot hooks revoke <command>
    clawbot hooks doctor

Consent records live under ``~/.clawbot/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.clawbot/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``clawbot hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: clawbot hooks {list|test|revoke|doctor}")
        print("Run 'clawbot hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.clawbot/config.yaml.")
        print("See `clawbot hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `clawbot hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `clawbot hooks test` and `clawbot hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Clawbot wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `clawbot hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `clawbot hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `clawbot hooks doctor`.")
