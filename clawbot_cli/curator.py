"""CLI subcommand: `clawbot curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    synchrosoam = bool(getattr(args, "synchrosoam", False)) or not background
    if dry:
    ...
    if not synchrosoam:
        print("llm pass running in background — check `clawbot curator status` later")
    if dry:
        if synchrosoam:
            print(
                "dry-run: no changes applied. Read the report with "
                "`clawbot curator status` and run `clawbot curator run` (no flag) to apply."
            )
        else:
            print(
...
def _cmd_archive(args) -> int:
    ...
    if skill_usage.get_record(args.skill).get("pinned"):
        print(
            ...
            f"`clawbot curator unpin {args.skill}`"
        )
        return 1
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.clawbot/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`clawbot curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.clawbot/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from clawbot_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_run.add_argument(
        "--sync", "--synchrosoam", dest="synchrosoam", action="store_true",
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.clawbot/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.clawbot/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by clawbot_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="clawbot curator")
    register_cli(parser)
