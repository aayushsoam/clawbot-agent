"""
Status command for clawbot CLI.

Shows the status of all Clawbot Agent components.
"""
...
from clawbot_cli.auth import AuthError, resolve_provider
from clawbot_cli.colors import Colors, color
from clawbot_cli.config import get_env_path, get_env_value, get_clawbot_home, load_config
from clawbot_cli.models import provider_label
from clawbot_cli.soam_subscription import get_soam_subscription_features
from clawbot_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``clawbot config``'s
...
def show_status(args):
    """Show status of all Clawbot Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚡Clawbot Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    soam_logged_in = bool(soam_status.get("logged_in"))
    soam_error = soam_status.get("error")
    soam_label = "logged in" if soam_logged_in else "not logged in (run: clawbot auth add soam --type oauth)"
    print(
        f"  {'Soam Portal':<12}  {check_mark(soam_logged_in)} "
        f"{soam_label}"
    )
    portal_url = soam_status.get("portal_base_url") or "(unknown)"
    access_exp = _format_iso_timestamp(soam_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(soam_status.get("agent_key_expires_at"))
    refresh_label = "yes" if soam_status.get("has_refresh_token") else "no"
    if soam_logged_in or portal_url != "(unknown)" or soam_error:
        print(f"    Portal URL: {portal_url}")
    if soam_logged_in or soam_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if soam_logged_in or soam_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if soam_logged_in or soam_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if soam_error and not soam_logged_in:
        print(f"    Error:      {soam_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: clawbot model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: clawbot auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed Soam/Codex/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: clawbot auth add xai-oauth)'}"
    )
    ...
    # =========================================================================
    # Soam Subscription Features
    # =========================================================================
    if managed_soam_tools_enabled():
        features = get_soam_subscription_features(config)
        print()
        print(color("◆ Soam Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.soam_auth_present:
            print("  Soam Portal   ✗ not logged in")
        else:
            print("  Soam Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_soam:
                state = "active via Soam subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.soam_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.soam_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_soam)} {state}")
    elif soam_logged_in:
        # Logged into Soam but on the free tier — show upgrade nudge
        print()
        print(color("◆ Soam Tool Gateway", Colors.CYAN, Colors.BOLD))
        print("  Your free-tier Soam account does not include Tool Gateway access.")
        print("  Upgrade your subscription to unlock managed web, image, TTS, and browser tools.")
        try:
            portal_url = soam_status.get("portal_base_url", "").rstrip("/")
            if portal_url:
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: clawbot model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from clawbot_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'clawbot-agent[vercel]')"
    ...
    try:
        from clawbot_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   clawbot gateway")
    ...
    jobs_file = get_clawbot_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_clawbot_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'clawbot doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'clawbot setup' to configure", Colors.DIM))
    print()
