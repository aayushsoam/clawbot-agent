"""Profile distributions — shareable, packaged Clawbot profiles via git.

A distribution is a Clawbot profile published as a git repository (or
...
* ``clawbot profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``clawbot skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``clawbot profile``, not a parallel tree):

    clawbot profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "clawbot_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".clawbot_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "clawbot-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            clawbot_requires=str(data.get("clawbot_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.clawbot_requires:
            out["clawbot_requires"] = self.clawbot_requires
        if self.author:
...
def check_clawbot_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Clawbot {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``clawbot profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Clawbot profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Clawbot distribution."
        )

    # Version check up-front so we fail fast
    check_clawbot_requires(manifest.clawbot_requires, clawbot_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.clawbot).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="clawbot_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `clawbot profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`clawbot profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="clawbot_dist_update_") as tmp:
        plan = plan_install(
