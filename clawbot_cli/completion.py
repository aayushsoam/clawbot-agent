"""Shell completion script generation for clawbot CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_clawbot_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Clawbot Agent bash completion
# Add to ~/.bashrc:
#   eval "$(clawbot completion bash)"

_clawbot_profiles() {{
    local profiles_dir="$HOME/.clawbot/profiles"
...
_clawbot_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_clawbot_profiles)" -- "$cur"))
        return
    fi
...
complete -F _clawbot_completion clawbot
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef clawbot
# Clawbot Agent zsh completion
# Add to ~/.zshrc:
...
_clawbot_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.clawbot/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.clawbot/profiles 2>/dev/null)}}")
    fi
...
_clawbot() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_clawbot_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'clawbot command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Clawbot Agent fish completion",
        "# Add to your config:",
        "#   clawbot completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __clawbot_profiles",
        "    echo default",
        "    if test -d $HOME/.clawbot/profiles",
        "        ls $HOME/.clawbot/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c clawbot -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c clawbot -f -s p -l profile"
        " -d 'Profile name' -xa '(__clawbot_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__clawbot_profiles)' -d 'Profile name'"
                )
