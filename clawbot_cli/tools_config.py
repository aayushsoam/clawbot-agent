"""
Unified tool configuration for Clawbot Agent.

`clawbot tools` and `clawbot setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.clawbot/config.yaml under
...
from clawbot_cli.soam_subscription import (
    apply_soam_managed_defaults,
    get_soam_subscription_features,
)
from tools.tool_backend_helpers import fal_key_is_configured, managed_soam_tools_enabled
...
from clawbot_cli.cli_output import (  # noqa: E402 — late import block
...
# who want it opt in via `clawbot tools` → Video Generation, which walks
...
# `clawbot tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from clawbot_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `clawbot tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``clawbot tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from clawbot_cli.plugins import discover_plugins, get_plugin_toolsets
...
from clawbot_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "tts_provider": "openai",
                "requires_soam_auth": True,
                "managed_soam_feature": "tts",
    ...
    "web": {
        ...
        #   - "Soam Subscription" — managed Firecrawl billed via Soam
        #     subscription (requires_soam_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_soam_auth": True,
                "managed_soam_feature": "image_gen",
    ...
    "x_search": {
        ...
        "setup_note": (
            "Clawbot routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        #   - "Soam Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via Soam subscription (requires_soam_auth +
        ...
        "providers": [
            {
                "name": "Soam Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_soam_auth": True,
                "managed_soam_feature": "browser",
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``clawbot update`` and
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_clawbot_home()}/clawbot-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in clawbot-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/aayushsoam/clawbot-agent:latest"
            )
            return
        ...
        if not camofox_dir.exists() and _npm_bin:
            ...
            # package.json so that `clawbot update` does not silently pull
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.clawbot/config.yaml")
        ...
        # Run the full `clawbot auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.clawbot/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from clawbot_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: clawbot auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `clawbot auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: clawbot auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: clawbot auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `clawbot auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: clawbot auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: clawbot auth add xai-oauth"
                )
        else:
...
def _get_platform_tools(
    ...
    *,
    ...
    # "clawbot-cli" (which include all _CLAWBOT_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[clawbot-cli, spotify]`` after enabling Spotify via ``clawbot
        ...
        # (e.g. "clawbot-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``clawbot tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `clawbot tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"clawbot-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("clawbot-")}
    ...
    # shipped as a bundled plugin but user must opt in via `clawbot tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `clawbot tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by clawbot tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (clawbot-cli, clawbot-telegram, etc.)
    ...
    # Opening `clawbot tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(ts_key: str, config: dict = None) -> bool:
    ...
    if ts_key in {"web", "image_gen", "tts", "browser"}:
        features = get_soam_subscription_features(config)
        feature = features.features.get(ts_key)
        if feature and (feature.available or feature.managed_by_soam):
            return True
...
# ("Soam Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("Soam Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(cat: dict, config: dict) -> list[dict]:
    ...
    for provider in cat.get("providers", []):
        if provider.get("managed_soam_feature") and not managed_soam_tools_enabled():
            continue
        if provider.get("requires_soam_auth") and not features.soam_auth_present:
            continue
        visible.append(provider)

    # Inject plugin-registered image_gen backends (OpenAI today, more
    # later) so the picker lists them alongside FAL / Soam Subscription.
    ...
    # remaining hardcoded rows ("Soam Subscription", "Firecrawl
    ...
    # the hardcoded "Soam Subscription" / "Local Browser" / "Camofox" rows
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `clawbot tools` gets a silent no-op
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_soam
        if provider.get("tts_provider"):
            return (
                feature.managed_by_soam
                and cfg_get(config, "tts", "provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = cfg_get(config, "browser", "cloud_provider")
            return feature.managed_by_soam and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_soam and current == provider["web_backend"]
        return feature.managed_by_soam
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("requires_soam_auth"):
        features = get_soam_subscription_features(config)
        if not features.soam_auth_present:
            _print_warning("  Soam Subscription is only available after logging into Soam Portal.")
            return
    ...
    if not env_vars:
        ...
        if managed_feature:
            _print_info("  Requests for this tool will be billed to your Soam subscription.")
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `clawbot tools` and `clawbot setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚡Clawbot Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://clawbot-agent.aayushsoam.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_soam_managed_defaults(
                config,
                enabled_toolsets=new_enabled,
            )
            if managed_soam_tools_enabled():
                for ts_key in sorted(auto_configured):
                    label = next((l for k, l, _ in CONFIGURABLE_TOOLSETS if k == ts_key), ts_key)
                    print(color(f"  ✓ {label}: using your Soam subscription defaults", Colors.GREEN))
    ...
    print()
    from clawbot_constants import display_clawbot_home
    print(color(f"  Tool configuration saved to {display_clawbot_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'clawbot' or gateway restart.", Colors.DIM))
    print()
