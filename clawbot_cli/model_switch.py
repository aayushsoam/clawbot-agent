...
- ``clawbot_cli.providers``        -- canonical provider identity + overlays
- ``clawbot_cli.model_normalize``  -- per-provider name formatting
...
_CLAWBOT_MODEL_WARNING = (
    "Aayush soam Clawbot 3 & 4 models are NOT agentic and are not designed "
    "for use with Clawbot Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Aayush soam Clawbot 3 / Clawbot 4 chat families.
# The previous substring check (`"clawbot" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``clawbot-brain:qwen3-14b-ctx16k`` that just
# happen to carry "clawbot" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   Aayushsoam/Clawbot-3-Llama-3.1-70B, clawbot-4-405b, openrouter/clawbot3:70b
# Negative examples it must NOT match:
#   clawbot-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_SOAM_CLAWBOT_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])clawbot[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_soam_clawbot_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Soam Clawbot 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_SOAM_CLAWBOT_NON_AGENTIC_RE.search(model_name))


def _check_clawbot_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Soam Clawbot 3/4 chat model."""
    if is_soam_clawbot_non_agentic(model_name):
        return _CLAWBOT_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``clawbot config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from clawbot_cli.model_switch import
...
def _resolve_alias_fallback(
    ...
    Falls back to ``("openrouter", "soam")`` only when no authenticated
    providers are supplied (backwards compat for non-interactive callers).
    """
    providers = authenticated_providers or ("openrouter", "soam")
...
def resolve_display_context_length(
    ...
    about Codex OAuth, Copilot, Soam, and falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'clawbot model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'clawbot doctor' — config issues detected:"
    ...
    # Mirrors the same logic in clawbot_cli.runtime_provider.resolve_runtime_provider;
    ...
    clawbot_warn = _check_clawbot_model_warning(new_model)
    if clawbot_warn:
        warnings.append(clawbot_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from clawbot_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from clawbot_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by clawbot provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "soam" pulls from the remote model-catalog manifest published at
    # https://clawbot-agent.aayushsoam.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Clawbot release. Falls back to the in-repo
    # _PROVIDER_MODELS["soam"] snapshot when the manifest is unreachable.
    curated["soam"] = get_curated_soam_model_ids()
    ...
    # --- 1. Check Clawbot-mapped providers ---
    for clawbot_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(clawbot_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (CLAWBOT_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from clawbot_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and clawbot_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Clawbot release.
        model_ids = curated.get(clawbot_id, [])
        if clawbot_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(clawbot_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Clawbot-only providers (soam, openai-codex, copilot, opencode-go) ---
    from clawbot_cli.providers import CLAWBOT_OVERLAYS
    from clawbot_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Clawbot provider ID.
    # CLAWBOT_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Clawbot IDs ("copilot").
    _mdev_to_clawbot = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in CLAWBOT_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Clawbot slug — e.g. "github-copilot" → "copilot"
        clawbot_slug = _mdev_to_clawbot.get(pid, pid)
        if clawbot_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(clawbot_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, clawbot_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or clawbot_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(clawbot_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", clawbot_slug, exc)
        ...
        # configured.
        if not has_creds and clawbot_slug == "anthropic":
            try:
                ...
                if (clawbot_creds and clawbot_creds.get("accessToken")) or \
        ...
        if clawbot_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = provider_model_ids(clawbot_slug)
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(clawbot_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(clawbot_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by Clawbot slug, fall back to overlay key
            model_ids = curated.get(clawbot_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if clawbot_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(clawbot_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": clawbot_slug,
            "name": get_label(clawbot_slug),
            "is_current": clawbot_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "clawbot",
        ...
        seen_slugs.add(clawbot_slug.lower())
        _record_builtin_endpoint(clawbot_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or CLAWBOT_OVERLAYS (keeps /model in sync
    # with `clawbot model`).
    try:
        from clawbot_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Clawbot's canonical write key (matches
            ...
            # Clawbot writes ``models:`` as a dict keyed by model id
            # (see clawbot_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Clawbot's own writer uses; a hyphen variant is accepted
            ...
            # active model. Clawbot's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, clawbot_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`clawbot_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from clawbot_cli.models import fetch_openrouter_models
