#!/usr/bin/env python3
"""
Clawbot CLI - Main entry point.

Usage:
    clawbot                     # Interactive chat (default)
    clawbot chat                # Interactive chat
    clawbot gateway             # Run gateway in foreground
    clawbot gateway start       # Start gateway as service
    clawbot gateway stop        # Stop gateway service
    clawbot gateway status      # Show gateway status
    clawbot gateway install     # Install gateway service
    clawbot gateway uninstall   # Uninstall gateway service
    clawbot setup               # Interactive setup wizard
    clawbot logout              # Clear stored authentication
    clawbot status              # Show status of all components
    clawbot cron                # Manage cron jobs
    clawbot cron list           # List cron jobs
    clawbot cron status         # Check if cron scheduler is running
    clawbot doctor              # Check configuration and dependencies
    clawbot honcho setup                    # Configure Honcho AI memory integration
    clawbot honcho status                   # Show Honcho config and connection status
    clawbot honcho sessions                 # List directory → session name mappings
    clawbot honcho map <name>               # Map current directory to a session name
    clawbot honcho peer                     # Show peer names and dialectic settings
    clawbot honcho peer --user NAME         # Set user peer name
    clawbot honcho peer --ai NAME           # Set AI peer name
    clawbot honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    clawbot honcho mode                     # Show current memory mode
    clawbot honcho mode [hybrid|honcho|local]  # Set memory mode
    clawbot honcho tokens                   # Show token budget settings
    clawbot honcho tokens --context N       # Set session.context() token cap
    clawbot honcho tokens --dialectic N     # Set dialectic result char cap
    clawbot honcho identity                 # Show AI peer identity representation
    clawbot honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    clawbot honcho migrate                  # Step-by-step migration guide: OpenClaw native → Clawbot + Honcho
    clawbot version             Show version
    clawbot update              Update to latest version
    clawbot uninstall           Uninstall Clawbot Agent
    clawbot acp                 Run as an ACP server for editor integration
    clawbot sessions browse     Interactive session picker with search

    clawbot claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: clawbot_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``clawbot_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``clawbot update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``clawbot_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, clawbot crashes on import and the user can't run
# ``clawbot update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import clawbot_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to CLAWBOT_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (clawbot tools, clawbot setup, clawbot model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'clawbot {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any clawbot module import.
#
# Many modules cache CLAWBOT_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CLAWBOT_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.clawbot/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CLAWBOT_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors clawbot_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If CLAWBOT_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.clawbot/profiles/coder or
    # /opt/data/profiles/coder).  If CLAWBOT_HOME points to the clawbot root
    # instead (e.g. systemd hardcodes CLAWBOT_HOME=/root/.clawbot), we must
    # still read active_profile — the user may have switched profiles via
    # `clawbot profile use` and the gateway should honour that choice.
    # See issue #22502.
    clawbot_home_env = os.environ.get("CLAWBOT_HOME", "")
    if profile_name is None and clawbot_home_env:
        if Path(clawbot_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the clawbot root
    if profile_name is None:
        try:
            from clawbot_constants import get_default_clawbot_root

            active_path = get_default_clawbot_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set CLAWBOT_HOME
    if profile_name is not None:
        try:
            from clawbot_cli.profiles import resolve_profile_env

            clawbot_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent clawbot from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["CLAWBOT_HOME"] = clawbot_home
...
# Load .env from ~/.clawbot/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from clawbot_cli.config import get_clawbot_home
from clawbot_cli.env_loader import load_clawbot_dotenv

load_clawbot_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → CLAWBOT_REDACT_SECRETS env
# var BEFORE clawbot_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "CLAWBOT_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_clawbot_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["CLAWBOT_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `clawbot` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from clawbot_logging import setup_logging as _setup_logging
...
try:
    from clawbot_cli.config import load_config as _load_config_early
    from clawbot_constants import apply_ipv4_preference as _apply_ipv4
...
from clawbot_cli import __version__, __release_date__
from clawbot_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from clawbot_cli.config import get_env_path, get_clawbot_home, load_config
    from clawbot_cli.auth import get_auth_status

    # Determine whether Clawbot itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from clawbot_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_clawbot_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from clawbot_cli.auth import PROVIDER_REGISTRY
    ...
    # Check for Soam Portal OAuth credentials
    auth_file = get_clawbot_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Clawbot has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Clawbot to use their tokens.
    if _has_clawbot_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, clawbot_bin
        cli_args: the original CLI arguments (everything after 'clawbot')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    clawbot_bin = container_info["clawbot_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo clawbot {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo clawbot {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  clawbot --tui --resume {target}")
    if title:
        print(f'  clawbot --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @clawbot/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@clawbot" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``CLAWBOT_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("CLAWBOT_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    clawbot_home = os.environ.get("CLAWBOT_HOME") or str(Path.home() / ".clawbot")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "CLAWBOT_HOME": clawbot_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(clawbot_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(clawbot_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if clawbot_cli_dir is None:
        clawbot_cli_dir = Path(__file__).parent
    bundled = clawbot_cli_dir / "tui_dist" / "entry.js"
    return bundled if bundled.is_file() else None


def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (CLAWBOT_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("CLAWBOT_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from clawbot_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("CLAWBOT_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with CLAWBOT_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset CLAWBOT_TUI_DIR (e.g. `unset CLAWBOT_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("CLAWBOT_QUIET"):
    ...
    if tui_dev:
        # Keep the local @clawbot/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @clawbot/ink resolves through
        # packages/clawbot-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "clawbot-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from clawbot_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="clawbot-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["CLAWBOT_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["CLAWBOT_PYTHON_SRC_ROOT"] = os.environ.get(
        "CLAWBOT_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("CLAWBOT_PYTHON", sys.executable)
    env.setdefault("CLAWBOT_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["CLAWBOT_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["CLAWBOT_MODEL"] = model
        env["CLAWBOT_INFERENCE_MODEL"] = model
    if provider:
        env["CLAWBOT_TUI_PROVIDER"] = provider
        env["CLAWBOT_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["CLAWBOT_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["CLAWBOT_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["CLAWBOT_TUI_SKILLS"] = value
    if query:
        env["CLAWBOT_TUI_QUERY"] = query
    if image:
        env["CLAWBOT_TUI_IMAGE"] = image
    if checkpoints:
        env["CLAWBOT_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["CLAWBOT_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["CLAWBOT_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["CLAWBOT_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["CLAWBOT_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["CLAWBOT_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["CLAWBOT_TUI_RESUME"] = resume_session_id
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``CLAWBOT_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``clawbot kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``clawbot kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("CLAWBOT_KANBAN_BOARD"):
        return
    try:
        from clawbot_cli.kanban_db import get_current_board

        os.environ["CLAWBOT_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("CLAWBOT_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'clawbot sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Clawbot isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  clawbot setup")
        print()

        from clawbot_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'clawbot setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from clawbot_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["CLAWBOT_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.clawbot/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["CLAWBOT_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["CLAWBOT_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["CLAWBOT_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from clawbot_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from clawbot_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    from clawbot_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Clawbot?")
        print()
    ...
    # has no creds.json.  Every subsequent `clawbot gateway` then paid a 30s
    ...
    session_dir = get_clawbot_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: clawbot gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `clawbot gateway` skips it cleanly instead of paying a 30s
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  clawbot gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚡Clawbot Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  clawbot gateway")
        ...
        print()
        print("  Or install as a service: clawbot gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'clawbot whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from clawbot_cli.setup import run_setup_wizard
...
def cmd_postinstall(args):
    ...
    from clawbot_cli.config import stamp_install_method
    from clawbot_cli.dep_ensure import ensure_dependency

    stamp_install_method("pip")

    print("⚡Clawbot post-install bootstrap")
    print()
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``clawbot model``) and the setup wizard
    ...
    from clawbot_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from clawbot_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("CLAWBOT_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'clawbot model' for "
                "available providers, or run 'clawbot doctor' to diagnose config "
                "issues."
            )
    ...
    from clawbot_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from clawbot_cli.config import read_raw_config
    ...
    elif selected_provider == "soam":
        _model_flow_soam(config, current_model, args=args)
    ...
    # a leftover OPENAI_BASE_URL in ~/.clawbot/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.clawbot/.env if the active provider is not 'custom'.
    ...
    """
    from clawbot_cli.config import get_env_value, save_env_value, load_config
...
#
# Clawbot uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `clawbot model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `clawbot model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from clawbot_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from clawbot_cli.config import load_config
    ...
    while True:
        ...
        print("  Clawbot only falls back to a lightweight backend (OpenRouter,")
        print("  Soam Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``clawbot model`` flow, then route aux tasks to them here.
    """
    from clawbot_cli.config import load_config
    from clawbot_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from clawbot_cli.auth import _prompt_model_selection
    from clawbot_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from clawbot_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.clawbot/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `clawbot setup` from scratch. OpenRouter
    ...
    from clawbot_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.clawbot/.env by hand.
    ...
    from clawbot_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_soam(config, current_model="", args=None):
    """Soam Portal provider: ensure logged in, then pick model."""
    from clawbot_cli.auth import (
    ...
    from clawbot_cli.soam_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("soam")
    if not state or not state.get("access_token"):
        print("Not logged into Soam Portal. Starting login...")
        print()
        try:
            ...
            _login_soam(mock_args, PROVIDER_REGISTRY["soam"])
            ...
            return
        # login_soam already handles model selection + config update
        return

    # Already logged in — use curated model list (same as OpenRouter defaults).
    # The live /models endpoint returns hundreds of models; the curated list
    # shows only agentic models users recognize from OpenRouter.
    from clawbot_cli.models import (
        get_curated_soam_model_ids,
        get_pricing_for_provider,
        check_soam_free_tier,
        partition_soam_models_by_tier,
        union_with_portal_free_recommendations,
        union_with_portal_paid_recommendations,
    )

    model_ids = get_curated_soam_model_ids()
    if not model_ids:
        print("No curated models available for Soam Portal.")
        return

    # Verify credentials are still valid (catches expired sessions early)
    try:
        creds = resolve_soam_runtime_credentials(min_key_ttl_seconds=5 * 60)
    except Exception as exc:
        relogin = isinstance(exc, AuthError) and exc.relogin_required
        msg = format_auth_error(exc) if isinstance(exc, AuthError) else str(exc)
        if relogin:
            print(f"Session expired: {msg}")
            print("Re-authenticating with Soam Portal...\n")
            try:
                mock_args = argparse.Namespace(
                    portal_url=None,
                    inference_url=None,
    ...
    # Fetch live pricing (non-blocking — returns empty dict on failure)
    pricing = get_pricing_for_provider("soam")

    # Check if user is on free tier
    free_tier = check_soam_free_tier()

    # Resolve portal URL early — needed both for upgrade links and for the
    # freeRecommendedModels endpoint below.
    _soam_portal_url = ""
    try:
        _soam_state = get_provider_auth_state("soam")
        if _soam_state:
            _soam_portal_url = _soam_state.get("portal_base_url", "")
    except Exception:
        pass
    ...
    if free_tier:
        model_ids, pricing = union_with_portal_free_recommendations(
            model_ids, pricing, _soam_portal_url,
        )
        model_ids, unavailable_models = partition_soam_models_by_tier(
    ...
    if not model_ids and not unavailable_models:
        print("No models available for Soam Portal after filtering.")
        return

    if free_tier and not model_ids:
        print("No free models currently available.")
        if unavailable_models:
            from clawbot_cli.auth import DEFAULT_SOAM_PORTAL_URL

            _url = (_soam_portal_url or DEFAULT_SOAM_PORTAL_URL).rstrip("/")
    ...
    selected = _prompt_model_selection(
        ...
        portal_url=_soam_portal_url,
    )
    if selected:
        _save_model_choice(selected)
        # Reactivate Soam as the provider and update config
        inference_url = creds.get("base_url", "")
        _update_config_for_provider("soam", inference_url)
        ...
        save_config(config)
        print(f"Default model set to: {selected} (via Soam Portal)")
        # Offer Tool Gateway enablement for paid subscribers
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from clawbot_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `clawbot auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from clawbot_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_xai_oauth(_config, current_model=""):
    ...
    from clawbot_cli.models import _PROVIDER_MODELS
    ...
    # live only in the pool (e.g. after ``clawbot auth add xai-oauth``).  Fall
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from clawbot_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from clawbot_cli.auth import _save_model_choice, deactivate_provider
    from clawbot_cli.config import get_env_value, load_config, save_config
    ...
    from clawbot_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `clawbot model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from clawbot_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Clawbot URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from clawbot_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from clawbot_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from clawbot_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  Clawbot will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from clawbot_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from clawbot_cli.config import save_env_value, load_config, save_config
    from clawbot_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from clawbot_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from clawbot_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Clawbot turns to `copilot --acp`.")
    print("  Clawbot currently starts its own ACP subprocess for each request.")
    print("  Clawbot uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set CLAWBOT_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``clawbot setup`` / ``clawbot model``.
    ...
    recover from a malformed paste without editing ``~/.clawbot/.env`` by hand.
    ...
    from clawbot_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from clawbot_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `clawbot setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so clawbot knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   Clawbot typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with Clawbot, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, Soam."
                )
                print()
    ...
    if provider_id == "lmstudio":
        from clawbot_cli.auth import AuthError
        from clawbot_cli.models import fetch_lmstudio_models
        ...
        from clawbot_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from clawbot_constants import display_clawbot_home as _dhh_fn

            print(
                f"    Clawbot will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from clawbot_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'clawbot model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Clawbot CLI with a provider."""
    from clawbot_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from clawbot_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from clawbot_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from clawbot_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from clawbot_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from clawbot_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``clawbot slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: clawbot slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `clawbot slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from clawbot_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from clawbot_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from clawbot_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from clawbot_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from clawbot_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from clawbot_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from clawbot_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Clawbot home directory to a zip file."""
    if getattr(args, "quick", False):
        from clawbot_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from clawbot_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Clawbot backup from a zip file."""
    from clawbot_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Clawbot Agent v{__version__} ({__release_date__})")
    ...
    # Show update status (synchrosoam — acceptable since user asked for version info)
    try:
        from clawbot_cli.banner import check_for_updates
        from clawbot_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Clawbot Agent."""
    _require_tty("uninstall")
    from clawbot_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``clawbot update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``clawbot_cli/web_dist/`` (per vite.config.ts
    outDir: "../clawbot_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "clawbot_cli" / "web_dist"
...
_WEB_UI_NODE_REQUIREMENT = "Node.js 20.19+ or 22.12+"


def _parse_node_version(version_text: str) -> Optional[tuple[int, int, int]]:
    """Parse ``node --version`` output into a version tuple."""
    stripped = (version_text or "").strip()
    if not stripped:
        return None
...
def _node_version_supports_web_ui(version: tuple[int, int, int]) -> bool:
    """Return True when Node is new enough for the Vite 7 dashboard."""
...
def _node_version_from_executable(node: str) -> tuple[Optional[str], Optional[tuple[int, int, int]]]:
    try:
        result = subprocess.run(
            [node, "--version"],
            capture_output=True,
    ...
    return version_text, _parse_node_version(version_text)
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``clawbot update`` to stash the
    lockfile — repeatedly.
    """
    lockfile = cwd / "package-lock.json"
    ci_result: Optional[subprocess.CompletedProcess] = None
    if lockfile.exists():
    ...
    if capture_output and ci_result is not None and install_result.returncode != 0:
        def _failure_report(label: str, result: subprocess.CompletedProcess) -> str:
            chunks = [f"{label} exited with code {result.returncode}."]
            for stream_name, blob in (("stdout", result.stdout), ("stderr", result.stderr)):
                ...
                text = text.rstrip()
                if text:
                    chunks.append(f"{label} {stream_name}:\n{text}")
            return "\n".join(chunks)

        combined = "\n\n".join(
            (
                _failure_report("npm ci", ci_result),
                _failure_report("npm install", install_result),
            )
        )
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``clawbot web``).
    ...
    # build path usable on a stock `py -m clawbot_cli.main web` invocation.
    ...
    if not npm:
        if fatal:
            ...
            _say(f"Install {_WEB_UI_NODE_REQUIREMENT}, then run:  cd web && npm ci && npm run build")
        return not fatal

    node = shutil.which("node")
    if not node:
        if fatal:
            _say("Web UI frontend not built and node is not available.")
            _say(f"Install {_WEB_UI_NODE_REQUIREMENT}, then run:  cd web && npm ci && npm run build")
        return not fatal

    node_version_text, node_version = _node_version_from_executable(node)
    if node_version is not None and not _node_version_supports_web_ui(node_version):
        found = f"Node.js {node_version_text}" if node_version_text else "the current Node.js"
        if fatal:
            _say(f"Web UI requires {_WEB_UI_NODE_REQUIREMENT}; found {found}.")
            _say("Update Node.js, then run:  cd web && npm ci && npm run build")
            if os.name != "nt":
                _say(
                    "Or install Clawbot-managed Node.js 22:  "
                    "curl -fsSL https://raw.githubusercontent.com/aayushsoam/clawbot-agent/main/scripts/install.sh "
                    "| bash -s -- --ensure node"
                )
        return not fatal
    ...
    if npm_check.returncode != 0:
        if fatal:
            _say("Web UI frontend not built because npm is installed but not runnable.")
            _relay(npm_check)
            _say(f"Repair or reinstall {_WEB_UI_NODE_REQUIREMENT} with npm, then run:")
            _say("  cd web && npm ci && npm run build")
        return not fatal
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (clawbot web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "clawbot.*dashboard"`) keeps us consistent
            # with `clawbot_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `clawbot update`.
    ...
    print("  Preview now:  clawbot curator run --dry-run")
    print("  Pause it:     clawbot curator pause")
    print(
        "  Docs:         https://clawbot-agent.aayushsoam.com/docs/user-guide/features/curator"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``clawbot update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``clawbot update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: clawbot curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``clawbot dashboard`` processes.

    Called at the end of ``clawbot update`` (default ``reason``) and also
    from ``clawbot dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    clawbot dashboard --port <port>")
...
def _update_via_zip(args):
    """Update Clawbot Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/aayushsoam/clawbot-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="clawbot-update-")
        zip_path = os.path.join(tmp_dir, f"clawbot-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to clawbot-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"clawbot-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "clawbot-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Clawbot behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes clawbot completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Clawbot couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Clawbot couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Clawbot behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `clawbot update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/aayushsoam/clawbot-agent.git",
    "git@github.com:Aayushsoam/clawbot-agent.git",
    "https://github.com/aayushsoam/clawbot-agent",
    "git@github.com:Aayushsoam/clawbot-agent",
}
OFFICIAL_REPO_URL = "https://github.com/aayushsoam/clawbot-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_clawbot_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_clawbot_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Clawbot repository.")
        print("  This means you may miss updates from Aayushsoam/clawbot-agent.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/aayushsoam/clawbot-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/aayushsoam/clawbot-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``clawbot update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from clawbot_constants import get_default_clawbot_root

    default_home = get_default_clawbot_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``clawbot update`` is still progressing even if pip/uv itself is silent.
    """
...
def _clawbot_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    if not _is_windows():
        return []
    return [
        scripts_dir / "clawbot.exe",
        scripts_dir / "clawbot-gateway.exe",
    ]


def _quarantine_running_clawbot_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``clawbot.exe``.
    ...
    when ``clawbot update`` runs, ``clawbot.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``clawbot.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next clawbot invocation by ``_cleanup_quarantined_exes``.
    ...
    for shim in _clawbot_exe_shims(scripts_dir):
        if not shim.exists():
            continue
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_clawbot_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``clawbot.exe.old.*`` left by prior updates.

    Called early on every clawbot invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``clawbot update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `clawbot update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``clawbot.exe`` / ``clawbot-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_clawbot_exe`` for the rationale.
    """
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_clawbot_exe(scripts_dir)
        try:
...
def _update_node_dependencies() -> None:
    ...
    for label, path in paths:
        ...
        # streamed output to ``~/.clawbot/logs/update.log`` so nothing is lost.
...
class _UpdateOutputStream:
    """Stream wrapper used during ``clawbot update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.clawbot/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``clawbot update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``clawbot update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.clawbot/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``clawbot update --gateway``) the update is already
    ...
    try:
        ...
        # clawbot_cli.config.get_clawbot_home to simulate setup failure.
        from clawbot_cli.config import get_clawbot_home as _get_clawbot_home

        logs_dir = _get_clawbot_home() / "logs"
...
def _cmd_update_check():
    """Implement ``clawbot update --check``: fetch and report without installing."""
    from clawbot_cli.config import detect_install_method
    method = detect_install_method(PROJECT_ROOT)
    if method == "pip":
        from clawbot_cli.config import recommended_update_command
        from clawbot_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``clawbot update`` without requiring a reinstall.  The
    ...
    ``clawbot`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/clawbot.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v clawbot'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/clawbot, code at /usr/local/lib/clawbot-agent).
    fhs_link = Path("/usr/local/bin/clawbot")
    ...
    path_comment = (
        "# Clawbot Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of CLAWBOT_HOME before running the update.
    ...
    CLAWBOT_HOME directories.  The ``--backup`` flag on ``clawbot update``
    ...
    try:
        from clawbot_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_clawbot_home so the user sees ~/.clawbot/...
    try:
        from clawbot_constants import get_clawbot_home, display_clawbot_home

        home = get_clawbot_home()
        try:
            display_path = f"{display_clawbot_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  clawbot import {out_path}")
...
def cmd_update(args):
    """Update Clawbot Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from clawbot_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Clawbot Agent")
        return
...
def _cmd_update_pip(args):
    """Update Clawbot via pip (for PyPI installs)."""
    ...
    uv = shutil.which("uv")
    if uv:
        cmd = [uv, "pip", "install", "--upgrade", "clawbot-agent"]
    else:
        cmd = [sys.executable, "-m", "pip", "install", "--upgrade", "clawbot-agent"]
    ...
    print("✓ Update complete! Restart clawbot to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_clawbot_home added to clawbot_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload clawbot_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_clawbot_home() added since the last release.
        try:
            import importlib
            import clawbot_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit CLAWBOT_HOME so
        # it is not affected by sync_skills()'s module-level CLAWBOT_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's CLAWBOT_HOME env var points at the default or a named profile.
        try:
            from clawbot_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: clawbot config migrate")
            else:
                print()
                print("Skipped. Run 'clawbot config migrate' later to configure.")
        else:
        ...
        # check `clawbot curator status`. Self-stamps after printing so it
        ...
        # When running as ``clawbot update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_clawbot_home() / ".update_exit_code"
            try:
        ...
        try:
            from clawbot_cli.gateway import (
                is_macos,
            ...
            # Discover all clawbot-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. clawbot-gateway.service or clawbot-gateway-coder.service
                            ...
                            # path in `clawbot gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: clawbot gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: clawbot -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Clawbot gateway unit files are still installed.
        # When both clawbot.service (from a pre-rename install) and the
        # current clawbot-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `clawbot update` surfaces the issue until the user migrates.
        try:
            from clawbot_cli.gateway import (
                has_legacy_clawbot_units,
                _find_legacy_clawbot_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_clawbot_units():
                print()
                print("⚠ Legacy Clawbot gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_clawbot_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (clawbot.service) fight the current")
                print("  clawbot-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    clawbot gateway migrate-legacy")
                print()
        ...
        print("  clawbot model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``clawbot -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from clawbot_constants import display_clawbot_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `clawbot profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_clawbot_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → clawbot -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.clawbot)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  clawbot profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     clawbot -p {name} chat")
                else:
        ...
        from clawbot_cli import profiles as _profiles_mod
        ...
        if name and not text_value and not auto_flag:
            try:
                if _profiles_mod.normalize_profile_name(name) == "default":
                    from clawbot_constants import get_clawbot_home as _hh
        ...
        from clawbot_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `clawbot profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from clawbot_cli.profiles import profile_exists
        ...
        from clawbot_cli.profiles import rename_profile
        ...
        from clawbot_cli.profiles import export_profile
        ...
        from clawbot_cli.profiles import import_profile
        ...
        import tempfile
        from clawbot_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="clawbot_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  clawbot -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      clawbot -p {plan.manifest.name} chat")
        ...
        from clawbot_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `clawbot profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"clawbot -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from clawbot_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("clawbot_requires"):
            print(f"Requires:     Clawbot {data['clawbot_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from clawbot_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.clawbot_requires:
        print(f"  Requires: Clawbot {mf.clawbot_requires}")
...
def cmd_dashboard(args):
    ...
    if "CLAWBOT_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["CLAWBOT_WEB_DIST"])
            if "CLAWBOT_WEB_DIST" in os.environ
            else PROJECT_ROOT / "clawbot_cli" / "web_dist"
        )
...
def cmd_completion(args, parser=None):
    ...
    from clawbot_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter Clawbot log files."""
    from clawbot_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from clawbot_cli.models import CANONICAL_PROVIDERS as _cp
        return ["auto"] + [p.slug for p in _cp]
    except Exception:
        # Fallback: static list guarantees the CLI always works
        return [
            "auto", "openrouter", "soam", "openai-codex", "xai-oauth", "copilot-acp", "copilot",
...
# so that in ``clawbot -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``clawbot_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``clawbot -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``clawbot`` or only flags → defaults to ``chat``.
        return False
...
def main():
    """Main entry point for clawbot CLI."""
    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from clawbot_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``clawbot.exe.old.*`` quarantine files left by previous
    # ``clawbot update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_clawbot_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    from clawbot_cli._parser import build_top_level_parser
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for Soam login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for Soam login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Soam login (default: clawbot-cli)",
    )
    model_parser.add_argument(
        "--scope", default=None, help="OAuth scope to request for Soam login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during Soam login",
    )
    model_parser.add_argument(
        "--timeout",
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for Soam login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for Soam TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for Soam login (testing only)",
    )
    ...
    # =========================================================================
    from clawbot_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://clawbot-agent.aayushsoam.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `clawbot model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy clawbot.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Clawbot gateway unit files "
            "(e.g. clawbot.service) left over from older installs. Profile "
            "units (clawbot-gateway-<profile>.service) and unrelated "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Soam Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="soam",
        help="Upstream provider (default: soam). See `clawbot proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Clawbot Agent with an interactive wizard. "
        "Run a specific section: clawbot setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'clawbot setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for Clawbot.",
    )
    ...
    slack_manifest.add_argument(
        ...
        "writes to $CLAWBOT_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "Clawbot")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    # =========================================================================
    from clawbot_cli.send_cmd import register_send_subparser
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Clawbot CLI",
    )
    login_parser.add_argument(
        "--provider",
        choices=["soam", "openai-codex", "xai-oauth"],
        default=None,
        help="Provider to authenticate with (default: soam)",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: clawbot-cli)"
    )
    ...
    logout_parser.add_argument(
        "--provider",
        choices=["soam", "openai-codex", "xai-oauth", "spotify"],
        default=None,
    ...
    auth_add.add_argument("--portal-url", help="Soam portal base URL")
    auth_add.add_argument("--inference-url", help="Soam inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate Clawbot with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set CLAWBOT_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Clawbot Agent components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.clawbot/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.clawbot/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from clawbot_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.clawbot/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.clawbot/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Clawbot Agent setup",
    )
    ...
    doctor_parser.add_argument(
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `clawbot "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your Clawbot setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Clawbot Agent. Use 'clawbot debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    clawbot debug share              Upload debug report and print URL
    clawbot debug share --lines 500  Include more log lines
    clawbot debug share --expire 30  Keep paste for 30 days
    clawbot debug share --local      Print report locally (no upload)
    clawbot debug share --no-redact  Disable upload-time secret redaction
    clawbot debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'clawbot debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up Clawbot home directory to a zip file",
        description="Create a zip archive of your entire Clawbot configuration, "
        "skills, sessions, and data (excludes the clawbot-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/clawbot-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.clawbot/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo clawbot uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from clawbot_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a Clawbot backup from a zip file",
        description="Extract a previously created Clawbot backup into your "
        "Clawbot home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Clawbot Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from clawbot_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.clawbot/skills/.bundled_manifest) "
            "so future 'clawbot update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from clawbot_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from clawbot_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/clawbot-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `clawbot plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from clawbot_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``clawbot --help``, ``clawbot version``, ``clawbot logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from clawbot_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from clawbot_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_clawbot_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_clawbot_home()}/memories/\n")
        else:
            from clawbot_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'clawbot tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # clawbot tools list [--platform cli]
    ...
    # clawbot tools disable <name...> [--platform cli]
    ...
    # clawbot tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from clawbot_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from clawbot_cli.tools_config import tools_command
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            ...
            if path:
                ...
                print("  Refresh to latest: clawbot computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: clawbot computer-use install")
            return
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Clawbot as an MCP server",
        description=(
            "Manage MCP server connections and run Clawbot as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'clawbot mcp add' to connect to a new server, or\n"
            "'clawbot mcp serve' to expose Clawbot conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Clawbot as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `clawbot mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from clawbot_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_clawbot_home() / "sessions"
            ...
            # Launch clawbot --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from clawbot_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Clawbot",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Clawbot",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.clawbot/ (by default a "
        "single restore-point archive is written to ~/.clawbot/backups/ "
        "before apply; restorable with 'clawbot import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from clawbot_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Clawbot Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'clawbot config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Clawbot Agent",
        description="Remove Clawbot Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Clawbot Agent as an ACP (Agent Client Protocol) server",
        description="Start Clawbot Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print Clawbot ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Clawbot provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.clawbot/node/ "
    ...
    def cmd_acp(args):
        """Launch Clawbot Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Clawbot instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `clawbot update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
        help="One- or two-sentence description of what this profile is good at. "
             ...
             "of profile name alone. Skip and add later via `clawbot profile describe`.",
    )
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a Clawbot profile distribution. SOURCE can be a git URL "
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Clawbot log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    clawbot logs                    Show last 50 lines of agent.log
    clawbot logs -f                 Follow agent.log in real time
    clawbot logs errors             Show last 50 lines of errors.log
    clawbot logs gateway -n 100     Show last 100 lines of gateway.log
    clawbot logs --level WARNING    Only show WARNING and above
    clawbot logs --session abc123   Filter by session ID
    clawbot logs --component tools  Only show tool-related lines
    clawbot logs --since 1h         Lines from the last hour
    clawbot logs --since 30m -f     Follow, starting from 30 min ago
    clawbot logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``clawbot -c Pokemon Agent Dev`` → ``clawbot -c 'Pokemon Agent Dev'``
    ...
    from clawbot_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'clawbot -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (clawbot hooks list, cron
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from clawbot_cli.plugins import discover_plugins
    ...
    if getattr(args, "oneshot", None):
        from clawbot_cli.oneshot import run_oneshot
