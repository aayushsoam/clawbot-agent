"""
Multi-provider authentication system for Clawbot Agent.

Supports OAuth device code flows (Soam Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.clawbot/auth.json with cross-process file locking.
...
- Legacy session key (fallback): mint an opaque 24h key when JWT auth is
  unavailable, or when CLAWBOT_AGENT_USE_LEGACY_SESSION_KEYS is set for
...
from clawbot_cli.config import get_clawbot_home, get_config_path, read_raw_config
from clawbot_constants import OPENROUTER_BASE_URL
...
SOAM_LEGACY_SESSION_KEYS_ENV = "CLAWBOT_AGENT_USE_LEGACY_SESSION_KEYS"
SOAM_DEVICE_CODE_SOURCE = "device_code"
SOAM_INFERENCE_AUTH_MODE_AUTO = "auto"
SOAM_INFERENCE_AUTH_MODE_FRESH = "fresh"
SOAM_INFERENCE_AUTH_MODE_LEGACY = "legacy"
SOAM_INFERENCE_AUTH_MODES = frozenset({
    SOAM_INFERENCE_AUTH_MODE_AUTO,
    SOAM_INFERENCE_AUTH_MODE_FRESH,
    SOAM_INFERENCE_AUTH_MODE_LEGACY,
})
SOAM_AUTH_PATH_INVOKE_JWT = "invoke_jwt"
SOAM_AUTH_PATH_LEGACY_SESSION_KEY_CACHE = "legacy_session_key_cache"
SOAM_AUTH_PATH_LEGACY_SESSION_KEY_MINT = "legacy_session_key_mint"
DEFAULT_AGENT_KEY_MIN_TTL_SECONDS = 30 * 60  # 30 minutes
ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120       # refresh 2 min before expiry
SOAM_INVOKE_JWT_MIN_TTL_SECONDS = ACCESS_TOKEN_REFRESH_SKEW_SECONDS
...
SPOTIFY_DOCS_URL = "https://clawbot-agent.aayushsoam.com/docs/user-guide/features/spotify"
SPOTIFY_DASHBOARD_URL = "https://developer.spotify.com/dashboard"
SPOTIFY_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120

XAI_OAUTH_DOCS_URL = "https://clawbot-agent.aayushsoam.com/docs/guides/xai-grok-oauth"
OAUTH_OVER_SSH_DOCS_URL = "https://clawbot-agent.aayushsoam.com/docs/guides/oauth-over-ssh"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from clawbot_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.clawbot/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `clawbot model` to re-authenticate."

    if error.code == "subscription_required":
        return (
            "No active paid subscription found on Soam Portal. "
    ...
    if error.code == "insufficient_credits":
        return (
            ...
            "Top up/renew credits in Soam Portal, then retry."
        )
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("CLAWBOT_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.clawbot/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_clawbot_home() / "auth.json"
    # Seat belt: if pytest is running and CLAWBOT_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch CLAWBOT_HOME, tests invoked without the
    # clawbotic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".clawbot" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set CLAWBOT_HOME to a tmp_path in your test fixture, or run "
                "via scripts/run_tests.sh for clawbotic CI-parity env."
            )
    return path
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom CLAWBOT_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_clawbot_root()
    except Exception:
        return None
    profile_home = get_clawbot_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.clawbot/auth.json`` even when CLAWBOT_HOME is set to a profile
    path. The clawbotic conftest does not redirect ``HOME``, so
    ``get_default_clawbot_root()`` for a profile-shaped CLAWBOT_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".clawbot" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared Soam store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    ``_soam_shared_store_lock``, acquire ``_auth_store_lock`` FIRST
    (outer) and the shared Soam lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and isinstance(raw.get("systems"), dict):
        systems = raw["systems"]
        providers = {}
        if "soam_portal" in systems:
            providers["soam"] = systems["soam_portal"]
        return {"version": AUTH_STORE_VERSION, "providers": providers,
                "active_provider": "soam" if providers else None}
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``clawbot auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or Soam device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Clawbot.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `clawbot logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from clawbot_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'clawbot doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'clawbot model' for available providers, or run 'clawbot doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'clawbot model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.clawbot/.env.",
...
def _soam_legacy_session_keys_forced() -> bool:
    return is_truthy_value(os.getenv(SOAM_LEGACY_SESSION_KEYS_ENV), default=False)


def _soam_scope_has_invoke(raw_scope: Any) -> bool:
    return SOAM_INFERENCE_INVOKE_SCOPE in _scope_values(raw_scope)


def _normalize_soam_inference_auth_mode(inference_auth_mode: Optional[str]) -> str:
    mode = str(inference_auth_mode or SOAM_INFERENCE_AUTH_MODE_AUTO).strip().lower()
    if mode not in SOAM_INFERENCE_AUTH_MODES:
        allowed = ", ".join(sorted(SOAM_INFERENCE_AUTH_MODES))
        raise ValueError(
            "Invalid Soam inference auth mode "
            f"{inference_auth_mode!r}; expected one of: {allowed}"
        )
    return mode


def _soam_invoke_jwt_status(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    min_ttl_seconds: int = SOAM_INVOKE_JWT_MIN_TTL_SECONDS,
) -> Optional[str]:
    ...
    if SOAM_INFERENCE_INVOKE_SCOPE not in scopes:
...
def _soam_invoke_jwt_is_usable(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
...
def _soam_legacy_session_key_reason(
    token: Any,
    *,
    scope: Any = None,
    expires_at: Any = None,
    inference_auth_mode: str = SOAM_INFERENCE_AUTH_MODE_AUTO,
) -> str:
    if inference_auth_mode == SOAM_INFERENCE_AUTH_MODE_LEGACY:
        return "forced_legacy_session_key"
    if _soam_legacy_session_keys_forced():
        return "forced_legacy_session_keys"
    return (
        _soam_invoke_jwt_status(token, scope=scope, expires_at=expires_at)
        or "invoke_jwt_unavailable"
    )


def _choose_soam_inference_auth_path(
    state: Dict[str, Any],
    *,
    ...
    inference_auth_mode = _normalize_soam_inference_auth_mode(inference_auth_mode)
    token = state.get("access_token") if access_token is None else access_token
    if (
        not _soam_legacy_session_keys_forced()
        and inference_auth_mode != SOAM_INFERENCE_AUTH_MODE_LEGACY
        and _soam_invoke_jwt_is_usable(
            token,
            scope=state.get("scope"),
            expires_at=state.get("expires_at"),
        )
    ):
        return SOAM_AUTH_PATH_INVOKE_JWT, None
    if (
        inference_auth_mode == SOAM_INFERENCE_AUTH_MODE_AUTO
        and _agent_key_is_usable(
            state,
            max(60, int(min_key_ttl_seconds)),
        )
    ):
        return SOAM_AUTH_PATH_LEGACY_SESSION_KEY_CACHE, None
    return (
        SOAM_AUTH_PATH_LEGACY_SESSION_KEY_MINT,
        _soam_legacy_session_key_reason(
            token,
            scope=state.get("scope"),
            expires_at=state.get("expires_at"),
...
def _log_soam_invoke_jwt_selected(
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
) -> None:
    logger.info("Soam inference auth: using NAS invoke JWT")
    _oauth_trace(
        "soam_invoke_jwt_selected",
        sequence_id=sequence_id,
        access_token_fp=_token_fingerprint(access_token),
    )


def _log_soam_legacy_session_key_selected(
    reason: str,
    *,
    access_token: Any,
    sequence_id: Optional[str] = None,
    ...
    logger.info(
        "Soam inference auth: using legacy session key path (%s)",
        reason,
    )
    _oauth_trace(
        "soam_legacy_session_key_selected",
        sequence_id=sequence_id,
        reason=reason,
        access_token_fp=_token_fingerprint(access_token),
    )
...
def _soam_jwt_expires_at(token: Any, fallback_expires_at: Any = None) -> Optional[str]:
    claims = _decode_jwt_claims(token)
    exp = claims.get("exp")
    if isinstance(exp, (int, float)):
        try:
...
def _set_soam_agent_key_from_invoke_jwt(
    state: Dict[str, Any],
    *,
    ...
    expires_at = _soam_jwt_expires_at(access_token, state.get("expires_at"))
...
def _select_soam_invoke_jwt(
    ...
    *,
    ...
    _set_soam_agent_key_from_invoke_jwt(state)
    _log_soam_invoke_jwt_selected(
        access_token=state.get("access_token"),
        sequence_id=sequence_id,
    )


_SOAM_EFFECTIVE_STATE_IGNORED_KEYS = frozenset({
...
def _soam_effective_provider_state(state: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: value
        for key, value in state.items()
        if key not in _SOAM_EFFECTIVE_STATE_IGNORED_KEYS
    }
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("CLAWBOT_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.clawbot/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `clawbot auth list` / `clawbot status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.clawbot/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("CLAWBOT_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set CLAWBOT_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("CLAWBOT_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLAWBOT_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("CLAWBOT_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `clawbot auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `clawbot auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `clawbot auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `clawbot auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.clawbot/.env, and return it.
    ...
    print("       App name:     anything (e.g. clawbot-agent)")
    ...
    # Persist so subsequent `clawbot auth spotify` runs skip the wizard.
    save_env_value("CLAWBOT_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("CLAWBOT_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved CLAWBOT_SPOTIFY_CLIENT_ID to ~/.clawbot/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "CLAWBOT_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Clawbot:")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.clawbot/auth.json (not ~/.codex/)
#
# Clawbot maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Clawbot auth store (~/.clawbot/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `clawbot auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `clawbot auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `clawbot auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `clawbot auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Clawbot auth store (~/.clawbot/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Clawbot auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `clawbot auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Clawbot auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Clawbot's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CLAWBOT_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Clawbot processes
    ...
    base_url = (
        os.getenv("CLAWBOT_CODEX_BASE_URL", "").strip().rstrip("/")
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.clawbot/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok Subscription) in `clawbot model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `clawbot model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `clawbot model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `clawbot model`.",
            provider="xai-oauth",
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.clawbot/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `clawbot model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `clawbot model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Clawbot (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `clawbot model` to refetch.
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(os.getenv("CLAWBOT_XAI_REFRESH_TIMEOUT_SECONDS", "20"))
    ...
    base_url = (
        os.getenv("CLAWBOT_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("CLAWBOT_CA_BUNDLE")
...
def _is_soam_invoke_scope_refusal(exc: Exception) -> bool:
    ...
    return (
        ...
        or "scope" in text and SOAM_INFERENCE_INVOKE_SCOPE in text
    )


def _soam_device_scope_with_env_override(
    requested_scope: Optional[str],
    *,
    default_scope: str = DEFAULT_SOAM_SCOPE,
) -> Tuple[str, bool]:
    explicit_scope = requested_scope is not None
    scope = requested_scope or default_scope
    if _soam_legacy_session_keys_forced():
        scope = SOAM_LEGACY_AGENT_KEY_SCOPE
    return scope, explicit_scope


def _request_soam_device_code_with_scope_fallback(
    *,
    client: httpx.Client,
    portal_base_url: str,
    client_id: str,
    ...
    try:
        ...
        if (
            ...
            and _soam_scope_has_invoke(scope)
            and _is_soam_invoke_scope_refusal(exc)
        ):
            logger.info("Soam inference auth: NAS refused invoke scope, retrying legacy scope")
            _oauth_trace("soam_device_code_invoke_scope_refused")
            retry_scope = SOAM_LEGACY_AGENT_KEY_SCOPE
            return (
                _request_device_code(
                    client=client,
                    portal_base_url=portal_base_url,
...
# =============================================================================
# Soam Portal — token refresh, agent key minting, model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared Soam token store — lets OAuth credentials persist across profiles
# so a new `clawbot --profile <name> auth add soam --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${CLAWBOT_SHARED_AUTH_DIR}/soam_auth.json, defaulting to
# ``<clawbot-root>/shared/soam_auth.json`` where ``<clawbot-root>`` is what
# ``get_default_clawbot_root()`` returns — ``~/.clawbot`` on Linux/macOS,
# ``%LOCALAPPDATA%\clawbot`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's CLAWBOT_HOME so named profiles (which
# typically live under ``<clawbot-root>/profiles/<name>/``) all see the
# same file.
#
...
SOAM_SHARED_STORE_FILENAME = "soam_auth.json"
_soam_shared_lock_holder = threading.local()


def _soam_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared Soam token store.

    Honors ``CLAWBOT_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<clawbot-root>/shared/``, where ``<clawbot-root>`` is what
    :func:`clawbot_constants.get_default_clawbot_root` returns — so
    Linux/macOS classic installs land at ``~/.clawbot/shared/``, native
    Windows installs at ``%LOCALAPPDATA%\\clawbot\\shared\\``, and
    Docker / custom ``CLAWBOT_HOME`` deployments at
    ``<CLAWBOT_HOME>/shared/``. Sits outside any named profile so all
    profiles under the same root share the store.
    """
    override = os.getenv("CLAWBOT_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from clawbot_constants import get_default_clawbot_root
    return get_default_clawbot_root() / "shared"


def _soam_shared_store_path() -> Path:
    path = _soam_shared_auth_dir() / SOAM_SHARED_STORE_FILENAME
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Clawbot root, refuse rather than silently corrupt cross-profile
    # state. Tests must set CLAWBOT_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from clawbot_constants import get_default_clawbot_root
        real_home_shared = (
            get_default_clawbot_root() / "shared" / SOAM_SHARED_STORE_FILENAME
        ...
        if resolved == real_home_shared:
            raise RuntimeError(
                f"Refusing to touch real user shared Soam auth store during test run: "
                f"{path}. Set CLAWBOT_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path


@contextmanager
def _soam_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared Soam OAuth store.
    ...
    ``_try_import_shared_soam_state``, which holds this lock alone for
    ...
    """
    try:
        lock_path = _soam_shared_store_path().with_suffix(".lock")
    except RuntimeError:
        # No CLAWBOT_HOME yet (pre-setup): fall through without locking.
        yield
        return

    with _file_lock(
        lock_path,
        _soam_shared_lock_holder,
        timeout_seconds,
        "Timed out waiting for shared Soam auth lock",
    ):
        yield


def _merge_shared_soam_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local Soam state."""
...
def _write_shared_soam_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the Soam OAuth state to the shared store.
    ...
    shared = {
        ...
        "scope": state.get("scope") or DEFAULT_SOAM_SCOPE,
        "client_id": state.get("client_id") or DEFAULT_SOAM_CLIENT_ID,
        "portal_base_url": state.get("portal_base_url") or DEFAULT_SOAM_PORTAL_URL,
        "inference_base_url": state.get("inference_base_url") or DEFAULT_SOAM_INFERENCE_URL,
        "obtained_at": state.get("obtained_at"),
        "expires_at": state.get("expires_at"),
        "updated_at": datetime.now(timezone.utc).isoformat(),
    }
    try:
        with _soam_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed Soam
            ...
            try:
                ...
                try:
                    ...
                    pass
        _oauth_trace(
            "soam_shared_store_written",
            path=str(path),
            refresh_token_fp=_token_fingerprint(refresh_token),
        )
    except Exception as exc:
        logger.debug("Failed to write shared Soam auth store: %s", exc)


def _read_shared_soam_state() -> Optional[Dict[str, Any]]:
    """Return the shared Soam OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared Soam auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_soam_state(reason: str) -> None:
    """Remove the shared Soam OAuth store after a terminal token failure."""
    try:
        with _soam_shared_store_lock():
            path = _soam_shared_store_path()
            try:
                ...
                pass
        _oauth_trace("soam_shared_store_cleared", reason=reason)
    except Exception as exc:
        logger.debug("Failed to clear shared Soam auth store: %s", exc)


def _is_terminal_soam_refresh_error(exc: Exception) -> bool:
    """True when retrying the same Soam refresh token cannot succeed."""
    return (
        isinstance(exc, AuthError)
        and exc.provider == "soam"
        and exc.code in {"invalid_grant", "invalid_token", "refresh_token_reused"}
        and bool(exc.relogin_required)
    )


def _quarantine_soam_oauth_state(
    state: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
    ...
    _clear_shared_soam_state(reason)
    invalidate_soam_auth_status_cache()


def _quarantine_soam_pool_entries(
    auth_store: Dict[str, Any],
    error: AuthError,
    *,
    reason: str,
) -> bool:
    """Remove singleton-seeded Soam pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
    entries = pool.get("soam")
    if not isinstance(entries, list):
        return False

    retained = []
    removed = False
    singleton_sources = {SOAM_DEVICE_CODE_SOURCE, f"manual:{SOAM_DEVICE_CODE_SOURCE}"}
    ...
    if removed:
        pool["soam"] = retained
        _oauth_trace(
            "soam_pool_device_code_quarantined",
            reason=reason,
...
def _try_import_shared_soam_state(
    *,
    timeout_seconds: float = 15.0,
    min_key_ttl_seconds: int = 5 * 60,
) -> Optional[Dict[str, Any]]:
    """Attempt to rehydrate Soam OAuth state from the shared store.
    ...
    """
    try:
        with _soam_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            shared = _read_shared_soam_state()
            if not shared:
                return None

            # Build a full state dict so refresh_soam_oauth_from_state has every
            ...
            state: Dict[str, Any] = {
                ...
                "client_id": shared.get("client_id") or DEFAULT_SOAM_CLIENT_ID,
                "portal_base_url": shared.get("portal_base_url") or DEFAULT_SOAM_PORTAL_URL,
                "inference_base_url": shared.get("inference_base_url") or DEFAULT_SOAM_INFERENCE_URL,
                "token_type": shared.get("token_type") or "Bearer",
                "scope": shared.get("scope") or DEFAULT_SOAM_SCOPE,
            ...
            def _persist_shared_refresh(updated_state: Dict[str, Any], _reason: str) -> None:
                _write_shared_soam_state(updated_state)

            refreshed = refresh_soam_oauth_from_state(
                state,
            ...
            _write_shared_soam_state(refreshed)
    except AuthError as exc:
        _oauth_trace(
            "soam_shared_import_failed",
            error_type=type(exc).__name__,
            error_code=getattr(exc, "code", None),
        )
        if _is_terminal_soam_refresh_error(exc):
            _clear_shared_soam_state("shared_import_terminal_refresh_failure")
        logger.debug("Shared Soam import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "soam_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    response = client.post(
        f"{portal_base_url}/api/oauth/token",
        headers={"x-soam-refresh-token": refresh_token},
        data={
            "grant_type": "refresh_token",
            "client_id": client_id,
        },
    ...
    if response.status_code == 200:
        ...
        if "access_token" not in payload:
            raise AuthError("Refresh response missing access_token",
                            provider="soam", code="invalid_token", relogin_required=True)
        return payload
    ...
    try:
        ...
        raise AuthError("Refresh token exchange failed",
                        provider="soam", relogin_required=True) from exc

    code = str(error_payload.get("error", "invalid_grant"))
    description = str(error_payload.get("error_description") or "Refresh token exchange failed")
    relogin = code in {"invalid_grant", "invalid_token", "refresh_token_reused"}

    # Detect the OAuth 2.1 "refresh token reuse" signal from the Soam portal
    ...
    # called POST /api/oauth/token with Clawbot's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Clawbot's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Soam Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Clawbot install sharing "
            "~/.clawbot/auth.json) called POST /api/oauth/token with Clawbot's "
            "refresh token without persisting the rotated token back.\n"
            "Soam refresh tokens are single-use — only Clawbot may call the "
            "refresh endpoint. For health checks, use `clawbot auth status` "
            "instead.\n"
            "Re-authenticate with: clawbot auth add soam"
        )
        relogin = True

    raise AuthError(description, provider="soam", code=code, relogin_required=relogin)
    ...
    *,
    ...
    if response.status_code == 200:
        ...
        if "api_key" not in payload:
            raise AuthError("Mint response missing api_key",
                            provider="soam", code="server_error")
        return payload
    ...
    try:
        ...
        raise AuthError("Agent key mint request failed",
                        provider="soam", code="server_error") from exc
...
def fetch_soam_models(
    *,
    ...
    """Fetch available model IDs from the Soam inference API."""
    ...
    if response.status_code != 200:
        ...
        raise AuthError(description, provider="soam", code="models_fetch_failed")
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Clawbot models — they're not reliable for agentic tool-calling
            if "clawbot" in mid.lower():
                continue
...
def _agent_key_is_usable(state: Dict[str, Any], min_ttl_seconds: int) -> bool:
    ...
    if _decode_jwt_claims(key):
        if _soam_legacy_session_keys_forced():
            return False
        return _soam_invoke_jwt_is_usable(
            key,
            scope=state.get("scope"),
...
def resolve_soam_access_token(
    *,
    ...
    """Resolve a refresh-aware Soam Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state = _load_provider_state(auth_store, "soam")

        if not state:
            raise AuthError(
                "Clawbot is not logged into Soam Portal.",
                provider="soam",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("CLAWBOT_PORTAL_BASE_URL")
            or os.getenv("SOAM_PORTAL_BASE_URL")
            or DEFAULT_SOAM_PORTAL_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_SOAM_CLIENT_ID)
        verify = _resolve_verify(insecure=insecure, ca_bundle=ca_bundle, auth_state=state)

        with _soam_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            merged_shared = _merge_shared_soam_oauth_state(state)
            access_token = state.get("access_token")
            refresh_token = state.get("refresh_token")
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Soam Portal login.",
                    provider="soam",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state(auth_store, "soam", state)
                    _save_auth_store(auth_store)
                return access_token
            ...
            with httpx.Client(
                ...
                try:
                    ...
                    if _is_terminal_soam_refresh_error(exc):
                        _quarantine_soam_oauth_state(
                            state,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
                        _quarantine_soam_pool_entries(
                            auth_store,
                            exc,
                            reason="managed_access_token_refresh_failure",
                        )
...
def refresh_soam_oauth_pure(
    ...
    *,
    ...
    """Refresh Soam OAuth state without mutating auth.json directly.
    ...
    state: Dict[str, Any] = {
        "access_token": access_token,
        "refresh_token": refresh_token,
        "client_id": client_id or DEFAULT_SOAM_CLIENT_ID,
        "portal_base_url": (portal_base_url or DEFAULT_SOAM_PORTAL_URL).rstrip("/"),
        "inference_base_url": (inference_base_url or DEFAULT_SOAM_INFERENCE_URL).rstrip("/"),
        "token_type": token_type or "Bearer",
        "scope": scope or DEFAULT_SOAM_SCOPE,
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        legacy_session_keys = _soam_legacy_session_keys_forced()
        ...
        selected_auth_path, fallback_reason = _choose_soam_inference_auth_path(
            state,
            min_key_ttl_seconds=min_agent_key_ttl,
            inference_auth_mode=inference_auth_mode,
        )
        if selected_auth_path == SOAM_AUTH_PATH_INVOKE_JWT:
            _select_soam_invoke_jwt(state)
        elif selected_auth_path == SOAM_AUTH_PATH_LEGACY_SESSION_KEY_MINT:
            _log_soam_legacy_session_key_selected(
...
def refresh_soam_oauth_from_state(
    state: Dict[str, Any],
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    ...
    """Refresh Soam OAuth from a state dict. Thin wrapper around refresh_soam_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_soam_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "clawbot-cli"),
        state.get("portal_base_url", DEFAULT_SOAM_PORTAL_URL),
        state.get("inference_base_url", DEFAULT_SOAM_INFERENCE_URL),
        token_type=state.get("token_type", "Bearer"),
        scope=state.get("scope", DEFAULT_SOAM_SCOPE),
        obtained_at=state.get("obtained_at"),
        expires_at=state.get("expires_at"),
...
def persist_soam_credentials(
    creds: Dict[str, Any],
    *,
    label: Optional[str] = None,
):
    """Persist minted Soam OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    Soam credentials are read at runtime from two independent locations:

    - ``providers.soam``: singleton state read by
      ``resolve_soam_runtime_credentials()`` during 401 recovery and by
      ``_seed_from_singletons()`` during pool load.
    - ``credential_pool.soam``: used by the runtime ``pool.select()`` path.

    Historically ``clawbot auth add soam`` wrote a ``manual:device_code`` pool
    entry only, skipping ``providers.soam``.  When the 24h agent_key TTL
    expired, the recovery path read the empty singleton state and raised
    ``AuthError`` silently (``logger.debug`` at INFO level).

    This helper writes ``providers.soam`` then calls ``load_pool("soam")`` so
    ...
    ``clawbot auth add soam --label <name>``).  It gets embedded in the
    singleton state so that ``_seed_from_singletons`` uses it as the pool
    entry's label on every subsequent ``load_pool("soam")`` instead of the
    ...
    # these credentials via `clawbot auth add soam --type oauth`. Best-
    ...
    pool = load_pool("soam")
    return next(
        (e for e in pool.entries() if e.source == SOAM_DEVICE_CODE_SOURCE),
        None,
    )


def _sync_soam_pool_from_auth_store() -> None:
    """Best-effort pool reseed after providers.soam changes; never fail login."""
    try:
        from agent.credential_pool import load_pool

        load_pool("soam")
    except Exception as exc:
        logger.debug("Failed to sync Soam credential pool from auth store: %s", exc)


def resolve_soam_runtime_credentials(
    *,
    min_key_ttl_seconds: int = DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
    timeout_seconds: float = 15.0,
    insecure: Optional[bool] = None,
    ...
    """
    Resolve Soam inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Clawbot is not logged into Soam Portal.",
                            provider="soam", relogin_required=True)
        ...
        inference_base_url = (
            ...
            or os.getenv("SOAM_INFERENCE_BASE_URL")
            or DEFAULT_SOAM_INFERENCE_URL
        ).rstrip("/")
        client_id = str(state.get("client_id") or DEFAULT_SOAM_CLIENT_ID)
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed Soam auth-status cache warm during read paths.
            if (
                _soam_effective_provider_state(state)
                == _soam_effective_provider_state(persisted_state)
            ):
                _oauth_trace(
                    "soam_state_persist_skipped",
                    sequence_id=sequence_id,
                    reason=reason,
                )
                return
        ...
        timeout = httpx.Timeout(timeout_seconds if timeout_seconds else 15.0)
        _oauth_trace(
            "soam_runtime_credentials_start",
            sequence_id=sequence_id,
            inference_auth_mode=inference_auth_mode,
            min_key_ttl_seconds=min_key_ttl_seconds,
            refresh_token_fp=_token_fingerprint(state.get("refresh_token")),
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Soam Portal login.",
                                provider="soam", relogin_required=True)
            ...
            if (
                ...
                with _soam_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    if _merge_shared_soam_oauth_state(state):
                        access_token = state.get("access_token")
                        refresh_token = state.get("refresh_token")
                    ...
                    if (
                        _is_expiring(state.get("expires_at"), ACCESS_TOKEN_REFRESH_SKEW_SECONDS)
                        and not _soam_invoke_jwt_is_usable(
                            access_token,
                            scope=state.get("scope"),
                            expires_at=state.get("expires_at"),
                        )
            ...
            if selected_auth_path == SOAM_AUTH_PATH_INVOKE_JWT:
                _select_soam_invoke_jwt(
                    state,
                    access_token=access_token,
                    sequence_id=sequence_id,
                )
            elif selected_auth_path == SOAM_AUTH_PATH_LEGACY_SESSION_KEY_CACHE:
                used_cached_key = True
                logger.info("Soam inference auth: using cached agent_key")
                ...
                try:
                    ...
                    if (
                        ...
                        retry_inference_auth_mode = (
                            SOAM_INFERENCE_AUTH_MODE_LEGACY
                            if inference_auth_mode == SOAM_INFERENCE_AUTH_MODE_LEGACY
                            else SOAM_INFERENCE_AUTH_MODE_FRESH
                        )
                        retry_auth_path, _ = _choose_soam_inference_auth_path(
                            state,
                            access_token=access_token,
                            min_key_ttl_seconds=min_key_ttl_seconds,
                            inference_auth_mode=retry_inference_auth_mode,
                        )
                        if retry_auth_path == SOAM_AUTH_PATH_INVOKE_JWT:
                            mint_payload = None
                            selected_auth_path = SOAM_AUTH_PATH_INVOKE_JWT
                            _select_soam_invoke_jwt(
                                state,
                                access_token=access_token,
                                sequence_id=sequence_id,
        ...
        _persist_state("resolve_soam_runtime_credentials_final")
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a Soam inference API key",
                        provider="soam", code="server_error")
...
def _empty_soam_auth_status() -> Dict[str, Any]:
    return {
        "logged_in": False,
...
def _snapshot_soam_pool_status() -> Dict[str, Any]:
    ...
    try:
        ...
        if not pool or not pool.has_credentials():
            return _empty_soam_auth_status()
...
# ── Process-level memo for get_soam_auth_status() ──
# get_soam_auth_status() validates state by calling resolve_soam_runtime_credentials(),
# which does a synchrosoam OAuth refresh POST to portal.aayushsoam.com. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`clawbot tools`, status panels,
# subscription-feature checks) call it many times per render — `clawbot tools` → "All Platforms"
# was firing the refresh ~31× during one menu paint, racking up >13s of HTTP and burning
# single-use refresh tokens. Cache the snapshot for a few seconds, keyed on the auth.json
# mtime so that `clawbot auth login/logout/add/remove` invalidate naturally on the next call.
_SOAM_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_soam_auth_status_cache: Optional[Tuple[float, Optional[float], Dict[str, Any]]] = None
...
def invalidate_soam_auth_status_cache() -> None:
    """Clear the get_soam_auth_status() process-level memo.

    Call this from any code path that mutates Soam auth state without going
    through resolve_soam_runtime_credentials() (e.g. tests). Login/logout
    ...
    """
    global _soam_auth_status_cache
    _soam_auth_status_cache = None


def get_soam_auth_status() -> Dict[str, Any]:
    """Status snapshot for Soam auth.
    ...
    ``invalidate_soam_auth_status_cache()`` explicitly.
    """
    global _soam_auth_status_cache
    now = time.monotonic()
    mtime = _auth_file_mtime()
    cached = _soam_auth_status_cache
    if cached is not None:
        cached_at, cached_mtime, cached_status = cached
        if (
            cached_mtime == mtime
            and (now - cached_at) < _SOAM_AUTH_STATUS_CACHE_TTL
        ):
            return dict(cached_status)

    status = _compute_soam_auth_status()
    _soam_auth_status_cache = (now, mtime, dict(status))
    return status


def _compute_soam_auth_status() -> Dict[str, Any]:
    """Uncached implementation of get_soam_auth_status(). See that function."""
    ...
    if state:
        ...
        try:
            creds = resolve_soam_runtime_credentials(min_key_ttl_seconds=60)
            refreshed_state = get_provider_auth_state("soam") or state
            base_status.update(
                {
                    "logged_in": True,
    ...
    return _snapshot_soam_pool_status()


def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for Codex auth.
    
    Checks the credential pool first (where `clawbot auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `clawbot auth` and
    # `clawbot model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("CLAWBOT_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("CLAWBOT_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set CLAWBOT_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `clawbot logout` historically keyed off auth.json.active_provider only.
    ...
    if provider in {"soam", "openai-codex", "xai-oauth"}:
        return provider
    return None
...
def _prompt_model_selection(
    ...
    """
    from clawbot_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        _upgrade_url = (portal_url or DEFAULT_SOAM_PORTAL_URL).rstrip("/")
        if _unavailable:
            print(menu_title)
            print()
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from clawbot_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'clawbot model' or 'clawbot setup' instead."""
    print("The 'clawbot login' command has been removed.")
    print("Use 'clawbot auth' to manage credentials,")
    print("'clawbot model' to select a provider, or 'clawbot setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.clawbot/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Clawbot-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing Codex credentials found in Clawbot auth store.")
                try:
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Clawbot will create its own session to avoid conflicts with Codex CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("CLAWBOT_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if Codex CLI refreshes its token,")
                print("Clawbot will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Clawbot gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Clawbot creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Clawbot auth store
    _save_codex_tokens(creds["tokens"], creds.get("last_refresh"))
    config_path = _update_config_for_provider("openai-codex", creds.get("base_url", DEFAULT_CODEX_BASE_URL))
    print()
    print("Login successful!")
    from clawbot_constants import display_clawbot_home as _dhh
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Clawbot auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Clawbot creates its own local OAuth session)")
    print()
...
def _xai_oauth_build_authorize_url(
    *,
    ...
    # `referrer=clawbot-agent` lets xAI attribute Clawbot-originated logins
    ...
    *,
    ...
    if not code_verifier:
        raise AuthError(
            ...
            "This is a bug in Clawbot — please report at "
            "https://github.com/aayushsoam/clawbot-agent/issues/26990.",
            provider="xai-oauth",
...
def _xai_oauth_loopback_login(
    *,
    ...
    try:
        ...
        print("Open this URL to authorize Clawbot with xAI:")
        print(authorize_url)
        print()
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Clawbot auth store (~/.clawbot/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Clawbot login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `clawbot model` and select "
            "MiniMax (OAuth).",
...
def _soam_device_code_login(
    *,
    ...
    """Run the Soam device-code flow and return full OAuth state without persisting."""
    pconfig = PROVIDER_REGISTRY["soam"]
    portal_base_url = (
        portal_base_url
        or os.getenv("CLAWBOT_PORTAL_BASE_URL")
        or os.getenv("SOAM_PORTAL_BASE_URL")
    ...
    scope, explicit_scope = _soam_device_scope_with_env_override(
        scope,
    ...
    print(f"Starting Clawbot login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        device_data, scope = _request_soam_device_code_with_scope_fallback(
            client=client,
            portal_base_url=portal_base_url,
            client_id=client_id,
            scope=scope,
    ...
    try:
        return refresh_soam_oauth_from_state(
            auth_state,
            min_key_ttl_seconds=min_key_ttl_seconds,
            timeout_seconds=timeout_seconds,
            force_refresh=False,
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("Your Soam Portal account does not have an active subscription.")
            print(f"  Subscribe here: {portal_url}/billing")
            print()
            print("After subscribing, run `clawbot model` again to finish setup.")
            raise SystemExit(1)
        raise


def _login_soam(args, pconfig: ProviderConfig) -> None:
    """Soam Portal device authorization flow."""
    ...
    try:
        ...
        # flow, check the shared store for an existing Soam credential
        # from any other profile. If present, offer to rehydrate it.
        shared = _read_shared_soam_state()
        if shared:
            try:
                shared_path = _soam_shared_store_path()
            except RuntimeError:
                shared_path = None
            print()
            if shared_path:
                print(f"Found existing Soam OAuth credentials at {shared_path}")
            else:
                print("Found existing shared Soam OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating Soam session from shared credentials...")
                auth_state = _try_import_shared_soam_state(
                    timeout_seconds=timeout_seconds,
        ...
        if auth_state is None:
            auth_state = _soam_device_code_login(
        ...
        # overwrites it to "soam".  If the user picks "Skip (keep current)"
        ...
        with _auth_store_lock():
            auth_store = _load_auth_store()
            _save_provider_state(auth_store, "soam", auth_state)
            saved_to = _save_auth_store(auth_store)
        ...
        _write_shared_soam_state(auth_state)
        _sync_soam_pool_from_auth_store()
        ...
        # leave the config in a half-updated state (provider=soam but model
        ...
        try:
            ...
            from clawbot_cli.models import (
                get_curated_soam_model_ids, get_pricing_for_provider,
                check_soam_free_tier, partition_soam_models_by_tier,
            ...
            if model_ids:
                ...
                if free_tier:
                    ...
                    # as free so users on older Clawbot builds still see
            ...
            if model_ids:
                ...
                _url = (_portal or DEFAULT_SOAM_PORTAL_URL).rstrip("/")
        ...
        # them to Soam with a mismatched model.  The Soam OAuth tokens
        # stay saved for future use.
        if not selected_model:
            # Restore the prior active_provider that _save_provider_state
            # overwrote to "soam".  config.yaml model.provider is left
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. Soam credentials saved for future use.")
            print("  Run `clawbot model` again to switch to Soam Portal.")
            return

        config_path = _update_config_for_provider(
            "soam", inference_base_url, default_model=selected_model,
        )
        if selected_model:
            _save_model_choice(selected_model)
            print(f"Default model set to: {selected_model}")
        print(f"  Config updated: {config_path} (model.provider=soam)")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Clawbot will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `clawbot model` or configure an API key to use Clawbot.")
        else:
