...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or clawbot -c.",
    ...
    "/skin changes the CLI theme — try ares, mono, slate, Water, or flame's.",
    ...
    "Ctrl+Z suspends Clawbot to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "clawbot -c resumes your most recent CLI session. clawbot -c \"project name\" resumes by title.",
    "clawbot -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "clawbot -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "clawbot chat -t web,terminal enables only specific toolsets for a focused session.",
    "clawbot chat -s github-pr-workflow preloads a skill at launch.",
    "clawbot chat -q \"query\" runs a single non-interactive query and exits.",
    "clawbot chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "clawbot chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "clawbot --yolo bypasses all dangerous command approval prompts for the entire session.",
    "clawbot chat --source telegram tags the session for filtering in clawbot sessions list.",
    "clawbot -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "clawbot doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "clawbot dump outputs a compact setup summary — great for bug reports.",
    "clawbot config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "clawbot config edit opens config.yaml in your default editor.",
    "clawbot config check scans for missing or stale configuration options.",
    "clawbot sessions browse opens an interactive session picker with search.",
    "clawbot sessions stats shows session counts by platform and database size.",
    "clawbot sessions prune --older-than 30 cleans up old sessions.",
    "clawbot skills search react --source skills-sh searches the skills.sh public directory.",
    "clawbot skills check scans installed hub skills for upstream updates.",
    "clawbot skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "clawbot skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "clawbot mcp add github --command npx adds MCP servers from the command line.",
    "clawbot mcp serve runs Clawbot itself as an MCP server for other agents.",
    "clawbot auth add lets you add multiple API keys for credential pool rotation.",
    "clawbot completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "clawbot logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "clawbot backup creates a zip backup of your entire Clawbot home directory.",
    "clawbot profile create coder creates an isolated profile that becomes its own command.",
    "clawbot profile create work --clone copies your current config and keys to a new profile.",
    "clawbot update syncs new bundled skills to ALL profiles automatically.",
    "clawbot gateway install sets up Clawbot as a system service (systemd/launchd).",
    "clawbot memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "clawbot webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: clawbot tools disables unused tools, clawbot skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "clawbot models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as clawbot -w).",
    ...
    "CLAWBOT_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Clawbot tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'clawbot profile create coder' creates the 'coder' command.",
    "clawbot profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Clawbot prints a resume command with session ID and stats.",
    "clawbot sessions export backup.jsonl exports all sessions for backup or analysis.",
    "clawbot -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "clawbot skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .clawbot/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: clawbot cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Clawbot runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "clawbot gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Clawbot via a local macOS server.",
    ...
    "Context files (.clawbot.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.clawbot/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "Clawbot loads project context from .clawbot.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — clawbot tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.clawbot/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "clawbot chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "clawbot plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "clawbot acp runs Clawbot as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "CLAWBOT_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "clawbot login supports OAuth-based auth for Soam and OpenAI Codex providers.",
    ...
    "clawbot status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.clawbot/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Clawbot your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.clawbot/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Clawbot auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.clawbot/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at CLAWBOT_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "CLAWBOT_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in CLAWBOT_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.clawbot/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'clawbot profile create ls' would be rejected.",
    "clawbot profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "clawbot claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "CLAWBOT_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via clawbot tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.clawbot/.env.",
    "Skills from trusted repos (Aayushsoam) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.clawbot/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Clawbot auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Clawbot config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Clawbot is working.',
    ...
    '/reload reloads ~/.clawbot/.env into the running session — pick up new API keys without restarting.',
    ...
    "CLAWBOT_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.clawbot/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.clawbot/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'clawbot curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "clawbot curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'clawbot curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'clawbot auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    'credential_pool_strategies.<provider>: round_robin cycles keys evenly instead of the fill_first default.',
    'use_gateway: true per-tool routes web, image, tts, or browser through your Soam subscription — no extra keys.',
    ...
    # --- TUI & Dashboard ---
    'CLAWBOT_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "CLAWBOT_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'clawbot dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'clawbot dashboard --tui embeds the full Clawbot TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.clawbot/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.clawbot/dashboard-plugins/ — no npm build required.',
    ...
    'CLAWBOT_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'CLAWBOT_WRITE_SAFE_ROOT restricts write_file and patch to a directory prefix; writes outside require approval.',
    'CLAWBOT_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'CLAWBOT_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.clawbot/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.clawbot/skills/ so drop-in skills appear without restarting the session.',
    ...
    'clawbot -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'clawbot chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'clawbot chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'clawbot chat --ignore-user-config skips ~/.clawbot/config.yaml — reproducible bug reports and CI runs.',
    "clawbot chat --source tool tags programmatic chats so they don't clutter clawbot sessions list.",
    'clawbot dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'clawbot sessions rename <ID> "new title" renames any past session; clawbot sessions delete <ID> removes one.',
    'clawbot import restores a session export or profile archive produced by sessions export or profile export.',
    'clawbot fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'clawbot pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'clawbot setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'clawbot status --deep runs the full health sweep across every component; plain clawbot status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'CLAWBOT_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'CLAWBOT_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.clawbot/plugins/ — trust-gated by design.',
    "CLAWBOT_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'CLAWBOT_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'CLAWBOT_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'CLAWBOT_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'CLAWBOT_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'CLAWBOT_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'CLAWBOT_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'CLAWBOT_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'CLAWBOT_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'CLAWBOT_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'CLAWBOT_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Clawbot block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from clawbot sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.clawbot/dashboard-plugins/.',
]
