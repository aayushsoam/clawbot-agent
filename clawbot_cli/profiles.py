"""
Profile management for multiple isolated Clawbot instances.

Each profile is a fully independent CLAWBOT_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.clawbot/profiles/<name>/`` by default.

The "default" profile is ``~/.clawbot`` itself — backward compatible,
zero migration needed.

Usage::

    clawbot profile create coder          # fresh profile + bundled skills
    clawbot profile create coder --clone  # also copy config, .env, SOUL.md, skills
    clawbot profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    clawbot -p coder chat                 # or via flag
    clawbot profile use coder             # set as sticky default
    clawbot profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See clawbot_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
# default profile (``~/.clawbot``).  Named profiles never contain these
# directories at root, so the exclusion is gated to avoid silently dropping
# user data from a named-profile source.
#
# Rationale per item:
#   clawbot-agent  — git repo checkout (~84 MB source + ~3 GB venv)
...
# Marker file written by `clawbot profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `clawbot
...
# `clawbot skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Clawbot infrastructure directories that only the default profile
         (``~/.clawbot``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_clawbot_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.clawbot) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "clawbot-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
_RESERVED_NAMES = frozenset({
    "clawbot", "default", "test", "tmp", "root", "sudo",
})

# Clawbot subcommands that cannot be used as profile names/aliases
_CLAWBOT_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the clawbot root, NOT to the current CLAWBOT_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where CLAWBOT_HOME points outside
    ``~/.clawbot``, profiles live under ``CLAWBOT_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_clawbot_home() / "profiles"


def _get_default_clawbot_home() -> Path:
    """Return the default (pre-profile) CLAWBOT_HOME path.

    In standard deployments this is ``~/.clawbot``.
    In Docker/custom deployments where CLAWBOT_HOME is outside ``~/.clawbot``
    (e.g. ``/opt/data``), returns CLAWBOT_HOME directly.
    """
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_clawbot_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Clawbot installation itself or a common system binary.  "
            f"Pick a different name."
        )


def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CLAWBOT_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_clawbot_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, clawbot subcommands, existing binaries in PATH.
    """
    canon = normalize_profile_name(name)
    if canon in _RESERVED_NAMES:
        return f"'{canon}' is a reserved name"
    if canon in _CLAWBOT_SUBCOMMANDS:
        return f"'{canon}' conflicts with a clawbot subcommand"
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``clawbot profile list`` for an unrelated profile.
    """
...
# ``config.yaml``. ``config.yaml`` is the user-facing Clawbot config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_clawbot_home()
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``clawbot update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.clawbot)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.clawbot/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
    ...
    # Write the opt-out marker so seed_profile_skills() and `clawbot update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`clawbot profile create --no-skills`).\n"
                "Delete this file to re-enable sync on the next `clawbot update`.\n",
                encoding="utf-8",
            )
        except OSError:
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `clawbot profile describe`
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CLAWBOT_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``clawbot profile create
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CLAWBOT_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.clawbot).\n"
            "To remove everything, use: clawbot uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CLAWBOT_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CLAWBOT_HOME")
    try:
        os.environ["CLAWBOT_HOME"] = str(profile_dir)
        from clawbot_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CLAWBOT_HOME"] = old_home
        elif "CLAWBOT_HOME" in os.environ:
            del os.environ["CLAWBOT_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.clawbot/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: clawbot profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CLAWBOT_HOME.

    Returns ``"default"`` if CLAWBOT_HOME is not set or points to ``~/.clawbot``.
    Returns the profile name if CLAWBOT_HOME points into ``~/.clawbot/profiles/<name>``.
    Returns ``"custom"`` if CLAWBOT_HOME is set to an unrecognized path.
    """
    from clawbot_constants import get_clawbot_home
    clawbot_home = get_clawbot_home()
    resolved = clawbot_home.resolve()

    default_resolved = _get_default_clawbot_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.clawbot itself — its parent is ~/ and its
        # directory name is ".clawbot", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: clawbot profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.clawbot itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.clawbot). "
            "Specify a different name: clawbot profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="clawbot_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"clawbot.{old_name}"
    new_host = f"clawbot.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_clawbot_home() / "honcho.json",
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CLAWBOT_HOME path string.

    Called early in the CLI entry point, before any clawbot modules
    are imported, to set the CLAWBOT_HOME environment variable.
    """
