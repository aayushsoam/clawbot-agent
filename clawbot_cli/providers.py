"""
Single source of truth for provider identity in Clawbot Agent.
...
2. **Clawbot overlays** — transport type, auth patterns, aggregator flags,
...
# -- Clawbot overlay ----------------------------------------------------------
# Clawbot-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class ClawbotOverlay:
    """Clawbot-specific provider metadata layered on top of models.dev."""
...
CLAWBOT_OVERLAYS: Dict[str, ClawbotOverlay] = {
    "openrouter": ClawbotOverlay(
    ...
    "soam": ClawbotOverlay(
        ...
        base_url_override="https://inference-api.aayushsoam.com/v1",
    ),
    "openai-codex": ClawbotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "xai-oauth": ClawbotOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": ClawbotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="CLAWBOT_QWEN_BASE_URL",
    ),
    "google-gemini-cli": ClawbotOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": ClawbotOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": ClawbotOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": ClawbotOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": ClawbotOverlay(
    ...
    "kimi-for-coding": ClawbotOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": ClawbotOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": ClawbotOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": ClawbotOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": ClawbotOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": ClawbotOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": ClawbotOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": ClawbotOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": ClawbotOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": ClawbotOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": ClawbotOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": ClawbotOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": ClawbotOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": ClawbotOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": ClawbotOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": ClawbotOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": ClawbotOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "clawbot", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Clawbot overlays (for providers not in models.dev: soam, openai-codex, etc.)
      2. models.dev catalog + Clawbot overlay
    ...
    overlay = CLAWBOT_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + clawbot extra
    ...
    if overlay is not None:
        # Clawbot-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in CLAWBOT_OVERLAYS
    if provider == "bedrock":
