"""
Clawbot Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m clawbot_cli.main web          # Start on http://127.0.0.1:9119
    python -m clawbot_cli.main web --port 8080
"""
...
try:
    ...
    # running `clawbot dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["CLAWBOT_WEB_DIST"]) if "CLAWBOT_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Clawbot Agent", version=__version__)
...
# or CLAWBOT_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.clawbot/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_clawbot_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "clawbot-update": "clawbot-update.log",
}
...
def _spawn_clawbot_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``clawbot <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``clawbot_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "clawbot_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "CLAWBOT_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``clawbot gateway restart`` in the background."""
    try:
        proc = _spawn_clawbot_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/clawbot/update")
async def update_clawbot():
    """Kick off ``clawbot update`` in the background."""
    try:
        proc = _spawn_clawbot_action(["update"], "clawbot-update")
    except Exception as exc:
        _log.exception("Failed to spawn clawbot update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Clawbot supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
# in clawbot_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.clawbot/config.yaml`` — applies to **new** sessions only.
...
# Anthropic, device-code for Soam/Codex) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``clawbot auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    Clawbot resolves Anthropic creds in this order at runtime:
    1. ``~/.clawbot/.anthropic_oauth.json`` — Clawbot-managed PKCE flow
    ...
    try:
        from agent.anthropic_adapter import (
            read_clawbot_oauth_credentials,
            read_claude_code_credentials,
            _CLAWBOT_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_clawbot_oauth_credentials = None  # type: ignore
        _CLAWBOT_OAUTH_FILE = None  # type: ignore

    clawbot_creds = None
    if read_clawbot_oauth_credentials:
        try:
            clawbot_creds = read_clawbot_oauth_credentials()
        except Exception:
            clawbot_creds = None
    if clawbot_creds and clawbot_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "clawbot_pkce",
            "source_label": f"Clawbot PKCE ({_CLAWBOT_OAUTH_FILE})",
            "token_preview": _truncate_token(clawbot_creds.get("accessToken")),
            "expires_at": clawbot_creds.get("expiresAt"),
            "has_refresh_token": bool(clawbot_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Clawbot even
    when they also have a separate Clawbot-managed PKCE login.
    """
    try:
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "clawbot auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "clawbot auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "clawbot auth add qwen-oauth",
    ...
    },
    {
        ...
        # as Soam's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "clawbot auth add minimax-oauth",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from clawbot_cli import auth as hauth
        if provider_id == "soam":
            raw = hauth.get_soam_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "soam_portal",
                "source_label": raw.get("portal_base_url") or "Soam Portal",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("clawbot_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same Clawbot-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in {"anthropic", "claude-code"}:
        try:
            from agent.anthropic_adapter import _CLAWBOT_OAUTH_FILE
            if _CLAWBOT_OAUTH_FILE.exists():
                _CLAWBOT_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from clawbot_cli.auth import clear_provider_auth
...
#          → persists to ~/.clawbot/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (Soam, OpenAI Codex):
#     1. POST /api/providers/oauth/{soam|openai-codex}/start
...
# Guarded so clawbot web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Clawbot file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``clawbot auth add anthropic``.
    """
    from agent.anthropic_adapter import _CLAWBOT_OAUTH_FILE
    payload = {
    ...
    _CLAWBOT_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _CLAWBOT_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
def _submit_anthropic_pkce(session_id: str, code_input: str) -> Dict[str, Any]:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "clawbot-dashboard/1.0",
        },
        method="POST",
    )
    try:
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    """Initiate a device-code flow (Soam, OpenAI Codex, or MiniMax).
    ...
    if provider_id == "soam":
        from clawbot_cli.auth import (
            _soam_device_scope_with_env_override,
            _request_soam_device_code_with_scope_fallback,
            PROVIDER_REGISTRY,
        )
        import httpx
        pconfig = PROVIDER_REGISTRY["soam"]
        portal_base_url = (
            os.getenv("CLAWBOT_PORTAL_BASE_URL")
        ...
        def _do_soam_device_request():
            with httpx.Client(
                timeout=httpx.Timeout(15.0),
                headers={"Accept": "application/json"},
            ) as client:
                return _request_soam_device_code_with_scope_fallback(
                    client=client,
        ...
        device_data, effective_scope = await asyncio.get_running_loop().run_in_executor(
            None, _do_soam_device_request
        )
        sid, sess = _new_oauth_session("soam", "device_code")
        ...
        threading.Thread(
            target=_soam_poller, args=(sid,), daemon=True, name=f"oauth-poll-{sid[:6]}"
        ).start()
        return {
            "session_id": sid,
            "flow": "device_code",
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to Soam's device-code
...
def _soam_poller(session_id: str) -> None:
    """Background poller that drives a Soam device-code flow to completion."""
    from clawbot_cli.auth import (
        SOAM_INFERENCE_AUTH_MODE_FRESH,
        _poll_for_token,
        refresh_soam_oauth_from_state,
    )
    ...
    try:
        ...
        # Same post-processing as _soam_device_code_login (mint agent key)
        ...
        full_state = refresh_soam_oauth_from_state(
            auth_state,
        ...
        persist_soam_credentials(full_state)
        with _oauth_sessions_lock:
            sess["status"] = "approved"
        _log.info("oauth/device: soam login completed (session=%s)", session_id)
    except Exception as e:
        _log.warning("soam device-code poll failed (session=%s): %s", session_id, e)
        with _oauth_sessions_lock:
            sess["status"] = "error"
...
def _minimax_poller(session_id: str) -> None:
    ...
    Mirrors `_soam_poller` but calls the MiniMax-specific token endpoint,
    which uses a PKCE-style ``code_verifier`` + ``user_code`` rather than
    the ``device_code`` field used by Soam. On success, builds the same
...
async def get_logs(
    ...
    from clawbot_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_clawbot_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from clawbot_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _profile_setup_command(name: str) -> str:
    ...
    return "clawbot setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from clawbot_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from clawbot_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``clawbot --tui`` binary the CLI uses, behind
...
try:
    from clawbot_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
async def _send_pty_error_and_close(ws: WebSocket, text: str, code: int = 1011) -> None:
    """Send a PTY startup error if the browser is still connected."""
    try:
        await ws.send_text(text)
    except WebSocketDisconnect:
        return
    except Exception as exc:
        _log.debug("PTY websocket disconnected before error could be sent: %s", exc)
        return

    try:
        await ws.close(code=code)
    except Exception as exc:
        _log.debug("PTY websocket disconnected before close frame could be sent: %s", exc)


def _running_under_wsl() -> bool:
    """Best-effort WSL detection for dashboard browser launch behavior."""
    if sys.platform != "linux":
        return False
    if os.environ.get("WSL_DISTRO_NAME") or os.environ.get("WSL_INTEROP"):
        return True

    for marker in ("/proc/sys/kernel/osrelease", "/proc/version"):
        try:
            text = Path(marker).read_text(encoding="utf-8", errors="ignore").lower()
        except OSError:
            continue
...
def _open_dashboard_url(url: str) -> bool:
    """Open the dashboard URL without leaking helper-tool stderr to the console."""
...
def _resolve_chat_argv(
    ...
    Default: whatever ``clawbot --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``CLAWBOT_TUI_RESUME`` env var —
    matching what ``clawbot_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``CLAWBOT_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from clawbot_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("CLAWBOT_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["CLAWBOT_TUI_RESUME"] = resume

    if sidecar_url:
        env["CLAWBOT_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await _send_pty_error_and_close(
            ws,
            "\r\n\x1b[31mChat unavailable: the embedded terminal requires a "
            "POSIX PTY, which native Windows Python doesn't provide.\x1b[0m\r\n"
            "\x1b[33mInstall Clawbot inside WSL2 to use the dashboard's /chat "
...
# CLAWBOT_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/clawbot"`` (no trailing slash) or ``""`` when
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/clawbot/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /clawbot`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__CLAWBOT_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/clawbot``)
        or empty string when served at root.
        """
        html = _index_path.read_text()
        chat_js = "true" if _DASHBOARD_EMBEDDED_CHAT_ENABLED else "false"
        token_script = (
            f'<script>window.__CLAWBOT_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__CLAWBOT_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__CLAWBOT_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
    ...
    # under ``/clawbot`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Clawbot backend. Intercept CSS asset
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.clawbot/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.clawbot/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_clawbot_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.clawbot/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as clawbot_cli.plugins):
    1. User plugins:    ~/.clawbot/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.clawbot/plugins/  (only if CLAWBOT_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from clawbot_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_clawbot_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("CLAWBOT_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".clawbot" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from clawbot_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_clawbot_home() / "plugins").resolve()
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"clawbot auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from clawbot_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from clawbot_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from clawbot_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from clawbot_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"clawbot_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    if open_browser:
        ...
        if _has_display or _running_under_wsl():
            def _open():
                time.sleep(1.0)
