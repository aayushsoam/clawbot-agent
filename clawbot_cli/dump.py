"""
Dump command for clawbot CLI.

Outputs a compact, plain-text summary of the user's Clawbot setup
...
from clawbot_cli.config import get_clawbot_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``clawbot dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from clawbot_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(clawbot_home: Path) -> int:
...
def _cron_summary(clawbot_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = clawbot_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"clawbot_home:      {display_clawbot_home()}")
    ...
    toolsets = config.get("toolsets", ["clawbot-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(clawbot_home)}")
    lines.append(f"  skills:             {_count_skills(clawbot_home)}")
