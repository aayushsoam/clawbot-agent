...
Used by ``clawbot profile describe <name> --auto`` and the dashboard's
...
Design notes
------------
- Mirrors the shape of ``clawbot_cli/kanban_specify.py``: lazy aux
...
_SYSTEM_PROMPT = """You are a profile-describer for the Clawbot Agent kanban board.
...
Rules:
  ...
  - Never write "Clawbot Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_clawbot_home())
        else:
