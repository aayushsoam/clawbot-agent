"""
Unified self-relaunch for Clawbot CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``clawbot sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``clawbot`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from clawbot_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched clawbot."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_clawbot_bin() -> Optional[str]:
    """Find the clawbot entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("clawbot")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m clawbot_cli.main``.
    ...
    ``clawbot.exe`` on PATH or the ``sys.executable -m clawbot_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with clawbot.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_clawbot_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "clawbot_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh clawbot invocation.
    ...
    relaunch contract wants: "run clawbot again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``clawbot.exe`` (a Python console-script shim that wraps
    ``python -m clawbot_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "clawbot exited, then
    new clawbot started" — just with two PIDs in play instead of one.
    """
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            # cryptic.  Common causes: ``clawbot`` not on PATH yet (install
            # hasn't propagated User PATH into this shell) or a stale shim.
            print(
                f"\nClawbot relaunch failed: {exc}\n"
                f"Command: {' '.join(new_argv)}\n"
                f"Fix: open a new terminal so PATH picks up, then re-run clawbot.",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
