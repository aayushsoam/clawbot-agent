"""
Clawbot Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.clawbot/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Clawbot PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing clawbot-agent or clawbot PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Clawbot Agent" comment and following line
                if '# Clawbot Agent' in line or '# clawbot-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('clawbot' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing clawbot
                if 'clawbot' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the clawbot wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "clawbot",
        Path("/usr/local/bin/clawbot"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains clawbot_cli reference)
                content = wrapper.read_text()
                if 'clawbot_cli' in content or 'clawbot-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``clawbot gateway run`` processes
    ...
    try:
        from clawbot_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from clawbot_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``CLAWBOT_HOME`` and ``CLAWBOT_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\clawbot\git\cmd``, ``%LOCALAPPDATA%\clawbot\git\bin``,
#      ``%LOCALAPPDATA%\clawbot\git\usr\bin``, ``%LOCALAPPDATA%\clawbot\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\clawbot\git\`` and Node to
#      ``%LOCALAPPDATA%\clawbot\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``clawbot dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\clawbot\gateway-service\`` and sometimes
...
def _clawbot_path_markers(clawbot_home: Path) -> list[str]:
    """Path-entry substrings that identify Clawbot-owned User-PATH entries."""
    root = str(clawbot_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare clawbot-agent install dir.
    markers = [root + "\\clawbot-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if CLAWBOT_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "clawbot".  We don't want to catch
    # unrelated entries like "cclawbot-foo" or "epclawbotal", so we look for
    # backslash-clawbot as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(clawbot_home: Path) -> list[str]:
    """Strip Clawbot-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _clawbot_path_markers(clawbot_home)
            kept: list[str] = []
...
def remove_clawbot_env_vars_windows() -> list[str]:
    """Delete CLAWBOT_HOME and CLAWBOT_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("CLAWBOT_HOME", "CLAWBOT_GIT_BASH_PATH"):
                try:
...
def _is_default_clawbot_home(clawbot_home: Path) -> bool:
    """Return True when ``clawbot_home`` points at the default (non-profile) root."""
    try:
        from clawbot_constants import get_default_clawbot_root
        return clawbot_home.resolve() == get_default_clawbot_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its CLAWBOT_HOME directory.

    We shell out to ``clawbot -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current CLAWBOT_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m clawbot_cli.main` so we don't depend on a `clawbot`
    #    wrapper that may be half-removed mid-uninstall.
    clawbot_invocation = [_sys.executable, "-m", "clawbot_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                clawbot_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's CLAWBOT_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.clawbot/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.clawbot/ for future reinstall
    """
    project_root = get_project_root()
    clawbot_home = get_clawbot_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie CLAWBOT_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_clawbot_home(clawbot_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚡Clawbot Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {clawbot_home / 'config.yaml'}")
    print(f"  Secrets: {clawbot_home / '.env'}")
    print(f"  Data:    {clawbot_home / 'cron/'}, {clawbot_home / 'sessions/'}, {clawbot_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their CLAWBOT_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Clawbot data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Clawbot code but keep your configuration and data.")
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in clawbot_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\clawbot\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(clawbot_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Clawbot-owned PATH entries in User environment")

        log_info("Removing CLAWBOT_HOME / CLAWBOT_GIT_BASH_PATH User env vars...")
        removed_env = remove_clawbot_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Clawbot-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing clawbot command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.clawbot/, just remove the clawbot-agent subdir
            if clawbot_home in project_root.parents or project_root.parent == clawbot_home:
    ...
    #     under CLAWBOT_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(clawbot_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(clawbot_home)
    ...
    # 5. Optionally remove ~/.clawbot/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile CLAWBOT_HOME dirs live under
        ...
        try:
            if clawbot_home.exists():
                shutil.rmtree(clawbot_home)
                log_success(f"Removed {clawbot_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {clawbot_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {clawbot_home}")
    ...
    if not full_uninstall:
        ...
        print(f"  {clawbot_home}/")
        print()
        print("To reinstall later with your existing settings:")
        if _is_windows():
            print(color("  irm https://raw.githubusercontent.com/Aayushsoam/clawbot-agent/main/scripts/install.ps1 | iex", Colors.DIM))
        else:
            print(color("  curl -fsSL https://raw.githubusercontent.com/Aayushsoam/clawbot-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    ...
    print()
    print("Thank you for using Clawbot Agent! ⚡")
    print()
