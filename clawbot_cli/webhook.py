"""clawbot webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.clawbot/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _clawbot_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: clawbot gateway run
"""
...
def webhook_command(args):
    """Entry point for 'clawbot webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: clawbot webhook {subscribe|list|remove|test}")
        print("Run 'clawbot webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (clawbot gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: clawbot webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from clawbot webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (clawbot gateway run)")
