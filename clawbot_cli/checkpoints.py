"""`clawbot checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.clawbot/checkpoints/``.  Actions:

    clawbot checkpoints               # same as `status`
    clawbot checkpoints status        # total size, project count, breakdown
    clawbot checkpoints list          # per-project checkpoint counts + workdir
    clawbot checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    clawbot checkpoints clear [-f]    # nuke the entire base (asks first)
    clawbot checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    clawbot checkpoints
    clawbot checkpoints prune --retention-days 3 --max-size-mb 200
    clawbot checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: clawbot checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``clawbot checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `clawbot checkpoints` → status
