"""
Interactive setup wizard for Clawbot Agent.
...
from clawbot_cli.soam_subscription import get_soam_subscription_features
from tools.tool_backend_helpers import managed_soam_tools_enabled
...
_DOCS_BASE = "https://clawbot-agent.aayushsoam.com/docs"
...
# display_clawbot_home imported lazily at call sites (stale-module safety during clawbot update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚡Clawbot Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Clawbot using environment variables or config commands:")
    print_info("  clawbot config set model.provider custom")
    print_info("  clawbot config set model.base_url http://localhost:8080/v1")
    print_info("  clawbot config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'clawbot setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from clawbot_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'clawbot setup')")


def _print_setup_summary(config: dict, clawbot_home):
    ...
    tool_status = []
    subscription_features = get_soam_subscription_features(config)
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'clawbot setup' to configure"))
    ...
    if subscription_features.web.managed_by_soam:
        tool_status.append(("Web Search & Extract (Soam subscription)", True, None))
    ...
    if subscription_features.browser.managed_by_soam:
        tool_status.append(("Browser Automation (Soam Browser Use)", True, None))
    ...
    # Image generation — FAL (direct or via Soam), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_soam:
        tool_status.append(("Image Generation (Soam subscription)", True, None))
        ...
        try:
            ...
            from clawbot_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `clawbot tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    try:
        from agent.video_gen_registry import list_providers as _list_video_providers
        from clawbot_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
        _ensure_plugins()
        _video_backend = None
    ...
    if subscription_features.tts.managed_by_soam:
        tool_status.append(("Text-to-Speech (OpenAI via Soam subscription)", True, None))
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'clawbot setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'clawbot setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_soam:
        tool_status.append(("Modal Execution (Soam subscription)", True, None))
    elif cfg_get(config, "terminal", "backend") == "modal":
        if subscription_features.modal.direct_override:
            tool_status.append(("Modal Execution (direct Modal)", True, None))
        else:
            tool_status.append(("Modal Execution", False, "run 'clawbot setup terminal'"))
    elif managed_soam_tools_enabled() and subscription_features.soam_auth_present:
        tool_status.append(("Modal Execution (optional via Soam subscription)", True, None))

    # Home Assistant
    if get_env_value("HASS_TOKEN"):
        tool_status.append(("Smart Home (Home Assistant)", True, None))

    # Spotify (OAuth via clawbot auth spotify — check auth.json, not env vars)
    try:
        from clawbot_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'clawbot setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {clawbot_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('clawbot setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('clawbot setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('clawbot setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('clawbot setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('clawbot setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('clawbot config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('clawbot config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('clawbot config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('clawbot', Colors.GREEN)}              Start chatting")
    print(f"   {color('clawbot gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('clawbot doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `clawbot tools` and `clawbot setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``clawbot model``)
    ...
    provider added to ``clawbot model`` is automatically available here.
    ...
    # Delegate to the shared clawbot model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from clawbot_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: clawbot model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from clawbot_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Clawbot can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Clawbot can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'clawbot setup' or configure AUXILIARY_VISION_* settings")


    # Tool Gateway prompt is already shown by _model_flow_soam() above.
    save_config(config)

    if not quick and selected_provider != "soam":
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``clawbot model`` -> xAI Grok OAuth (SuperGrok Subscription).
    """
    try:
        from clawbot_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_soam_tools_enabled() and subscription_features.soam_auth_present:
        choices.append("Soam Subscription (managed OpenAI TTS, billed to your subscription)")
        providers.append("soam-openai")
    choices.extend(
        [
    ...
    selected_via_soam = selected == "soam-openai"
    if selected == "soam-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed Soam gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.clawbot/.env."
            )
    ...
    if selected == "neutts":
    ...
    elif selected == "openai" and not selected_via_soam:
        ...
        # via the Clawbot auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via clawbot setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'clawbot setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Clawbot runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        managed_modal_available = bool(
            managed_soam_tools_enabled()
            and
            get_soam_subscription_features(config).soam_auth_present
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed Soam gateway and bill to your subscription.")
        ...
        print_info("Requires the optional SDK: pip install 'clawbot-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'clawbot-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into CLAWBOT_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("CLAWBOT_MAX_ITERATIONS")
    ...
    print_info("  Run `clawbot setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # CLAWBOT_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Clawbot delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `clawbot update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://clawbot-agent.aayushsoam.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under CLAWBOT_HOME, and print
    ...
    """
    try:
        from clawbot_cli.slack_cli import _build_full_manifest
        from clawbot_constants import get_clawbot_home

        manifest = _build_full_manifest(
            bot_name="Clawbot",
            bot_description="Your Clawbot agent on Slack",
        )
        target = Path(get_clawbot_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `clawbot slack manifest --write` anytime to refresh after "
            "Clawbot adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Clawbot delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where Clawbot delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  clawbot config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Clawbot to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from clawbot_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://clawbot-agent.aayushsoam.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://clawbot-agent.aayushsoam.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  clawbot config edit")
    print_info("   Open config in your editor:  clawbot config edit")
...
def setup_gateway(config: dict):
    ...
    from clawbot_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Clawbot from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'clawbot setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     clawbot config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_clawbot_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  clawbot gateway")
        else:
            from clawbot_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   clawbot gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   clawbot gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `clawbot setup tools` and `clawbot tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from clawbot_cli.config
    ...
    if section_key == "model":
        ...
        from clawbot_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_clawbot.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_clawbot migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_clawbot", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Clawbot.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Clawbot to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Clawbot at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Clawbot at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Clawbot at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Clawbot at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Clawbot equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Clawbot config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Clawbot.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Clawbot's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(clawbot_home: Path) -> bool:
    ...
    print_info("Clawbot can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: clawbot claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=clawbot_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: clawbot claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Clawbot configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Clawbot config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Clawbot (use clawbot claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      clawbot setup           — full or quick (auto-detected)
      clawbot setup model     — just model/provider
      clawbot setup tts       — just text-to-speech
      clawbot setup terminal  — just terminal backend
      clawbot setup gateway   — just messaging platforms
      clawbot setup tools     — just tool configuration
      clawbot setup agent     — just agent settings
    """
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, clawbot_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Clawbot configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'clawbot setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(clawbot_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up Clawbot?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, clawbot_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {clawbot_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'clawbot config edit'")
...
def _run_first_time_quick_setup(config: dict, clawbot_home, is_existing: bool):
    ...
    the user can customize later via ``clawbot setup <section>``.
    """
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'clawbot setup gateway'",
        ],
        0,
    )
...
def _run_quick_setup(config: dict, clawbot_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'clawbot setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Clawbot to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'clawbot setup gateway'.")
