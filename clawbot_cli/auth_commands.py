...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "soam", "openai-codex", "xai-oauth", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_clawbot_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:clawbot_pkce",
    ...
    if provider == "soam":
        # Codex-style auto-import: if a shared Soam credential lives at
        # <clawbot-root>/shared/soam_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `clawbot --profile <name>
        # auth add soam --type oauth` a one-tap operation for users who
        # run multiple profiles.
        shared = auth_mod._read_shared_soam_state()
        if shared:
            try:
                path = auth_mod._soam_shared_store_path()
            except RuntimeError:
                path = None
            print()
            if path:
                print(f"Found existing Soam OAuth credentials at {path}")
            else:
            ...
            if do_import in {"", "y", "yes"}:
                ...
                rehydrated = auth_mod._try_import_shared_soam_state(
                ...
                if rehydrated is not None:
                    ...
                    entry = auth_mod.persist_soam_credentials(rehydrated, label=custom_label)
        ...
        creds = auth_mod._soam_device_code_login(
        ...
        # Honor `--label <name>` so soam matches other providers' UX.  The
        # helper embeds this into providers.soam so that label_from_token
        # doesn't overwrite it on every subsequent load_pool("soam").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_soam_credentials(creds, label=custom_label)
        shown_label = entry.label if entry is not None else label_from_token(
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `clawbot auth
    ...
    raise SystemExit(f"`clawbot auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Clawbot reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `clawbot auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `clawbot auth` is called bare."""
