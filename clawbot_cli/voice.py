...
# lookup, and clawbot-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and clawbot-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when CLAWBOT_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("CLAWBOT_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def stop_continuous(force_transcribe: bool = False) -> None:
    ...
    True, the recorder stops synchrosoamly, then transcription/cleanup runs on a
