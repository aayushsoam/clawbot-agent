"""Helpers for Soam subscription managed-tool capabilities."""
...
from clawbot_cli.config import get_env_value, load_config
...
class SoamFeatureState:
    key: str
    label: str
    included_by_default: bool
    available: bool
    active: bool
    managed_by_soam: bool
...
@dataclass(frozen=True)
class SoamSubscriptionFeatures:
    subscribed: bool
    soam_auth_present: bool
    provider_is_soam: bool
    features: Dict[str, SoamFeatureState]

    @property
    def web(self) -> SoamFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> SoamFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> SoamFeatureState:
        return self.features["tts"]

    @property
    def browser(self) -> SoamFeatureState:
        return self.features["browser"]

    @property
    def modal(self) -> SoamFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[SoamFeatureState]:
...
def get_soam_subscription_features(
    config: Optional[Dict[str, object]] = None,
) -> SoamSubscriptionFeatures:
    if config is None:
        config = load_config() or {}
    config = dict(config)
    model_cfg = _model_config_dict(config)
    provider_is_soam = str(model_cfg.get("provider") or "").strip().lower() == "soam"
    ...
    managed_tools_flag = managed_soam_tools_enabled()
    soam_auth_present = bool(soam_status.get("logged_in"))
    subscribed = provider_is_soam or soam_auth_present
    ...
    # Tool Gateway via `clawbot model`, so direct credentials should NOT
    ...
    managed_web_available = managed_tools_flag and soam_auth_present and is_managed_tool_gateway_ready("firecrawl")
    managed_image_available = managed_tools_flag and soam_auth_present and is_managed_tool_gateway_ready("fal-queue")
    managed_tts_available = managed_tools_flag and soam_auth_present and is_managed_tool_gateway_ready("openai-audio")
    managed_browser_available = managed_tools_flag and soam_auth_present and is_managed_tool_gateway_ready("browser-use")
    managed_modal_available = managed_tools_flag and soam_auth_present and is_managed_tool_gateway_ready("modal")
    ...
    features = {
        "web": SoamFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_soam=web_managed,
        ...
        "image_gen": SoamFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_soam=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("Soam Subscription" if image_managed else ""),
            explicit_configured=direct_fal,
        ),
        "tts": SoamFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_soam=tts_managed,
        ...
        "browser": SoamFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_soam=browser_managed,
        ...
        "modal": SoamFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_soam=modal_managed,
    ...
    return SoamSubscriptionFeatures(
        subscribed=subscribed,
        soam_auth_present=soam_auth_present,
        provider_is_soam=provider_is_soam,
        features=features,
    )
...
def apply_soam_managed_defaults(
    ...
    *,
    ...
    if not features.provider_is_soam:
        return set()
...
def get_gateway_eligible_tools(
    ...
    All lists are empty when the user is not a paid Soam subscriber or
    is not using Soam as their provider.
    """
    if not managed_soam_tools_enabled():
        return [], [], []
    ...
    # Quick provider check without the heavy get_soam_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "soam":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_soam which fires implicitly when
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from clawbot_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    desc_parts: list[str] = [
        ...
        "  text-to-speech, and browser automation through your Soam subscription.",
    ...
    try:
        idx = prompt_choice(
            "Your Soam subscription includes the Tool Gateway.",
            choices,
            default_idx,
    ...
    if changed:
        ...
        for key in sorted(newly_switched):
            ...
            print(f"  ✓ {label}: enabled via Soam subscription")
