"""CLI handlers for the ``clawbot proxy`` subcommand."""
...
from clawbot_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "clawbot proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "soam"
    try:
    ...
    if not adapter.is_authenticated():
        print(
            ...
            f"Run `clawbot login {adapter.name}` first.",
            file=sys.stderr,
        )
        return 2
    ...
    print(
        f"Starting Clawbot proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Clawbot proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: clawbot proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``clawbot proxy <subcommand>``."""
    ...
    print(
        "clawbot proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  clawbot proxy start [--provider soam] [--host 127.0.0.1] [--port 8645]\n"
