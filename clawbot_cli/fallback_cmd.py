"""
clawbot fallback — manage the fallback provider chain.
...
Subcommands:
  clawbot fallback [list]   Show the current fallback chain (default when no subcommand)
  clawbot fallback add      Pick provider + model via the same picker as `clawbot model`,
                           then append the selection to the chain
  clawbot fallback remove   Pick an entry to delete from the chain
  clawbot fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.clawbot/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from clawbot_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `clawbot model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  clawbot fallback add")
        print()
        return
    ...
    print("  Docs: https://clawbot-agent.aayushsoam.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `clawbot model`, then append the selection to the chain."""
    from clawbot_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `clawbot model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `clawbot fallback list` to view, or `clawbot fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``clawbot fallback [subcommand]``."""
