...
The Clawbot docs site hosts a JSON manifest of curated models for providers
we want to update without shipping a release (currently OpenRouter and
Soam Portal). This module fetches, validates, and caches that manifest,
...
def _cache_path() -> Path:
    ...
    return get_clawbot_home() / "cache" / "model_catalog.json"
...
def get_curated_soam_models() -> list[str] | None:
    """Return Soam Portal's curated list of model ids from the manifest.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("soam")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``clawbot model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
