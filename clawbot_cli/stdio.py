...
on Japanese locales, etc.).  Clawbot's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``clawbot_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Clawbot is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``CLAWBOT_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("CLAWBOT_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Clawbot-managed Git install directories so
    ...
    # already-running shells don't see the change — which means clawbot
    ...
    # install without Clawbot-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Clawbot's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Clawbot's
    ``clawbot config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Clawbot (User env var in Windows Settings, or
    export in a PowerShell profile) and Clawbot picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Clawbot-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\clawbot\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``clawbot`` from right after install) retain their old PATH.

    Any subprocess Clawbot spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Clawbot-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "clawbot", "git", "cmd"),
        os.path.join(local_appdata, "clawbot", "git", "bin"),
        os.path.join(local_appdata, "clawbot", "git", "usr", "bin"),
        # Clawbot venv Scripts directory — host of the clawbot.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes clawbot, but harmless to include.
        os.path.join(local_appdata, "clawbot", "clawbot-agent", "venv", "Scripts"),
