"""
Configuration management for Clawbot Agent.

Config files are stored in ~/.clawbot/ for easy access:
- ~/.clawbot/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.clawbot/.env         - API keys and secrets

This module provides:
- clawbot config          - Show current configuration
- clawbot config edit     - Open config in editor
- clawbot config set      - Set a specific value
- clawbot config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.clawbot/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``clawbot logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  clawbot config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `clawbot plugins enable
    # observability/langfuse`); credentials gate the plugin at runtime.
    "CLAWBOT_LANGFUSE_ENV",
    "CLAWBOT_LANGFUSE_RELEASE",
    "CLAWBOT_LANGFUSE_SAMPLE_RATE",
    "CLAWBOT_LANGFUSE_MAX_CHARS",
    "CLAWBOT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from clawbot_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CLAWBOT_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_clawbot_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Clawbot is running in package-manager-managed mode.

    Two signals: the CLAWBOT_MANAGED env var (set by the systemd service),
    or a .managed marker file in CLAWBOT_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade clawbot-agent"
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Clawbot was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Stamped ``~/.clawbot/.install_method`` file (written by installers)
    2. CLAWBOT_MANAGED env / .managed marker (NixOS, Homebrew)
    3. Container detection (/.dockerenv, /run/.containerenv, cgroup)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'
    """
    stamp = get_clawbot_home() / ".install_method"
    try:
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.clawbot/.install_method."""
    stamp = get_clawbot_home() / ".install_method"
    try:
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade clawbot-agent"
    if method == "docker":
        return "docker pull aayushsoam/clawbot-agent:latest"
    if method == "pip":
        import shutil
        uv = shutil.which("uv")
        if uv:
            return "uv pip install --upgrade clawbot-agent"
        return "pip install --upgrade clawbot-agent"
    return "clawbot update"
...
def format_managed_message(action: str = "modify this Clawbot installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CLAWBOT_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Clawbot installation is managed by NixOS "
            f"(CLAWBOT_MANAGED={env_hint}).\n"
            "Edit services.clawbot-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Clawbot installation is managed by Homebrew "
            f"(CLAWBOT_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade clawbot-agent"
        )

    return (
        f"Cannot {action}: this Clawbot installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Clawbot."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from CLAWBOT_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, clawbot_bin
    or None if container mode is not active, we're already inside the
    container, or CLAWBOT_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("CLAWBOT_DEV") == "1":
        return None
    ...
    container_mode_file = get_clawbot_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "clawbot-agent")
    exec_user = info.get("exec_user", "clawbot")
    clawbot_bin = info.get("clawbot_bin", "/data/current-package/bin/clawbot")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "clawbot_bin": clawbot_bin,
    }
...
# Re-export from clawbot_constants — canonical definition lives there.
from clawbot_constants import get_clawbot_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_clawbot_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_clawbot_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the clawbot group can
    share state with the gateway service.

    The mode can be overridden via the CLAWBOT_HOME_MODE environment variable
    (e.g. CLAWBOT_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse CLAWBOT_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("CLAWBOT_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When Clawbot runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("CLAWBOT_CONTAINER") or os.environ.get("CLAWBOT_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set CLAWBOT_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CLAWBOT_HOME if the user doesn't have one yet."""
...
def ensure_clawbot_home():
    """Ensure ~/.clawbot directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_clawbot_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_clawbot_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"CLAWBOT_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Clawbot-level retry loop that wraps the whole call.  Lower
        ...
        # blocks indefinitely (input() is synchrosoam) and ignores this.
        "clarify_timeout": 600,
    ...
    "terminal": {
        ...
        # skipped. When empty, Clawbot auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Clawbot sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Clawbot
        ...
        # bundled Clawbot image, which drops to the `clawbot` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Clawbot sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Clawbot should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``clawbot chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.clawbot/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: clawbot sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, Clawbot truncates the
    ...
    "auxiliary": {
        "vision": {
            "provider": "auto",    # auto | openrouter | soam | codex | custom
        ...
        # Invoked by ``clawbot kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``clawbot kanban decompose`` and the kanban
        ...
        # ``clawbot profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `clawbot model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `clawbot --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `clawbot -c` muscle memory.  Default off so existing
        # users aren't surprised.  CLAWBOT_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/clawbot`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.clawbot/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.clawbot/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, Soam, Z.ai, Kimi, etc.) are supported.
    "delegation": {
    ...
    # response. If not, Clawbot feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Clawbot auto-pauses the goal and
    ...
    # always goes to ~/.clawbot/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${CLAWBOT_SKILL_DIR} and ${CLAWBOT_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `clawbot curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.clawbot/skills/ into
        # ~/.clawbot/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `clawbot curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for clawbot-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚡*Clawbot Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (CLAWBOT_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.clawbot/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or CLAWBOT_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `clawbot doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``clawbot_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Clawbot to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # Also overridable via CLAWBOT_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `clawbot -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # falls back to the default profile (the one `clawbot` launches with
        ...
        # decomposition is manual via `clawbot kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     clawbot-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.clawbot/logs/.
    ...
    # curated model lists for OpenRouter and Soam Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a clawbot-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://clawbot-agent.aayushsoam.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `clawbot model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.clawbot/state.db.
    ...
    "sessions": {
        ...
        # default of ``clawbot sessions prune``.
        "retention_days": 90,
    ...
    # ``clawbot update`` behaviour.
    "updates": {
        # Run a full ``clawbot backup``-style zip of CLAWBOT_HOME before every
        # ``clawbot update``.  Backups land in ``<CLAWBOT_HOME>/backups/`` and
        # can be restored with ``clawbot import <path>``.  Off by default —
        # on large CLAWBOT_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<CLAWBOT_HOME>/lsp/bin/`` on first use.
    ...
    # `clawbot tools`. These settings tune the backing Responses API call.
    "x_search": {
...
# selection step (Soam Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "SOAM_BASE_URL": {
        "description": "Soam Portal base URL override",
        "prompt": "Soam Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'clawbot model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for Soam Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Soam Subscribers only, used to derive vendor hosts, e.g. aayushsoam.com -> firecrawl-gateway.aayushsoam.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for Soam Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Soam Subscriber access token for tool-gateway requests (optional; otherwise read from the Clawbot auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @clawbot:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. CLAWBOT_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #clawbot)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: clawbot-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'clawbot-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Clawbot API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Clawbot API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Clawbot API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # CLAWBOT_TOOL_PROGRESS and CLAWBOT_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "CLAWBOT_TOOL_PROGRESS": {
    ...
    "CLAWBOT_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.clawbot.config`` entries, then checks
    ...
    try:
        ...
        # should never break `clawbot update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Clawbot versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``clawbot_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'clawbot setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Clawbot won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'clawbot doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("CLAWBOT_HOME", "~/.clawbot")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CLAWBOT_TOOL_PROGRESS")
            old_mode = get_env_value("CLAWBOT_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CLAWBOT_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CLAWBOT_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CLAWBOT_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CLAWBOT_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$CLAWBOT_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_clawbot_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `clawbot plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `clawbot curator status` has no
    ...
    #   3. Creates `~/.clawbot/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_clawbot_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_clawbot_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: clawbot config set <key> <value>")
    ...
    # metadata.clawbot.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _clawbot_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.clawbot/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.clawbot/config.yaml.
    ...
    (which change ``CLAWBOT_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_clawbot_home()
...
#   openai-codex (OAuth — clawbot auth) — OpenAI Codex
#   soam         (OAuth — clawbot auth) — Soam Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.clawbot/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.clawbot/.env.
    ...
    interactive menu render (`clawbot tools`, `clawbot setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `clawbot tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Clawbot env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.clawbot/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.clawbot/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.clawbot/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.clawbot/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Clawbot — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Clawbot vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.clawbot/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚡Clawbot Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  clawbot config edit     # Edit config file", Colors.DIM))
    print(color("  clawbot config set <key> <value>", Colors.DIM))
    print(color("  clawbot setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: clawbot config set <key> <value>")
            print()
            print("Examples:")
            print("  clawbot config set model anthropic/claude-sonnet-4")
            print("  clawbot config set terminal.backend docker")
            print("  clawbot config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'clawbot config migrate' to add them")
        ...
        print("  clawbot config           Show current configuration")
        print("  clawbot config edit      Open config in editor")
        print("  clawbot config set <key> <value>   Set a config value")
        print("  clawbot config check     Check for missing/outdated config")
        print("  clawbot config migrate   Update config with new options")
        print("  clawbot config path      Show config file path")
        print("  clawbot config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``clawbot config`` UI so users
