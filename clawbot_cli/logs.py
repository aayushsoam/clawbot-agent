"""``clawbot logs`` — view and filter Clawbot log files.
...
Usage examples::
    ...
    clawbot logs                    # last 50 lines of agent.log
    clawbot logs -f                 # follow agent.log in real time
    clawbot logs errors             # last 50 lines of errors.log
    clawbot logs gateway -n 100    # last 100 lines of gateway.log
    clawbot logs --level WARNING    # only WARNING+ lines
    clawbot logs --session abc123   # filter by session ID substring
    clawbot logs --component tools  # only tool-related lines
    clawbot logs --since 1h         # lines from the last hour
    clawbot logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_clawbot_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Clawbot runs — try 'clawbot chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_clawbot_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_clawbot_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_clawbot_home()}/logs/")
        return

    print(f"Log files in {display_clawbot_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'clawbot chat' to generate logs)")
