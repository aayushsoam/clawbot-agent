...
Add, remove, or reorder entries here — both `clawbot setup` and
`clawbot` provider-selection will pick up the change automatically.
"""
...
_CLAWBOT_USER_AGENT = f"clawbot-cli/{_CLAWBOT_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `clawbot model`
    flow without maintaining a separate static list.
    """
    from clawbot_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $CLAWBOT_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $CLAWBOT_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Clawbot refreshes the cache from
...
# ---------------------------------------------------------------------------
# Soam Portal free-model helper
# ---------------------------------------------------------------------------
# The Soam Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Soam Portal account tier detection
# ---------------------------------------------------------------------------

def fetch_soam_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    """Fetch the user's Soam Portal account/subscription info.
    ...
    """
    base = (portal_base_url or "https://portal.aayushsoam.com").rstrip("/")
...
def is_soam_free_tier(account_info: dict[str, Any]) -> bool:
...
def partition_soam_models_by_tier(
    model_ids: list[str],
    pricing: dict[str, dict[str, str]],
    free_tier: bool,
) -> tuple[list[str], list[str]]:
    """Split Soam models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    The Portal's ``/api/soam/recommended-models`` endpoint advertises which
    models are free *right now* — independent of what the in-repo
    ``_PROVIDER_MODELS["soam"]`` list happens to contain or whether the
    ...
    older Clawbot that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    users. The Portal's ``/api/soam/recommended-models`` endpoint advertises
    which paid models are blessed *right now* — independent of what the
    in-repo ``_PROVIDER_MODELS["soam"]`` list happens to contain or whether
    ...
    picker even if the user is running an older Clawbot that doesn't ship
    ...
    * ``pricing`` is left untouched — we deliberately do NOT synthesize
      ...
      :func:`partition_soam_models_by_tier` keeps free models selectable;
...
def check_soam_free_tier() -> bool:
    """Check if the current Soam Portal user is on a free (unpaid) tier.
    ...
    try:
        from clawbot_cli.auth import get_provider_auth_state, resolve_soam_runtime_credentials

        # Ensure we have a fresh token (triggers refresh if needed)
        resolve_soam_runtime_credentials(min_key_ttl_seconds=60)
        ...
        account_info = fetch_soam_account_tier(access_token, portal_url)
        result = is_soam_free_tier(account_info)
...
# ---------------------------------------------------------------------------
# Soam Portal recommended models
#
...
SOAM_RECOMMENDED_MODELS_PATH = "/api/soam/recommended-models"
_SOAM_RECOMMENDED_CACHE_TTL: int = 600  # seconds (10 minutes)
# (result_dict, timestamp) keyed by portal_base_url so staging vs prod don't collide.
_soam_recommended_cache: dict[str, tuple[dict[str, Any], float]] = {}


def fetch_soam_recommended_models(
    portal_base_url: str = "",
    timeout: float = 5.0,
    *,
    force_refresh: bool = False,
) -> dict[str, Any]:
    """Fetch the Soam Portal's curated recommended-models payload.

    Hits ``<portal>/api/soam/recommended-models``. The endpoint is public —
    no auth is required. Results are cached per portal URL for
    ``_SOAM_RECOMMENDED_CACHE_TTL`` seconds; pass ``force_refresh=True`` to
    bypass the cache.
    ...
    base = (portal_base_url or "https://portal.aayushsoam.com").rstrip("/")
    now = time.monotonic()
    cached = _soam_recommended_cache.get(base)
    if not force_refresh and cached is not None:
        payload, cached_at = cached
        if now - cached_at < _SOAM_RECOMMENDED_CACHE_TTL:
            return payload

    url = f"{base}{SOAM_RECOMMENDED_MODELS_PATH}"
    try:
    ...
    _soam_recommended_cache[base] = (data, now)
    return data


def _resolve_soam_portal_url() -> str:
    """Best-effort lookup of the Portal base URL the user is authed against."""
    try:
        from clawbot_cli.auth import (
            DEFAULT_SOAM_PORTAL_URL,
            get_provider_auth_state,
        )
        state = get_provider_auth_state("soam") or {}
        portal = str(state.get("portal_base_url") or "").strip()
        if portal:
            return portal.rstrip("/")
        return str(DEFAULT_SOAM_PORTAL_URL).rstrip("/")
    except Exception:
        return "https://portal.aayushsoam.com"
...
def get_soam_recommended_aux_model(
    *,
    vision: bool = False,
    ...
    * ``vision=False`` → ``paidRecommendedCompactionModel`` or
    ...
    via :func:`check_soam_free_tier`. Pass an explicit bool to bypass the
    ...
    ``google/gemini-3-flash-preview``).
    """
    base = portal_base_url or _resolve_soam_portal_url()
    payload = fetch_soam_recommended_models(base, force_refresh=force_refresh)
    if not payload:
        return None
...
# this list:  clawbot model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `clawbot model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `clawbot model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("groq",           "Groq",                     "Groq (fast OpenAI-compatible API via GROQ_API_KEY)"),
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``clawbot model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``clawbot auth add openai-codex`` without
    ``clawbot model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    clawbot-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Soam Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from clawbot_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — clawbot-agent
...
def get_curated_soam_model_ids() -> list[str]:
    """Return the curated Soam Portal model-id list.

    Prefers the remotely-hosted catalog manifest (published under
    ``website/static/api/model-catalog.json``); falls back to the in-repo
    snapshot in ``_PROVIDER_MODELS["soam"]`` when the manifest is
    unreachable. Always returns a list (never None).
    """
    try:
        from clawbot_cli.model_catalog import get_curated_soam_models
        remote = get_curated_soam_models()
    except Exception:
        remote = None
    if remote:
        return list(remote)
    return list(_PROVIDER_MODELS.get("soam", []))
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from clawbot_constants import AI_GATEWAY_BASE_URL
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Soam Portal).
    """
    ...
    headers: dict[str, str] = {
        "Accept": "application/json",
        "User-Agent": _CLAWBOT_USER_AGENT,
    }
    if api_key:
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return clawbot-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; clawbot's picker expects
...
_DEFAULT_SOAM_INFERENCE_BASE = "https://inference-api.aayushsoam.com"


def _resolve_soam_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Soam Portal pricing.

    The Soam inference ``/v1/models`` endpoint exposes pricing without
    ...
    """
    try:
        from clawbot_cli.auth import resolve_soam_runtime_credentials
        creds = resolve_soam_runtime_credentials()
        if creds:
            return (creds.get("api_key", ""), creds.get("base_url", ""))
    except Exception:
        pass
    return ("", _DEFAULT_SOAM_INFERENCE_BASE)


def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    """Return live pricing for providers that support it (openrouter, soam, ai-gateway, novita)."""
    ...
    if normalized == "soam":
        api_key, base_url = _resolve_soam_pricing_credentials()
        if base_url:
            # Soam base_url typically looks like https://inference-api.aayushsoam.com/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``clawbot model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from clawbot_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Codex, Soam, etc. all support /models)
...
_AGGREGATOR_PROVIDERS = frozenset(
    {"soam", "openrouter", "ai-gateway", "copilot", "kilocode"}
)
...
def _resolve_static_model_alias(
    ...
    try:
        from clawbot_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_static_provider_for_model(
    ...
    # If someone types `/model soam` or `/model anthropic`, treat it as a
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Clawbot' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``clawbot_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Clawbot should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``clawbot auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.clawbot/.env``.
    ...
    """
    try:
        from clawbot_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from clawbot_cli.auth import read_credential_pool
        from clawbot_cli.copilot_auth import (
...
#   - "soam": curated list and Portal /models endpoint are the source of
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Codex, Soam),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Clawbot release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Clawbot release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "soam":
        # Try live Soam Portal /models endpoint
        try:
            from clawbot_cli.auth import fetch_soam_models, resolve_soam_runtime_credentials
            creds = resolve_soam_runtime_credentials()
            if creds:
                live = fetch_soam_models(api_key=creds.get("api_key", ""), inference_base_url=creds.get("base_url", ""))
                if live:
                    return live
        except Exception:
            pass
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _CLAWBOT_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Clawbot' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _CLAWBOT_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_clawbot_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Clawbot will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Clawbot cannot verify the model name."
