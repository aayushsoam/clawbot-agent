"""
Clawbot MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Clawbot-specific extra)

Usage:
    clawbot mcp serve
    clawbot mcp serve --verbose
...
logger = logging.getLogger("clawbot.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using CLAWBOT_HOME."""
    try:
        from clawbot_constants import get_clawbot_home
        return get_clawbot_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("CLAWBOT_HOME", Path.home() / ".clawbot")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from clawbot_constants import get_clawbot_home
        directory_file = get_clawbot_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("CLAWBOT_HOME", Path.home() / ".clawbot")
...
class EventBridge:
    ...
    This is the Clawbot equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from clawbot_constants import get_clawbot_home
            db_file = get_clawbot_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("CLAWBOT_HOME", Path.home() / ".clawbot")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Clawbot MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "clawbot",
        instructions=(
            "Clawbot Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Clawbot MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
