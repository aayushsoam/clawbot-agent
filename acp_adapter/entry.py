"""CLI entry point for the clawbot-agent ACP adapter.

Loads environment variables from ``~/.clawbot/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from CLAWBOT_HOME (default ``~/.clawbot``)."""
    from clawbot_cli.env_loader import load_clawbot_dotenv

    clawbot_home = get_clawbot_home()
    loaded = load_clawbot_dotenv(clawbot_home=clawbot_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", clawbot_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="clawbot-acp",
        description="Run Clawbot Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Clawbot version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
...
def _print_version() -> None:
    ...
    print(clawbot_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import ClawbotACPAgent  # noqa: F401

    print("Clawbot ACP check OK")


def _run_setup() -> None:
    from clawbot_cli.main import main as clawbot_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "clawbot", "model"]
        clawbot_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``clawbot postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting clawbot-agent ACP adapter")
    ...
    import acp
    from .server import ClawbotACPAgent
