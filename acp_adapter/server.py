"""ACP agent server — exposes Clawbot Agent via the Agent Client Protocol."""
...
try:
    from clawbot_cli import __version__ as CLAWBOT_VERSION
except Exception:
    CLAWBOT_VERSION = "0.0.0"

# Thread pool for running AIAgent (synchrosoam) in parallel.
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Clawbot running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Clawbot cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Clawbot/OpenAI-compatible user content payload."""
...
class ClawbotACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Clawbot AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Clawbot version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Clawbot maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from clawbot_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from clawbot_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Clawbot estimates ``used`` from the same
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["clawbot-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="clawbot-agent", version=CLAWBOT_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Clawbot' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Clawbot setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Clawbot
    ...
    async def load_session(
        ...
        **kwargs: Any,
        ...
        # spec-compliant ACP client that measures notifications synchrosoamly
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Clawbot on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Clawbot' local kawaii waiting/status
        ...
        # not the executor's. Also set CLAWBOT_INTERACTIVE so approval.py
        # takes the CLI-interactive path (which calls the registered
        # callback via prompt_dangerous_approval) instead of the
        # non-interactive auto-approve branch (GHSA-96vc-wcxf-jjff).
        # ACP's conn.request_permission maps cleanly to the interactive
        # callback shape — not the gateway-queue CLAWBOT_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind CLAWBOT_SESSION_KEY for this session so per-session caches
            ...
            previous_interactive = os.environ.get("CLAWBOT_INTERACTIVE")
            ...
            try:
                ...
                # Restore CLAWBOT_INTERACTIVE.
                if previous_interactive is None:
                    os.environ.pop("CLAWBOT_INTERACTIVE", None)
                else:
                    os.environ["CLAWBOT_INTERACTIVE"] = previous_interactive
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            ...
            # stomp on each other's ContextVar writes (CLAWBOT_SESSION_KEY in
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Clawbot Agent v{CLAWBOT_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Clawbot has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
