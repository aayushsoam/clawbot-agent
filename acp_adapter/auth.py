"""ACP auth helpers — detect and advertise Clawbot authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "clawbot-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Clawbot runtime provider, or None if unavailable."""
    try:
...
def has_provider() -> bool:
    """Return True if Clawbot can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Clawbot.
    ...
    not have Clawbot provider credentials configured yet, so Clawbot always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Clawbot using the currently configured "
