...
upstream prefix cache warm.  See ``clawbot-agent-dev``'s
...
* ``stable``   — identity (SOUL.md or DEFAULT_AGENT_IDENTITY), tool
  guidance, computer-use guidance, soam subscription block, tool-use
...
def _ra():
    ...
    ``build_context_files_prompt``, ``build_soam_subscription_prompt``,
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Clawbot never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want CLAWBOT_HOME persona while keeping
    ...
    # Pointer to the clawbot-agent skill + docs for user questions about Clawbot itself.
    stable_parts.append(CLAWBOT_AGENT_HELP_GUIDANCE)
    ...
    # CLAWBOT_KANBAN_TASK env var). Normal chat sessions never see
    # this block.
    ...
    soam_subscription_prompt = _r.build_soam_subscription_prompt(agent.valid_tool_names)
    if soam_subscription_prompt:
        stable_parts.append(soam_subscription_prompt)
    ...
    if not agent.skip_context_files:
        ...
        # mode).  The gateway process runs from the clawbot-agent install
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Clawbot never rebuilds or reinjects parts of it
