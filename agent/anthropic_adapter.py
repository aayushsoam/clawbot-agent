"""Anthropic Messages API adapter for Clawbot Agent.

Translates between Clawbot's internal OpenAI-style message format and
...
# Clawbot effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Clawbot historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Clawbot)
    ...
    # 1. Clawbot-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Clawbot configs.
...
# ── Clawbot-native PKCE OAuth flow ────────────────────────────────────────
...
_CLAWBOT_OAUTH_FILE = get_clawbot_home() / ".anthropic_oauth.json"
...
def run_clawbot_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Clawbot-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Clawbot with your Claude Pro/Max subscription.")
    print()
...
def read_clawbot_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Clawbot-managed OAuth credentials from ~/.clawbot/.anthropic_oauth.json."""
    if _CLAWBOT_OAUTH_FILE.exists():
        try:
            data = json.loads(_CLAWBOT_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Clawbot OAuth credentials: %s", e)
    return None
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    for m in messages:
        ...
        if role == "assistant":
            ...
            # can validate the message history.  See clawbot-agent#13848.
            #
    ...
    # are preserved.  See clawbot-agent#13848 (Kimi) and #16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Clawbot Agent", "Claude Code")
                text = text.replace("Clawbot agent", "Claude Code")
                text = text.replace("clawbot-agent", "claude-code")
                text = text.replace("Aayush soam", "Anthropic")
                block["text"] = text
    ...
    # silently hides reasoning text that Clawbot surfaces in its CLI. We
