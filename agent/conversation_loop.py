...
from agent.soam_rate_guard import (
    clear_soam_rate_limit,
    is_genuine_soam_rate_limit,
    soam_rate_limit_remaining,
    record_soam_rate_limit,
)
...
from clawbot_logging import set_session_context
...
def run_conversation(
    ...
    # ``clawbot logs --session <id>`` can filter a single conversation.
    from clawbot_logging import set_session_context
    ...
    # are reused.  The system prompt is Clawbot's territory; plugins
    ...
    # all run inside Codex). Default Clawbot path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Clawbot internals.
        #
        # Clawbot invariant: the system prompt is built ONCE per session
        ...
        soam_auth_retry_attempted=False
        ...
        while retry_count < max_retries:
            # ── Soam Portal rate limit guard ──────────────────────
            # If another session already recorded that Soam is rate-
            # limited, skip the API call entirely.  Each attempt
            # (including SDK-level retries) counts against RPH and
            # deepens the rate limit hole.
            if agent.provider == "soam":
                try:
                    from agent.soam_rate_guard import (
                        soam_rate_limit_remaining,
                        format_remaining as _fmt_soam_remaining,
                    )
                    _soam_remaining = soam_rate_limit_remaining()
                    if _soam_remaining is not None and _soam_remaining > 0:
                        _soam_msg = (
                            f"Soam Portal rate limit active — "
                            f"resets in {_fmt_soam_remaining(_soam_remaining)}."
                        )
                        agent._vprint(
                            f"{agent.log_prefix}⏳ {_soam_msg} Trying fallback...",
                            force=True,
                        )
                        agent._emit_status(f"⏳ {_soam_msg}")
            ...
            try:
                ...
                if env_var_enabled("CLAWBOT_DUMP_REQUESTS"):
                ...
                # Clear Soam rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting Soam.
                if agent.provider == "soam":
                    try:
                        from agent.soam_rate_guard import clear_soam_rate_limit
                        clear_soam_rate_limit()
                    except Exception:
                        pass
                ...
                if (
                    ...
                    and agent.provider == "soam"
                    and status_code == 401
                    and not soam_auth_retry_attempted
                ):
                    soam_auth_retry_attempted = True
                    if agent._try_refresh_soam_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 Soam agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 Soam 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: clawbot login --provider soam")
                    print(f"{agent.log_prefix}     • Check credits / billing: https://portal.aayushsoam.com")
                ...
                if (
                    ...
                    from clawbot_constants import display_clawbot_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Clawbot-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: clawbot config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: clawbot config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Soam Portal: record rate limit & skip retries ─────
                # When Soam returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Soam Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, MiMo, Clawbot).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every Soam model for minutes.  We use
                # ``is_genuine_soam_rate_limit`` to tell the two
                ...
                if (
                    ...
                    _genuine_soam_rate_limit = False
                    try:
                        from agent.soam_rate_guard import (
                            is_genuine_soam_rate_limit,
                            record_soam_rate_limit,
                        ...
                        _genuine_soam_rate_limit = is_genuine_soam_rate_limit(
                            headers=_err_hdrs,
                            last_known_state=agent._rate_limit_state,
                        )
                        if _genuine_soam_rate_limit:
                            record_soam_rate_limit(
                                headers=_err_hdrs,
                                error_context=error_context,
                            )
                            ...
                            logging.info(
                                "Soam 429 looks like upstream capacity "
                ...
                # which Clawbot' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Clawbot' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`clawbot",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if _provider in {"openai-codex", "xai-oauth"} and status_code == 401:
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `clawbot auth` to re-authenticate.", force=True)
                            else:
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok Subscription) from `clawbot model`.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: clawbot setup", force=True)
    ...
    if final_response is None and (
        ...
        _kanban_task = os.environ.get("CLAWBOT_KANBAN_TASK")
        if _kanban_task:
            try:
