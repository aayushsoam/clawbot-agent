...
``kind: backend``) or ``~/.clawbot/plugins/browser/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class BrowserProvider(abc.ABC):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        Used by :mod:`clawbot_cli.tools_config` to inject this provider as a
        ...
        expose API key prompts, badges, managed-Soam gating, and the
