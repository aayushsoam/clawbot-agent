...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Clawbot starts and finds ``~/.openclaw/``.

    Points users at ``clawbot claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``clawbot claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Clawbot, run "
        "`clawbot claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `clawbot claw cleanup` (renames it to ~/.openclaw.pre-migration — "
