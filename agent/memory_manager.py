...
class MemoryManager:
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``clawbot_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_clawbot_home()`` themselves.
        """
        if "clawbot_home" not in kwargs:
            from clawbot_constants import get_clawbot_home
            kwargs["clawbot_home"] = str(get_clawbot_home())
        for provider in self._providers:
            try:
