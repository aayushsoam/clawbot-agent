...
from clawbot_constants import get_clawbot_home, get_skills_dir, is_wsl
...
_CLAWBOT_MD_NAMES = (".clawbot.md", "CLAWBOT.md")


def _find_clawbot_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.clawbot.md`` or ``CLAWBOT.md``.
...
DEFAULT_AGENT_IDENTITY = (
    "You are Clawbot Agent, an intelligent AI assistant created by Aayush soam. "
...
CLAWBOT_AGENT_HELP_GUIDANCE = (
    "If the user asks about configuring, setting up, or using Clawbot Agent "
    "itself, load the `clawbot-agent` skill with skill_view(name='clawbot-agent') "
    "before answering. Docs: https://clawbot-agent.aayushsoam.com/docs"
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.clawbot/kanban.db`. Your task id is in "
    "`$CLAWBOT_KANBAN_TASK`; your workspace is `$CLAWBOT_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $CLAWBOT_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `clawbot kanban <verb>` for board operations. Use "
...
PLATFORM_HINTS = {
    ...
    "webui": (
        "You are in the Clawbot WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Clawbot itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Clawbot runs on.
    """
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Clawbot itself is running. The host OS, home, and cwd "
                f"of the Clawbot process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Clawbot "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_clawbot_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.clawbot/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("CLAWBOT_PLATFORM")
        or get_session_env("CLAWBOT_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Clawbot Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `clawbot-agent` skill "
            "first. It has the actual commands (e.g. `clawbot config set …`, `clawbot tools`, "
            "`clawbot setup`) so you don't have to guess or invent workarounds.\n"
...
def build_soam_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    """Build a compact Soam subscription capability block for the system prompt."""
    try:
        ...
        logger.debug("Failed to import Soam subscription helper: %s", exc)
        return ""

    if not managed_soam_tools_enabled():
        return ""
    ...
    features = get_soam_subscription_features()

    def _status_line(feature) -> str:
        if feature.managed_by_soam:
            return f"- {feature.label}: active via Soam subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.soam_auth_present:
            return f"- {feature.label}: included with Soam subscription, not currently selected"
        if feature.key == "modal" and features.soam_auth_present:
            return f"- {feature.label}: optional via Soam subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# Soam Subscription",
        "Soam subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a Soam-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, or Browser-Use API keys.",
            "If the user is not subscribed and asks for a capability that Soam subscription would unlock or simplify, suggest Soam subscription as one option alongside direct setup or local alternatives.",
            "Do not mention subscription unless the user asks about it or it directly solves the current missing capability.",
            "Useful commands: clawbot setup, clawbot setup tools, clawbot setup terminal, clawbot status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from CLAWBOT_HOME and return its content, or None.
    ...
    """
    try:
        from clawbot_cli.config import ensure_clawbot_home
        ensure_clawbot_home()
    except Exception as e:
        logger.debug("Could not ensure CLAWBOT_HOME before loading SOUL.md: %s", e)

    soul_path = get_clawbot_home() / "SOUL.md"
...
def _load_clawbot_md(cwd_path: Path) -> str:
    """.clawbot.md / CLAWBOT.md — walk to git root."""
    clawbot_md_path = _find_clawbot_md(cwd_path)
    if not clawbot_md_path:
        return ""
    try:
        content = clawbot_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = clawbot_md_path.name
        try:
            rel = str(clawbot_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".clawbot.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", clawbot_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .clawbot.md / CLAWBOT.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from CLAWBOT_HOME is independent and always included when present.
    ...
    # SOUL.md from CLAWBOT_HOME only — skip when already loaded as identity
    if not skip_soul:
