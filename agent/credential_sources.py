"""Unified removal contract for every credential source Clawbot reads from.

Clawbot seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.clawbot/.env
    claude_code   — ~/.claude/.credentials.json
    clawbot_pkce   — ~/.clawbot/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (soam, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `clawbot auth add`
...
unify here is **removal**:
    ...
    ``clawbot auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, soam device_code (partial), clawbot_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"soam"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.clawbot/.env  → clear it
    ...
    """
    from clawbot_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.clawbot/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Clawbot.",
            f"  The pool entry is now suppressed — Clawbot will ignore "
            f"{env_var} until you run `clawbot auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Clawbot stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `clawbot auth add anthropic` to re-enable if needed.",
    ])


def _remove_clawbot_pkce(provider: str, removed) -> RemovalResult:
    """~/.clawbot/.anthropic_oauth.json is ours — delete it outright."""
    from clawbot_constants import get_clawbot_home

    result = RemovalResult()
    oauth_file = get_clawbot_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Clawbot Anthropic OAuth credentials")
...
def _remove_soam_device_code(provider: str, removed) -> RemovalResult:
    """Soam OAuth lives in auth.json providers.soam — clear it and suppress.

    We suppress in addition to clearing because nothing else stops the
    user's next `clawbot login` run from writing providers.soam again
    before they decide to.  Suppression forces them to go through
    `clawbot auth add soam` to re-engage, which is the documented re-add
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as Soam: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_loopback_pkce(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``clawbot auth remove xai-oauth <N>`` silently undoes
    ...
    Belt-and-braces against the manual entry path: ``clawbot auth add
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `clawbot model` → xAI Grok OAuth (SuperGrok Subscription) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Clawbot auth store is not enough — _seed_from_singletons() would
    ...
    via ``clawbot auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `clawbot auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `clawbot auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Clawbot stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `clawbot auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="clawbot_pkce",
        remove_fn=_remove_clawbot_pkce,
        description="~/.clawbot/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="soam", source_id="device_code",
        remove_fn=_remove_soam_device_code,
        description="auth.json providers.soam",
    ))
    register(RemovalStep(
