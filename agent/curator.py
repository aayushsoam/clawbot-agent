...
def _state_file() -> Path:
    return get_clawbot_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.clawbot/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``clawbot update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``clawbot curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `clawbot curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Clawbot' background skill CURATOR. This is an "
    ...
    "into ~/.clawbot/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: clawbot-config-*, clawbot-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.clawbot/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.clawbot/logs/curator/``)
    ...
    authored skill data in ``~/.clawbot/skills/``.

    ``ensure_clawbot_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_clawbot_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every clawbot install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: clawbot curator status
        keep an umbrella stable: clawbot curator pin document-tools
    ...
    lines.append("full report: clawbot curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: clawbot curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.clawbot/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.clawbot/skills/.archive/` for "
            "safety and can be restored via `clawbot curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.clawbot/skills/.archive/`. "
            "Restore any via `clawbot curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `clawbot curator restore <name>`")
    lines.append("- All archives live under `~/.clawbot/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    If *synchrosoam* is True, the LLM review runs in the calling thread; the
    ...
    # real pass out. We still record a summary so `clawbot curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `clawbot curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``clawbot model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `clawbot model` → auxiliary
    ...
    try:
        ...
        # run_curator_review(synchrosoam=True) from the CLI.
