...
from clawbot_cli.config import get_env_value, load_env
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "soam":
            # Soam stores the runtime inference credential in agent_key for
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``clawbot model`` / ``clawbot auth`` which
        ...
        Mirrors the Soam/Anthropic resync paths above.  Only applies to
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Clawbot process
    ...
    def _sync_soam_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Soam pool entry from auth.json if tokens differ.

        Soam OAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_soam_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the Soam Portal.
        """
        if self.provider != "soam" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing Soam state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync Soam entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Soam, OpenAI Codex, and xAI Grok OAuth).
        ...
        here would mean every Soam/Codex/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``clawbot`` invocation that defaults to the active provider
        (e.g. setup wizard, ``clawbot auth status``) would land on
        ...
        try:
            with _auth_store_lock():
                ...
                if self.provider == "soam":
                    ...
                    _store_provider_state(auth_store, "soam", state, set_active=False)
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("clawbot_pkce"),
                )
                updated = replace(
                    entry,
            ...
            elif self.provider == "soam":
                synced = self._sync_soam_entry_from_auth_store(entry)
                if synced is not entry:
                    entry = synced
                auth_mod.resolve_soam_runtime_credentials(
                    min_key_ttl_seconds=DEFAULT_AGENT_KEY_MIN_TTL_SECONDS,
                    inference_auth_mode=(
                        auth_mod.SOAM_INFERENCE_AUTH_MODE_LEGACY
                        if force
                        else auth_mod.SOAM_INFERENCE_AUTH_MODE_AUTO
                    ),
                )
                updated = self._sync_soam_entry_from_auth_store(entry)
            else:
                return entry
        except Exception as exc:
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("clawbot_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For xai-oauth: same race as soam — another process may have
            ...
            if self.provider == "xai-oauth":
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    self._persist()
                    return updated
            # For soam: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "soam":
                synced = self._sync_soam_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Soam refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                    ...
                    return updated
                if auth_mod._is_terminal_soam_refresh_error(exc):
                    logger.debug("Soam refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                            state = _load_provider_state(auth_store, "soam") or {
                            ...
                            if not store_refresh or store_refresh == entry_refresh:
                                auth_mod._quarantine_soam_oauth_state(
                                    state,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                                auth_mod._quarantine_soam_pool_entries(
                                    auth_store,
                                    exc,
                                    reason="credential_pool_refresh_failure",
                                )
                        ...
                        logger.debug("Failed to clear terminal Soam OAuth state: %s", clear_exc)

                    singleton_sources = {
                        auth_mod.SOAM_DEVICE_CODE_SOURCE,
                        f"manual:{auth_mod.SOAM_DEVICE_CODE_SOURCE}",
                    }
                    self._entries = [
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "soam":
            # Soam refresh/mint can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Clawbot profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For soam entries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_soam_runtime_credentials(), making this entry's
            ...
            # re-authed via `clawbot model` / `clawbot auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `clawbot model` ->
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `clawbot auth remove <provider> <N>` is stable across all source types.
    try:
        from clawbot_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Clawbot PKCE)
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_clawbot_oauth_credentials

        for source_name, creds in (
            ("clawbot_pkce", read_clawbot_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_soam_credentials(label=...) when the user ran
            # `clawbot auth add soam --label <name>`).  Fall back to the
        ...
        # Clawbot auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from clawbot_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.clawbot/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `clawbot auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `clawbot auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Clawbot auth store.  Without this gate the removal is instantly
        ...
        # Clawbot owns its own Codex auth state — we do NOT auto-import from
        ...
        # When the user logs in via ``clawbot model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``clawbot auth list`` reflects the logged-in state and so the pool
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.clawbot/.env over os.environ — the user's config file is the
    # authoritative source for Clawbot credentials. Stale env vars from parent
    ...
    # Honour user suppression — `clawbot auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.clawbot/.env.
    ...
    try:
        from clawbot_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.clawbot/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "clawbot_pkce"}
        )
    ]
    if len(retained) == len(entries):
        return False
