...
as ``kind: backend``) or ``~/.clawbot/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``clawbot tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``clawbot tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``clawbot tools`` picker."""
        return {
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``clawbot tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$CLAWBOT_HOME/cache/videos/``, creating parents as needed."""
    from clawbot_constants import get_clawbot_home

    path = get_clawbot_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$CLAWBOT_HOME/cache/videos/``.
