...
appropriate.  All installs go to a Clawbot-owned bin staging dir,
``<CLAWBOT_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``clawbot lsp
  status``.
- ``off`` — same as ``manual`` for now (kept distinct so we can
  evolve behavior later, e.g. logging differently).

The actual installs happen synchrosoamly the first time a server is
...
# ``<CLAWBOT_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def clawbot_lsp_bin_dir() -> Path:
    """Return the Clawbot-owned bin staging dir for LSP servers."""
    home = os.environ.get("CLAWBOT_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".clawbot")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = clawbot_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = clawbot_lsp_bin_dir().parent  # <CLAWBOT_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = clawbot_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a clawbot-owned target dir.
    ...
    console script.
    """
    pip_target = clawbot_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = clawbot_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``clawbot lsp status`` CLI to give users a quick
