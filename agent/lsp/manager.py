...
The :class:`LSPService` is the bridge between the synchrosoam
...
class _BackgroundLoop:
    ...
    Provides :meth:`run` for synchrosoam callers — submits a coroutine
...
class LSPService:
    ...
    def create_from_config(cls) -> Optional["LSPService"]:
        """Build a service from ``clawbot_cli.config`` settings.
    ...
    def enabled_for(self, file_path: str) -> bool:
        ...
        timeout cost — until the service is restarted (``clawbot lsp
    ...
    # ------------------------------------------------------------------
    # status / introspection (used by ``clawbot lsp status``)
    # ------------------------------------------------------------------
