...
Owns one Codex thread per Clawbot session. Drives `turn/start`, consumes
...
call is synchrosoam and behaves like AIAgent's existing chat_completions loop.
"""
...
# Clawbot' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_CLAWBOT_TO_CODEX_PERMISSION_PROFILE = {
...
class CodexAppServerSession:
    """One Codex thread per Clawbot session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _CLAWBOT_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("CLAWBOT_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="clawbot",
            client_title="Clawbot Agent",
            client_version=_get_clawbot_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Clawbot' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Clawbot' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own clawbot-tools callback we
            # auto-accept — the user already approved Clawbot' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Clawbot approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Clawbot returns 'once', 'session', 'always', or 'deny'.
...
def _get_clawbot_version() -> str:
    """Best-effort Clawbot version string for codex's userAgent line."""
    try:
        from importlib.metadata import version

        return version("clawbot-agent")
