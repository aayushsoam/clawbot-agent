...
projection into Clawbot' display, approval bridging, transcript projection into
...
"codex_app_server"`. Clawbot' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    Threading model:
      - Spawning thread (caller) drives request/response pairs synchrosoamly.
    ...
    Intentionally NOT async. AIAgent.run_conversation() is synchrosoam and
    ...
    def __init__(
        ...
        if spawn_env.get("CLAWBOT_KANBAN_TASK"):
            kanban_db = spawn_env.get("CLAWBOT_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "CLAWBOT_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("CLAWBOT_HOME", os.path.expanduser("~/.clawbot")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "clawbot",
        client_title: str = "Clawbot Agent",
