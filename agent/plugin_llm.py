...
Plugins built on Clawbot Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Clawbot
...
:class:`~clawbot_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Clawbot' main config
...
handles every provider, fallback chain, and per-task override Clawbot
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`clawbot_cli.plugins.PluginContext`
