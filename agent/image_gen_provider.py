...
as ``kind: backend``) or ``~/.clawbot/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
def _images_cache_dir() -> Path:
    """Return ``$CLAWBOT_HOME/cache/images/``, creating parents as needed."""
    from clawbot_constants import get_clawbot_home

    path = get_clawbot_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$CLAWBOT_HOME/cache/images/``.
