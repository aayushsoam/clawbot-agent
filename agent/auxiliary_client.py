...
Resolution order for text tasks (auto mode):
  ...
  3. Soam Portal (~/.clawbot/auth.json active provider)
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before Clawbot triggers summarization.  Higher values delay
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://clawbot-agent.aayushsoam.com",
    "X-Title": "Clawbot Agent",
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``CLAWBOT_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``CLAWBOT_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("CLAWBOT_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("CLAWBOT_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "ClawbotAgent",
}
...
from clawbot_cli import __version__ as _CLAWBOT_VERSION

_AI_GATEWAY_HEADERS = {
    "HTTP-Referer": "https://clawbot-agent.aayushsoam.com",
    "X-Title": "Clawbot Agent",
    "User-Agent": f"ClawbotAgent/{_CLAWBOT_VERSION}",
}

# Soam Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Soam Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with clawbot_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import soam_portal_tags as _soam_portal_tags


def _soam_extra_body() -> dict:
    """Return a fresh Soam Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``clawbot_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _soam_portal_tags()}


# Backwards-compatible module attribute. Some callers (tests, third-party
# plugins) read ``SOAM_EXTRA_BODY`` directly; keep it as a snapshot of the
# current tags. Callers that need the freshest value should call
# ``_soam_extra_body()`` or import ``soam_portal_tags`` directly.
SOAM_EXTRA_BODY = _soam_extra_body()

# Set at resolve time — True if the auxiliary client points to Soam Portal
auxiliary_is_soam: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_SOAM_MODEL = "google/gemini-3-flash-preview"
_SOAM_DEFAULT_BASE_URL = "https://inference-api.aayushsoam.com/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_clawbot_home() / "auth.json"
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Clawbot Agent)",
...
def _pool_runtime_api_key(entry: Any) -> str:
    ...
    # provider-specific fallback (e.g. agent_key for soam).
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    # runtime_base_url handles provider-specific logic (e.g. soam prefers inference_base_url).
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``clawbot_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_soam_auth() -> Optional[dict]:
    """Read and validate ~/.clawbot/auth.json for an active Soam provider.

    Returns the provider state dict if Soam is active with tokens,
    otherwise None.
    """
    pool_present, entry = _select_pool_entry("soam")
    if pool_present:
        if entry is None:
            return None
        return {
            ...
            "inference_base_url": _pool_runtime_base_url(entry, _SOAM_DEFAULT_BASE_URL),
    ...
    try:
        ...
        if data.get("active_provider") != "soam":
            return None
        provider = data.get("providers", {}).get("soam", {})
        # Must have at least an access_token or agent_key
        if not provider.get("agent_key") and not provider.get("access_token"):
            return None
        return provider
    except Exception as exc:
        logger.debug("Could not read Soam auth: %s", exc)
        return None


def _soam_api_key(provider: dict) -> str:
    """Extract the Soam runtime credential from the compatibility field."""
    return provider.get("agent_key") or provider.get("access_token", "")


def _soam_base_url() -> str:
    """Resolve the Soam inference base URL from env or default."""
    return os.getenv("SOAM_INFERENCE_BASE_URL", _SOAM_DEFAULT_BASE_URL)


def _resolve_soam_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh Soam runtime credentials when available.
    ...
    try:
        ...
        creds = resolve_soam_runtime_credentials(
            min_key_ttl_seconds=max(60, int(os.getenv("CLAWBOT_SOAM_MIN_KEY_TTL_SECONDS", "1800"))),
            timeout_seconds=float(os.getenv("CLAWBOT_SOAM_TIMEOUT_SECONDS", "15")),
            inference_auth_mode=(
                SOAM_INFERENCE_AUTH_MODE_LEGACY
                if force_refresh
                else SOAM_INFERENCE_AUTH_MODE_AUTO
            ),
        )
    except Exception as exc:
        logger.debug("Auxiliary Soam runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``clawbot auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``clawbot_cli.auth``'s singleton runtime resolver for older
    auth-store-only logins. Returns ``None`` if the user is not authenticated
    with xAI Grok OAuth.
    """
    try:
        from clawbot_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Clawbot auth store.
    ...
    try:
        from clawbot_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from clawbot_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from clawbot_cli.models import copilot_default_headers
...
def _try_soam(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting Soam —
    # if another session already recorded a 429, skip Soam entirely
    # to avoid piling more requests onto the tapped RPH bucket.
    try:
        from agent.soam_rate_guard import soam_rate_limit_remaining
        _remaining = soam_rate_limit_remaining()
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Soam Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
            _mark_provider_unhealthy("soam", ttl=_remaining)
            return None, None
    except Exception:
        pass

    soam = _read_soam_auth()
    runtime = _resolve_soam_runtime_api(force_refresh=False)
    if runtime is None and not soam:
        logger.warning(
            "Auxiliary Soam client unavailable: no Soam authentication found "
            "(run: clawbot auth)."
        )
        _mark_provider_unhealthy("soam", ttl=60)
        return None, None
    if runtime is None and soam:
        # Runtime credential mint failed but stored Soam auth is still present.
        ...
        logger.debug(
            "Auxiliary Soam: runtime credential mint failed; falling back to "
            "stored auth.json token."
        )
    global auxiliary_is_soam
    auxiliary_is_soam = True
    logger.debug("Auxiliary client: Soam Portal")

    # Ask the Portal which model it currently recommends for this task type.
    # The /api/soam/recommended-models endpoint is the authoritative source:
    # it distinguishes paid vs free tier recommendations, and get_soam_recommended_aux_model
    # auto-detects the caller's tier via check_soam_free_tier().  Fall back to
    # _SOAM_MODEL (google/gemini-3-flash-preview) when the Portal is unreachable
    # or returns a null recommendation for this task type.
    model = _SOAM_MODEL
    try:
        from clawbot_cli.models import get_soam_recommended_aux_model
        recommended = get_soam_recommended_aux_model(vision=vision)
        if recommended:
            model = recommended
            logger.debug(
    ...
    if runtime is not None:
        ...
        api_key = _soam_api_key(soam or {})
        base_url = str((soam or {}).get("inference_base_url") or _soam_base_url()).rstrip("/")
    return (
        OpenAI(
            api_key=api_key,
            base_url=base_url,
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `clawbot setup` or `clawbot model` and enter a valid http(s) base URL."
        ) from exc
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second clawbot
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``clawbot config aux reset``)."""
...
def _is_unsupported_parameter_error(exc: Exception, param: str) -> bool:
    ...
    ``seed``, ``top_p``, and any future quirk. Credit @aayushsoam 
...
def _recoverable_pool_provider(resolved_provider: str, client: Any) -> Optional[str]:
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference-api.aayushsoam.com"):
        return "soam"
    ...
    try:
        ...
        if normalized == "soam":
            ...
            creds = resolve_soam_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("CLAWBOT_SOAM_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("CLAWBOT_SOAM_TIMEOUT_SECONDS", "15")),
                inference_auth_mode=SOAM_INFERENCE_AUTH_MODE_LEGACY,
            )
            if not str(creds.get("api_key", "") or "").strip():
                return False
            _evict_cached_clients(normalized)
...
def _try_payment_fallback(
    ...
    _alias_to_label = {"openrouter": "openrouter", "soam": "soam",
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/Soam get their chosen chat model; users
      ...
      2. OpenRouter → Soam → custom → Codex → API-key providers (fallback
         chain, only used when the main provider has no working client).
    """
    global auxiliary_is_soam, _stale_base_url_warned
    auxiliary_is_soam = False  # Reset — _try_soam() will set True if it wins
    runtime = _normalize_main_runtime(main_runtime)
    ...
    #    scenario where a user switches providers via `clawbot model` but the
    #    old OPENAI_BASE_URL lingers in ~/.clawbot/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: clawbot model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.clawbot/.env",
    ...
    # on aggregators (OpenRouter, Soam) who previously got routed to a
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from clawbot_cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from clawbot_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    Args:
        provider: Provider identifier.  One of:
            "openrouter", "soam", "openai-codex" (or "codex"),
    ...
    if provider == "soam":
        ...
        client, default = _try_soam(vision=_is_vision)
        if client is None:
            logger.warning("resolve_provider_client: soam requested "
                           "but Soam Portal not configured (run: clawbot auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: clawbot model)")
                return None, None
    ...
    # OpenRouter / Soam bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: clawbot model -> xAI Grok OAuth — SuperGrok Subscription)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    try:
        from clawbot_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider (e.g. ``soam``)
    ...
    try:
        ...
        logger.debug("clawbot_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from clawbot_cli.models import _should_use_copilot_responses_api
    ...
    if pconfig.auth_type == "external_process":
        ...
        if provider == "soam":
            return resolve_provider_client("soam", model, async_mode)
        if provider == "openai-codex":
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → Soam → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. Soam — skip if already covered by main provider.
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Soam Portal product tags when the auxiliary client is backed
    by Soam Portal. Returns empty dict otherwise.
    """
    return _soam_extra_body() if auxiliary_is_soam else {}
...
def _refresh_soam_auxiliary_client(
    *,
    cache_provider: str,
    model: Optional[str],
    async_mode: bool,
    ...
    """Refresh Soam runtime creds, rebuild the client, and replace the cache entry."""
    runtime = _resolve_soam_runtime_api(force_refresh=True)
    if runtime is None:
        return None, model
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Codex adapter handles max_tokens internally; OpenRouter/Soam use max_tokens.
    ...
    if provider == "soam" or auxiliary_is_soam:
        merged_extra.setdefault("tags", []).extend(_soam_portal_tags())
    if merged_extra:
...
def call_llm(
    ...
    *,
    ...
    """Centralized synchrosoam LLM call.
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `clawbot model`."
                )
    ...
    try:
        ...
        # ── Soam auth refresh parity with main agent ──────────────────
        client_is_soam = (
            resolved_provider == "soam"
            or base_url_host_matches(_base_info, "inference-api.aayushsoam.com")
        )
        if _is_auth_error(first_err) and client_is_soam:
            refreshed_client, refreshed_model = _refresh_soam_auxiliary_client(
                cache_provider=resolved_provider or "soam",
                model=final_model,
                async_mode=False,
                base_url=resolved_base_url,
                api_key=resolved_api_key,
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed Soam runtime credentials after 401, retrying",
                            task or "call")
...
async def async_call_llm(
    ...
    *,
    ...
    """Centralized asynchrosoam LLM call.
    ...
    try:
        ...
        client_is_soam = (
            resolved_provider == "soam"
            or base_url_host_matches(_client_base, "inference-api.aayushsoam.com")
        )
        if _is_auth_error(first_err) and client_is_soam:
            refreshed_client, refreshed_model = _refresh_soam_auxiliary_client(
                cache_provider=resolved_provider or "soam",
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed Soam runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
