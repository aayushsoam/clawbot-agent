"""Cross-session rate limit guard for Soam Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Soam Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from Soam triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Clawbot retries), and every one of those calls counts
...
_STATE_FILENAME = "soam.json"


def _state_path() -> str:
    """Return the path to the Soam rate limit state file."""
    try:
        from clawbot_constants import get_clawbot_home
        base = get_clawbot_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".clawbot")
...
def record_soam_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Soam Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "Soam rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write Soam rate limit state: %s", exc)


def soam_rate_limit_remaining() -> Optional[float]:
    """Check if Soam Portal is currently rate-limited.
...
def clear_soam_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful Soam request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear Soam rate limit state: %s", exc)
...
def is_genuine_soam_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Soam Portal is a real account rate limit.

    Soam Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    MiMo, Clawbot, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on Soam is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on Soam.

    Tripping the cross-session breaker on (b) blocks ALL Soam requests
    (and all models, since Soam is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  Soam emits
