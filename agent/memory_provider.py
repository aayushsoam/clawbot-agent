...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - clawbot_home (str): The active CLAWBOT_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.clawbot``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "clawbot").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'clawbot memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], clawbot_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'clawbot memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``clawbot_home`` is the active CLAWBOT_HOME directory path.
