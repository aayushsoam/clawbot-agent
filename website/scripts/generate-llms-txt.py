#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Clawbot docs site.
...
Both publish at:
  https://clawbot-agent.aayushsoam.com/docs/llms.txt
  https://clawbot-agent.aayushsoam.com/docs/llms-full.txt
...
SITE_BASE = "https://clawbot-agent.aayushsoam.com/docs"
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Clawbot as a Python Library", None),
        ("guides/use-mcp-with-clawbot", "Use MCP with Clawbot", None),
        ("guides/use-voice-mode-with-clawbot", "Use Voice Mode with Clawbot", None),
        ("guides/use-soul-with-clawbot", "Use SOUL.md with Clawbot", None),
        ("guides/build-a-clawbot-plugin", "Build a Clawbot Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Clawbot"),
...
def emit_llms_index() -> str:
    ...
    lines: list[str] = []
    lines.append("# Clawbot Agent")
    lines.append("")
    lines.append(
        "> The self-improving AI agent built by Aayush soam. A terminal-native "
        ...
        "backends. Works with Soam Portal, OpenRouter, OpenAI, Anthropic, Google, or any "
        "OpenAI-compatible endpoint."
    )
    lines.append("")
    lines.append(
        "Install: `curl -fsSL https://raw.githubusercontent.com/Aayushsoam/"
        "clawbot-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/aayushsoam/clawbot-agent")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Clawbot Agent — Full Documentation\n",
        (
            "This file is the entire Clawbot Agent documentation concatenated for LLM "
            "context ingestion. Section order reflects docs-site navigation: Getting "
            "Started, Using Clawbot, Features, Messaging, Integrations, Guides, "
            "Developer Guide, Reference, then everything else.\n"
        ),
        "Canonical site: https://clawbot-agent.aayushsoam.com/docs\n",
        "Short index: https://clawbot-agent.aayushsoam.com/docs/llms.txt\n",
        "\n---\n\n",
    ]
