...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/aayushsoam/clawbot-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    clawbot_meta = (fm.get("metadata") or {}).get("clawbot") or {}
    tags = clawbot_meta.get("tags") or []
    related = clawbot_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `clawbot skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Clawbot Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Clawbot ships with a large built-in skill library copied into `~/.clawbot/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Clawbot also syncs bundled skills on `clawbot update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.clawbot/skills/` tree, it is still shipped with Clawbot; restore it with `clawbot skills reset <name> --restore`.",
        "",
