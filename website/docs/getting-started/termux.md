...
description: "Run Clawbot Agent directly on an Android phone with Termux"
---

# Clawbot on Android with Termux

This is the tested path for running Clawbot Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Clawbot from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Clawbot now ships a Termux-aware installer path:
...
- links `clawbot` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `clawbot` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/clawbot" "$PREFIX/bin/clawbot"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `clawbot` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.clawbot/.env`.
...
### `clawbot doctor` says ripgrep or Node is missing
