...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Clawbot Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Clawbot setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full Clawbot Agent Tutorials & Use Cases.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src=""
    title="Clawbot Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Clawbot for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Clawbot working on my machine | `clawbot setup` | Run a real chat and verify it responds |
| I already know my provider | `clawbot model` | Save the config, then start chatting |
| I want a bot or always-on setup | `clawbot gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `clawbot model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `clawbot model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Clawbot cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Clawbot Agent

**Option A — pip (simplest):**

```bash
pip install clawbot-agent
clawbot postinstall     # optional: installs Node.js, browser, ripgrep, ffmpeg + runs setup
```
...
The single most important setup step. Use `clawbot model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Soam Portal** | Subscription-based, zero-config | OAuth login via `clawbot model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `clawbot model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `clawbot model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax-M2.7 via browser OAuth — no API key needed | `clawbot model` → MiniMax (OAuth) |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `clawbot model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `clawbot model` (requires `copilot` CLI + `copilot login`) |
...
:::tip
You can switch providers at any time with `clawbot model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Clawbot separates secrets from normal config:

- **Secrets and tokens** → `~/.clawbot/.env`
- **Non-secret settings** → `~/.clawbot/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
clawbot config set model anthropic/claude-opus-4.6
clawbot config set terminal.backend docker
clawbot config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
clawbot            # classic CLI
clawbot --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Clawbot ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `clawbot` vs `clawbot --tui`.
:::
...
```bash
clawbot --continue    # Resume the most recent session
clawbot -c            # Short form
```
...
```bash
clawbot gateway setup    # Interactive platform configuration
```
...
- `clawbot tools` — tune tool access per platform
- `clawbot skills` — browse and install reusable workflows
...
```bash
clawbot config set terminal.backend docker    # Docker isolation
clawbot config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Clawbot install directory (the curl installer placed it at
# ~/.clawbot/clawbot-agent on Linux/macOS or %LOCALAPPDATA%\clawbot\clawbot-agent on Windows):
...
(If you installed without `[all]`, run `cd ~/.clawbot/clawbot-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Clawbot opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `clawbot model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `clawbot gateway setup` and check `clawbot gateway status` |
| `clawbot --continue` can't find old session | Switched profiles or session never saved | Check `clawbot sessions list` and confirm you're in the right profile |
...
| `clawbot doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `clawbot model`
3. `clawbot setup`
4. `clawbot sessions list`
5. `clawbot --continue`
6. `clawbot gateway status`
...
| Command | Description |
|---------|-------------|
| `clawbot` | Start chatting |
| `clawbot model` | Choose your LLM provider and model |
| `clawbot tools` | Configure which tools are enabled per platform |
| `clawbot setup` | Full setup wizard (configures everything at once) |
| `clawbot doctor` | Diagnose issues |
| `clawbot update` | Update to latest version |
| `clawbot gateway` | Start the messaging gateway |
| `clawbot --continue` | Resume last session |
