...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Clawbot Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Clawbot Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Clawbot](/docs/guides/use-voice-mode-with-clawbot).

## Prerequisites

Before using voice features, make sure you have:

1. **Clawbot Agent installed** — `pip install clawbot-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `clawbot model` or set your preferred provider credentials in `~/.clawbot/.env`
3. **A working base setup** — run `clawbot` to verify the agent responds to text before enabling voice

:::tip
The `~/.clawbot/` directory and default `config.yaml` are created automatically the first time you run `clawbot`. You only need to create `~/.clawbot/.env` manually for API keys.
:::
...
pip install "clawbot-agent[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
pip install "clawbot-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "clawbot-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "clawbot-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`clawbot chat`) and the **TUI** (`clawbot --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.clawbot/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
1. Start the CLI with `clawbot` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.clawbot/config.yaml` (default: `ctrl+b`).
:::
...
```bash
clawbot gateway        # Start the gateway (connects to configured platforms)
clawbot gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@clawbotbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.clawbot/.env`:
```bash
...
```bash
clawbot gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.clawbot/.env`
...
- Review gateway logs: `tail -f ~/.clawbot/logs/gateway.log`
