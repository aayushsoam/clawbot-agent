...
Type `@` followed by a reference to inject content directly into your message. Clawbot expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Clawbot env: `$CLAWBOT_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CLAWBOT_HOME/skills/.hub/`
