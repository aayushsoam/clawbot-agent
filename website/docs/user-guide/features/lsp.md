...
Clawbot runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Clawbot
...
1. Clawbot captures a baseline of current diagnostics for the file.
...
…). Clawbot auto-detects the binary on PATH or in
`<CLAWBOT_HOME>/lsp/bin/`.
...
`node_modules` tree — Clawbot installs both packages together when you
run `clawbot lsp install typescript` or auto-install fires on first
use.

## CLI

```
clawbot lsp status          # service state + per-server install status
clawbot lsp list            # registry, optionally --installed-only
clawbot lsp install <id>    # eagerly install one server
clawbot lsp install-all     # try every server with a known recipe
clawbot lsp restart         # tear down running clients
clawbot lsp which <id>      # print resolved binary path
```

`clawbot lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <CLAWBOT_HOME>/lsp/bin
...
When `install_strategy: auto`, Clawbot installs binaries into
`<CLAWBOT_HOME>/lsp/bin/`. NPM packages land in
`<CLAWBOT_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Clawbot-owned and is
...
Servers are kept alive for the life of the Clawbot process. There's
...
**`clawbot lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<CLAWBOT_HOME>/lsp/bin/`. Run
`clawbot lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `clawbot lsp status`**
...
When `clawbot lsp status` shows a `Backend warnings` section, install
...
`~/.clawbot/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.clawbot/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `clawbot lsp restart` to clear the set;
