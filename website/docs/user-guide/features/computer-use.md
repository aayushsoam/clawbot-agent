...
Use `clawbot computer-use status` to verify the install.
...
3. Grant macOS permissions when prompted:
   - **System Settings → Privacy & Security → Accessibility** → allow the
     terminal (or Clawbot app).
   - **System Settings → Privacy & Security → Screen Recording** → allow
     the same.
4. Start a session with the toolset enabled:
   ```
   clawbot -t computer_use chat
   ```
   or add `computer_use` to your enabled toolsets in `~/.clawbot/config.yaml`.

## Keeping cua-driver up to date

The cua-driver project ships fixes regularly (e.g. v0.1.6 fixed a Safari
window-focus bug for UTM workflows). Clawbot refreshes the binary in two
places so you don't get stuck on a stale release:

- **`clawbot update`** — when you update Clawbot itself, if `cua-driver` is
...
- **`clawbot computer-use install --upgrade`** — manual force-refresh.
...
`clawbot computer-use status` shows the installed version next to the
binary path.
...
Clawbot applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.clawbot/config.yaml` if you want every action confirmed.

## Token efficiency

Screenshots are expensive. Clawbot applies four layers of optimisation:
...
- **Private SPI risk.** Apple can change SkyLight's symbol surface in any
  OS update. Pin the driver version with the `CLAWBOT_CUA_DRIVER_VERSION`
...
```
CLAWBOT_CUA_DRIVER_CMD=/opt/homebrew/bin/cua-driver
CLAWBOT_CUA_DRIVER_VERSION=0.5.0    # optional pin
```

Swap the backend entirely (for testing):

```
CLAWBOT_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

## Troubleshooting

**`computer_use backend unavailable: cua-driver is not installed`** — Run
`clawbot computer-use install` to fetch the cua-driver binary, or run
`clawbot tools` and enable the Computer Use toolset.
...
- [Universal skill: `macos-computer-use`](https://github.com/aayushsoam/clawbot-agent/blob/main/skills/apple/macos-computer-use/SKILL.md)
