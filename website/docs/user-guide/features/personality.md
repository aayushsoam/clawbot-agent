...
description: "Customize Clawbot Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Clawbot Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CLAWBOT_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Clawbot is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Clawbot now seeds a default `SOUL.md` automatically in:

```text
~/.clawbot/SOUL.md
```

More precisely, it uses the current instance's `CLAWBOT_HOME`, so if you run Clawbot with a custom home directory, it will use:

```text
$CLAWBOT_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Clawbot creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Clawbot loads `SOUL.md` only from `CLAWBOT_HOME`
- Clawbot does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Clawbot falls back to a built-in default identity
...
If Clawbot loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CLAWBOT_HOME`, the personality belongs to the Clawbot instance itself.

That also makes it easier to teach users:
- "Edit `~/.clawbot/SOUL.md` to change Clawbot' default personality."
...
- how Clawbot should handle uncertainty, disagreement, or ambiguity
...
## What Clawbot injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Clawbot falls back to a built-in default identity ("You are Clawbot Agent, an intelligent AI assistant created by Aayush soam..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Clawbot ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Clawbot, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Clawbot its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.clawbot/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.clawbot/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-clawbot)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Clawbot speaks
- `display.skin` and `/skin` affect how Clawbot looks in the terminal
