...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Clawbot's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Clawbot instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
clawbot memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$CLAWBOT_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `clawbot` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Clawbot syncs them back at session init.
...
The `clawbot honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). Run `clawbot memory setup` and pick Honcho first; the subcommand appears on the next invocation.

```bash
clawbot honcho status          # Connection status, config, and key settings
clawbot honcho setup           # Redirects to `clawbot memory setup`
clawbot honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
clawbot honcho peer            # Show or update peer names + dialectic reasoning level
clawbot honcho mode            # Show or set recall mode (hybrid/context/tools)
clawbot honcho tokens          # Show or set token budget for context and dialectic
clawbot honcho identity        # Seed or show the AI peer's Honcho identity
clawbot honcho sync            # Sync Honcho config to all existing profiles
clawbot honcho peers           # Show peer identities across all profiles
clawbot honcho sessions        # List known Honcho session mappings
clawbot honcho map             # Map current directory to a Honcho session name
clawbot honcho enable          # Enable Honcho for the active profile
clawbot honcho disable         # Disable Honcho for the active profile
clawbot honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `clawbot honcho`

If you previously used the standalone `clawbot honcho setup`:
...
No re-login or re-setup needed. Run `clawbot memory setup` and select "honcho" — the wizard detects your existing config.
