...
The web dashboard is a browser-based UI for managing your Clawbot Agent installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.
...
| `--tui` | off | Expose the in-browser Chat tab (embedded `clawbot --tui` via PTY/WebSocket). Alternatively set `CLAWBOT_DASHBOARD_TUI=1`. |

```bash
# Custom port
clawbot dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
clawbot dashboard --host 0.0.0.0
...
The default `clawbot-agent` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
pip install 'clawbot-agent[web,pty]'
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `pip install clawbot-agent[all]` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `clawbot dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is intentionally off for a plain `clawbot dashboard` launch. Start the dashboard with `clawbot dashboard --tui` or set `CLAWBOT_DASHBOARD_TUI=1` when you want the embedded browser chat pane.
...
- Node.js (same requirement as `clawbot --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`pip install 'clawbot-agent[web,pty]'`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `clawbot config set` and the gateway read from.
:::
...
Browse, search, and toggle skills and toolsets. Skills are loaded from `~/.clawbot/skills/` and grouped by category.
...
This re-reads `~/.clawbot/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
The frontend is built with React 19, TypeScript, Tailwind CSS v4, and shadcn/ui-style components. Production builds output to `clawbot_cli/web_dist/` which the FastAPI server serves as a static SPA.

## Automatic Build on Update

When you run `clawbot update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `clawbot dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Clawbot Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Clawbot Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
