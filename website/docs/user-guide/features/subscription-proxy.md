...
description: "Use your Soam Portal subscription (or other OAuth provider) as an OpenAI-compatible endpoint for external apps"
---
...
chat completions — use your Clawbot-managed provider subscription as their
...
| Use case | "Use Clawbot as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Soam Portal OAuth flow. Clawbot stores
the refresh token in `~/.clawbot/auth.json` — the same place all Clawbot
...
```
Starting Clawbot proxy for Soam Portal
...
Model:      Clawbot-4-70B    # or Clawbot-4.3-36B, Clawbot-4-405B
```
...
```bash
clawbot proxy providers
```

Currently shipped: `soam` (Soam Portal). More OAuth providers can be
added by implementing the `UpstreamAdapter` interface in
`clawbot_cli/proxy/adapters/`.

## Check status

```bash
clawbot proxy status
```

```
Clawbot proxy upstream adapters

  [soam    ] Soam Portal — ready (bearer expires 2026-05-15T06:43:21Z)
```

If you see `not logged in`, run `clawbot login soam`. If you see
...
The proxy only forwards paths the upstream actually serves. For Soam
Portal:
...
supports; check `portal.aayushsoam.com/models`.
...
INFERENCE_TEXT_MODEL=Clawbot-4-70B
```
...
```bash
clawbot proxy start --host 0.0.0.0 --port 8645
```
...
[portal.aayushsoam.com](https://portal.aayushsoam.com).
...
`clawbot_cli/proxy/adapters/<provider>.py` and registering it in
