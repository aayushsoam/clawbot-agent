...
When Clawbot Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
custom instructions in `~/.clawbot/config.yaml` under `agent.custom_instructions`.
...
If you use Clawbot' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.clawbot/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Clawbot Agent's deliverable
