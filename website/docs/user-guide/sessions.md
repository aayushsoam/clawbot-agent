...
Clawbot Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.clawbot/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.clawbot/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
Clawbot stores session history so it can resume conversations, but it does not
...
Clawbot explicitly injects for that turn.
...
For example, if a user sends an image and asks Clawbot to make a meme from it,
Clawbot may inspect that image once with vision and run an image-processing
...
`clawbot sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`clawbot` or `clawbot chat`) |
...
clawbot -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
clawbot -c "my project"   # → resumes "my project #3"
```
...
clawbot --resume 20250305_091523_a1b2c3d4
clawbot -r 20250305_091523_a1b2c3d4

# Resume by title
clawbot --resume "refactoring auth"

# Or with the chat subcommand
clawbot chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `clawbot sessions list`.

### Conversation Recap on Resume

When you resume a session, Clawbot displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Clawbot session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.clawbot/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `clawbot -r "<title>"` from the shell) and pick up where the platform left off.
...
```bash
clawbot sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Clawbot creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`clawbot -c "my project"`), it automatically picks the most recent session in the lineage.
...
Clawbot provides a full set of session management commands via `clawbot sessions`:
...
# Filter by platform
clawbot sessions list --source telegram

# Show more sessions
clawbot sessions list --limit 50
```
...
clawbot sessions export backup.jsonl

# Export sessions from a specific platform
clawbot sessions export telegram-history.jsonl --source telegram

# Export a single session
clawbot sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
clawbot sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
clawbot sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
clawbot sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
clawbot sessions prune

# Custom age threshold
clawbot sessions prune --older-than 30

# Only prune sessions from a specific platform
clawbot sessions prune --source telegram --older-than 60

# Skip confirmation
clawbot sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`clawbot insights`](/docs/reference/cli-commands#clawbot-insights).
...
When Clawbot cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Clawbot uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Clawbot in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.clawbot/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.clawbot/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.clawbot/sessions/sessions.json` | Maps session keys to active session IDs |
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Clawbot process in the same `CLAWBOT_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.clawbot/config.yaml`:
...
clawbot sessions delete <session_id>
