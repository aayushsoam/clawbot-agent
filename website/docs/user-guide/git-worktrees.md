...
title: "Git Worktrees"
description: "Run multiple Clawbot agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Clawbot Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Clawbot so each session has a clean, isolated working directory.

## Why Use Worktrees with Clawbot?

Clawbot treats the **current working directory** as the project root:

- CLI: the directory where you run `clawbot` or `clawbot chat`
...
git worktree add ../repo-feature feature/clawbot-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/clawbot-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Clawbot there:

```bash
cd ../repo-feature

# Start Clawbot in the worktree
clawbot
```
...
git worktree add ../repo-experiment-a feature/clawbot-a
git worktree add ../repo-experiment-b feature/clawbot-b
```
...
Each Clawbot process:

- Works on its own branch (`feature/clawbot-a` vs `feature/clawbot-b`).
...
- Clawbot checkpoint data under `~/.clawbot/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Clawbot experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/clawbot-checkpoints-docs`, `feature/clawbot-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Clawbot from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `clawbot -w` (Automatic Worktree Mode)

Clawbot has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `clawbot/clawbot-<hash>`).
...
```bash
clawbot -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `clawbot -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Clawbot session its own clean checkout.
