...
description: "Set up Clawbot Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Clawbot Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Clawbot strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create clawbot-bot --allow-anonymous
devtunnel port create clawbot-bot -p 3978 --protocol https  # replace 3978 with TEAMS_PORT if changed
devtunnel host clawbot-bot
...
```bash
CLAWBOT_UID=$(id -u) CLAWBOT_GID=$(id -g) docker compose up -d gateway
```
...
Alternatively, configure via `~/.clawbot/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.clawbot/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create clawbot-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs clawbot` for tracebacks |
...
- Store credentials in `~/.clawbot/.env` with permissions `600` (`chmod 600 ~/.clawbot/.env`)
