...
Connect Clawbot to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Clawbot supports two WeCom integration modes:
...
3. Clawbot decrypts the message, queues it for the agent
...
(Use `clawbot gateway start` only after `clawbot gateway install` has registered the systemd/launchd service.)
