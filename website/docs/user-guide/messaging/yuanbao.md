...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Clawbot Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Clawbot to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.clawbot/.env`:
...
All standard Clawbot commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.clawbot/logs/gateway.log`
...
4. Consider enabling verbose logging: `CLAWBOT_LOG_LEVEL=debug`
...
Yuanbao has a maximum message size. Clawbot automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
CLAWBOT_LOG_LEVEL=debug clawbot gateway
```
...
```bash
clawbot chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
