...
description: "Chat with Clawbot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Clawbot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/docs/user-guide/features/voice-mode) and [Use Voice Mode with Clawbot](/docs/guides/use-voice-mode-with-clawbot).
...
```bash
clawbot gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
clawbot gateway              # Run in foreground
clawbot gateway setup        # Configure messaging platforms interactively
clawbot gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo clawbot gateway install --system   # Linux only: install a boot-time system service
clawbot gateway start        # Start the default service
clawbot gateway stop         # Stop the default service
clawbot gateway status       # Check default service status
clawbot gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Clawbot Agent to the latest version |
...
Configure per-platform overrides in `~/.clawbot/gateway.json`:
...
clawbot pairing approve telegram XKGH5N7P

# Other pairing commands:
clawbot pairing list          # View pending + approved users
clawbot pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Clawbot appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.clawbot/config.yaml`:
...
Each `/background` prompt spawns a **separate agent instance** that runs asynchrosoamly:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.clawbot/config.yaml`:
...
```bash
CLAWBOT_BACKGROUND_NOTIFICATIONS=result
```
...
```bash
clawbot gateway install               # Install as user service
clawbot gateway start                 # Start the service
clawbot gateway stop                  # Stop the service
clawbot gateway status                # Check status
journalctl --user -u clawbot-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo clawbot gateway install --system
sudo clawbot gateway start --system
sudo clawbot gateway status --system
journalctl -u clawbot-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Clawbot will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Clawbot installations on the same machine (with different `CLAWBOT_HOME` directories), each gets its own systemd service name. The default `~/.clawbot` uses `clawbot-gateway`; other installations use `clawbot-gateway-<hash>`. The `clawbot gateway` commands automatically target the correct service for your current `CLAWBOT_HOME`.
:::

### macOS (launchd)

```bash
clawbot gateway install               # Install as launchd agent
clawbot gateway start                 # Start the service
clawbot gateway stop                  # Stop the service
clawbot gateway status                # Check status
tail -f ~/.clawbot/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.clawbot.gateway.plist`. It includes three environment variables:
...
- **CLAWBOT_HOME** — scopes the gateway to your Clawbot installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `clawbot gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `CLAWBOT_HOME` directory gets its own launchd label. The default `~/.clawbot` uses `ai.clawbot.gateway`; other installations use `ai.clawbot.gateway-<suffix>`.
:::
...
|----------|---------|--------------|
| CLI | `clawbot-cli` | Full access |
| Telegram | `clawbot-telegram` | Full tools including terminal |
| Discord | `clawbot-discord` | Full tools including terminal |
| WhatsApp | `clawbot-whatsapp` | Full tools including terminal |
| Slack | `clawbot-slack` | Full tools including terminal |
| Google Chat | `clawbot-google_chat` | Full tools including terminal |
| Signal | `clawbot-signal` | Full tools including terminal |
| SMS | `clawbot-sms` | Full tools including terminal |
| Email | `clawbot-email` | Full tools including terminal |
| Home Assistant | `clawbot-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `clawbot-mattermost` | Full tools including terminal |
| Matrix | `clawbot-matrix` | Full tools including terminal |
| DingTalk | `clawbot-dingtalk` | Full tools including terminal |
| Feishu/Lark | `clawbot-feishu` | Full tools including terminal |
| WeCom | `clawbot-wecom` | Full tools including terminal |
| WeCom Callback | `clawbot-wecom-callback` | Full tools including terminal |
| Weixin | `clawbot-weixin` | Full tools including terminal |
| BlueBubbles | `clawbot-bluebubbles` | Full tools including terminal |
| QQBot | `clawbot-qqbot` | Full tools including terminal |
| Yuanbao | `clawbot-yuanbao` | Full tools including terminal |
| Microsoft Teams | `clawbot-teams` | Full tools including terminal |
| API Server | `clawbot-api-server` | Full tools (drops `clarify`, `send_message`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `clawbot-webhook` | Full tools including terminal |
