...
sidebar_position: 2
title: "TUI"
description: "Launch the modern terminal UI for Clawbot — mouse-friendly, rich overlays, and non-blocking input."
---

# TUI

The TUI is the modern front-end for Clawbot — a terminal UI backed by the same Python runtime as the [Classic CLI](cli.md). Same agent, same sessions, same slash commands; a cleaner, more responsive surface for interacting with them.

It's the recommended way to run Clawbot interactively.
...
clawbot --tui -c
clawbot --tui --continue

# Resume a specific session by ID or title
clawbot --tui -r 20260409_000000_aa11bb
clawbot --tui --resume "my t0p session"
...
```bash
export CLAWBOT_TUI=1
clawbot          # now uses the TUI
clawbot chat     # same
```
...
- **Instant first frame** — the banner paints before the app finishes loading, so the terminal never feels frozen while Clawbot is starting.
...
- **Node.js** ≥ 20 — the TUI runs as a subprocess launched from the Python CLI. `clawbot doctor` verifies this.
- **TTY** — like the classic CLI, piping stdin or running in non-interactive environments falls back to single-query mode.

On first launch Clawbot installs the TUI's Node dependencies into `ui-tui/node_modules` (one-time, a few seconds). Subsequent launches are fast. If you pull a new Clawbot version, the TUI bundle is rebuilt automatically when sources are newer than the dist.

### External prebuild

Distributions that ship a prebuilt bundle (Nix, system packages) can point Clawbot at it:

```bash
export CLAWBOT_TUI_DIR=/path/to/prebuilt/ui-tui
clawbot --tui
```
...
| `/reload` | Re-reads `~/.clawbot/.env` into the running TUI process so newly added API keys take effect without a restart |
...
1. `CLAWBOT_TUI_THEME` env var — highest priority. Values: `light`, `dark`, or a raw 6-char background hex (e.g. `ffffff`, `1a1a2e`).
...
```bash
export CLAWBOT_TUI_THEME=light
```

## Busy indicator styles

The status-bar busy indicator is pluggable — the default rotates Clawbot' kawaii face palette every 2.5 seconds during agent work. Pick a different style via config or the `/indicator` slash command:
...
By default, `clawbot --tui` starts a fresh session each launch. To re-attach to the most recent TUI session automatically (useful when your terminal or SSH connection drops unexpectedly), opt in:

```bash
export CLAWBOT_TUI_RESUME=1          # most-recent TUI session
# or:
export CLAWBOT_TUI_RESUME=<session-id>   # specific session
```
...
- **Working directory with git branch** — `~/projects/clawbot-agent (docs/two-week-gap-sweep)`. The branch suffix updates when you `git checkout` in a side terminal (mtime-cached) so the TUI reflects your actual active branch, not whatever it was at launch.
...
The TUI respects all standard Clawbot config: `~/.clawbot/config.yaml`, profiles, personalities, skins, quick commands, credential pools, memory providers, tool/skill enablement. No TUI-specific config file exists.
...
Sessions are shared between the TUI and the classic CLI — both write to the same `~/.clawbot/state.db`. You can start a session in one, resume in the other. The session picker surfaces sessions from both sources, with a source tag.

See [Sessions](sessions.md) for lifecycle, search, compression, and export.

## Reverting to the classic CLI

Launching `clawbot` (without `--tui`) stays on the classic CLI. To make a machine prefer the TUI, set `CLAWBOT_TUI=1` in your shell profile. To go back, unset it.

If the TUI fails to launch (no Node, missing bundle, TTY issue), Clawbot prints a diagnostic and falls back — rather than leaving you stuck.
