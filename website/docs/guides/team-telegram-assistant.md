...
This tutorial walks you through setting up a Telegram bot powered by Clawbot Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Clawbot Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.clawbot/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Clawbot itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Clawbot Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_clawbot_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Clawbot Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.clawbot/.env`:
...
```
[Gateway] Starting Clawbot Gateway...
...
# View live logs
journalctl --user -u clawbot-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `clawbot gateway install` to update the plist.
:::
...
```bash
clawbot gateway stop && clawbot gateway start
```
...
clawbot pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.clawbot/.env`:
...
Control how much detail the bot shows when using tools. In `~/.clawbot/config.yaml`:
...
Customize how the bot communicates by editing `~/.clawbot/SOUL.md`:

For a full guide, see [Use SOUL.md with Clawbot](/docs/guides/use-soul-with-clawbot).
...
```bash
# From the CLI
clawbot cron list          # View all scheduled jobs
clawbot cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u clawbot-gateway` (Linux) or `~/.clawbot/logs/gateway.log` (macOS) |
| Cron job output | `~/.clawbot/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.clawbot/cron/jobs.json` |
| Pairing data | `~/.clawbot/pairing/` |
| Session history | `~/.clawbot/sessions/` |
