...
description: "Sign in with your SuperGrok subscription to use Grok models in Clawbot Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok Subscription)

Clawbot Agent supports xAI Grok through a browser-based OAuth login flow against [accounts.x.ai](https://accounts.x.ai), using your existing **SuperGrok subscription**. No `XAI_API_KEY` is required — log in once and Clawbot automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Clawbot — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Clawbot opens your browser to accounts.x.ai
...
After the first login, credentials are stored under `~/.clawbot/auth.json` and refreshed automatically before they expire.
...
```bash
clawbot auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, or SSH sessions where no browser is available, Clawbot detects the remote environment and prints the authorization URL instead of opening a browser.
...
clawbot auth add xai-oauth --no-browser
...
1. Clawbot opens your browser to `accounts.x.ai`.
2. You sign in (or confirm your existing session) and approve access.
3. xAI redirects back to Clawbot and the tokens are saved to `~/.clawbot/auth.json`.
4. From then on, Clawbot refreshes the access token in the background — you stay signed in until you `clawbot auth remove xai-oauth` or revoke access from your xAI account settings.
...
```bash
clawbot config set model.default grok-4.3
clawbot config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.clawbot/config.yaml` will contain:
...
```bash
clawbot --provider xai-oauth        # canonical
clawbot --provider grok-oauth       # alias
clawbot --provider x-ai-oauth       # alias
clawbot --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `clawbot tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search is off by default
The `x_search` toolset is disabled by default. Enable it in `clawbot tools` → `🐦 X (Twitter) Search` (press space) before the agent can call `x_search`. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
| `CLAWBOT_INFERENCE_PROVIDER` | Force the active provider at runtime, e.g. `CLAWBOT_INFERENCE_PROVIDER=xai-oauth clawbot`. |

## Troubleshooting

### Token expired — not re-logging in automatically

Clawbot refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Clawbot surfaces a typed re-auth message instead of crashing.

**Fix:** run `clawbot auth add xai-oauth` again to start a fresh login.

### Authorization timed out

The loopback listener has a finite expiry window (default 180 s). If you don't approve the login in time, Clawbot raises a timeout error.

**Fix:** re-run `clawbot auth add xai-oauth` (or `clawbot model`). The flow starts fresh.

### State mismatch (possible CSRF)

Clawbot detected that the `state` value returned by the authorization server doesn't match what it sent.
...
On SSH or container sessions Clawbot prints the authorization URL instead of opening a browser. The loopback callback listener still binds `127.0.0.1:56121` on the remote host — your laptop's browser can't reach it without an SSH local-forward:
...
**Fix:** run `clawbot model` and pick the xAI Grok OAuth provider, or run `clawbot auth add xai-oauth`.
...
```bash
clawbot auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `clawbot auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `clawbot auth list xai-oauth` to see them).

## See Also

- [OAuth over SSH / Remote Hosts](./oauth-over-ssh.md) — required reading if Clawbot is on a different machine than your browser
