...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `clawbot send`."
---

# Pipe Script Output to Messaging Platforms

`clawbot send` is a small, scriptable CLI that pushes a message to any
messaging platform Clawbot is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | clawbot send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `clawbot
...
clawbot send --to telegram "deploy finished"

# Pipe in stdout from anything
echo "RAM 92%" | clawbot send --to telegram:-1001234567890

# Send a file
clawbot send --to discord:#ops --file /tmp/report.md

# Attach a subject/header line
clawbot send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
clawbot send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
clawbot send --list

# Filter by platform
clawbot send --list telegram
```
...
Any platform Clawbot ships adapters for works as a target:
...
`clawbot send` resolves the message body in this order:

1. **Positional argument** — `clawbot send --to telegram "hi"`
2. **`--file PATH`** — `clawbot send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | clawbot send --to telegram`

When stdin is a TTY (no pipe), Clawbot does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  clawbot send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `clawbot send` reuses your Clawbot config, the same script works on
any host where Clawbot is installed — no need to export bot tokens into
...
`clawbot send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  clawbot send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | clawbot send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.clawbot/bin/clawbot send \
...
```bash
./train.py --epochs 200 && \
  clawbot send --to telegram "training done" || \
  clawbot send --to telegram "training failed (exit $?)"
```
...
clawbot send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(clawbot send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `clawbot send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `clawbot send` calls
the platform's REST endpoint directly using credentials from
`~/.clawbot/.env` and `~/.clawbot/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `clawbot gateway start` and retry.
...
# Machine-readable
clawbot send --list --json
```

The listing is built from `~/.clawbot/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`clawbot gateway
...
| Approach | Multi-platform | Reuses Clawbot creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `clawbot send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`clawbot send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `clawbot send`.
...
- [Gateway Internals](/docs/developer-guide/gateway-internals) —
  the delivery router that `clawbot send` shares with cron delivery.
