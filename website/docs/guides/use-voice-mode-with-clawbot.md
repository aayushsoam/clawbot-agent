---
sidebar_position: 8
title: "Use Voice Mode with Clawbot"
description: "A practical guide to setting up and using Clawbot voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Clawbot
...
- you want Clawbot sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Clawbot.
...
## Step 1: make sure normal Clawbot works first
...
Clawbot supports both local and cloud speech stacks.
...
### If you use `clawbot setup`

If you choose NeuTTS in the setup wizard, Clawbot checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Clawbot transcribes and responds
...
- asking Clawbot to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Clawbot loop.

## Tuning CLI behavior

### Silence threshold

If Clawbot starts/stops too aggressively, tune:
...
Clawbot stays a normal chat bot, but can speak replies.
...
- you want Clawbot to function like a portable research or ops assistant
...
Clawbot joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Clawbot detects speech boundaries
- transcripts are posted in the associated text channel
- Clawbot responds in text and audio
...
1. get text Clawbot working
2. install `clawbot-agent[voice]`
