...
```bash
clawbot teams-pipeline validate
```
...
```bash
clawbot teams-pipeline token-health
clawbot teams-pipeline token-health --force-refresh
```
...
```bash
clawbot teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
clawbot teams-pipeline maintain-subscriptions
clawbot teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Clawbot cron (recommended if you already run the Clawbot gateway)

Clawbot ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.clawbot/scripts/`. First create the script:

```bash
mkdir -p ~/.clawbot/scripts
cat > ~/.clawbot/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec clawbot teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.clawbot/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
clawbot cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/clawbot-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Clawbot Teams pipeline subscription maintenance
...
ExecStart=/usr/local/bin/clawbot teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/clawbot-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Clawbot Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now clawbot-teams-pipeline-maintain.timer
systemctl list-timers clawbot-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/clawbot teams-pipeline maintain-subscriptions >> /var/log/clawbot/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.clawbot/.env` at the top of a wrapper script that crontab calls.
...
```bash
clawbot teams-pipeline subscriptions   # should show expirationDateTime advanced
clawbot teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
clawbot teams-pipeline list
clawbot teams-pipeline list --status failed
clawbot teams-pipeline show <job-id>
```

### Replay a stored job

```bash
clawbot teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
clawbot teams-pipeline fetch --meeting-id <meeting-id>
clawbot teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `clawbot teams-pipeline maintain-subscriptions --dry-run`
- inspect `clawbot teams-pipeline list --status failed`
...
- run `clawbot teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `clawbot teams-pipeline run`
...
- [ ] `clawbot teams-pipeline validate` returns an OK snapshot
- [ ] `clawbot teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Clawbot cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
