---
sidebar_position: 6
title: "Use MCP with Clawbot"
description: "A practical guide to connecting MCP servers to Clawbot Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Clawbot

This guide shows how to actually use MCP with Clawbot Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Clawbot tool
- you want Clawbot to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Clawbot to internal APIs, databases, or company systems without modifying Clawbot core

Do not use MCP when:
- a built-in Clawbot tool already solves the job well
...
- Clawbot remains the agent
- MCP servers contribute tools
- Clawbot discovers those tools at startup or reload time
...
If you installed Clawbot with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Clawbot banner/status should show MCP integration when configured
- ask Clawbot what tools it has available
...
## WSL2: bridge Clawbot in WSL to Windows Chrome

This is the practical setup when:

- Clawbot runs inside WSL2
...
In this setup, Clawbot does **not** connect to Chrome directly. Instead:

- Clawbot runs in WSL
- Clawbot starts a local stdio MCP server
...
```text
Clawbot (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Clawbot stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Clawbot core browser transport
...
```bash
clawbot mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
clawbot mcp test chrome-devtools-win
```

Then start a fresh Clawbot session or run:
...
Once loaded, Clawbot can use the MCP-prefixed browser tools directly. For example:
...
If Clawbot runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Clawbot from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Clawbot from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Clawbot:
...
2. Clawbot-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Clawbot will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Clawbot to reason over a bounded workspace.
...
Now Clawbot can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Clawbot core.
...
Because Clawbot now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
