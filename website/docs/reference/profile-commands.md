...
This page covers all commands related to [Clawbot profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `clawbot profile` without a subcommand shows help.
...
```bash
clawbot profile use <name>
```

Sets `<name>` as the active profile. All subsequent `clawbot` commands (without `-p`) will use this profile.
...
clawbot profile use default
```

## `clawbot profile create`

```bash
clawbot profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `clawbot profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
...
clawbot profile create mybot

# Clone config only from current profile
clawbot profile create work --clone

# Clone everything from current profile
clawbot profile create backup --clone-all

# Clone config from a specific profile
clawbot profile create work2 --clone --clone-from work
```

## `clawbot profile describe`

```bash
clawbot profile describe [<name>] [options]
```
...
clawbot profile describe researcher

# Set it explicitly
clawbot profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
clawbot profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
clawbot profile describe --all --auto
```

## `clawbot profile delete`

```bash
clawbot profile delete <name> [options]
```
...
```bash
clawbot profile delete mybot
clawbot profile delete mybot --yes
```
...
## `clawbot profile show`

```bash
clawbot profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Clawbot home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ clawbot profile show work
Profile: work
Path:    ~/.clawbot/profiles/work
...
## `clawbot profile alias`

```bash
clawbot profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Clawbot installation.
...
```bash
clawbot profile alias work
# Creates/updates ~/.local/bin/work

clawbot profile alias work --name mywork
# Creates ~/.local/bin/mywork

clawbot profile alias work --remove
# Removes the wrapper script
```

## `clawbot profile rename`

```bash
clawbot profile rename <old-name> <new-name>
```
...
```bash
clawbot profile rename mybot assistant
# ~/.clawbot/profiles/mybot → ~/.clawbot/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `clawbot profile export`

```bash
clawbot profile export <name> [options]
```
...
```bash
clawbot profile export work
# Creates work.tar.gz in the current directory

clawbot profile export work -o ./work-2026-03-29.tar.gz
```

## `clawbot profile import`

```bash
clawbot profile import <archive> [options]
```
...
```bash
clawbot profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

clawbot profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`clawbot profile export` / `import` are still the right commands for
...
### `clawbot profile install`

```bash
clawbot profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `clawbot -p telemetry`). |
...
clawbot profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
clawbot profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
clawbot profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
clawbot profile install ./telemetry/
```

### `clawbot profile update`

```bash
clawbot profile update <name> [--force-config] [--yes]
```
...
### `clawbot profile info`

```bash
clawbot profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Clawbot version, author, env var requirements, the source URL/path, and
...
`clawbot profile list` also shows the distribution name and version in a
`Distribution` column, and `clawbot profile show <name>` / `delete <name>`
...
clawbot profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
clawbot profile install https://github.com/your-org/internal-assistant.git
```
...
`clawbot_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Clawbot version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Clawbot can clone from.
3. Tell recipients to run `clawbot profile install <your-repo-url>`.
...
## `clawbot -p` / `clawbot --profile`

```bash
clawbot -p <name> <command> [options]
clawbot --profile <name> <command> [options]
```

Global flag to run any Clawbot command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
clawbot -p work chat -q "Check the server status"
clawbot --profile dev gateway start
clawbot -p personal skills list
clawbot -p work config edit
```

## `clawbot completion`

```bash
clawbot completion <shell>
```
...
- `clawbot profile <TAB>` — subcommands (list, use, create, etc.)
- `clawbot profile use <TAB>` — profile names
- `clawbot -p <TAB>` — profile names
