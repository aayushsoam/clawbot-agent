...
description: "How Clawbot stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `clawbot_cli/cron.py` | CLI `clawbot cron` subcommands |
...
Jobs are stored in `~/.clawbot/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `clawbot cron` commands are run or during active CLI sessions.
...
```python
# ~/.clawbot/scripts/check_competitors.py
...
2. **Environment variable** — `CLAWBOT_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.clawbot/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `clawbot cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `clawbot cron` CLI provides direct job management:

```bash
clawbot cron list                    # Show all jobs
clawbot cron create                  # Interactive job creation (alias: add)
clawbot cron edit <job_id>           # Edit job configuration
clawbot cron pause <job_id>          # Pause a running job
clawbot cron resume <job_id>         # Resume a paused job
clawbot cron run <job_id>            # Trigger immediate execution
clawbot cron remove <job_id>         # Delete a job
```
