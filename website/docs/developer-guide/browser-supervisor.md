...
[PR #12550](https://github.com/aayushsoam/clawbot-agent/pull/12550) proposed a
...
`window.alert`/`confirm`/`prompt` with a synchrosoam XHR to a magic host
(`clawbot-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per Clawbot `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `clawbot-acp`, `clawbot-api-server`, core toolsets (gated on CDP reachability)
...
- `clawbot_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
