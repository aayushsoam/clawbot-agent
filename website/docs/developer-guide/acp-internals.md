...
The ACP adapter wraps Clawbot' synchrosoam `AIAgent` in an async JSON-RPC stdio server.
...
```text
clawbot acp / clawbot-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.clawbot/.env
  -> configure stderr logging
  -> construct ClawbotACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'clawbot-agent[acp]==<version>' clawbot-acp`, pointed at the `clawbot-agent` PyPI release.
...
- `allow_once` -> Clawbot `once`
- `allow_always` -> Clawbot `always`
- reject options -> Clawbot `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Clawbot tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["clawbot-acp"])
...
Instead it reuses Clawbot' runtime resolver:

- `acp_adapter/auth.py`
- `clawbot_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Clawbot provider/credentials. It also always advertises a terminal setup auth method (`clawbot-setup`, args `--setup`) so first-run registry clients can open Clawbot' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.clawbot/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `clawbot-acp` toolset definition
- `clawbot_cli/main.py` — `clawbot acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `clawbot-acp` script
