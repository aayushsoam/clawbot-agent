...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Clawbot already has applies. The plugin never sees a
...
[`clawbot-example-plugins`](https://github.com/Aayushsoam/clawbot-example-plugins/tree/main/plugin-llm-example)
repo (companion repo for reference plugins — not bundled with
clawbot-agent itself). For the async surface (`acomplete()` /
`acomplete_structured()` with `asyncio.gather()`), see
[`plugin-llm-async-example`](https://github.com/Aayushsoam/clawbot-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Clawbot provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Clawbot provider
        # (must be one Clawbot already knows about — same names as
        # `clawbot model` and config.yaml model.provider).
        ...
        # plugin sends — Clawbot does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.clawbot/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Clawbot' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Clawbot subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/aayushsoam/clawbot-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/aayushsoam/clawbot-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/Aayushsoam/clawbot-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/Aayushsoam/clawbot-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
