...
sidebar_position: 1
title: "Architecture"
description: "Clawbot Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Clawbot Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ clawbot_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # ClawbotCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── clawbot_state.py           # SQLite session/state database with FTS5
├── clawbot_constants.py       # CLAWBOT_HOME, profile-aware paths
...
│
├── clawbot_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `clawbot` subcommands (large file)
...
│   ├── skills_config.py      # clawbot skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # clawbot tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # clawbot gateway start/stop
│
...
```text
User input → ClawbotCLI.process_input()
...
The synchrosoam orchestration engine (`AIAgent` in `run_agent.py`). Handles provider selection, prompt construction, tool execution, retries, fallback, callbacks, compression, and persistence. Supports three API modes for different provider backends.
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .clawbot.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.clawbot/plugins/` (user), `.clawbot/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `clawbot plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-clawbot-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Clawbot as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`clawbot -p <name>`) gets its own CLAWBOT_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
